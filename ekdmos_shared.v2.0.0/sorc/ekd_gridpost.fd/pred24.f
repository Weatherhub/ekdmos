      SUBROUTINE PRED24(KFILDO,KFIL10,KFILIO,KFILGO,KFILIN,NAMIN,
     ...
     5                  LKHERE,MSDATE,NDATE,KFILRA,RACESS,NUMRA,
     ...
     D                  FD1,FD2,FD3,FD4,FD5,FD6,FD7,DATACDF,NCDF,
     E                  FDA,FDVERT,FDTIME,FDSINS,FDMS,ND2X3,
     F                  IP12,IP13,IP14,IP15,IP16,IP23,
...
C   WRITTEN TO THE OUTPUT FILE.  PRED24 IS ENTERED ONCE FOR
C   EACH DATE/TIME AFTER THE FIRST.  GRIDPOINT
...
C   WILL BE DONE IN OPTION.
C
C   PRED24 FOR U202 WAS ADAPTED FROM PRED22 FOR U201.  IT'S
...
C   RETAINED, THE GRIDPOINT DATA ARE UNPACKED AS READ, AND
C   THEREFORE HAVE TO BE REPACKED, EVEN IF UNMODIFIED.
C   
C PROGRAM HISTORY LOG:
C   00-12-01  GLAHN
C   01-02-01  GLAHN   MODIFIED TO NOT DO GRIDPOINT CALCULATIONS
C                     WHEN FULL ID IS FOUND; ADDED IFDATE( );
C                     SUBSTITUTED TEST ON IFDATE(N) FOR TEST ON 
C                     INDEX(N) IN DO 116 LOOP; CHANGED LT TO LE IN
C                     LOOP DO 1015 AND EXECUTED LOOP ONLY WHEN NODATA LT 3
C   02-11-01  GLAHN   CHANGED ISDATA( ) TO FD1( ) AND ND1 TO ND2X3 
C                     IN CALL TO PACKG
C   02-12-01  SFANOS  MODIFIED FORMAT STATEMENTS TO CONFORM TO
C                     FORTRAN90 STANDARDS ON THE IBM-SP
C   03-02-01  GLAHN   ADDED NAME AND NGRID TO CALL TO OPTION
C   03-09-01  GLAHN   REPLACED KSTART=KEND+1 WITH KSTART=MAX(KSTART+1,MITEMS) 
C                     BELOW 116 AND 1165; CHANGED HOW NSIZE IS DEFINED 
C                     OUT OF OPTION; COMMENTS; OMITTED TEST ON IFIND( ) 
C                     AT BEGINNING OF DO 700 LOOP
C   05-01-28  MALONEY ADDED NCEP DOCBLOCK.
...
C                       FOR PRED24, NFIRST WILL NEVER BE 1.  (INPUT)  
...
C            IFIND(N) = 1 = WHEN THE VARIABLE MAY BE FOUND DIRECTLY
C                           FROM GFETCH.  THIS INCLUDES CASE WHEN IT HAS
C                           NOT BEEN FOUND, BUT IS NOT IDENTIFIED IN OPTION.
C                       0 = THE VARIABLE CAN AND MUST BE COMPUTED THRU
C                           OPTION.
C                       (INPUT)
C            ISTAV(N) = INDICATES FOR EACH VARIABLE (N=1,NPRED) WHETHER 
C                       DATA ARE CURRENTLY VECTOR (=1), GRIDPOINT (=0),
C                       OR UNKNOWN (=2).  (INPUT)
...
C                       U202 WAS WRITTEN ONLY FOR PREDICTORS.  (INPUT)
...
C                       FURNISHED ON THIS CALL TO PRED24.  (INPUT)
...
C                       NOTE THAT MSTORE IN U202 AND PRED24 IS NOT EXACTLY
C                       THAT IN U600 AND RDVECT.  U202 DOES NOT USE RDVECT.
...
C              FDA(J) = USED TO RETAIN A VARIABLE WITHIN PRED24 SO THAT IT
...
C                       ABOUT EOF AND FILE OPENINGS AND CLOSINGS,
C                       AND NO DATA ON A PARTICULAR FILE WILL
...
C           IFDATE(N) = HOLDS THE DATE/TIME OF THE VARIABLE FOUND, 
C                       CORRESPONDING TO INDEX(N).  IFDATE( ) AND
C                       INDEX( ) TOGETHER ALLOW A MISSING DAY (SUCH
C                       THAT A VARIABLE FOR TWO (OR MORE) DATES HAVE
C                       TO BE SAVED WHILE READING THE DATA) CAN BE
C                       HANDLED CORRECTLY.  (AUTOMATIC) (INTERNAL)
C        1         2         3         4         5         6         7 X
C 
C        SUBPROGRAMS CALLED: GFETCH, OPTION, GRIDB, SMTH5, SMTH9, SMTH25,
C            SMTH2X, SMTH3X, INTRP, INTRPA, INTRPB, TRANS, BINARY, PREDX1,
C            PACKV, UNPACK, UNPKBG, UPDAT, GRCOMB, GSTORE, TIMPR
C          UNIQUE: NONE
C          LIBRARY:
C           MOSLIB - ALL
C
...
C
     ...
     3          SMULT(NPRED),SADD(NPRED),ORIGIN(NPRED),CINT(NPRED)
      DIMENSION IFDATE(NPRED)
C        IFDATE( ) IS AN AUTOMATIC VARIABLE.
...
C        U202 AND WILL NOT HAVE A MODEL NUMBER (E.G. HOURLY DATA).
...
C        FROM A PREVIOUS RUN OF U202 AND HAVE A MODEL NUMBER
...
C        THE FIRST AND LAST DATE/TIMES, RESPECTIVELY, NEEDED FOR DAY 1.
 105  CONTINUE
C
D     ICOUNT=0
C
...
C
 ...
 1102 FORMAT(/' ****ERROR READING PACKED RECORD ON UNIT NO.',I3,
     1        ' PROCESSING DATE',I11,' IN PRED24 AT 1102, IOSTAT =',I5/
     2        '     FILE = ',A60)
      ISTOP(1)=ISTOP(1)+1
      LSTOPC=LSTOPC+1
...
C
 ...
 1105    FORMAT(/' ****ERROR IN PRED24 AT 1105.',
...
C
 ...
 1111    FORMAT(/' ****ERROR CLOSING FILE ON UNIT NO.',I3,
     1           ' PROCESSING DATE',I11,' IN PRED24 AT 1111,',
     2           ' IOSTAT =',I5/
     3           '     FILE = ',A60)
         ISTOP(1)=ISTOP(1)+1
 1114    LKHERE(IN+1)=0
...
D    1       ' PROCESSING DATE'I11,' IN PRED24 AT 1156')
         GO TO 400
      ENDIF
C
...
C
            ...
            IF(IFDATE(N).EQ.IDATE)GO TO 116
C              WHEN THE ABOVE TEST IS MET, THE VARIABLE HAS ALREADY
C              BEEN FOUND FOR THE SAME DATE.  THIS CAN HAPPEN WHEN
C              A PREVIOUS RUN OF U202 IS INPUT, AS WELL AS THE 
C              INPUT TO THE PREVIOUS U202 RUN.
...
C
C              MSTORE( , ) CAN HOLD THE ID( ) OR JD( ), DEPENDING ON WHICH
C              ONE WAS FOUND IN PRED25.  IF THE VARIABLE READ IS THE 
C              SAME AS ID( ), THEN NO PROCESSING SHOULD BE DONE.  IN THAT
C              CASE, SET IDYES = 1.
C
            IF(IPACK(6).EQ.ID(1,N).AND.
     1         IPACK(7).EQ.ID(2,N).AND.
     2         IPACK(8).EQ.ID(3,N).AND.
     3         IPACK(9).EQ.ID(4,N))THEN
                  IDYES=1
            ELSE
               IDYES=0
            ENDIF
...
C              WHEN THE ABOVE TEST IS MET, THE VARIABLE HAS ALREADY
C              BEEN FOUND FOR THE SAME DATE.  THIS CAN HAPPEN WHEN
C              A PREVIOUS RUN OF U201 IS INPUT, AS WELL AS THE 
C              INPUT TO THE PREVIOUS U202 RUN.
C              TEST ON MSTORE(7,M) ADDED 9/7/03, NECESSARY TO SAVE
C              PAST DATA IN SOME CIRCUMSTANCES; THIS MIGHT CAUSE
C              DUPLICATE PROCESSING IF DUPLICATES WERE INPUT.
            CALL UPDAT(IDATE,MSTORE(7,M),NEWDAT)
C              USUALLY THE LOOKBACK FEATURE WON'T BE OPERATIVE
C              AND MSTORE(7,M) WILL BE ZERO, IN WHICH CASE
C              UPDAT IS NOT EXPENSIVE.  NEWDAT IS USED LATER.
...
C               CASE, SET IDYES = 1.
C
            IF(IPA6.EQ.ID(1,N).AND.
     1         IPA7.EQ.ID(2,N).AND.
     2         IPA8.EQ.ID(3,N).AND.
     3         IPA9.EQ.ID(4,N))THEN
                  IDYES=1
            ELSE
               IDYES=0
            ENDIF
...
C           AN ERROR IN GRCOMB WILL HAVE CREATED A DIAGNOSTIC. 
         GO TO 800
C           IER NE 0 IS TREATED AS FATAL IN PRED24 WITH RETURN TO CALLING
...
C
C        CONTROL NEVER GOES BEYOND THIS POINT IN THE INITAL LOOP TO 
...
C
      NSIZE=IS2(3)*IS2(4)
C        NSIZE IS THE SIZE OF THE GRID.
C
...
C
 ...
 1178    FORMAT(/' ****ERROR IN PRED24 AT 1178.',
     1           '  ND5 MUST BE INCREASED FROM',I8,' TO GE',I8/
     2           '     READING ON UNIT NO.',I3,' PROCESSING DATE',I11,
     3           '    FILE = ',A60)
         ISTOP(1)=ISTOP(1)+1
...
C
 ...
 1182    FORMAT(/' ****ND5 =',I6,' TOO SMALL FOR DATA ARRAY',
     1           ' IN PRED24 AT 1182.  INCREASE TO GE',I6/
...
C           ISTAV( ) WAS UNDETERMINED UNTIL NOW.
 ...
 1189    FORMAT(/' ****MODEL NO.',I3,' ON UNIT NO.',I3,
     ...
     2           '     WITH TYPE OF DATA READ IN PRED24. ',
...
D1190 FORMAT( ' RESTORING GRID AT 1190 IN PRED24',I11.9,3I11)
C
      DO 120  K=1,NSIZE
      DATA(K)=FDA(K)
 120  CONTINUE
...
C
      ...
      LPARS=IDPARS(15,N)
      LDYES=IDYES
C        PRESERVE IDYES FOR LATER USE.
C
D     WRITE(KFILDO,139)(JD(K,N),K=1,4)
D139  FORMAT(/' SAVING GRID AT 139 IN PRED24    ',I11.9,3I11)
C
      DO 140 K=1,NSIZE
      FDA(K)=DATA(K)
 140  CONTINUE
...
C           ARE RETURNED AS MISSING.  LET IT PROCEED.  ISTOP(2) IS
C           INCREMENTED HERE BECAUSE IT PROBABLY WON'T BE LATER.
      ENDIF
C
...
C
     ...
     2      (IDPARS(2,N).EQ.361)))THEN
            ...
            ID(2,N)=CDFTABLE(K)
            ID(4,N)=000
C               CALL PACKG(KFILDO,KFILRA(JJ),RACESS(JJ),ID(1,N),
C     +                       IDPARS(1,N),
C     1                       ISCALD(N),0,NGRIDC(1,NSLAB),
C     2                       IPLAIN(1,1,N),PLAIN(N),NDATE,NYR,NMO,NDA,
C     3                       NHR,
C     4                       FD1,SDATACDF(1,K),ND2X3,IS2(3),IS2(4),
C     5                       IPACK,IWORK,ND5,
C     6                       MINPK,IS0,IS1,IS2,IS4,ND7,
C     7                       XMISSP,XMISSS,NWORDS,NTOTGB,NTOTGR,
C     8                       L3264B,L3264W,ISTOP(1),IER)
                CALL PACKG(KFILDO,KFILGO,ID(1,N),IDPARS(1,N),
     1           ISCALD(N),0,NGRIDC(1,NSLAB),
     2           IPLAIN(1,1,N),PLAIN(N),NDATE,NYR,NMO,NDA,NHR,
     3           FD1,SDATACDF(1,K),ND2X3,IS2(3),IS2(4),IPACK,IWORK,
     +           ND5,MINPK,
     4           IS0,IS1,IS2,IS4,ND7,XMISSP,XMISSS,
     5           NWORDS,NTOTBG,NTOTRG,
     6           L3264B,L3264W,ISTOP(1),IER)
...
C
 360  CALL PACKV(KFILDO,KFILIO,ID(1,N),IDPARS(1,N),JP(1,N),ISCALD(N),0,
 ...
 389  IF(INDEX(N).NE.1)GO TO 395
C
      IF(KFILIO.EQ.0)THEN
         WRITE(KFILDO,390)(ID(I,N),I=1,4),NDATE
 390     FORMAT(/' ****VARIABLE ',I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     1           ' COMPUTED MORE THAN ONCE FOR DATE',I11)
      ELSE
         WRITE(KFILDO,391)(ID(I,N),I=1,4),NDATE
 391     FORMAT(/' ****VARIABLE ',I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
...
C
 ...
 397  FORMAT(/' ****ERROR IN UNPKBG IN PRED24.  IER =',I4,
     1        ', PROCESSING DATE',I11)
      ISTOP(1)=ISTOP(1)+1
...
C        OUTPUT UP TO 3 DIAGNOSTICS IF NO DATA AVAILABLE ON A
C        PARTICULAR INPUT FILE FOR UP TO 3 DIAGNOSTICS.
C 
      IF(NODATA.GE.3)GO TO 4016
C
      DO 4015 J=1,NUMIN
C
...
C        CCC = 799 IS A DUMMY THAT HAS NO PURPOSE IN PRED24 EXCEPT
...
C***D410  FORMAT(' PRED24 AT 410'I4,3I10,6X3I6/
C***D    1       '              '4X,3I10,  4I6)
...
C           BE MET IN U202.  IDYES IS PRESERVED.
C           TRANSFER TO 510 RATHER THAN 520 ALLOWS GRIDPRINTING.
C           IDYES IS PRESERVED.
C
...
D419  FORMAT( ' RESTORING GRID AT  419 IN PRED24',I11.9,3I11)
C
      DO 420  K=1,NSIZE
      DATA(K)=FDA(K)
 420  CONTINUE
...
C
 425  IF(IFIND(N).EQ.0)GO TO 432
C        WHEN IFIND(N) EQ.0, THIS VARIABLE MUST BE COMPUTED IN OPTION,
C        IFIND( ) BEING SET IN PRED21.  THIS IS DONE FOR EFFICIENCY ONLY.
C      
...
C           OPTION.  THIS KEEPS CONTROL OUT OF GFETCH.
         WRITE(KFILDO,426)(ID(K,N),K=1,4),NDATE
 426     FORMAT(/' ****VARIABLE ',I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     1           ' NOT FOUND ON INPUT FILE(S) FOR DATE',I11/
     2           '     VARIABLE WILL NOT BE WRITTEN.')
         ISTOP(1)=ISTOP(1)+1
         ISTOP(2)=ISTOP(2)+1
         GO TO 695
      ENDIF
...
C
         IF(IER.EQ.0)THEN
            IDYES=0
C              IDYES = 0 INDICATES FULL ID WAS NOT FOUND.
C
            IF(NSLAB.EQ.0)THEN
...
C
     ...
     1               IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,SDATA,ND1,NSIZE,
     2               NPACK,NDATE,NTIMES,CORE,ND10,
     3               NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,ITIME(N),
     4               IER)
         LD(1)=0
...
C
         IF(IER.EQ.0)THEN
            IDYES=1
C              IDYES = 1 INDICATES FULL ID WAS FOUND.
C
            IF(NSLAB.EQ.0)THEN
C                 DATA ARE VECTOR.  THIS IS ALREADY KNOWN UNLESS
C                 DATA WERE MISSING UP TO THIS POINT.
...
C
 432  CALL OPTION(KFILDO,KFIL10,NFIRST,
...
C 
      IF(IER.EQ.0)THEN
         IFIND(N)=0
         IDYES=1
C           IDYES = 1 ALLOWS GRIDPOINT PROCESSING FROM OPTION.         
         GO TO 4325
C
      ELSEIF(IER.EQ.47)THEN
...
C           VARIABLE IS TO COME FROM.
         WRITE(KFILDO,4321)(ID(J,N),J=1,4)
 4321    FORMAT('     VARIABLE                   ',
...
C      
 ...
 433     FORMAT(/' ****ISTAV RETURNED FROM OPTION IN PRED24 DOES',
     1           ' NOT AGREE WITH WHAT WAS EXPECTED FOR VARIABLE',/
     2           '     ',I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     3           ' PROCESSING DATE',I11/
     3           '     VARIABLE WILL NOT BE WRITTEN.')
         ISTOP(2)=ISTOP(2)+1
         GO TO 695
      ENDIF
C
      IF(ISTAV(N).EQ.1)GO TO 650
...
C
      ...
      LPARS=IDPARS(15,N)
      LDYES=IDYES
C        KEEP IDYES WITH THE DATA.
C
D     WRITE(KFILDO,439)(JD(K,N),K=1,4)
D439  FORMAT(/' SAVING GRID AT 439 IN PRED24    ',I11.9,3I11)
C
      DO 440 K=1,NSIZE
      FDA(K)=DATA(K)
 440  CONTINUE
...
C     +                       IDPARS(1,N),
C     1                       ISCALD(N),0,NGRIDC(1,NSLAB),
C     2                       IPLAIN(1,1,N),PLAIN(N),NDATE,NYR,NMO,NDA,NHR,
C     3                       FD1,SDATACDF(1,K),ND2X3,IS2(3),IS2(4),
C     +                       IPACK,IWORK,ND5,
C     4                       MINPK,IS0,IS1,IS2,IS4,ND7,
C     5                       XMISSP,XMISSS,NWORDS,NTOTGB,NTOTGR,
C     6                       L3264B,L3264W,ISTOP(1),IER)
               CALL PACKG(KFILDO,KFILGO,ID(1,N),IDPARS(1,N),
     1           ISCALD(N),0,NGRIDC(1,NSLAB),
     2           IPLAIN(1,1,N),PLAIN(N),NDATE,NYR,NMO,NDA,NHR,
     3           FD1,SDATACDF(1,K),ND2X3,IS2(3),IS2(4),IPACK,IWORK,
...
C
      GO TO 670
C
C        THE DATA ARE MISSING.  SET ALL VALUES TO 9999.
C
 600  DO 610 K=1,NSTA
      SDATA(K)=9999.
 610  CONTINUE
...
C        PACK AND WRITE THE DATA.
C
 660  CALL PACKV(KFILDO,KFILIO,ID(1,N),IDPARS(1,N),JP(1,N),ISCALD(N),0,
     1           IPLAIN(1,1,N),PLAIN(N),NDATE,NYR,NMO,NDA,NHR,CCALL,
     2           ISDATA,SDATA,ND1,NSTA,IPACK,ND5,MINPK,
     3           IS0,IS1,IS2,IS4,ND7,XMISSP,XMISSS,
     4           IP15,NWORDS,NTOTBY,NTOTRC,
     5           L3264B,L3264W,ISTOP(1),IER)
 670  IF(INDEX(N).NE.1)GO TO 695
C
      IF(KFILIO.EQ.0)THEN
         WRITE(KFILDO,690)(ID(I,N),I=1,4),NDATE
 690     FORMAT(/' ****VARIABLE ',I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     1           ' COMPUTED MORE THAN ONCE PROCESSING DATE',I11)
      ELSE
         WRITE(KFILDO,691)(ID(I,N),I=1,4),NDATE
 691     FORMAT(/' ****VARIABLE ',I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     1           ' COMPUTED AND WRITTEN MORE THAN ONCE PROCESSING DATE',
     2             I11)
      ENDIF
C
