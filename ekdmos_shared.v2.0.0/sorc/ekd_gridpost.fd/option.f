...
C       DECEMBER   2011    JLW    COMMENTED OUT CALLS TO _OLD SUBROUTINES.
C
C        PURPOSE 
...
D 100  FORMAT(' *********** IN OPTION *************'/,' '4I10,15I5)
C
      IER=0
C
...
C
C         CALCULATE THE MEAN OF THE ENSEMBLE MEMBERS AND
C         CREATE A RAW CDF
C
      ELSEIF(((IDPARS(4).EQ.62).OR.(IDPARS(4).EQ.63).OR.
     1       (IDPARS(4).EQ.61)).AND.(IDPARS(1).LT.100))THEN
         CALL GRMEAN(KFILDO,KFIL10,IDPARS,JD,NDATE,
...
C
     ...
     3                ICALL,CCALL,ICALLD,CCALLD,NAME,
     4                NELEV,STALAT,STALON,
     5                ITIMEZ,ISDATA,DATA,SDATA1,DIR,ND1,NSTA,
...
C
     ...
     B               ISTAV,L3264B,L3264W,MISTOT,IER)
C
C       LOOK FOR THE DIFFERENCE BETWEEN TWO THRESHOLDS IN A CDF  
C
       ELSEIF(IDPARS(5).EQ.5.AND.IDPARS(4).EQ.61)THEN
       CALL CMPTAILS(KFILDO,KFIL10,
...
C
C      ELSEIF(IDPARS(1).EQ.002.AND.(IDPARS(2).EQ.042.OR.
C     1       IDPARS(2).EQ.043.OR.IDPARS(2).EQ.044.OR.
C     2       IDPARS(2).EQ.045)) THEN
C         CALL FRZLVL_OLD(KFILDO,KFIL10,IDPARS,JD,NDATE,
C     1               NGRIDC,ND11,NSLAB,IPACK,IWORK,DATA,ND5,
C     2               LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
C     3               IS0,IS1,IS2,IS4,ND7,FD1,FD2,FD3,FD4,
C     4               ND2X3,ISTAV,L3264B,MISTOT,IER)
C
...
C
C      ELSEIF((IDPARS(1).EQ.003).AND.((IDPARS(2).EQ.151).OR.
C     2      (IDPARS(2).EQ.155)))THEN
C         CALL ZRPRED_OLD(KFILDO,KFIL10,
C     1               IDPARS,JD,NDATE,NGRIDC,ND11,
C     2               NSLAB,IPACK,IWORK,DATA,ND5,LSTORE,ND9,
C     3               LITEMS,CORE,ND10,NBLOCK,NFETCH,IS0,IS1,
C     4               IS2,IS4,ND7,ND2X3,ISTAV,L3264B,MISTOT,IER)
C
...
C        ARE REDEVELOPED.
C
C      ELSEIF(IDPARS(1).EQ.003.AND.(IDPARS(2).EQ.225.OR.
C     1                             IDPARS(2).EQ.255.OR.
C     2                             IDPARS(2).EQ.285))THEN
C         CALL TPCP24_OLD(KFILDO,KFIL10,IDPARS,JD,NDATE,
C     1               NGRIDC,ND11,NSLAB,IPACK,IWORK,
C     2               DATA,ND5,LSTORE,ND9,LITEMS,CORE,ND10,
C     3               NBLOCK,NFETCH,IS0,IS1,IS2,IS4,ND7,FD1,
C     4               FD2,FD3,ND2X3,ISTAV,L3264B,MISTOT,
C     5               IER)
C
...
C
C      ELSEIF(IDPARS(1).EQ.007.AND.
C     1       IDPARS(2).EQ.140.OR.IDPARS(2).EQ.150.OR.
C     2       IDPARS(2).EQ.145.OR.IDPARS(2).EQ.155)THEN
C         CALL BULKRN(KFILDO,KFIL10,
C     1               IDPARS,JD,NDATE,
C     2               SDATA,DIR,ND1,NSTA,
C     3               NGRIDC,ND11,NSLAB,IPACK,IWORK,DATA,ND5,
C     4               FD1,FD2,FD3,FD4,FD5,FD6,FD7,ND2X3,       
C     5               LSTORE,ND9,LITEMS,CORE,ND10,
C     6               LASTL,NBLOCK,LASTD,NSTORE,NFETCH,
C     7               IS0,IS1,IS2,IS4,ND7,
C     8               ISTAV,L3264B,MISTOT,IER)
C
...
C         CALL SWTXRF_OLD(KFILDO,KFIL10,IDPARS,JD,NDATE,NGRIDC,ND11,
C     1               NSLAB,IPACK,IWORK,DATA,ND5,LSTORE,ND9,LITEMS,
C     2               CORE,ND10,NBLOCK,NFETCH,IS0,IS1,IS2,IS4,ND7,
C     3               ND2X3,ISTAV,L3264B,MISTOT,FD1,
C     4               FD2,FD3,FD4,FD5,FD6,FD7,KFILRA,RACESS,NUMRA,
C     5               ND1,CCALL,ICALLD,CCALLD,ISDATA,SDATA,NSTA,
C     *               NAME,STALAT,STALON,NGRID,
C     6               LASTL,LASTD,NSTORE,IP16,L3264W,DIR,IP12,IER)
C
...
C
C      ELSEIF(IDPARS(1).EQ.704.AND.(IDPARS(2).EQ.020).OR.
C     1                            (IDPARS(2).EQ.120)) THEN
C         CALL OBSADJUV(KFILDO,KFIL10,IDPARS,JD,NDATE,
C     1                 SDATA,ND1,NSTA,IPACK,IWORK,FD1,FD2,FD3,ND2X3,
C     2                 LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
C     3                 IS0,IS1,IS2,IS4,ND7,
C     4                 ISTAV,L3264B,IER)
C
...
C
C      ELSEIF(IDPARS(1).EQ.704.AND.IDPARS(2).EQ.230) THEN
C         CALL OBSADJDIR(KFILDO,KFIL10,IDPARS,JD,NDATE,
C     1                  SDATA,ND1,NSTA,IPACK,IWORK,FD1,FD2,ND2X3,
C     2                  LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
C     3                  IS0,IS1,IS2,IS4,ND7,
C     4                  ISTAV,L3264B,IER)
C
...
C
C      ELSEIF(IDPARS(1).EQ.704.AND.IDPARS(2).EQ.330)THEN
C         CALL OBSADJSPD(KFILDO,KFIL10,IP12,
C     1                  IDPARS,JD,NDATE,
C     2                  KFILRA,RACESS,NUMRA,CCALL,ICALLD,
C     3                  CCALLD,ISDATA,SDATA,ND1,NSTA,FD1,ND2X3,
C     4                  NSLAB,IPACK,IWORK,
C     5                  DATA,ND5,LSTORE,ND9,LITEMS,CORE,ND10,LASTL,
C     6                  NBLOCK,LASTD,NSTORE,NFETCH,
C     7                  IS0,IS1,IS2,IS4,ND7,
C     8                  ISTAV,L3264B,L3264W,IER)
C
C        LOOK FOR COMPUTATION OF MAX WIND SPEED IN 12-H
C
      ELSEIF((IDPARS(1).EQ.704).AND.(IDPARS(2).EQ.420))THEN
...
C
C      ELSEIF(IDPARS(1).EQ.007.AND.(IDPARS(2).EQ.460.OR.
C     1                             IDPARS(2).EQ.465.OR.
C     2                             IDPARS(2).EQ.470.OR.
C     3                             IDPARS(2).EQ.475.OR.
C     4                             IDPARS(2).EQ.480.OR.
C     5                             IDPARS(2).EQ.485.OR.
C     6                             IDPARS(2).EQ.490))THEN
C         CALL KINXRF_OLD(KFILDO,KFIL10,IP12,IP16,IDPARS,JD,NDATE,
C     1               KFILRA,RACESS,NUMRA,
C     2               CCALL,ICALLD,CCALLD,
C     3               ISDATA,SDATA,DIR,ND1,NSTA,
C     4               NGRIDC,ND11,NSLAB,IPACK,IWORK,DATA,ND5,
C     5               LSTORE,ND9,LITEMS,CORE,ND10,
C     6               LASTL,NBLOCK,LASTD,NSTORE,NFETCH,
C     6               IS0,IS1,IS2,IS4,ND7,
C     8               FD1,FD2,FD3,FD5,FD6,FD4,ND2X3,
