      SUBROUTINE CMPTAILS(KFILDO,KFIL10,
...
C
C
C        JANUARY 2008   WIEDENFELD  MDL MOS-2000. 
C
C        PURPOSE
C            TO COMPUTE THE DIFFERENCE OF TWO ENSEMBLE MOS CDF THRESHOLD
C            GRIDS.  THIS CAN BE USED TO COMPUTE THE "WARM" AND "COOL" TAILS
C            OF A SAMPLE
C
...
C
C        HISTORY:   
C        01/11/08    WIEDENFELD     NEW SUBROUTINE                  
C
C        DATA SET USE
...
C                       (INPUT)
C             DATA( ) = THE CALCULATED GRIDDED WIND DIRECTION IN
C                       DEGREES (OUTPUT)
...
C
C
C        FILL LD( ) AND LDPARS( ) AND GET THE FIRST CDF THRESHOLD
C        IN FD1
C
        LD(1)=JD(1)
        LD(2)=(IDPARS(6)/100)*10000
...
C
 140    MSLAB(1)=NSLAB
        ...
        NY(1)=IS2(4)
C
C        FILL LD1( ) AND LDPARS1( ) AND GET THE SECOND CDF THRESHOLD
C        IN FD2
C
        LD1(1)=JD(1)
        LD1(2)=IDPARS(7)*10000
...
C
      ...
      IF ((NX(1).NE.NX(2)).OR.(NY(1).NE.NY(2))) THEN
        ...
        GOTO 800
      ENDIF
 200  FORMAT(/,' ****MISMATCH FOUND IN INPUT THRESHOLDS ')
 201  FORMAT('  NSLAB VALUES NOT EQUAL')
 202  FORMAT('  GRID DIMENSIONS NOT EQUIVALENT')
C
C    SUBTRACT THE TWO GRIDS TO GET THE DIFFERENCE OF THE THRESHOLDS
C
