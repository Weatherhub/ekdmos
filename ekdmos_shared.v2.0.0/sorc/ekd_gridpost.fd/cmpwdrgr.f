      SUBROUTINE CMPWDRGR(KFILDO,KFIL10,
...
C
C
C        MARCH   2006   COSGROVE  MDL MOS-2000. 
C
C        PURPOSE
C            TO COMPUTE A GRID OF WIND DIRECTION USING GRIDS OF U AND V
C            WINDS.  IN CASES OF CALM WINDS WHERE THE WIND SPEED IS 0,
C            THE WIND DIRECTION IS SET TO 0.
C
...
C
C                224 250 - GRIDDED WIND DIRECTION
C
C        HISTORY:   
C        03/17/06    COSGROVE     NEW SUBROUTINE                  
C        10/03/06    GILBERT      MODIFIED TO SET NORTHERLY WINDS TO 360 INSTEAD OF 0.
C                                 CHANGED NINT=0 CHECKS ON DIRECTION TO ABS <0.1 CHECKS.
C
C        DATA SET USE
...
C                       GRIDDED WIND SPEED BY EITHER
C                       GFETCH OR CONST1. (INTERNAL)
...
C        WIND DIRECTION.
C
      IF((IDPARS(1).NE.224).AND.(IDPARS(2).NE.250))THEN
C
      WRITE(KFILDO,110)(JD(L),L=1,4)
 110  FORMAT(/,' ****CMPWDRGR ENTERED FOR VARIABLE',
     ...
     2       ' NOT ACCOMMODATED.')
      ...
      ENDIF
C
C        FILL LD( ) AND LDPARS( ) AND GET THE U WIND 
C        IN FD1
C
        LD(1)= 224 * 1000000 + 060 * 1000 + IDPARS(4)
        LD(2)=JD(2)
        LD(3)=JD(3)
...
C
 140    MSLAB(1)=NSLAB
        ...
        NY(1)=IS2(4)
C
C        FILL LD1( ) AND LDPARS1( ) AND GET THE V WIND 
C        IN FD2
C
        LD1(1)= 224 * 1000000 + 160 * 1000 + IDPARS(4)
        LD1(2)=JD(2)
        LD1(3)=JD(3)
...
C
 150    MSLAB(2)=NSLAB1
        ...
        NY(2)=IS2(4)
C
C        FILL LD1( ) AND LDPARS1( ) AND GET THE WIND 
C        SPEED IN FD3
C
        LD1(1)= 224 * 1000000 + 360 * 1000 + IDPARS(4)
        LD1(2)=JD(2)
        LD1(3)=JD(3)
        LD1(4)=IDPARS(13)*100+IDPARS(14)*10+IDPARS(15)
        CALL PRSID1(KFILDO,LD1,LDPARS1)
...
C
      ...
      IF ((NX(1).NE.NX(2)).OR.(NY(1).NE.NY(2))) THEN
        ...
        GOTO 800
      ENDIF
 200  FORMAT(/,' ****MISMATCH FOUND IN INPUT U ',
     1       'AND V GRIDS:')
 201  FORMAT('  NSLAB VALUES NOT EQUAL')
 202  FORMAT('  GRID DIMENSIONS NOT EQUIVALENT')
C
      IF (MSLAB(1).NE.MSLAB(3)) THEN
        WRITE(KFILDO,203)
        GOTO 800
      ENDIF
      IF ((NX(1).NE.NX(3)).OR.(NY(1).NE.NY(3))) THEN
        WRITE(KFILDO,202)
        GOTO 800
      ENDIF
 203  FORMAT(/,' ****MISMATCH FOUND IN INPUT U ',
     1       'AND SPEED GRIDS:')
C
C        GRIDS ARE IDENTICAL. NOW LOAD DATA ARRAY;
C        IF WIND SPEED IS ZERO, SET DIRECTION TO 0;
C        OTHERWISE, COMPUTE DIRECTION FROM U AND V.
C
      DO 450 J=1,NX(1)*NY(1)
C
        IF(NINT(FD1(J)).NE.9999.AND.NINT(FD2(J)).NE.9999.AND.
     1     NINT(FD3(J)).NE.9999)THEN
C
C        IF WIND IS CALM (SPEED OR U&V=0), SET DIRECTION TO ZERO
C
          IF((FD3(J).LT.0.1).OR.((ABS(FD1(J)).LT.0.1).AND.  
     1         (ABS(FD2(J)).LT.0.1)))THEN                  
            DATA(J)=0.0
          ELSE
C
C        IF THERE IS WIND, COMPUTE DIRECTION FROM U AND V
            IF(ABS(FD2(J)).LT.0.1)THEN        
               DATA(J)=SIGN(90.,FD1(J)) + 180.
            ELSE
               DATA(J)=57.29578*ATAN2(FD1(J),FD2(J))+180.
C        DO NOT RETURN A DIRECTION OF 0 IF NOT CALM, SET TO 360.0
               IF(DATA(J).LT.0.5) DATA(J)=360.0   
            ENDIF
C
C
          ENDIF
