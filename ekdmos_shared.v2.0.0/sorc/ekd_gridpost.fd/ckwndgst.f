      SUBROUTINE CKWNDGST(KFILDO,KFIL10,
...
C
C        MARCH    2007   MALONEY  MDL MOS-2000. (FROM CKTMPDP)
C
C        PURPOSE
C            TO CHECK AN INPUT GRID OF WIND GUSTS/MAX WIND SPEEDS (224 385) AGAINST
C            THE GRIDDED WIND SPEED (224 370) TO ENSURE CONSISTENCY:
C       
C               GUST >= WIND SPEED
C
C            AS ANY NEW CCCFFF IS REMOVED OR ADDED TO ITABLE( , ),
C            CHANGE THE PARAMETER NDIM APPROPRIATELY; THIS FUNCTIONALITY IS RETAINED
C            IN THE INTEREST OF GENERALITY, THOUGH IT IS NOT ANTICIPATED THAT THIS 
C            SUBROUTINE WILL BE USED FOR VARIABLES OTHER THAN THOSE ORIGINALLY 
C            ACCOMODATED.
C
...
C
C                224 390 - GRIDDED WIND GUSTS IN KNOTS.
C
C        HISTORY:   
C        2007/03/19      MALONEY	NEW SUBROUTINE (FROM CKTMPDP)
C
C        DATA SET USE
...
C                       (INPUT)
C             DATA( ) = THE GRIDDED CONSISTENCY-CHECKED WIND GUSTS IN
C                       KNOTS. (OUTPUT)
...
C                       GRIDDED WIND GUSTS IN KNOTS BY EITHER 
C                       GFETCH OR CONST1. (INTERNAL)
C              FD2( ) = WORK ARRAY (J=1,ND2X3) USED TO FETCH THE 
C                       GRIDDED WIND SPEEDS IN KNOTS BY EITHER
C                       GFETCH OR CONST1. (INTERNAL)
...
C                 OUT     IN      IN    
C                 GUST    GUST    WSPEED
      DATA ITABLE/224390, 224385, 224370/
C
C
      IER=0
      ISTAV=0
C
C        MAKE SURE THE REQUESTED PREDICTOR IS A GRIDDED 
C        WIND GUST FIELD.
C
...
C
 ...
 110  FORMAT(/,' ****CKWNDGST ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 800
C
C        FILL LD( ) AND LDPARS( ) AND GET THE WIND GUSTS 
C        AND WIND SPEEDS
C
 120  DO 150 II=2,3
         LD(1)=ITABLE(II,JJ)*1000+IDPARS(4)
         LD(2)=JD(2)
         LD(3)=JD(3)
...
C
 140     MSLAB(II-1)=NSLAB
         NX(II-1)=IS2(3)
         NY(II-1)=IS2(4)
C
C           DATA ARE READ INTO FD2 EACH TIME.  FOR THE FIRST VARIABLE,
C           COPY FD2 TO FD1.
C
         IF((II-1).EQ.1) THEN
            DO I=1,ND2X3
...
C
      ...
      IF ((NX(1).NE.NX(2)).OR.(NY(1).NE.NY(2))) THEN
         ...
         GOTO 800
      ENDIF
 200  FORMAT(/,' ****MISMATCH FOUND IN INPUT WIND ',
     1       'AND GUST GRIDS:')
 201  FORMAT('  NSLAB VALUES NOT EQUAL')
 202  FORMAT('  GRID DIMENSIONS NOT EQUIVALENT')
C
C        GRIDS ARE IDENTICAL. NOW LOAD DATA ARRAY;
C        ENFORCE WIND, GUST CONSISTENCY.
C
 400  DO 450 J=1,NX(1)*NY(1)
         DATA(J)=FD1(J)
         IF(NINT(FD1(J)).EQ.9999.OR.NINT(FD2(J)).EQ.9999) GOTO 450
C
C               IF GUST < WIND SPEED, SET GUST = WIND
C
         IF(FD1(J).LT.FD2(J)) THEN
            DATA(J)=FD2(J)
         ENDIF
C
 450  CONTINUE
