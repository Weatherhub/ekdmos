      SUBROUTINE U905(KFILDI,KFILDO,KFILEQ,EQNNAM,
...
C
C ABSTRACT: PROGRAM U905 IS USED TO MAKE FORECASTS FROM REGRESSION
C   EQUATIONS, PROBABLY PRODUCED BY U605.  THIS PROGRAM SHOULD
...
C   COMPUTED WITHIN U905.
C
C   NORMALLY, U905 IS USED FOR ONE DATE/TIME IN OPERATIONS,
C   BUT CAN OPERATE ON MULTIPLE DATE/TIMES.
C
C
C PROGRAM HISTORY LOG:
C   07-10-25  WIEDENFELD             ADAPTED FROM U900 AND U705
C   10-11-18  VEENHUIS               ADDED NESNUM TO CONTROL FILE.
C                                    THIS ALLOWS THE 'G' IN THE OUTPUT
C                                    ID TO BE SET TO A SPECIFIC VALUE.
C
C USAGE      CALL U905 (KFILDI,KFILDO,KFILEQ,EQNNAM,ICALL,CCALL,ISDATA,
...
C                       JD,ITAU,IUSED,CONST,AVG,ESS,CORR,COEF,X,VECTOR,
C                       IDTAND,IDTPAR,IDTNSD,IDPRSD,PLNSD,IPLNSD,ISCLSD,
C                       NGP,LGP,MTRMS,ICAT,LOCSTA,FCST,ND2,ND3,ND11,ND13,
C                       CORE,ND10,NBLOCK)
C   INPUT ARGUMENT LIST:
C              KFILDI - UNIT NUMBER TO READ INPUT FILE 'U905.CN'.
...
C                       AND NAMIN(J) (J=1,ND6).  NOT USED IN U905.
...
C                       THIS VARIABLE WAS ESTABLISHED TO ALLOW U905 
...
C                       PREDICTAND NN (NN=1,MTANDS(I))
C        IDPRSD(J,NN) = THE PARSED, INDIVIDUAL COMPONENTS OF THE PREDICTAND
C                       SD ID'S (J=1,15) (NN=1,MTANDS)
C      IPLNSD(L,J,NN) = 32 CHARACTERS (L=1,L3264W) (J=1,4) OF PLAIN
C                       LANGUAGE DESCRIPTION OF THE PREDICTAND
C                       NN (NN=1,MTANDS(I)).  NOTE THAT THIS REQUIRES
C                       TWO 32-BIT WORDS TO HOLD THE DESCRIPTION
C                       BUT ONLY ONE 64-BIT WORD.  EQUIVALENCED
C                       TO PLNSD( ).
C           PLNSD(NN) = THE PLAIN LANGUAGE DESCRIPTION OF THE
C                       PREDICTAND SD'S (ERROR ESTIMATES) NN
C                       (NN=1,MTANDS(I)). EQUIVALENCED TO
C                       IPLAIN( , , ).  (CHARACTER*32)
...
C                       NN (NN=1,MTANDS(I) ISCLSD( , ) COMES FROM THE
C                       VARIABLE CONSTANT FILE, MODIFIED IN SETPLN
...
C                       THE FILE NAMES WILL BE 4 CHARACTERS 'U905',
C                       THEN 4 CHARACTERS FROM IPINIT, THEN 2 CHARACTERS
C                       FROM IP(J) (E.G., 'U905HRG130').  THE ARRAY IS
...
C                           IN THE ORDER TO BE DEALT WITH IN U905.  THE
...
C                           VIEWING. ALSO, THE ERROR ESTIMATES ARE WRITTEN.
...
C                       IS USED.  NORMALLY, U905 WOULD USE ONLY ONE
...
C              NESNUM = THE ENSEMBLE SYSTEM NUMBER. USED TO DIFFERENTIATE
C                       BETWEEN DIFFERENT MODESL WITH THE SAME DD (INPUT).
...
C             UNIQUE    - EST905, FCST95, RDSTR9, SETJDT
C          LIBRARY:
C             ENSLIB    - IPOPEN, IERX, RDI, RDSNAM, RDSTAD, RDSTAL, IPRINT,
C                         TIMPR, SKIPWR, TANDID, TANDSD, RDEQN95, TRAIL, DATPRO,
C                         CLFILM 
C
C        EXIT STATES:
...
C     CALL W3TAGB('U905',2000,0100,0050,'OSD21')
C
...
C     
      STATE='105 '
C     OPEN(UNIT=KFILDI,FILE='U905.CN',STATUS='OLD',IOSTAT=IOS,ERR=900)
C
...
C
 ...
 111  CONTINUE
C
 112  CALL TIMPR(IP(J),IP(J),'START U905          ')
 113  CONTINUE
C
...
C
     ...
     2     NREPLA,NCHECK,MEMNUM,NESNUM,PXMISS
 125  FORMAT(12(I10/),F10.0)
      GO TO 1255
...
C
 126  WRITE(KFILDO,127)KSKIP,KWRITE,NSKIP,JSTOP,
     ...
     2                 NREPLA,NCHECK,MEMNUM,NESNUM,NINT(PXMISS),L3264B
 127  FORMAT(/' KSKIP ',I10,'   SKIP PAST DATA FOR THIS DATE ON OUTPUT',
     ...
     X                      ' FILE, 1 = YES, 0 = NO'/
     9        ' MEMNUM',I10,'   MEMBER DD NUMBER, IF DD EQ 0, USE',
     X                      ' EQUATION DD, IF DD NE 0 USE VALUE'/
     A        ' NESNUM',I10,'   ENSEMBLE SYSTEM NUMBER. MUST BE LESS',
     X                       ' THAN 10.'/
     B        ' PXMISS',F10.2,'   SECONDARY MISSING VALUE TO INSERT',
     X                      ' FOR 9997'/
     C        ' L3264B',I10,'   INTEGER WORD SIZE OF MACHINE')
C
C        CHECK THE VALUE OF NESNUM TO MAKE SURE IT IS POSITIVE AND LESS
C        THAN 10 SINCE IT WILL BE USED AS THE 'G' VALUE.
C
         IF((NESNUM .LT. 0).OR.(NESNUM .GE. 10)) THEN
	   ISTOP(1)=ISTOP(1)+1
	   WRITE(KFILDO,128) NESNUM
 128       FORMAT(/'****NESNUM HAS BEEN SET TO AN INVALID NUMBER:',
     1            'NESNUM=',(I4),' NESNUM MUST BE BETWEEN 0 AND 9.',  
     2            'NESNUM WILL BE SET TO 0')
           NESNUM=0
	 ENDIF
C
...
C
 ...
 131     FORMAT(/' ****ERROR: CAN NOT READ NCEP DATE FILE - ',
     1           'CATASTROPHIC ERROR IN U905. STOP AT 131.')
         CALL W3TAGE('U905')
         STOP 131
      ENDIF
      NDATES=1
...
C        OUTPUT FILE.  SAVE THE FILE NAME IN CFILX.  U905
C        MUST HAVE AN OUTPUT EXTERNAL RANDOM ACCESS FILE;
C        WRITING IS U905'S ONLY PURPOSE.
C
      DO 1432 J=1,NUMRA
...
C        TO READ (AS IT CAN BE IN U700).  THIS IS IN U905
...
C           LAST DAY ON THE DATA SET.
         WRITE(KFILDO,1577)
 1577    FORMAT(' ****FATAL ERROR, STOP IN U905 AT 1577')
         CALL W3TAGE('U905')
         STOP 1577
      ENDIF
C
...
C
      ENDIF
C
 1580 CALL RDEQN95(KFILDO,KFILEQ(NSET),EQNNAM(NSET),IDATE(ND),
...
C          USER TO CHECK THIS OUT, BUT DON'T STOP THE PROGRAM.
 ...
 1582       FORMAT(' ****THE EQUATION FILE ON UNIT NO:',I3,' ',1X,
     ...
     1         'EMPTY. THE USER SHOULD INVESTIGATE, BUT U905 '/,
     2         ' WILL NOT STOP.')
	    GO TO 300
         ELSEIF(IER.EQ.21)THEN
            WRITE(KFILDO,158)
 158        FORMAT(/' STOP IN U905 AT 158;',
     1              ' INCREASE ND1 AND/OR ND5.')
            CALL W3TAGE('U905')
            STOP 158
         ELSEIF(IER.EQ.169)THEN
            WRITE(KFILDO,159)
 159        FORMAT(/' STOP IN U905 AT 159;',
     1              ' INCREASE ND1 AND/OR ND13.')
            CALL W3TAGE('U905')
            STOP 159
         ELSEIF(IER.EQ.166)THEN
            WRITE(KFILDO,160)
 160        FORMAT(/' STOP IN U905 AT 160; PROVIDE PREDICTANDS',
     1              ' OR INCREASE ND3')
            CALL W3TAGE('U905')
            STOP 160
         ELSEIF(IER.EQ.167)THEN
            WRITE(KFILDO,161)
 161        FORMAT(/' STOP IN U905 AT 161; PROVIDE EQUATIONS',
     1              ' OR INCREASE ND2')
            CALL W3TAGE('U905')
            STOP 161
...
C              EQUATIONS DON'T MATCH IDATE( ).
 ...
 162        FORMAT(/' STOP IN U905 AT 162, IER =',I5,'.')
            CALL W3TAGE('U905')
            STOP 162
         ENDIF
C
...
C164  FORMAT(/' IN U905 AT 164--ND3,ND11,NSETS,MTANDS(1)',4I6)
      CALL TANDID(KFILDO,KFILCP,KFILPF,PFCORR,IP(7),IP(9),IDTAND,
     1            IDTPAR,ISCALD,PLAIN,ICAT,MTANDS,1,ND3,1,
     2            ISTOP(1),IER)
C
      CALL TANDSD(KFILDO,KFILCP,IP(7),IP(9),
     1            IDTAND,IDTPAR,IDTNSD,IDPRSD,ISCLSD,PLNSD,ICAT,
     2            MTANDS,1,ND3,1,ISTOP,IER)
C        NOTE THAT NSETS AND ND11 IN TANDID AND TANDSD  MUST BE PASSED
C        AS 1 FROM U905.
C
...
C
     ...
     1            ND2,ND13,IER)
C
C         MAKE FORECASTS AND ERRORS OF ESTIMATE.
C
      CALL FCST95(KFILDO,KFIL10,IP12,KFILFC,KFILX,CFILX,
...
C        ERROR STOP BELOW IS FOR ERRORS OF CONTROL INFORMATION INPUT.
 900  CALL IERX(KFILDO,KFILDO,IOS,'U905  ',STATE)
      CALL W3TAGE('U905')
      STOP 9999
      END
