...
C                                 CALL TO ADJEXPQPF
C        OCTOBER   2008   JRW     ADDED CATGRQ FOR EKDMOS QPF.
C                                  
C
C        PURPOSE 
...
C            DC2PCT, GUSTPST, CATMOD, LPTSPP, DBTAIL, CIGBLOTDSC, IFRCAT
C
...
C        TO PRECENT.
C
      ELSEIF((IDPARS(1).EQ.207.AND.IDPARS(2).EQ.514))THEN
...
C 
     ...
     7               L3264B,L3264W,ISTAB,IER)
C
       ELSEIF(IDPARS(1).EQ.207.AND.(IDPARS(2).EQ.505.OR.
     1                              IDPARS(2).EQ.510)) THEN
C
C            LPTSPP POST-PROCESSES LAMP TSTM PROBABILITIES.
C
         CALL LPTSPP(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1               ID,IDPARS,JD,ITAU,
     2               NDATE,MDATE,CCALL,ISDATA,XDATA,ND1,NSTA,
     3               ICALLD,CCALLD,IPACK,IWORK,DATA,ND5,
     4               LSTORE,ND9,LITEMS,CORE,ND10,
...
C
     ...
     J                                 (IDPARS(2).EQ.465).OR.
     K                                 (IDPARS(2).EQ.620).OR.
     L                                 (IDPARS(2).EQ.630)).AND.
     M                                ((IDPARS(3).EQ.1).OR.
     N                                 (IDPARS(3).EQ.2))))THEN
...
C
      ELSEIF(IDPARS(1).EQ.203.AND.IDPARS(2).EQ.221.AND.
     1                            IDPARS(4).EQ.76)THEN
         CALL CATGRQ(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1               ID,IDPARS,JD,ITAU,
     2               NDATE,MDATE,CCALL,ISDATA,XDATA,ND1,NSTA,
     3               ICALLD,CCALLD,IPACK,IWORK,DATA,ND5,
     4               LSTORE,ND9,LITEMS,CORE,ND10,
...
C
     ...
     8                              (IDPARS(2).EQ.391).OR.
     9                              (IDPARS(2).EQ.461).OR.
     A                              (IDPARS(2).EQ.546).OR.
     B                              (IDPARS(2).EQ.556).OR.
     C                              (IDPARS(2).EQ.621).OR.
     D                              (IDPARS(2).EQ.646).OR.
     E                              (IDPARS(2).EQ.666))).OR.
     F       (IDPARS(1).EQ.203.AND.((IDPARS(2).EQ.221).OR.
     G                              (IDPARS(2).EQ.321).OR.
     H                              (IDPARS(2).EQ.331).OR.
     I                              (IDPARS(2).EQ.421).OR.
     J                              (IDPARS(2).EQ.431))).OR.
     K       (IDPARS(1).EQ.204.AND.((IDPARS(2).EQ.366))).OR.  
     L       (IDPARS(1).EQ.207.AND.((IDPARS(2).EQ.501))))THEN
