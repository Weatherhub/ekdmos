...
C          MAY      2012 WAGNER    ADDED AVGSD AND USED THIS TO CHECK WHEN TO USE
C                                  KERN=1 OR KERN=2.  ALSO REMOVED "-1" FROM FINC
C                                  WHEN KERN=2.
C          AUGUST   2012 WAGNER    ADDED AN ELSE STATEMENT TO SET RDATA AND SDATA
C                                  TO MISSING WHEN THE DEWPOINTS ARE MISSING.  
C                                  ADDED A GO TO AFTER LOOP 216.
C          AUGUST   2012 ZYLSTRA   INITIALIZED RDATA AND SDATA TO MISSING.  SET
C                                  XBAR TO MISSING WHEN APPROPRIATE.  ADDED NOHI
C
C        PURPOSE
...
C
C         LD(1)=204210000+(IDPARS(3,N)*100)+IDPARS(4,N)
         LD(1)=204317000+(IDPARS(3,N)*100)+IDPARS(4,N)
         LD(2)=ID(2,N)
         LD(3)=ID(3,N)
         LD(4)=ID(4,N)
         CALL PRSID1(KFILDO,LD,LDPARS)
...
!$OMP& PRIVATE(RFAC,TDATA,TSD,LOC,FACTOR,XLOW,NOHI)
!$OMP& PRIVATE(XBAR,SKEW,SKEWN,SKEWD,NDPT,RDATAMAX)
...
C
     ...
     1      .AND.WSDATA(K,L).NE.9999.)THEN
         ...
         RDATA(L)=9999.
         SDATA(L)=9999.
         IF(DPDATA(K,L).NE.9999.)NDPT=NDPT+1
 210  CONTINUE
C
      MMM=MM
      IF(MMM.GT.0)THEN
...
C
 ...
 214  CONTINUE 
C
      RDATAMAX=MAXVAL(RDATA(1:MM))
      IF(NOHI.NE.0)THEN
         IF(NDATE.EQ.NDATES)THEN
C              THIS DIAGNOSTIC WILL PRINT ON ONLY THE FIRST DAY.
            WRITE(KFILDO,218)CCALL(K),(ID(J,NN),J=1,4),NDATE,NDPT
 218        FORMAT(/,' CANNOT MAKE A HEAT INDEX FORECAST FOR STATION ',
     1              A8,
     2              ' FOR VARIABLE',
     3                2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     4               ' FOR DATE',I12,I2,'.  (PRINT ON DAY 1.)')
         ENDIF
C
...
C
  ...
  230   CONTINUE
C         DO 231 L=1,NCDFTH
C         WRITE(KFILDO,*) CCALL(K),L,CDFTH(L),XCDF(K,L)
C  231    CONTINUE
C
        IF(IP15.NE.0)THEN
