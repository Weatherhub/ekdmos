...
C   10-11-29  VEENHUIS    ADDED ND2 TO CALL LIST. ADDED CHECK
...
C          FORT.KFILAI = UNIT NUMBER FOR THE SPREAD-SKILL FILE (INPUT)
C        OUTPUT FILES: 
...
C               KFILAI = UNIT NUMBER OF SPREAD-SKILL ASCII INPUT FILE (INPUT)
...
C                        (OUTPUT)
C                SD(N) = THE CONSTANT KERNEL WIDTH FOR THIS VARIABLE N
...
C                        OUTPUT DATA (N=1,ND4).  ISCALD COMES FROM THE VARIABLE
...
C                        OUTPUTTING THE CDF VALUES (J=1,ND12).  (OUTPUT)
...
C                        PRINT IP( ) VALUES.
C                  II0 = COUNTER USED WHEN READING FROM KFILAI (INTERNAL)
C                   II = COUNTER USED WHEN READING FROM KFILAI (INTERNAL)
C                 NMEM = NUMBER OF ENSENBLE MEMBERS.  USED WHEN READING
C                        KFILAI (INTERNAL)
C              MAXNMEM = MAXIMUM NUMBER OF ENSEMBLE MEMBERS.  USED WHEN
C                        READING KFILAI (INTERNAL) 
C 
...
C                        EITHER KFILDT, KFILD(), KFILP, KFILCP, KFILIO, KFILAI,
C                        OR KFILDO
C                  175 - INCONSISTENCY OF INPUT NUMBERS IN KFILIO AND EITHER
C                        KFILDT, KFILD(), KFILP, KFILAI, OR KFILCP
...
C                                                                       
C ATTRIBUTES:                                                           
C   LANGUAGE:  FORTRAN 90 (xlf_r compiler) 
C   MACHINE:  IBM SP
C
C $$$               
C
...
C
 ...
 127  FORMAT(/' KSKIP ',I10,'   SKIP PAST THIS DATE ON OUTPUT FILE'/  
     ...
     C        ' MODRUN',I10,'   USED TO SET OUTPUT DD')
C
...
C
      CALL RDSNAM(KFILDI,KFILDO,KFILAI,CSPDNAM,IDUM,IDUM,1,N,'OLD',
     1            'FORMATTED',IP,IER)
      IF(IER.NE.0) ISTOP=ISTOP+1
      IF(KFILAI.NE.0)WRITE(KFILDO,154)KFILAI,CSPDNAM
 154  FORMAT(/,' SPREAD SKILL RELATIONSHIP FILE, UNIT AND NAME.',/,
     1        (' ',I4,2X,A60))
C
C        READ AND PROCESS UNIT NUMBER FOR THE CDF THRESHOLD FILE 
C
...
C
 160  DO 170 J=1,NUMRA
     ...
     6   KFILRA(J).NE.KFILAI  .AND.
     7   KFILRA(J).NE.KFILDO)GO TO 170
         WRITE(KFILDO,165)
 165  FORMAT(/' ****INCONSISTENCY IN INPUT UNIT NUMBERS IN KFILRA( )',
     1        ' WITH EITHER KFILDT, KFILD( ), KFILP, KFILAI, ',
...
C     IF(KFILIO.EQ.0)GO TO 180
     ...
     4   KFILIO.NE.KFILAI  .AND.
     5   KFILIO.NE.KFILCP)GO TO 176
         WRITE(KFILDO,175)
