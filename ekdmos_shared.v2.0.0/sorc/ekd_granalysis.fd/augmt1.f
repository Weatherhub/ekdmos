     ...
     2                  CCALL,NAME,XDATA,LNDSEA,
     3                  XLAPSE,ELEMOD,ELEV,NSTA,ND1,IPREX2,PREX3,
     4                  LSTORE,ND9,LITEMS,
...
C        OCTOBER   2009   GLAHN   ADDED CEILING AND VIS; IDCAT=15
C                                 CORRECTED JDATE=KDATE-MOD(NHR,6) TO
C                                 CALL UPDAT(KDATE,-MOD(NHR,6),JDATE)
C        OCTOBER   2009   GLAHN   MODIFIED SO WHEN EARLY CYCLES NOT
C                                 AVAILABLE, IT CYCLES. 
C        OCTOBER   2009   GLAHN   MODIFIED SREF PROJECTION FOR LAMP
C        OCTOBER   2009   GLAHN   ADDED WIND SPEED, U, V OBS; SET
C                                 NEGATIVE OBS, LAMP SPEEDS TO ZERO
C        OCTOBER   2009   GLAHN   ADDED CALLS TO CIGHTC FOR LAMP CIG
C        NOVEMBER  2009   GLAHN   ADDED PREX3, XLAPSE( ), ELEMOD( ),
C                                 ELEV( ) TO CALL; CODE TO USE THEM
C        NOVEMBER  2009   GLAHN   SUBSTITUTED IF(MOD(IFGTAU,3).EQ.0)
C                                 VICE MOD(IDPARS(12) FOR L=1-5
C        NOVEMBER  2009   GLAHN   ADDED IPREX2 TO CALL AND CHANGED
C                                 LIMIT FOR AVERAGING STATIONS FROM 5
C                                 TO IPRNO IN 2 PLACES; REQUIRED GE 2
C                                 STATIONS FOR AUGMENTING VICE GE 1
C        MARCH     2010   SDS     MODIFIED OPEN STATEMENT FOR OPERATIONS
C        MAY       2010   JLW     CORRECTED TYPO FOR EKDMOS DEWPOINT 
C                                 (L=7, NOT 9)    
C                                                            
C        PURPOSE
...
C            OR MIN TEMP FOR THE OTHER 3 TIMES.  NOTE THE NON
C            SYMMETRIC AUGMENTATION.  IT WAS THOUGHT THE MAX TEMP
C            WOULD, IN GENERAL, BE SMOOTHER AND FURNISH A BETTER
C            AUGMENTATION.  HOWEVER, IN THE DEEP NIGHT, THE MIN
C            IS USED.
C
C            THE SHORTEST PROJECTIONS ARE A PARTICULAR PROBLEM BECAUSE
C            THERE IS NO MAX OR MIN THAT PERTAINS TO THE SAME START
C            TIME.  THEREFORE, AUGMT1 CYCLES AND GETS A PREVIOUS
C            CYCLE WITH A LONGER PROJECTION.  THIS MEANS THAT A 12Z 6-H
C            MOS OR EKDMOS FORECAST (THE SHORTEST MADE) IS AUGMENTED
C            WITH A 30-H MAX FROM THE PREVIOUS CYCLE AND THE AVERAGED
C            FORECAST, AN 18-H FORECAST, IS AUGMENTED WITH A 42-H
C            MAX FORECAST.
C
...
C            AND MIN USING CO-OP STATIONS WERE NOT AVAILABLE FOR
C            EKDMOS.
C
...
C            WILL BE AUGMENTED, PROVIDED ITS AUGMETING LIST OF 
C            STATIONS ARE NOT MISSING.
C
C            THE LIMIT FOR COMPUTING THE DIFFERENCES AND AVERAGING
C            IS SET AT 10.  GENERALLY, THE LIMIT WILL BE SET BY
C            MAXSTA IN U179 WHEN THE ORDERED LISTS ARE PREPARED.  NOTE
C            THAT AUGMT1 WILL HANDLE ANY SIZED LIST, BUT IT WILL 
C            ONLY USE UP TO 10.
C
C        DATA SET USE
...
C
C              IT IS ASSUMED THE SREF FORECASTS RUN TIMES ARE
...
C
            ...
            LD(4)=ITABLE(4,M+1)
CD           WRITE(KFILDO,1118)NHR,JDATE,IDPARS(12),IFGTAU,
CD    1                        JTAU1,JTAU2,HNRRUN
CD1118       FORMAT(/' AT 1118--NHR,JDATE,IDPARS(12),IFGTAU,',
CD    1              'JTAU1,JTAU2,HNRRUN',I4,I12,5I6)
C
...
C                  MMTAU=30+24*IBASMX+6*KCYCLE
                  MMTAU=30+24*IBASMX+12*KCYCLE+NHRRUN
                  ICOR=2
...
CD    3                ITEST,MMTAU,KDATE,JDATE
CD112              FORMAT(/' AT 112--',
CD    1               'NAREA,ICYCLE,IDPARS(12),',
CD    2               'MAXTAB(1,NAREA,ICC),MAXTAB(2,NAREA,ICC),IBASMX,',
CD    3               'ITEST,MMTAU,KDATE,JDATE',/,8I5,2I12)
C
C                  IF(IDPARS(12).LT.15.AND.KCYCLE.EQ.0)THEN
                  IF((IDPARS(12)+NHRRUN).LT.15.AND.KCYCLE.EQ.0)THEN
...
C                  MMTAU=18+24*IBASMX+6*KCYCLE
                  MMTAU=18+24*IBASMX+12*KCYCLE+NHRRUN
C                  MMTAU=18+24*IBASMX+12*KCYCLE
                  ICOR=1
...
C                    GET MIN TEMP RATHER THAN MAX TEMP.
               ENDIF
C
C               LD(3)=ITABLE(3,M+1)+MMTAU+NHRRUN
               LD(3)=ITABLE(3,M+1)+MMTAU
C
            ELSE
C
...
C
         IF((L.EQ.11.OR.L.EQ.7).AND.ICOR.EQ.1)THEN
            ICOR=0
         ENDIF
C
...
C
C        IF(L.EQ.14)THEN
C           CALL CIGHTC(KFILDO,DATA,NWORDS,IER)
...
C
C        IF(L.EQ.14)THEN
C           CALL CIGHTC(KFILDO,DATA1,NWORDS1,IER)
C              IER RETURN ALWAYS = 0.
C        ENDIF
C
...
C                 THIS ASSUMES SREF IS AVAILABLE EVERY 3 HOURS. 
            ELSE
C               CALL UPDAT(KDATE,-6,KDATE) 
               CALL UPDAT(KDATE,-12,KDATE) 
...
CD    1                  DATA(K),XLAPSE(K)
CD2000 FORMAT(' AUGMT1 AT 2000--K,CCALL(K),LIST(K),LISTD(K),XDATA(K),',
CD    1       'DATA(K),XLAPSE(K)',I6,2X,A8,2I8,2F8.1,F8.4)
CD2005 CONTINUE
C
...
C        BE MODIFIED.
CD     CALL TIMPR(KFILDO,KFILDO,'AFTER FILE READ     ')
      IREPL=1
C
...
C              SUM SUMS THE DELTAS BETWEEN THE TWO SETS OF DATA.
            SUMLAP=0.
C              SUMLAP SUMS THE LAPSE RATES AT THE BASE STATIONS.
C              THIS IS USED FOR AUGMETING LAMP WITH SREF; MAY NOT
C              BE RELEVANT FOR ANYTHING ELSE.
            KOUNT=0
C
...
CD    1                'IALOC(LIST(K),M),LOC',6I9)
            ...
            ENDIF
C
CD           IF(CCALL(K).EQ.'260150  '.OR.
CD    1         CCALL(K).EQ.'042319  ')THEN
C                 260150 IS AT 2450 FT
C                 042319 IS DEATH VALLEY AT -194 FT
CD              WRITE(KFILDO,2025)K,LIST(K),CCALL(K),
CD    1                           LISTD(K),CCALLD(K),CCALL(LOC),
CD    2                           LOC,CCALL(LOC),DATA(LOC),XDATA(LOC),
CD    3                           XLAPSE(LOC)
CD2025          FORMAT(/' AT 2025--K,LIST(K),CCALL(K),',
CD    1                          'LISTD(K),CCALLD(K),CCALL(LOC),',
CD    2                          'LOC,CCALL(LOC),DATA(LOC),XDATA(LOC),',
CD    3                          'XLAPSE(LOC)',/,
CD    4              2I7,1X,A8,I7,1X,A8,1X,A8,I7,1X,A8,2F7.1,F7.4)
CD           ENDIF
C
            IF(L.EQ.14)THEN
...
C                     THESE ARE NOT CONSIDERED IN THE AUGMENTATION.
                  SUM=SUM+XDATA(LOC)
     1                    -DATA(LOC)
                  SUMLAP=SUMLAP+XLAPSE(LOC)
                  KOUNT=KOUNT+1
C
                  IF(KOUNT.GE.IPRNO)GO TO 2050
C                    LIMIT AVERAGING TO 10 STATIONS.  THE STATION
C                    LIST IS ORDERED, SO THE 10 CLOSEST ONES WITH DATA
C                    WILL BE USED.
               ENDIF
C
            ELSE    
...
C
 205        CONTINUE
C
 2050       IF(KOUNT.GE.2)THEN
C                 THIS IS SET TO REQUIRE A MINIMUM OF 2 STATIONS FOR
C                 AVERAGING.  KOUNT SHOULD NORMALLY BE GE 2. 
               DELTA=SUM/KOUNT
               AVGLAP=SUMLAP/KOUNT
C
...
C                       THE MIN. 
CD                    WRITE(KFILDO,2052)K,CCALL(K),DATA(K),DELTA,ELEV(K),
CD    1                                   ELEMOD(K),AVGLAP,PREX3
CD2052                FORMAT(/' AT 2052 IN AUGMT1--K,CCALL(K),DATA(K),',
CD    1                      'DELTA,ELEV(K),ELEMOD(K),AVGLAP,PREX3',
CD    2                       I6,1X,A8,2F6.1,2F8.1,F8.3,F3.0)
C
                     IF(PREX3.EQ.0.)THEN
                        TEMP(K)=DATA(K)+DELTA
                     ELSE               
                        TEMP(K)=DATA(K)+DELTA+
     1                         (ELEV(K)-ELEMOD(K))*AVGLAP*PREX3
C                          THE ABOVE IS TO APPLY THE AVERAGE LAPSE RATE
C                          CALCULATED AT LAMP STATIONS TO ADJUST THE
C                          SREF FROM THE MODEL TERRAIN TO THE ACTUAL
C                          TERRAIN.
C                          PREX3 IS THE FRACTIONAL ADJUSTMENT.
                     ENDIF
C
...
CD 2170                FORMAT(/' ####LIST AVERAGING FAILED FOR WATER',
CD     1                       ' STATION  ',A8,A20,
...
CD 2171             FORMAT(/' ####LIST AVERAGING FAILED FOR LAND',
...
C
            ELSE
               TEMP(K)=XDATA(K)
C                 WHEN THERE IS NO LIST, XDATA( ) MUST BE PRESERVED.
C                 (ADDED 8/4/2010 PER JOHN WAGNER.)
C
            ENDIF
C
         ENDIF
...
C
 ...
 222  CONTINUE
C
CD     DO 225 K=1,MIN(IALL,ND1)
C        IALL COULD EXCEED ND1.
CD     WRITE(KFILDO,224)K,CCALL(K),LIST(K),LISTD(K),XDATA(K),
CD    1                  DATA(K),XLAPSE(K)
CD224  FORMAT(' AUGMT1 AT 224--K,CCALL(K),LIST(K),LISTD(K),XDATA(K),',
CD    1       'DATA(K),XLAPSE(K)',I6,2X,A8,2I8,2F8.1,F8.4)
C        NOTE THAT THESE VALUES MAY NOT BE SCALED, DEPENDING ON THE
C        ORDER OF SCALING ROUTINE AND AUGMT1 IN U405A.CN FILE.
CD225  CONTINUE
C
