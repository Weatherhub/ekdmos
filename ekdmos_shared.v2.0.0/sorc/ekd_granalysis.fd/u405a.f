     ...
     9                 P,FD2,FD3,FD4,FD5,FD6,U,V,FDSINS,ND2X3,
...
C        DECEMBER  2004   GLAHN   ADDED IDST( , , , ), IDPARS( , , , ),
...
C        AUGUST    2009   GLAHN   MOVED LOOP DO 3449 ABOVE PACKING
C        AUGUST    2009   GLAHN   ADDED VARIABLE JSTTST
C        SEPTEMBER 2009   GLAHN   ADDED ITYPR = 3(NEED ROUTINE)
C        OCTOBER   2009   GLAHN   ADDED CALL TO SCLCIG; ADDED CHECK ON
C                                 IER AFTER SCLSKY
C        OCTOBER   2009   GLAHN   ADDED OBS WIND SPEED TO ASCII PLOT
C        NOVEMBER  2009   GLAHN   ADDED ACCOMMODATION FOR ASCII WRITING
C                                 OF WIND SPEED FOR GMOS_PLOT
C        NOVEMBER  2009   GLAHN   INSERTED CALL TO LAPSE BEFORE AUGMT1;
C                                 ADDED AUTOMATIC ARRAY ELEMOD( ); REX3,
C                                 XLAPSE( ), ELEV( ), ELEMOD( ) TO CALL
C                                 TO AUGMT1
C        NOVEMBER  2009   GLAHN   ADDED LLMT CALL TO LAPSE.
C        NOVEMBER  2009   GLAHN   ADDED AUGMT3
C        DECEMBER  2009   GLAHN   MODIFIED CALL TO SCLCIG
C        DECEMBER  2009   GLAHN   ADDED LTAGPT( ) AND IAUG; INCREMENTED
C                                 ISTOP(3) WHEN IER NE 0 ON RETURN FROM
C                                 RETVEC; CHANGED POSTPROCESSING CALL TO
C                                 CATEQ FROM SETCFT
C        JANUARY   2010   GLAHN   REVISED TO WRITE TRUNCATED INTEGER
C                                 VALUE FOR CEILING AND VISIBILITY VICE
C                                 NINT FOR OTHER DATA; ADDED PREX4A TO
C                                 CALL TO BCD
C        JANUARY   2010   GLAHN   ADDED KFILRA, RACESS, NUMRA, PLAINT,
C                                 PLAIN, AND DATA TO CALL TO BCD5
C        MARCH     2010   GLAHN   ADDED NPASSF; CHANGED WHERE LTAGPT( )
C                                 IS INITIALIZED
C        MARCH     2010   GLAHN   ADDED NPASSF TO CALL TO BCD5
C        MARCH     2010   GLAHN   ADDED XYP( , 2) AND SDATA( )
C        MARCH     2010   GLAHN   ADDED CALL TO AUGMT4; DEFINED NPASSF,
C                                 NSMNUM, NOCEAN, SQUEZE, WTAUG, RAY,
C                                 DIFFA FROM AUGMT4 AND PUT INTO CALL TO
C                                 BCD5; ELIMINATED PREX4A
C        MARCH     2010   GLAHN   ADDED WTAUG=PREX4(NN)WHEN AUGMT3 USED
C        MARCH     2010   GLAHN   ADDED PREPROCESSOR SPOTRM; ADDED
C                                 CPNDFD TO CALL TO BCDG; ADDED
C                                 LTAGPT(K)=9999 IN DO 2881 LOOP
C        MARCH     2010   GLAHN   ADDED PLOTTING OPTION LAMP TEMP/DP;
C                                 ADDED NOPTN AND DPOWER FOR SPOTRM
C        APRIL     2010   GLAHN   ADDED CALL TO SCLVIS
C        APRIL     2010   GLAHN   ADDED PASSSP AND IN CALL TO BCD5
C        APRIL     2010   GLAHN   INITIALIZED NAPSSF, NPASSP, AND RAY
C                                 ON ENTRY RATHER THAN DATA STATEMENT
C        APRIL     2010   GLAHN   ADDED AVERAGE LAPSE FOR CUMULATIVE
C                                 PROBABILITIES.
C        APRIL     2010   GLAHN   ADDED CALL TO CIGOBC
C        MAY       2010   GLAHN   ADDED CALL TO VISOBC
C        MAY       2010   GLAHN   ADDED SWITCH AND NPASSR
C        MAY       2010   GLAHN   CORRECTED LOGIC BELOW 2862
C        MAY       2010   GLAHN   REVISED VALUES OF RELVAR FOR 2.5 KM
C        MAY       2010   GLAHN   SET IER=0 BELOW 2888
C        JUNE      2010   GLAHN   ADDED CALL TO SKYCIG
C        JUNE      2010   GLAHN   CHANGED CALL TO CONCPR; SPELL CHECK
C        JUNE      2010   GLAHN   CHANGED PRINT OF POSTPROCESSING REAL
C                                 PARAMETERS FROM 8.0 TO 8.3
C        JUNE      2010   GLAHN   MODIFIED CALL TO CONVPR
C        JULY      2010   GLAHN   ADDED NCEPNO TO CALL AND TO CALL TO
C                                 LAPSUA
C        JULY      2010   GLAHN   CAHNGED CALL TO SKYCIG; MODIFIED
C                                 DIAGNOSTIC AT 362 AND 365; ADDED
C                                 DISCRETE PROBABILITY IN DETERMINING
C                                 PROBABILITY LEVELS AND REVISED CODE;
C                                 ADDED SCALING OF SKY FOR PLOTTING
...
C              NCEPNO = NCEP MODEL NUMBER USED AS DD WHEN RETRIEVING
C                       UPPER AIR DATA FOR LAPSE IN LAPSUA.  (INPUT)
...
C                       FURNISH TO CAKSNO AND LRSMTH.  EQUIVALENCED TO
C                       IPLANT.  (INTERNAL)
C          PLANT(L,J) = 32 CHARACTERS (L=1,L3264W) (J=1,4) OF PLAIN
C                       LANGUAGE DESCRIPTION USED IN CAKSNO.
...
C                       MESHLENGTH MESHE.  A "1" MEANS WITHIN THE AREA;
C                       A "0" MEANS CLIP IT OUT.  (INPUT)
...
C                       NOTE THAT DD = 0 AND WILL BE REPLACED WITH
...
C              NPASSF = THE NUMBER OF THE PASS TO START A
C                       A SPECIFIC ACTION, LIKE INTERPOLATING INTO
C                       THE CURRENT ANALYSIS TO GET STABALIZING
C                       VALUES FOR THE SUBSEQUENT PASSES.  A VALUE OF
C                       ZERO WILL BE TREATED AS A NOP.
C              NPASSP = THE SAME AS NPASSF, SPECIFICALLY FOR SPOTRM.
C              NPASSR = THE NUMBER OF THE PASS TO SWITCH FROM VARIABLE
C                       RADII TO CONSTANT RADII, UNLESS NPASSR NE 0.
C              NSMNUM = THE NUMBER TO INSERT INTO CALL TO SMOTHG
C                       INDICATING HOW MANY SMOOTHING PASSES TO MAKE
C                       AFTER SPOTRM.
C              NOCEAN = INDICATES WHETHER (=1) OR NOT (=0) THE SPOTRM
C                       SMOTHER WILL BE EXTENDED OVER THE OCEAN.
C              SQUEZE = THE FRACTION OF THE MAJOR AXIS OF THE 
C                       SMOOTHING ELIPSE THE MINOR AXIS IS.  WHEN
C                       A CIRCLE IS USED, SQUEZE = 1.  (INPUT)
C              DPOWER = THE POWER OF THE DISTANCE TO USE IN WEIGHTING.
C                       (INPUT)
C               WTAUG = THE WEIGHT OF THE AUGMENTING DATA TO USE WTIH
C                       AUGMT4.
C                 RAY = THE MULTIPLIER OF THE DISTANCE TO THE CLOSEST
C                       STATION TO USE AS THE RADIUS OF THE SMOOTHING
C                       CIRCLE (OR MAJOR AXIS OF ELIPSE).
...
C               NOPTN = OPTION NUMBER FOR SPOTRM.  (NO CURRENT USE.)
...
C                       IN A SIMILAR MANNER, IBACKN IS USED IN LAPSUA
C                       TO GET FIELDS FOR CALCULATION OF LAPSE RATE.
C                       IBACKN IS ALSO USED TO INDICATE IN AUGMT1 HOW
C                       MANY CYCLES TO GO BACK TO FIND AUGMENTATION 
C                       DATA.  IT IS POSSIBLE THERE COULD BE A CONFLICT
C                       IN THESE USES, BUT NOT LIKELY.  (INTERNAL)
C              IBACKL = NUMBER OF 6-H CYCLES TO LOOK BACK FOR FIRST
C                       GUESS WHEN IGUESS = 3.  IBACKL = 1 MEANS 
C                       CURRENT (MOST RECENT) CYCLE PLUS THE ONE
C                       6 HOURS BEFORE).  NORMALLY, THIS IS 0 FOR
C                       DEVELOPMENT; MAY BE OTHERWISE FOR OPERATIONS.
...
C                       IN A SIMILAR MANNER, IBACKL IS USED IN LAPSUA
C                       TO GET FIELDS FOR CALCULATION OF LAPSE RATE.
C                       (INTERNAL)
...
C                           WRITING PACKED DATA.  THIS VALUE WILL
C                           USE ISEED.  WILL USE SUBROUTINE WTHOL2.
C                       3 = USE WTHOL2--FOR USE IN STUDYING ERROR
C                           OF ANALYSIS.  IP24 WILL BE USED FOR
C                           WRITING PACKED DATA.  THIS VALUE WILL
C                           USE A SEED FROM THE SYSTEM CLOCK.  WILL
C                           USE SUBROUTINE WTHOL3.
...
C                       OF STATION K (K=1,NSTA).  READ FROM RANDOM
C                       ACCESS FILE WHEN NEEDED.  (INTERNAL)
C                       (AUTOMATIC) 
...
C                       FROM GSTORE.  (INTERNAL)
C           JSTTST(J) = NUMBER OF VALUES FOR ANALYSIS THAT IF NOT MET
C                       WILL TRIGGER A DIAGNOSTIC (J=1,4).
C                       J CORRESPONDS TO NAREA VALUES 1 THROUGH 4.
C                       (INTERNAL)
C                LLMT = MINIMUM NUMBER OF PAIRS TO USE IN LAPSE TO
C                       COMPUTE A LAPSE RATE.  (THIS HAS PREVIOUSLY
C                       BEEN SET IN LAPSE TO 4, BUT A LAPSE IN LAMP
C                       WITH ONLY LAMP STATIONS AND A PAIRS LIST 
C                       COMPUTED WITH BOTH LAMP AND A LARGE NUMBER OF
C                       MAY HAVE LESS THAN 4.  DEFINED BEFORE CALLS TO
C                       LAPSE.  (INTERNAL)
C           LTAGPT(K) = HOLDS THE LTAG VALUES TO BE USED BY SUBROUTINE
C                       SETPNT.  THE USABLE ORIGINAL DATA VALUES HAVE
C                       A ZERO IN LTAGPT( ) AND THE AUGMENTED OR
C                       MISSING ONES HAVE A 9999.  ALSO USED IN BCD5
C                       WHEN WEIGHTING AUGMENTED DATA.  (INTERNAL)
C                       (AUTOMATIC)
C                IAUG = 0 BEFORE AUGMT3 OR AUGMT4 IS CALLED, AND SET
C                       TO 1 AFTER THE CALL.  THIS IS TO SIGNAL BCD5
C                       WHETHER TO USE LTAG( ) OR LTAGPT( ) IN CALL TO
C                       SETPNT.  (INTERNAL)                     
C        1         2         3         4         5         6         7 X
C
...
C
     ...
     2          TOSS(ND1),QUEST(ND1),ISDATA(ND1),XYP(ND1,2),SDATA(ND1),
     ...
     1          MPAIRS(ND1),ELEMOD(ND1),LTAGPT(ND1)
C        WHOLD( ), LTAGWH( ), LOCWH( ), ERRWH( ), MPAIRS( ),
C        LTAGPT, AND ELEMOD( ) ARE AUTOMATIC ARRAYS.
      DIMENSION P(ND2X3)
     ...
     1          EX1D(3),EX2D(3),SEEDT(10)
      DIMENSION NROUGH(4),RELVAR(4),VARTAB(2,4),JSTTST(4)
...
C        FOR THE NEXT ITERATION.
     ...
     1     MFIRST/0/
      ...
      DATA IFILL/4/
      DATA MTEST/0/
      DATA JSTTST/1000,100,10,2/
      DATA NROUGH/8,4,2,1/,
     1     RELVAR/110.,90.,70.,54./,
     2     VARTAB/1.,2.,
     3            1.,2.,
     4            1.,2.,
     5            1.,2./
...
C        PUERTO RICO RESPECTIVELY.
      DATA NSMNUM/0/,DIFFA/0./,NOCEAN/0/,SQUEZE/0./,
     1     WTAUG/0./
C
...
C        NSTA MAY BE ALTERED IN U405A, AND IS RESET ON EXIT. 
      IPOPT=IPOPT+1
C        IOPT CONTROLS PRINT.
      NPASSF=0
      NPASSP=0
      NPASSR=0
      RAY=0.
C        NAPSSF, NPASSP, AND RAY MAY BE SET FROM THE U405A.CN FILE,
C        BUT SHOULD NOT CARRY OVER FROM THE PREVIOUS CLL TO U405A.
C
CD     WRITE(KFILDO,100)
CD100  FORMAT(' ')
...
CD     WRITE(KFILDO,1045)N,ND1,NSTA,IOPTB
CD1045 FORMAT(/' AT 1045 IN U405A--N,ND1,NSTA,IOPTB',20I6)
C
...
C
      IF(NWITH.GT.0.AND.(ITYPR.LT.1.OR.ITYPR.GT.3))THEN
...
C
 ...
 273     FORMAT('    PREPROCESSING  SUBROUTINE',I2,' AND FILE NAME:   ',
     1                              A6,I3,1X,A60,/,2I8,F8.3,2I8,3F8.3)
      ENDIF
C
 274  CONTINUE
C  
...
C         
      ENDIF
C
C        FILL LTAGPT( ) WITH MISSINGS.  AUGMT3, WHICH
C        IS CALLED FOR EACH INPUT (I.E., LEVEL, CYCLE),
C        AND IF ANY ONE HAS A GOOD VALUE IT EVENTUALLY GETS
C        INTO XDATA(1, , ) FOR ANALYSIS.  LTAGPT(K) INDICATES
C        THE INITIAL, NOT AUGMENTED, GOOD VALUES.
C
      DO 2857 K=1,ND1
      LTAGPT(K)=9999
 2857 CONTINUE
...
C        ALLOCATE XDATA(NSTA,NORUNS+1,LEVELS).  THE PROBABILITY
C        LEVELS OF A VARIABLE IN EKDMOS IS INDICATED BY IDPARS(6).
C        THE LEVELS WITH CUMULATIVE OR DISCRETE PROBABILITIES FOR
C        CATEGORICAL VARIABLES LIKE CEILING HEIGHT, VISIBILITY, AND
C        CLOUD AMOUNT ARE INDICATED BY 1, 2, OR 3 IN IDPARS(3) AND
C        A CORRESPONDING CHANGE IN THRESH( ).
C
      NS=N
...
C        REMAIN = NS = N.
C
      IF(NS.EQ.NPRED)GO TO 2863
C        THIS IS THE END OF THE LIST; ONLY ONE IN SEQUENCE.
      NE=N+1
C         
 2862 IF(IDPARS(1,NE).EQ.IDPARS(1,NS).AND.
     1   IDPARS(2,NE).EQ.IDPARS(2,NS).AND.
     2   IDPARS(4,NE).EQ.IDPARS(4,NS).AND.
     3   IDPARS(12,NE).EQ.IDPARS(12,NS))THEN
C           THE CCC, FFF, DD, AND TAU ARE THE SAME.  ALL THAT IS
C           NOW REQUIRED IS THAT B = 1, 2, OR 3, OR A PROBABILITY
C           LEVEL IS INDICATED IN IDPARS(6, ).
C
         IF(((IDPARS(6,NE).GT.0.AND.IDPARS(6,NE).LT.100).AND.
     1       (IDPARS(6,NS).GT.0.AND.IDPARS(6,NS).LT.100).AND.
     2       (IDPARS(6,NS).NE.IDPARS(6,NE))).OR.
     3       (IDPARS(3,NS).NE.0.AND.IDPARS(3,NS).EQ.IDPARS(3,NE).AND.
     4       THRESH(NS).NE.THRESH(NE)))THEN 
C
            IF(NE.EQ.NPRED)THEN
...
C
      ELSE
         NE=NE-1
C           NE IS THE END OF THE PROBABILITY SEQUENCE.  NOTE THAT
C           THE PROBABIITY LEVELS ARE NOT TESTED FOR INCREASING
C           OR DECREASING; THE LEVELS DON'T HAVE TO BE IN ORDER
C           FOR A CUMULATIVE LAPSE TO BE COMPUTED, BUT THEY MAY
C           NOT BE ABLE TO BE CHECKED FOR CONSISTENCY IF THEY ARE
C           NOT IN ORDER.
      ENDIF 
C
 2863 LEVELS=NE-NS+1
...
C
CCC      WRITE(KFILDO,9876)J,LEVELS,L,NORUNS,NS,M,(ID(MMM,M),MMM=1,4)
CCC 9876 FORMAT(/' AT 9876--J,LEVELS,L,NORUNS,NS,M,(ID(MMMM,M),MMM=1,4)',
CCC     1        6I4,4I12)
C
...
C        THE DATE/TIME IS DECREASED BY NHRRUN(L) HOURS. 
      ITAU=0
      MCAT=0
C        NDATE, MDATE, ITAU, AND MCAT SHOULD NOT BE NEEDED BY RETVEC.
...
C
CCCC      WRITE(KFILDO,9998)(K,CCALL(K,1),XDATA(K,L+1,J),K=1,NSTA)
CCCC 9998 FORMAT(/' AT 9998 IN U405A--(K,CCALL(1,K),XDATA(K,L+1,J),',
CCCC     1        'K=1,NSTA)',/,('     ',I6,2X,A8,F10.3))
C
      IF(IER.NE.0)THEN
...
C
CCCC      WRITE(KFILDO,9996)IER,J,L,NDATE,MDATE
CCCC 9996 FORMAT(/' IN U450 OUT OF OPTX,IER,J,L,NDATE,MDATE--',3I5,2I12)
C
         IF(IER.EQ.120)THEN
            IER=0
...
C              ONE PER FAILURE. 
            WRITE(KFILDO,2870)
 2870       FORMAT('     THE MISSING DATA ARE FROM THE CURRENT',
...
C              AVAILABLE.
            WRITE(KFILDO,2871)
 2871       FORMAT('     HOWEVER, THE MISSING DATA ARE FROM A',
...
C           THIS AUGMT1 IS FOR MOST AUGMENTATION, EXCEPT LAMP.
         IF(PREX3(NN).NE.0.)THEN
C              WHEN THE PREPROCESSING VARIABLE PREX3 IS NE 0, THE
C              LAPSE CALCULATED FROM ONLY THE BASE (LAMP) VARIABLE
C              ARE USED TO ADJUST THE AUGMENTING DATA FROM THE 
C              MODEL TERRAIN TO THE ACTUAL TERRAIN.  HAVE TO 
C              READ THE MODEL TERRAIN INTERPOLATED TO STATIONS.
            LD(1)=409390000
            LD(2)=0
            LD(3)=0
            LD(4)=200
            ...
            ITAU=0
            MCAT=0
C              MDATE, NDATE, ITAU, AND MCAT SHOULD NOT BE NEEDED 
C              RETVEC.
            CALL RETVEC(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  LD,LDPARS,JD(1,M),ITAU,
     2                  NDATE,MDATE,CCALL,ISDATA,ELEMOD,ND1,NSTA,
     3                  ICALLD,CCALLD,IPACK,IWORK,DATA,ND5,
     4                  LSTORE,ND9,LITEMS,CORE,ND10,
     5                  NBLOCK,NFETCH,
     6                  IS0,IS1,IS2,IS4,ND7,
     7                  L3264B,L3264W,IER)
C
            RLAP=R(1,1)*RSTAR(1,1)
C              RLAP IS THE RADIUS TO USE IN LAPSE.  MGUESS IS NOT YET 
C              KNOWN, SO USE 1, THE FIRST OPTION.
C
C              THIS CALL TO LAPSE IS TO CALCULATE THE LAPSE BASED
C              ON ONLY NON-AUGMENTED STATIONS.  PUT IN TO ACCOMMODATE
C              LAMP AUGMENTED BY SREF.  NOT ALL DATA ARE AVAILABLE
C              YET.  TREAT AS ONLY ONE LEVEL AND ONE RUN.  IF THERE
C              ARE MULTIPLE RUNS, THE FIRST ONE WILL BE COMPUTED
C              AGAIN.  THIS MAY NOT BE DESIRABLE FOR MULTIPLE
C              PROBABILITY LEVELS.
C         
            LLMT=2
C              ONLY TWO PAIARS ARE NECESSARY TO COMPUTE A LAPSE.
            IAUG=0
C              FOR SAFETY, SET IAUG = 0.  WEIGHTING COULD BE USED IN
C              AUGMT1 BUT HASN'T BEEN.  FOR AUGMT1 IT IS LIKELY THE
C              AUGMENTED DATA ARE OF NEARLY THE SAME QUALITY AS THE
C              BASE DATA AND SHOULD BE USED FULL FORCE.
            CALL LAPSE(KFILDO,KFILLP,IP14,CCALL,NAME,LNDSEA,ELEV,
     1                 NOPAR,LOCPAR,XDATA(1,1,1),XLAPSE,MPAIRS,NSTA,ND1,
     2                 NXL,NYL,XPL,YPL,RLAP,XP,YP,
     3                 ELCORR(1,1),NPASS,WTRUNL,1,1,
     4                 IALOC,ADIST,AELEV,ND13,
     5                 SEALND,TELEV,NXE,NYE,MESHE,P,NX,NY,MESH,
     6                 IBKPN,LPNO,LLMT,LAPFG,1,N4P,ISTOP,JER)
...
C
     ...
     2               CCALL,NAME,XDATA(1,L+1,J),LNDSEA,
     3               XLAPSE,ELEMOD,ELEV,NSTA,ND1,IPREX2(NN),PREX3(NN),
     4               LSTORE,ND9,LITEMS,
     ...
     8               L3264B,ISTOP,IER)
C
      ELSEIF(PREPRO(NN).EQ.'AUGMT3')THEN
C           THIS AUGMT3 IS FOR AUGMENTING LAMP WITH SREF.  PRECX3 SHOULD
C           BE ZERO OR ONE.  (ABANDONED AS OF MARCH 18, 2010, FOR CIG;
C           STILL USED FOR TEMP/DP.)
         IF(PREX3(NN).NE.0.)THEN
C              WHEN THE PREPROCESSING VARIABLE PREX3 IS NE 0, THE
C              ADJUSTMENT CALCULATED IN AUGMT3 NEEDS THE ELEVATIONS
C              OF SREF AT THE STATION LOCATIONS.  RETRIEVE THEM
C              IN ELEMOD( ).
            LD(1)=409390000
            LD(2)=0
            LD(3)=0
...
C              RETVEC.
     ...
     6                  IS0,IS1,IS2,IS4,ND7,
     7                  L3264B,L3264W,IER)
            IF(IER.NE.0)ISTOP(3)=ISTOP(3)+1
         ENDIF
C
         CALL AUGMT3(KFILDO,KFIL10,IP16,KFILPR(NN),PREPFL(NN),
     1               NAREA,JDATE(4),
     2               MDATE,ID(1,M),IDPARS(1,M),JD(1,M),IBACKN,NHRRUN(L),
     3               CCALL,NAME,PLAIN(M),XDATA(1,L+1,J),LNDSEA,
     4               XLAPSE,ELEMOD,ELEV,LTAGPT,NSTA,ND1,
     5               IPREX2(NN),PREX3(NN),
     6               LSTORE,ND9,LITEMS,
     7               IS0,IS1,IS2,IS4,ND7,
     8               IPACK,IWORK,DATA,ND5,
     9               CORE,ND10,LASTL,LASTD,NBLOCK,NSTORE,NFETCH,
     A               L3264B,ISTOP,IER)
         IAUG=1
C           IAUG IS AN INDICATOR IN BCD THAT AUGMT3 HAS BEEN CALLED 
C           LTAGPT( ) IS FILLED IN BCD5 AND SHOULD BE USED VICE LTAG( )
C           WHEN SETPNT OR CATEQ IS CALLED.
         WTAUG=PREX4(NN)
C
      ELSEIF(PREPRO(NN).EQ.'AUGMT4')THEN
C           THIS AUGMT4 IS FOR AUGMENTING LAMP WITH DATA FROM
C           THE PREVIOUS PASS.  THE ONLY THING DONE HERE IS TO DEFINE
c           SOME VARIABLES FOR EASIER RECOGNITION FOR USE IN BCD5.
C           LTAGPT( ) IS FILLED IN BCD5, AND IAUG SET = 1.
         NPASSF=NCAT(NN)
         WTAUG=PREX4(NN)
C
      ELSEIF(PREPRO(NN).EQ.'SWITCH')THEN
C           THIS SWITCH IS FOR SWITCHING FROM VARIABLE RADII
C           TO THE CONSTANT RADII IN THE U405A.CN FILE
         NPASSR=NCAT(NN)
C
      ELSEIF(PREPRO(NN).EQ.'SPOTRM')THEN
C           THIS SPOTRM IS FOR SMOOTHING TO REMOVE "HOLES" OR
C           "SPOTS."  THE ONLY THING DONE HERE IS TO DEFINE SOME
C           VARIABLES FOR EASIER RECOGNITION FOR USE IN BCD5.
C           LTAGPT( ) ALSO HAS TO BE FILLED IF IT HASN'T BEEN
C           ALREADY.  WHEN IAUG = 1 FROM EITHER AUGMT3 OR AUGMT4,
C           LTAGPT( ) WILL HAVE BEEN INITIALIZED.  IAUG = 0 FOR
C           ANY OTHER ANALYSIS, EITHER WITH OR WITHOUT AUGMT1.
         NPASSP=NCAT(NN)
         NSMNUM=NSCALE(NN)
         DIFFA=IPREX1(NN)
         NOPTN=NINT(CONST(NN))
...
C
         IF(IAUG.EQ.0)THEN
C              WHEN IAUG NE 0, LTAGPT( ) HAS ALREADY BEEN INITIALIZED
C
            IF(XDATA(K,L+1,J).LT.9998.)THEN
C                 LTAGPT( ) WAS INITIALIZED TO 9999.  ANY GOOD DATUM,
C                 FOR ANY L OR J, WILL BE SO INDICATED AND EVENTUALLY BE
C                 USED IN THE ANALYSIS.
               LTAGPT(K)=0
            ENDIF
C
         ENDIF
...
C           IS UNCHANGED.  NOTE THAT THIS CALL IS FOR THE VECTOR
C           DATA AND THE GRID VARIABLES (I.E., MESH) ARE NOT USED
C           IN SCLSKY, AND NX MUST = 0.
C
         IF(IER.NE.0)THEN
            JER=1
C
...
C
         ENDIF
C
      ELSEIF(PREPRO(NN).EQ.'SCLCIG')THEN
C           CEILING IS SCALED TO FRACTIONS OF CATEGORIES.
C           THERE ARE 8 CIG CATEGORIES.
         NZERO=0
C            NZERO USED TO SIGNAL VECTOR DATA TO SCLCIG.
         ITRPX=0
C           SEVERAL VARIABLES PERTAINING TO GRIDS, INCLUDING
C           ITRPX, ARE NOT USED IN THIS CALL TO SCLCIG.  THE
C           USE OF VECTOR DATA IS SIGNALED BY NX=NZERO=0.
         LD(1)=ITABLE(1,2)+IDPARS(4,M)
         LD(2)=ITABLE(2,2)
         LD(3)=ITABLE(3,2)+IDPARS(12,M)+NHRRUN(L)
C           THE PROJECTION IS INCREASED BY NHRRUN(L) HOURS.
         ...
         MESHX=99  
C           MESH HAS NOT BEEN DEFINED UP TO THIS POINT, AND
C           IS NOT USED IN THIS CALL TO SCLCIG FOR VECTOR DATA.
C           MESHX IS SET TO 99 HERE AS A SAFEGUARD FOR SCLCIG.
C
         CALL SCLCIG(KFILDO,KFIL10,MDATE,LD,LDPARS,JD(1,M),
     1               XDATA(1,L+1,J),FD2,FD3,ND1,NSTA,
     2               NCAT(NN),CONST(NN),NSCALE(NN),
     3               LSTORE,ND9,LITEMS,
     4               IS0,IS1,IS2,IS4,ND7,
     5               IPACK,IWORK,DATA,ND5,
     6               CORE,ND10,NBLOCK,NFETCH,
     7               L3264B,ISTOP,IER)
C           THE ABOVE CALL TO SCLCIG WILL SCALE THE 
C           VALUES IN EACH CEILING HEIGHT CATEGORY TO THE FULL
C           RANGE OF THE CATEGORY ACCORDING TO THE LOWEST
C           AND HIGHEST PROBABILITY OCCURRING FOR THIS CASE OVER
C           THE ANALYSIS AREA.  ERROR CHECKING AND INCREMENTING
C           OF ISTOP( ) IS TAKEN CARE OF IN SCLCIG.  NOTE
C           THAT THIS CALL IS FOR THE VECTOR DATA, AND THE GRID
C           VARIABLES (I.E., MESH) ARE NOT USED IN SCLCIG, AND
C           NX MUST = 0.
C
         IF(IER.NE.0)THEN
            JER=1
...
C
         ENDIF
C
      ELSEIF(PREPRO(NN).EQ.'SCLVIS')THEN
C           VISIBILITY IS SCALED TO FRACTIONS OF CATEGORIES.
C           THERE ARE 7 CIG CATEGORIES.
         NZERO=0
C            NZERO USED TO SIGNAL VECTOR DATA TO SCLCIG.
         ITRPX=0
C           SEVERAL VARIABLES PERTAINING TO GRIDS, INCLUDING
...
C           MESHX IS SET TO 99 HERE AS A SAFEGUARD FOR SCLCIG.
C
         CALL SCLVIS(KFILDO,KFIL10,MDATE,LD,LDPARS,JD(1,M),
     1               XDATA(1,L+1,J),FD2,FD3,ND1,NSTA,
     2               NCAT(NN),CONST(NN),NSCALE(NN),
     3               LSTORE,ND9,LITEMS,
     4               IS0,IS1,IS2,IS4,ND7,
     ...
     6               CORE,ND10,NBLOCK,NFETCH,
     7               L3264B,ISTOP,IER)
C           THE ABOVE CALL TO SCLVIS WILL SCALE THE 
C           VALUES IN EACH VISIBILITY CATEGORY TO THE FULL
C           RANGE OF THE CATEGORY ACCORDING TO THE LOWEST
C           AND HIGHEST PROBABILITY OCCURRING FOR THIS CASE OVER
C           THE ANALYSIS AREA.  ERROR CHECKING AND INCREMENTING
C           OF ISTOP( ) IS TAKEN CARE OF IN SCLVIS.  NOTE
C           THAT THIS CALL IS FOR THE VECTOR DATA, AND THE GRID
C           VARIABLES (I.E., MESH) ARE NOT USED IN SCLVIS, AND
C           NX MUST = 0.
C
         IF(IER.NE.0)THEN
            JER=1
...
C
         ENDIF
C
      ELSEIF(PREPRO(NN).EQ.'CIGOBC')THEN
         CALL CIGOBC(KFILDO,XDATA(1,L+1,J),NSTA,IER)
C           THE ABOVE CALL TO CIGOBC WILL PUT CEILING HEIGHTS IN 
C           HUNDREDS OF FEET INTO SCALED CATEGORIES.  THERE IS
C           NO NON ZERO ERROR RETURN.
C
      ELSEIF(PREPRO(NN).EQ.'VISOBC')THEN
         CALL VISOBC(KFILDO,XDATA(1,L+1,J),NSTA,IER)
C           THE ABOVE CALL TO VISOBC WILL PUT VISIBILITY IN MILES
C           INTO SCALED CATEGORIES.  THERE IS NO NON ZERO ERROR
C           RETURN.
C
      ENDIF
C
...
C
CCCC      WRITE(KFILDO,2878)(K,CCALL(K,1),XDATA(K,2,1),K=1,NSTA)
CCCC 2878 FORMAT(/' AT 2878 IN U405A--(K,CCALL(1,K),XDATA(K,L+1,J),',
CCCC     1        'K=1,NSTA)',/,('     ',I6,2X,A8,F10.3))
C
...
C           IN ANY PHASE OF THE ANALYSIS OR WRITTEN FOR PLOTTING.
         LTAGPT(K)=9999
C           WHILE THIS WAS A GOOD VALUE, IT WAS OUTLAWED BY IQUAL(K, ),
C           SO LTAGPT(K) HAS TO BE SET INDICATING AN AUGMENTED VALUE
C           WILL BE USED.         
 2881    CONTINUE
C
 2882    CONTINUE
...
C
CCCC         IF(CCALL(K,1).EQ.'KDCA    ')THEN
CCCC            WRITE(KFILDO,2283)CCALL(K,1),QUALST(K)
CCCC 2283       FORMAT(' IN U405A AT 2283--CCALL(K),QUALST(K)',A8,F10.2)
CCCC         ENDIF
C
      ELSE
...
C
      IER=0
C        IER COULD BE NON-ZERO FROM A ROUTINE ABOVE.
C
...
C*******************************************************ONLY 4*********
            LLMT=4
C              ONLY FOUR PAIARS ARE NECESSARY TO COMPUTE A LAPSE.
            CALL LAPSE(KFILDO,KFILLP,IP14,CCALL,NAME,LNDSEA,ELEV,
     ...
     4                 IALOC,ADIST,AELEV,ND13,
     5                 SEALND,TELEV,NXE,NYE,MESHE,P,NX,NY,MESH,
     6                 IBKPN,LPNO,LLMT,LAPFG,MGUESS,N4P,ISTOP,JER)
            IF(JER.NE.0)GO TO 500
C              A DIAGNOSTIC WILL HAVE BEEN WRITTEN IN LAPSE WHEN IT
C              IS ENTERED WITH LAPFG NE 0 OR 1.  THIS IS A SAFETY.
C
...
C              BE DIFFERENT.
            CALL LAPSUA(KFILDO,KFIL10,IP14,ID(1,N),IDPARS(1,N),NCEPNO,
     1                  CCALL,NAME,LNDSEA,
...
C
     ...
     1               NX,NY,0.)
      ENDIF
C
C        FLTAG WITH RMAX = 0. AND LTAG( ) WAS USED IN FSTGS5.  NOW
...
C
 ...
 300  CONTINUE
C
C        CHECK NUMBER OF STATIONS AGAINST A VALUE THAT
C        VARIES BY NAREA, SET BY DATA STATEMENT.
C
      IF(JSTAAN.EQ.0)THEN
         WRITE(KFILDO,310)
 310     FORMAT(/' ****NO STATIONS WITH DATA AVAILABLE',
     1           ' WITHIN THE ANALYSIS AREA.')
         GO TO 600
C
      ELSEIF(JSTAAN.LT.JSTTST(NAREA))THEN
...
C        X AND Y DIRECTIONS; U( ) AND V( ) ARE USED FOR THIS PURPOSE.
...
CCCC            WRITE(KFILDO,3423)K,MESH,MESHB,SINLAT,FAC,CORMSH,WNDGRD,
CCCC     1                       U(K),V(K)
CCCC 3423       FORMAT(' IN U405A--K,MESH,MESHB,SINLAT,FAC,CORMSH,WNDGRD,',
CCCC     1             'U(K),V(K)',3I4,6F10.3)
...
C              THE CHECK ON LE RATHER THAN EQ IS MADE BECAUSE THE
...
C
      ELSEIF(NWITH.NE.0.AND.(ITYPR.EQ.2.OR.ITYPR.EQ.3))THEN
...
C
     ...
     6                  ERRANL,ID2,IDIM,L3264B,L3264W,ISTOP,IER)
         ELSE
            CALL WTHOL3(KFILDO,IP24,ID(1,M),NDATE,CCALL,NAME,XDATA,ND1,
     1                  LTAG,LNDSEA,ELEV,WHOLD,LTAGWH,LOCWH,XP,YP,
     2                  DUMCAL,XLAPSE,NSTA,
     3                  CPNDFD,SEALND,TELEV,NXE,NYE,MESHE,NCLIPY,
     4                  NX,NY,MESH,SEED,NWITH,IEXTRA,
...
C
C           LAPSE REATES ARE USUALLY THOUSANDTHS.  SCALE FOR 
C           PLOTTING.
C
         DO 3448 K=1,NSTA
...
C
         IF(IP18.NE.0.AND.JP(3,M).NE.0)THEN
            WRITE(IP18,3449)
 3449       FORMAT(/' COMPUTED LAPSE RATES WRITTEN TO FILE KFILOV.')
         ENDIF
C
...
C
      ...
      CALL BCD5(KFILDO,KFILOG,KFILRA,RACESS,NUMRA,KFILOV,KFILQC,
     1          IP14,IP16,IP17,IP18,IP19,IP20,IP21,IP22,I405ADG,
     2          ID(1,M),IDPARS(1,M),JP(1,M),IVRBL,MODNO,
     3          ISCALD(M),JFIRST(IVRBL),
     4          NDATE,JDATE,XDATA(1,1,J),CCALL,NAME,XP,YP,XYP,
     5          XPL,YPL,XPE,YPE,SDATA,
     5          TOSS,QUEST,LTAG,LNDSEA,ELEV,QUALST,XLAPSE,
     6          VRAD,NSTA,ND1,ISETP,LTAGPT,IAUG,ILS,
     7          WTWTL,WTLTW,HGTTHA,HGTTHB,
     8          P,FD2,FD3,FD4,FD5,ND2X3,NX,NY,
     9          IPACK,DATA,IWORK,ND5,MINPK,
     A          TELEV,SEALND,CPNDFD,NXE,NYE,MESHE,EMESH,
     B          IS0,IS1,IS2,IS4,ND7,
     C          ITABLE(1,2),IPLAIN(1,1,M),PLAIN(M),PLAINT,PLAIN,
     D          NAREA,ALATL,ALONL,NPROJ,ORIENT,
     E          MESH,MESHB,MESHL,XLAT,
     F          MSHPAS(1,MGUESS),ER1(1,MGUESS,JDATE(2)),
     G          NTYPE(1,MGUESS),B(1,MGUESS),CSTSM,R(1,MGUESS),
     H          RSTAR(1,MGUESS),LNDWAT(1,MGUESS),ITRPLQ(1,MGUESS),
     I          IALGOR(1,MGUESS),ELCORR(1,MGUESS),IBKPN,BK(1,MGUESS),
     J          ELCORU(1,MGUESS),RWATO,RWATI,IVRAD,
     K          TLOD,SETLOD,THID,SETHID,CONSTD,NSCALD,EX1D,EX2D,
     L          IALOC,ADIST,AELEV,ND13,N4P,NSHLN,
     M          NSMTYP,U,V,WNDWT(1,MGUESS),WNDGRD,WNDTHR,WNDTRN,
     N          NPRT,JPRT,NTDL,JTDL,NPASS,MGUESS,
     O          NPASSP,NPASSF,NSMNUM,NPASSR,
     P          NOPTN,DIFFA,NOCEAN,SQUEZE,DPOWER,WTAUG,RAY,
     Q          CINT,ORIGIN,SMULT,SADD,TITLE,IOPT,POSTDS,
     R          JTOTBY,JTOTRC,MTOTBY,MTOTRC,ITOTBY,ITOTRC,NOTOSS,
     S          L3264B,L3264W,ISTOP,IER)
...
C
      ELSEIF(IP24.NE.0.AND.(ITYPR.EQ.2.OR.ITYPR.EQ.3).
     1                 AND.NWITH.NE.0)THEN
C
...
C           ARE FOUR DIGITS WHICH WAS THE UUUU POSITION.  THIS ASSUMES
...
C                 PROVIDED ROUTINE CAKSNO IS PROVIDED IN POSTAR( ).
               JBDUM=0
               JTDUM=0
C                 THE NUMBER OF INPUT BYTES OR RECORDS FROM THIS 
C                 SPECIALIZED CLIPPING GRID ARE NOT COUNTED.
...
C        (THIS IS FOR CHECKOUT AND HAS NOT BEEN IMPLEMENTED IN ALL
C        THE SUBROUTINES.)
C
 3522 DO 354 NN=1,3
C
...
C
         ELSEIF(POSTAR(NN).EQ.'ORVWSM')THEN
            IOCEXT=NINT(EX1A(NN))
            IOCINC=NINT(EX2A(NN))
            CONSTB=NSCALA(NN)
            SHOREA=TLOA(NN)
            SHOREB=SETLOA(NN)
            CALL ORVWSM(KFILDO,KFIL10,P,NX,NY,MESH,
     1                  SEALND,NXE,NYE,MESHE,NPROJ,
     2                  SHOREA,SHOREB,THIA(NN),SETHIA(NN),
     3                  CONSTA(NN),CONSTB,IOCEXT,IOCINC,
...
C              IPREX1( ) AND IPREX2( ) AS RAYS FROM THE CENTER,
C              STOPPING WHEN A MISSING OR LAND IS ENCOUNTERED.
C              SMOOTHING IS LIGHT NEAR SHORE, AND HEAVY AWAY FROM
C              THE COAST.
C
...
C
         ELSEIF(POSTAR(NN).EQ.'CATEQ ')THEN 
C              THIS IS TO MAKE SURE THE CLOSEST GRIDPOINT TO A STATION
C              MATCHES THE CATEGORICAL VALUE.
C
            IF(IAUG.NE.0)THEN 
C                 AUGMENTING HAS BEEN DONE SO THAT LTAGPT( ) SHOULD
C                 BE USED TO INDICATE BASE VALUES.                       
               CALL CATEQ(KFILDO,P,FD4,FD3,NX,NY,
     1                    CCALL,NAME,XP,YP,XDATA,LTAGPT,NSTA,IER)
            ELSE
C                 AUGMENTING HAS NOT BEEN DONE, SO USE LTAG( ) TO
C                 INDICATE BASE VALUES.
               CALL CATEQ(KFILDO,P,FD4,FD3,NX,NY,
     1                    CCALL,NAME,XP,YP,XDATA,LTAG,NSTA,IER)
            ENDIF
C
...
C
         ELSEIF(POSTAR(NN).EQ.'CONCPR')THEN
            CALL CONCPR(KFILDO,KFIL10,IP16,ID(1,M),IDPARS(1,M),
     1                  P,NX,NY,
     2                  LSTORE,ND9,LITEMS,NDATE,
     3                  IS0,IS1,IS2,IS4,ND7,
     4                  IPACK,IWORK,DATA,ND5,
     5                  CORE,ND10,NBLOCK,NFETCH,NSLAB,
     6                  IPLAIN(1,1,M),L3264W,L3264B,ISTOP,JER,IER)
C              THIS IS FOR MAKING CUMULATIVE CEILING PROBABILITY
C              GRIDS CONSISTENT (NON DECREASING FOR HIGHER THRESHOLDS).
C              THE "CONSISTENT" GRID WILL BE WRITTEN BY CONCPR WITH
C              IDPARS(15) = 1 SO IT WILL BE AVAILABLE FOR CHECKING
C              WITH THE NEXT HIGHER LEVEL.              
C
         ELSEIF(POSTAR(NN).EQ.'CONVPR')THEN
            CALL CONVPR(KFILDO,KFIL10,IP16,ID(1,M),IDPARS(1,M),
     1                  P,NX,NY,
     2                  LSTORE,ND9,LITEMS,NDATE,
     3                  IS0,IS1,IS2,IS4,ND7,
     4                  IPACK,IWORK,DATA,ND5,
...
C              GRIDS CONSISTENT (NON DECREASING FOR HIGHER THRESHOLDS).  
C
         ELSEIF(POSTAR(NN).EQ.'CONSPR')THEN
            CALL CONSPR(KFILDO,KFIL10,IP16,ID(1,M),IDPARS(1,M),
     1                  P,NX,NY,
     2                  LSTORE,ND9,LITEMS,NDATE,
     3                  IS0,IS1,IS2,IS4,ND7,
     4                  IPACK,IWORK,DATA,ND5,
     5                  CORE,ND10,NBLOCK,NFETCH,NSLAB,
     6                  IPLAIN(1,1,M),L3264W,L3264B,ISTOP,JER,IER)
C              THIS IS FOR MAKING DISCRETE SKY COVER PROBABILITY
C              GRIDS CONSISTENT. (TOTAL CANNOT BE GT 100 PERCENT).  
C
...
C
         ELSEIF(POSTAR(NN).EQ.'SKYCIG')THEN
            CALL SKYCIG(KFILDO,KFIL10,ID(1,M),IDPARS(1,M),
     1                  P,NX,NY,TLOA(NN),SETLOA(NN),
     2                  LSTORE,ND9,LITEMS,NDATE,
     3                  IS0,IS1,IS2,IS4,ND7,
     4                  IPACK,IWORK,DATA,ND5,
     5                  CORE,ND10,NBLOCK,NFETCH,NSLAB,
     6                  L3264B,ISTOP,IER)
C              THIS IS FOR MAKING SURE THE SKY GRID IS AT LEAST 
C              EQUAL TO CONST(NN) WHEN THERE IS A CEILING.
C
         ELSE
C
...
C
         IF(IER.NE.0)THEN
            WRITE(KFILDO,362)(ID(MMM,M),MMM=1,4),KFILRA(JJ),IER,MINPK
...
C
            IF(IER.NE.0)THEN
               WRITE(KFILDO,365)(LD(MMM),MMM=1,4),KFILRA(JJ),IER,MINPK
...
C                 NECESSARY.
     ...
     1                   1.,1.,2,DUM,DUM,IER)
C
            ELSEIF(ITABLE(1,2)/100.EQ.2083503.AND.ICOMPT(M).EQ.0)THEN
C                 THIS IS A SKY COVER PROBABILITY.  NEEDS TO BE 
C                 SCALED TO BE PLOTTED.  THE ICOMPT( ) IS LIKELY NOT
C                 NECESSARY.
               CALL POST(KFILDO,XDATA(1,1,J),NSTA,0.,0.,1.,
     1                   1.,1.,2,DUM,DUM,IER)
...
C
     ...
     2             ID(1,M)/1000.EQ.224325.OR.
     3             ID(1,M)/1000.EQ.724330)THEN
...
C
            DO 380 K=1,NSTA
C
C              THIS SECTION FOR WIND.  SPEED AND DIRECTION CAN BE
C              PLOTTED TOGETHER OR SEPARATELY IF ONE OR THE OTHER
C              IS MISSING.  DATA VALUES ARE TO THE CLOSEST INTEGER
C              SO MUST BE SCALED TO WHAT IS DESIRED.
C
...
C                       MISSING. 
                     IF(NINT(XDATA(K,1,J)).GT.99)XDATA(K,1,J)=99 
C                       THE ABOVE TO KEEP SPEED VALUES GT 99 TO NOT
C                       WRITE AS *'S THAT GMOS_PLOT WON'T HANDLE.  
...
C
               ENDIF
C
C                 THIS SECTION FOR CEILING HEIGHT OR VISIBILITY WHERE
C                 THE VALUES ARE CATEGORIES.  DATA VALUES ARE THE
C                 TRUNCATED INTEGER VALUE.  THAT IS, ANY VALUE IN THE
C                 RANGE 5.0 TO 5.99 IS CATEGORY 5.  NOTE ONLY PLOTTING
C                 THE ORIGINAL BASE VALUES (NOT AUGMENTED).
C
            ELSEIF(ID(1,M)/1000.EQ.228071.OR.
     1             ID(1,M)/1000.EQ.228131.OR.
     2             ID(1,M)/1000.EQ.728000.OR.
     3             ID(1,M)/1000.EQ.728100)THEN
C
               IF(IAUG.NE.0)THEN
C                    AUGMENTING HAS BEEN DONE SO THAT LTAGPT( ) SHOULD
C                    BE USED TO INDICATE BASE VALUES.  NOTE THAT THIS
C                    TRUNCATES, NOT ROUNDS (A SCALED CATEGORY OF
C                    7.00 TO 7.99 BECOMES CATEGORY 7).
C
                  IF(XDATA(K,1,J).LT.9998.9.AND.LTAGPT(K).EQ.0)THEN 
                     WRITE(KFILVO,379)-STALON(K),STALAT(K),
     1                                  INT(XDATA(K,1,J))
                  ENDIF
C
               ELSE
C                    AUGMENTING HAS NOT BEEN DONE SO THAT LTAG( )
C                    SHOULD BE USED TO INDICATE BASE VALUES.                       
C
                  IF(XDATA(K,1,J).LT.9998.9.AND.LTAG(K).EQ.0)THEN 
                     WRITE(KFILVO,379)-STALON(K),STALAT(K),
     1                                  INT(XDATA(K,1,J))
                  ENDIF
C
...
C              THIS SECTION FOR LAMP TEMPERATURE AND DEW POINT
C              WHERE ONLY PLOTTING THE ORIGINAL BASE VALUES
C              OVER LAND (NOT AUGMENTED), BUT SREF OVER WATER.
C
            ELSEIF(ID(1,M).EQ.222020005.OR.
     1             ID(1,M).EQ.223020005)THEN
C
               IF(XDATA(K,1,J).LT.9998.9)THEN
C
                  IF(LNDSEA(K).GE.3.5)THEN
CCCC                     WRITE(KFILDO,2795)K,CCALL(K,1),LTAGPT(K)
CCCC 2795                FORMAT(' U405 AT 2795--K,CCALL(K,1),LTAGPT(K)  ',
CCCC     1                      I6,2X,A8,I5)
C    
                     IF(LTAGPT(K).EQ.0)THEN 
                        WRITE(KFILVO,379)-STALON(K),STALAT(K),
     1                                NINT(XDATA(K,1,J))
                     ENDIF
C
...
C                    BE BYPASSED BY USING IWRITA = 0.
                  WRITE(KFILVO,379)-STALON(K),STALAT(K),
     1                             NINT(XDATA(K,1,J))
               ENDIF
C
