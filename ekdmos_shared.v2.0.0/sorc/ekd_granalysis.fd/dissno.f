      SUBROUTINE DISSNO(KFILDO,XDATA,NVAL,TRUNC,CONST,NSCALE,IER)
C
...
C
D     CALL TIMPR(KFILDO,KFILDO,'START DISSNO        ')
      IER=0
C
      FACTOR=CONST*10.**NSCALE
D     WRITE(KFILDO,102)NSCALE,CONST,TRUNC,FACTOR
D102  FORMAT(/' AT 102 IN DISSNO--NSCALE,CONST,TRUNC,FACTOR',I4,3F10.4)
C
      DO 120 K=1,NVAL
C
...
D125  FORMAT(/,' IN DISSNO--TRUNC',F8.2,/,(15F8.2))
C
      RETURN
      END
