...
C        OCTOBER   2009   GLAHN   SET DIMENSIONS FOR 2.5 NDFD CONUS GRID
C        DECEMBER  2009   GLAHN   INCREASED ND13
C        MARCH     2010   GLAHN   DEFINED XYP( ,2), EQUIVALENCED TO XP
C                                 AND YP, AND ADDED TO CALL TO U405A
C        1         2         3         4         5         6         7 X
C
C        PURPOSE
C           DRIVER FOR PROGRAM U155.  SEE U155 COMMENTS OR PROGRAM
C           WRITEUP FOR VARIABLE DEFINITIONS.  AN ATTEMPT HAS BEEN
C           MADE TO INCLUDE ALL INFORMATION IN THIS DRIVER THAT
C           THE USER OF U155 MIGHT HAVE TO CHANGE EXCEPT WHAT IS
C           READ IN CONTROL FILES.
C
C        DATA SET USE
...
C***      PARAMETER (NBLOCK=6400/L3264B)
C***      PARAMETER (MAXSTA=500)  ALASKA
C
      PARAMETER (MAXSTA=12200)
C        ABOVE FOR CONUS MAX/MIN TEMP
C
      PARAMETER (ND1=MAX(NBLOCK,MAXSTA))
C
C***      PARAMETER (ND2=1321,
C***     1           ND3=721)
C***         ABOVE FOR ALASKA 3-KM.
C
C***      PARAMETER (ND2=1073,
C***     1           ND3=689)
C****        ABOVE FOR CONUS NDFD 5 KM GRID.
CC
...
C***     1           ND3=81)
      ...
      PARAMETER (ND2=2145,
     1           ND3=1377)
C        ABOVE FOR CONUS NDFD 2.5 KM GRID.
      PARAMETER (ND4=30)
      PARAMETER (ND6=50)
      PARAMETER (ND7=54)
      PARAMETER (ND8=10)
      ...
      PARAMETER (ND5=MAX(ND2X3,2953665))
C        2,953,665 IS REQUIRED FRO READING 2.5 KM TERRAIN.
      PARAMETER (ND12=ND2*ND3)
C        THIS ASSUMES THE ND2*ND3 GRID WILL ACCOMMODATE A 2.5-KM
C        CONUS ANALYSIS.
C
C***      PARAMETER (ND13=100000)
C***         THE ABOVE FOR ALASKA.
C
      PARAMETER (ND13=850000)
C
...
C
      ...
      EQUIVALENCE (XYP(1,1),XP(1)),(XYP(1,2),YP(1))
C
      DATA KFILDI/5/,
     1     KFILDO/12/
      DATA PLAIN/ND4*' '/ 
      ...
      DATA LKHERE/ND6*1/
C
      CALL TIMPR(KFILDO,KFILDO,'START U155           ')
      OPEN(UNIT=KFILDI,FILE='U155.CN',STATUS='OLD',IOSTAT=IOS,ERR=900)
C
     ...
     J          DIR,NGRIDC,ND11)
C
      CALL TIMPR(KFILDO,KFILDO,'END U155             ')
      STOP 155
C
 900  CALL IERX(KFILDO,KFILDO,IOS,'DRU155','900 ')
      STOP 900
C
      END
