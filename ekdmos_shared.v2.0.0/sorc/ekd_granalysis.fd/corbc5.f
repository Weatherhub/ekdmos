     ...
     2                  ILS,LNDWAT,WTWTL,WTLTW,WTAUG,IAUG,LTAGPT,
...
C        JULY      2010   GLAHN   INSERTED OPEN MP STATEMENTS IN
C                                 3 PLACES
C                                 
C        PURPOSE
...
C            THE FIRST PASS SO THAT A GRIDPOINT AFFECTED ONLLY ON 
...
C            APPLYING IT ON THE CURRENT PASS, OR IT WIL BE APPLIED
...
C                IAUG = 0 IF WEIGHTING IS NOT TO BE USED, AND 1 IF IT IS.
C                       THIS IS TO SIGNAL WHETHER TO USE WTAUG AND
C                       LTAGPT( ) IN THE CORRECTION PROCESS.  (INPUT)
C           LTAGPT(K) = HOLDS THE LTAG VALUES USED TO INDICATE WHICH DATA
C                       POINTS ARE REAL (LTAGPT( ) = 0) AND WHICH ARE
C                       AUGMENTED (LTAGPT( ) = 9999).  (INPUT)
...
C
      ...
      IFIRST=0
C
CCC      WRITE(KFILDO,101)(K,CCALL(K),DATA(K),LTAG(K),LTAGPT(K),QUALST(K),
CCC     1                  K=1,NSTA)
CCC 101  FORMAT(' (K,CCALL(K),DATA(K),LTAG(K),LTAGPT(K),QUALST(K),',
CCC     1       'K=1,NSTA)',I6,1X,A8,F8.2,2I6,F15.3)
C
C         SAFETY CHECK ON LNDWAT.  (GO TO 500 CANNOT BE OPENMP
C         PARALLELIZATION LOOP.)
C
...
C*****************************************TEST
C
CCCD     IF(LNDSEA(K).LE.6)THEN
CCCD        WRITE(KFILDO,204)K,LNDSEA(K),CCALL(K),QUALST(K),
CCCD    1                       ELEV(K),XP(K),YP(K)
CCCD204  FORMAT(/' IN CORBC5 AT 204--K,LNDSEA(K),CCALL(K),QUALST(K),',
CCCD    1        'ELEV(K),XP(K),YP(K)',I6,I2,2X,A8,F15.3,3F9.2)
CCCD     ENDIF
C
...
CCCD        WRITE(KFILDO,2043)CCALL(K),QUALST(K),IAUG,LTAGPT(K),WTAUG
CCCD2043    FORMAT(/' AT 2043 IN CORBC5--',
CCCD    1           'CCALL(K),QUALST(K),IAUG,LTAGPT(K),WTAUG  ',
CCCD    2            A8,F10.2,2I6,F8.3)
CCCD     ENDIF
C*****************
C
...
C
      IF(IAUG.EQ.1.AND.LTAGPT(K).NE.0.AND.WTAUG.LT..001)GO TO 240
C        WHEN AUGMENTATION IS DONE, THE AUGMENTATION DATA CAN
C        HAVE A LESSER WEIGHT = WTAUG.  WHEN THIS IS ZERO,THE
C        CORRECTIONS SHOULD BE BYPASSED.
C
...
C
CCCD     CK='KDCA    '
CCCD     IF(CCALL(K).EQ.CK)THEN
CCCD        WRITE(KFILDO,2048)CCALL(K),LNDSEA(K),LP,IVRAD,VRAD(K),
CCCD    1                     RM999,RP001,RSQ,IAUG,WTAUG
CCCD2048    FORMAT(/,' AT 2048--CCALL(K),LNDSEA(K),LP,IVRAD,VRAD(K),',
CCCD    1            'RM999,RP001,RSQ,IAUG,WTAUG',/,
CCCD    2             A8,2I3,I3,4F10.3,I3,F6.3)
CCCD     ENDIF
C
...
C
!$OMP PARALLEL DO
!$OMP& PRIVATE(JY,JYE,DISTSQ,IX,IXE,ELEDIF,DX,DY,X,Y,BE,
!$OMP& DXLINE,B,BBPLUS,WT,WHT,W,IWINDX,IWINDY,F)
C
      DO 238 JY=JB,JE
C
      IF(MESH.EQ.MESHE)THEN
...
C
CCCCD     WRITE(KFILDO,2222)IX,XP(K),JY,YP(K),DISTSQ,RSQ
CCCCD2222 FORMAT(' AT 2222--IX,XP(K),JY,YP(K),DISTSQ,RSQ',I8,F8.2,I8,3F8.2)
C
...
C           WILL BE NEEDED.
C       
CCCD        IF(CCALL(K).EQ.'XXXC1  ')THEN  
CCCD           WRITE(KFILDO,2279)K,CCALL(K),ELCORR,XLAPSE(K),ELEDIF,
CCCD    1            IBKPN,ELCORR,ELCORU,WHT,WT,IX,JY,SEALND(IXE,JYE)
CCCD2279       FORMAT(' AT 2279--K,CCALL(K),ELCORR,,XLAPSE(K),ELEDIF,',
CCCD    1             'IBKPN,ELCORR,ELCORU,WHT,WT,IX,JY,',
CCCD    2              'SEALND(IXE,JYE)',/,
CCCD    3              I6,2X,A8,3F8.3,I3,4F8.2,2I5,F4.1)
CCCD        ENDIF
C
         IF(WHT.EQ.0.)THEN
            BBPLUS=BB
...
C        BE WEIGHTED LESS.  THIS IS DONE BY USING WTA VOR CORRECTONS
C        VICE WT.  IT HAS ALREADY BEEN DETERMINED THAT WTAUG IS NOT
C        ZERO WHEN LTAGPT(K) = 9999.***********PROPABLY NOT SO****************************
C
      IF(IAUG.EQ.1)THEN
C           IAUG = 1 MEANS AUGMENTATION HAS BEEN DONE AND LTAGPT( )
C           CONTAINS 0 FOR BASE DATA AND 9999 FOR AUGMENTED DATA.
C
         IF(LTAGPT(K).NE.0)THEN
C              WTAUG IS THE WEIGHT TO USE FOR AUGMENTED STATIONS
C              FOR THE CORRECTIONS.  WT IS NOT MODIFIED FOR BASE DATA.
            WTA=WT*WTAUG
...
C
      ...
      ENDIF
C
D     IF(CCALL(K).EQ.'QQQQ    ')THEN
D        WRITE(KFILDO,2283)K,NTYP,DATA(K),WT,WTA,WTLW,BBPLUS,LTAGPT(K)
D2283    FORMAT(' AT 2283--K,NTYP,DATA(K),WT,WTA,WTLW,BBPLUS,LTAGPT(K)',
D    1          2I6,5F10.3,I6)
D     ENDIF
C
      IF(NTYP.EQ.1)THEN
...
CCCCCC         IF(IX.EQ.510.AND.JY.EQ.52)THEN   JUST BELOW DUTCH HARBOR
C
CCCC         IF(IX.EQ.306.AND.JY.EQ.673)THEN
C              NEAR DEATH VALLEY.
CCCC            PRINT=WTLW*WTA*(DATA(K)-BBPLUS)*QUALST(K)
C                 CHANGED WT TO WTA 5/6/10
C              ADDED WTLW 10/15/08
CCCC            WRITE(KFILDO,2283)K,CCALL(K),ELEV(K),DATA(K),
CCCC     1             XLAPSE(K),TELEV(IXE,JYE),ELCORR,WT,BBPLUS,
CCCC     2             IX,JY,P(IX,JY),PRINT,CORR(IX,JY),COUNT(IX,JY),
CCCC     3             NCOUNT(IX,JY),SEALND(IXE,JYE)
CCCC 2283       FORMAT('AT 2283-K,CCALL(K),ELEV(K),DATA(K),',
CCCC     1             'XLAPSE(K),TELEV(IXE,JYE),ELCORR,WT,BBPLUS,',
CCCC     2             '    IX,   JY,P(IX,JY),PRINT,CORR(IX,JY),',
CCCC     3             'COUNT(IX,JY),NCOUNT(IX,JY),SEALND(IXE,JYE)',/,
CCCC     4             3X,I6,2X,A8,F6.1,F8.4,F8.4,F12.2,F10.2,F6.2,F7.2,I7,
CCCC     5             I6,F8.2,F7.3,2F10.3,I8,F4.1)
CCCC         ENDIF
C   
         CORR(IX,JY)=CORR(IX,JY)+WTLW*WTA*(DATA(K)-BBPLUS)*QUALST(K)
...
C
      ENDIF
C
CCC      IF(IX.EQ.206.AND.JY.EQ.556)THEN
C           FOR PRINTING, WHT IS UNDEFINED WHEN IBKPN = 99.
CCC      IF(CCALL(K).EQ.'SBIC1   '.AND.SEALND(IXE,JYE).EQ.9)THEN
...
C
C****************************************************************
CCCD        IF(IX.EQ.474.AND.JY.EQ.500)THEN
CCCD           WRITE(KFILDO,2405)IX,JY,P(IX,JY),CORR(IX,JY),
CCCD    1                        NCOUNT(IX,JY),NWIND(IX,JY)
CCCD2405       FORMAT(/' AT 2405--IX,JY,P(IX,JY),CORR(IX,JY),',
CCCD    1              'NCOUNT(IX,JY),NWIND(IX,JY)',2I5,2F8.2,2I5)
CCCD        ENDIF
C
...
C
CCCD        IF(IX.EQ.474.AND.JY.EQ.500)THEN
CCCD           WRITE(KFILDO,2406)IX,JY,P(IX,JY),CORR(IX,JY),
CCCD    1                        NCOUNT(IX,JY),NWIND(IX,JY)
CCCD2406       FORMAT(/' AT 2406--IX,JY,P(IX,JY),CORR(IX,JY),',
CCCD    1              'NCOUNT(IX,JY),NWIND(IX,JY)',2I5,2F8.2,2I5)
CCCD        ENDIF
C
...
C
D           IF(P(IX,JY).LT.1013.)THEN
D              WRITE(KFILDO,241)P(IX,JY),IX,JY,
D    1                      NCOUNT(IX,JY),NWIND(IX,JY)
D241           FORMAT(' TYPE 2 CORRECTION AT 241--',
...
C
                  ELSE
C
                     IF(LP.GE.4.AND.NCOUNT(IX,JY).EQ.2)THEN
...
C             ABOVE CORRECTION IS TYPE 3.
         ELSE
D           IF(IX.EQ.474.AND.JY.EQ.500)WRITE(KFILDO,2414)IX,JY
         ENDIF
D
C*****D        IF(NWIND(IX,JY).GT.0)THEN
D           IF(IX.EQ.474.AND.JY.EQ.220)
D    1                  WRITE(KFILDO,243)P(IX,JY),IX,JY,
D    2                  NCOUNT(IX,JY),NWIND(IX,JY)
C*****D        ENDIF
C
