...
C            IS ACCUMULATED FOR THE TERMS 0397XXXX AND 0497XXXX,
C            WHERE XXXX IS A RADIUS R:
C
C               039755 = AVERAGE OF THE ABSOLUTE VALUES OF THE 
C                        DIFFERENCES BETWEEN THE DATA VALUES AND THEIR
C                        MEAN WITHIN 55 GRIDLENGTHS OF THE GRIDPOINT
C        1         2         3         4         5         6         7 X
C                        AFTER APPLYING THE LAPSE RATE AND THE
C                        DIFFERENCE IN ELEVATION, AND
C               049745 = AVERAGE OF THE ABSOLUTE VALUES OF THE 
C                        DIFFERENCES BETWEEN THE DATA VALUES AND THEIR
...
C            2)  THE THE VALUES IN 1) ARE AVERAGED.
C            3)  THE AVERAGE DIFFERENCE OF THE INDIVIDUAL VALUES
C                AND THE AVERAGE COMPUTED IN 2) IS COMPUTED.
C
...
C
      SAVE IFIRST
C
CD     CALL TIMPR(KFILDO,KFILDO,'START VARIWG        ')
C
      IER=0
      IFIRST=0
      ICOUNT=0
...
C
      IF(ITABLE(J,L,M)/10000.NE.0397.AND.
     1   ITABLE(J,L,M)/10000.NE.0497)GO TO 399
C        TRANSFER OUT IF THIS TERM IS NOT 0397 OR 0497.
C
...
C
 ...
 101     FORMAT(/' ****NXE =',I5,' OR NYE =',I5,
     ...
     2           ' AT 101 IN VARIWG.  FATAL ERROR.')
         ISTOP=ISTOP+1
         IER=777
         GO TO 500
      ENDIF
...
C
         IF(DISTSQ.LT.RSQ)THEN       
            NCOUNT=NCOUNT+1
            TEMP(NCOUNT)=XDATA(K)+XLAPSE(K)*(TELEV(IX,JY)-ELEV(K))
C              TEMP(NCOUNT) IS THE VALUE IMPLIED BY STATION K AT
C              THE GRIDPOINT.
            VAR=VAR+TEMP(NCOUNT)
C
...
CD    1                  ' VARIWG WITHIN RADIUS R =',F6.1,' FOR',
CD    2                  ' GRIDPOINT IX,JY',2I6,'.  A LARGER RADIUS',
CD    3                  ' WILL BE TRIED.',/,
...
CD1634             FORMAT(/,' AT 1634 IN VARIWG--IX,JY,R,RSQ',2I5,2F10.2)
                  GO TO 120
               ENDIF
c
            ENDIF
...
CD1636       FORMAT(' AT 1636 IN VARIWG--IX,JY,R,RSQ',2I5,2F10.2)
         ENDIF
C
         VAR1=0.
C
...
CD1637       FORMAT(/,' AT 1637 IN VARIWG--J,L,M,IX,JY,VAR1,',
...
C
         IF(IX.EQ.300.AND.JY.EQ.400)THEN
            WRITE(KFILDO,165)J,L,M,IX,JY,ERR,RTABLE(J,L,M),
     1                       ERROR(IX,JY)
 165        FORMAT(/,' AT 165 IN VARIWG--J,L,M,IX,JY,ERR,',
...
C
         IF(DISTSQ.LT.RSQ)THEN
            NCOUNT=NCOUNT+1
            TEMP(NCOUNT)=XDATA(K)+XLAPSE(K)*(TELEV(IX,JY)-ELEV(K))
C              TEMP(NCOUNT) IS THE VALUE IMPLIED BY STATION KAT
C              THE GRIDPOINT.
            VAR=VAR+TEMP(NCOUNT)
C
         ENDIF
...
CD1734             FORMAT(/,' AT 1734 IN VARIWG--IX,JY,R,RSQ',
CD    1                     2I5,2F10.2)
                  GO TO 170
               ENDIF
C
...
CD1737       FORMAT(' AT 1737 IN VARIWG--IX,JY,R,RSQ',2I5,2F10.2)
         ENDIF
C
         VAR1=0.
C
...
C
     ...
     1                       ERROR(IX,JY)
 185        FORMAT(/,' AT 185 IN VARIWG--J,L,M,IX,JY,ERR,',
     1               'RTABLE(J,L,M),ERROR(IX,JY)',/,
     2               5I6,3F10.4)
         ENDIF
C
...
C
 ...
 250        FORMAT('THE RADIUS OF SEARCH DID NOT HAVE TO BE',
     1             ' INCREASED IN VARIWG FOR THIS VARIABLE ',2I10,
     2             ' FOR LAND  POINTS.')
         ELSE
CD           WRITE(KFILDO,251)JCOUNT,ID(1),ITABLE(J,L,M)
CD251        FORMAT('THE RADIUS OF SEARCH HAD TO BE INCREASED',
CD    1             I8,' TIMES IN VARIWG FOR THIS VARIABLE ',2I10,
...
C
 ...
 410     FORMAT(/' ####NUMBER OF POINTS SET TO MISSING IN VARIWG =',I9)
      ENDIF
C
CD     CALL TIMPR(KFILDO,KFILDO,'END   VARIWG        ')
C
 500  RETURN
      END
