      SUBROUTINE SCLSKY(KFILDO,KFIL10,NDATE,ID,IDPARS,JD,
...
C        MARCH     2007   GLAHN   CHANGED TABLE TO CORRECT VALUES
C        APRIL     2007   SMB     CHANGED FLOAT FUNCTIONS TO REAL.
C                                 ADDED COMMA TO FORMAT STATEMENT.
C        MAY       2007   SMB     COMMENTED OUT WRITE STATEMENT AT
C                                 100 FOR OPERATIONS
C        MAY       2007   GLAHN   MODIFIED /D TO NOT PRINT WHEN
C                                 ENTRY IS FOR A GRID
C        AUGUST    2007   GLAHN   INCREASED RANGE OF L FROM 2 TO 4
C                                 ACCOUNT FOR LAMP
C        OCTOBER   2008   COSGROVE   ADDED COMMAS FOR IBM COMPILE
C        NOVEMBER  2008   GLAHN   MADE NINT TEST ON DATA( ) AT DO 150;
C                                 2X PUT IN FORMAT AT 165; INSERTED
C                                 NINT(DATA(K).EQ.9999)GO TO 170
C                                 AT DO 170
C        NOVEMBER  2008   GLAHN   CORRECTION TO MIDDLE CATEGORY;
C                                 CHECKED FOR SMALL RANGE.
C        NOVEMBER  2008   GLAHN   REMOVED CHECK FOR SMALL RANGE.
C        OCTOBER   2009   GLAHN   REMOVED INCREMENTING ISTOP(1)
C                                 WHEN IER RETURNED NE 0
C
C        PURPOSE
...
C
C            NOTE THIS IS SPECIFIC TO SKY COVER.  THE PROBABILITES
C            ARE FOR DISCRETE CATEGORIES, OF WHICH THERE ARE 5.
C            FOR THE LOWER TWO, HIGH PROBABILITES ARE SCALED TO 
C            THE LOW END OF THE RANGE OF COVERAGE.  FOR THE HIGHER
C            TWO, HIGH PROBABILTIES RE SCALED TO THE HIGH END OF
C            THE RANGE OF THE COVERAGE.  THE MIDDLE CATEGORY IS
C            SPLIT WITH THE HIGH PROBABILITES GOING THE WAY OF 
C            THE HIGH CATEGOREIS AND THE LOW PROBABILITES GOING
C            THE WAY OF THE LOW CATEGORIES.
C
C        DATA SET USE
...
C                       SCLSKY CAN BE USED FOR EITHER VECTOR DATA
...
C                       (I=1,4) (J=1,NCAT) (L=1,4).  THE THIRD
...
C
      ...
      DIMENSION CORE(ND10)
      DIMENSION ISTOP(3),ITABLE(4,IDCAT,4),LD(4),TABLE(2,IDCAT-1)
C
     ...
     5            208381000,0,0,0,
C
     6            218384300,0,0,150001000,
     7            218384300,0,0,250001000,
     8            218384300,0,0,400001000,
     9            218384300,0,0,700001000,
     A            218384300,0,0,999905000,
     B            218381000,0,0,0,
C
     C            208350300,0,0,150001000,
     D            208350300,0,0,250001000,
     E            208350300,0,0,400001000,
     F            208350300,0,0,700001000,
     G            208350300,0,0,999905000,
     H            208351000,0,0,0,
C
     I            218351300,0,0,150001000,
     J            218351300,0,0,250001000,
     K            218351300,0,0,400001000,
     L            218351300,0,0,700001000,
     M            218351300,0,0,999905000,
     N            218351000,0,0,0/
C
      DATA TABLE/          .00,.05,
     1                     .05,.25,
...
C 100  FORMAT(/' AT 100 IN SCLSKY--NX,NY',2I6)
C
...
CD101  FORMAT(/' AT 101 IN SCLSKY--CAT,CONST,NSCALE',
...
C
 ...
 103     FORMAT(/' ****NCAT = ',I3,' NOT CORRECT IN SCLSKY.',
...
C
 ...
 110  FORMAT(/' ****VARIABLE ',I9.9,I10.9,I10.9,I4.3,' NOT',
     1        ' ACCOMMODATED IN SUBROUTINE SCLSKY.  IER =',I3)
      GO TO 900
C
...
CD1135 FORMAT(' AT 1135 IN SCLSKY--NVAL,FACTOR,(XDATA(M),M=1,NVAL)',
...
C
 ...
 120     FORMAT(/' ****COULD NOT FIND PROBABILITY RECORD',
     1           3I10.9,I10,'.  FATAL ERROR IN SCLSKY AT 120.')
         GO TO 900
      ENDIF
C
...
C 
 ...
 146           FORMAT(/' ****MAP PROJECTION NUMBER NPROJ =',I3,
     1           ' NOT 3, 5, OR 7.  FATAL ERROR IN SCLSKY AT',
     2           ' 146.')
         IER=777
         GO TO 900
      ENDIF
...
C
 ...
 148           FORMAT(/' ****NX AND NXI =,',2I6,' OR NY AND NYI =',2I6,
     1           ' DO NOT AGREE AT 148 IN SCLSKY.  FATAL ERROR.')
         IER=777
         GO TO 900
      ENDIF
C
CD           WRITE(KFILDO,149)MESHI,MESH,NXI,NYI,NX,NY
CD149        FORMAT(/' AT 149 IN SCLSKY--MESHI,MESH,NXI,NYI,NX,NY',
CD    1        6I6)
C
...
C
CD        IF(NX.EQ.0)THEN
C              NOTE THAT CCALL( ) IS DUMMY WHEN A GRID IS BEING
C              PROCESSED.  NX = 0 FOR VECTOR DATA.
CD           WRITE(KFILDO,1495)K,CCALL(K),XDATA(K),DATA(K),XMAX,XMIN,
CD    1                        ICOUNT
CD1495       FORMAT(' IN SCLSKY AT 1495--K,CCALL(K),XDATA(K),DATA(K),',
CD    1              'XMAX,XMIN,ICOUNT',I6,1X,A8,4F10.3,I7)
CD        ENDIF
C
      ENDIF
C
...
C           THIS GUARDS AGAINST A SMALL RANGE FOR A DIVISOR.
         A=(TABLE(2,J)-TABLE(1,J))/2.+TABLE(1,J)
         B=0.
         RANGE=0.
         HALFR=1.
         HALF=XMAX
         HTAB=TABLE(1,J)+(TABLE(2,J)-TABLE(1,J))/2.
         WRITE(KFILDO,1520)A,B,RANGE,HALFR,XMAX,HALF,HTAB
 1520    FORMAT(/' AT 1520--A,B,RANGE,HALFR,XMAX,HALF,HTAB',
     1           /,7F8.2)
C           SOME OF THE ABOVE ARE NOT USED FOR THIS SITUATION.
C
...
C           HIGH END.  FOR CLOUDS, NEITHER OF THESE
C           POSSIBILITIES WOULD BE LIKELY TO HAPPEN.
      ELSE
C
         IF(J.LE.2)THEN
C              FOR THE LOWEST 2 CATEGORIES, GIVE THE HIGH
C              PROBABILITIES A LOW VALUE.
            B=(TABLE(1,J)-TABLE(2,J))/RANGE
            A=TABLE(2,J)-B*XMIN
         ELSEIF(J.GE.4)THEN
C              FOR THE HIGHEST 2 CATEGORIES, GIVE THE HIGH
C              PROBABILITIES A HIGH VALUE.
            B=(TABLE(2,J)-TABLE(1,J))/RANGE
            A=TABLE(1,J)-B*XMIN
         ELSE
C              FOR THE MIDDLE CATEGORY, LEAVE THE VALUES
C              THE SAME.
            HALFR=RANGE/2.
            HALF=XMIN+HALFR
            HTAB=(TABLE(2,J)-TABLE(1,J))/2.
         ENDIF
C
      ENDIF
C
...
CD160  FORMAT(/,' IN SCLSKY AT 160--XMAX,XMIN,RANGE,',
CD    1         'TABLE(2,J),TABLE(1,J)',5F10.3)
C
      DO 170 K=1,NVAL
C
      IF(NINT(XDATA(K)).EQ.J)THEN
C              NOTE CHECKING FOR MISSING XDATA( ) HERE IS NOT NECESSARY.
c
...
C           HAPPEN, BUT HAS.  NOTE THAT FD2( ) HAS BEEN ALREADY
C           INITIALZED TO 9999.
C
         IF(J.EQ.3)THEN
C              THIS IS THE MID CATEGORY AND NEEDS SPECIAL
C              TREATMEMT.  HIGH PROBABILITES HIGHER THAN THE
C              MIDDLE OF THE RANGE INDICATE HIGH COVERAGE.
C              HIGH PROBABILITIES LOWER THAN THE MIDDLE
C              OF THE RANGE INDICATE LOW COVERAGE.
C
            IF(DATA(K).EQ.HALF)THEN
               FD2(K)=HTAB*FACTOR
               WRITE(KFILDO,1600)DATA(K),HALF,HTAB,FACTOR,FD2(K)
 1600          FORMAT(/,' AT 1600--DATA(K),HALF,HTAB,FACTOR,FD2(K)',
     1                /,5F8.2)
C                 THE ABOVE IS THE CASE IN WHICH THE XMAX = XMIN
C                 AND WOULD USUALLY BE WHEN ONLY ONE CASE IN THE
C                 CATEGORY.  THE VALUE IS PUT AT THE CENTER.
            ELSEIF(DATA(K).LT.HALF)THEN
               FD2(K)=(TABLE(1,J)+HTAB*(DATA(K)-XMIN)/HALFR)*FACTOR
            ELSE
               FD2(K)=(TABLE(2,J)-HTAB*(XMAX-DATA(K))/HALFR)*FACTOR
            ENDIF
C
         ELSE
...
C              NOTE THAT CCALL( ) IS DUMMY WHEN A GRID IS BEING
C              PROCESSED.  NX = 0 FOR VECTOR DATA.
CD           WRITE(KFILDO,165)J,K,CCALL(K),A,B,DATA(K),XDATA(K),FD2(K)
CD165        FORMAT(' AT 165--J,K,CCALL(K),A,B,DATA(K),XDATA(K),FD2(K)',
CD    1              2I6,2X,A8,5F10.3)
CD        ENDIF
C
      ENDIF
C
...
CD225  FORMAT(/,' IN SCLSKY AT 225',/,(15F8.2))
C
 900  RETURN
      END
