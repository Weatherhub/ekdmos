      SUBROUTINE SCLQ06(KFILDO,KFIL10,NDATE,ID,IDPARS,JD,
...
C        AUGUST    2008   GLAHN   CHECKED NCAT WITH IDCAT-1
C        NOVEMBER  2008   GLAHN   MADE NINT TEST ON DATA( ) AT DO 150;
C                                 INSERTED NINT(DATA(K).EQ.9999)
C                                 GO TO 170 AT DO 170
C        AUGUST    2009   GLAHN   COMMENT UNDER FORMAT 116
...
C            ARE SPECIFIC TO THOSE CATEGORIES.  THEREFORE, NCAT MUST
C            EQUAL IDCAT-1.
C
C        DATA SET USE
...
C                       SCLQ06 CAN BE USED FOR EITHER VECTOR DATA
...
C
      DATA ITABLE/        0,0,0,        0,
     1            203220100,0,0,950052000,         
     2            203220100,0,0,950051000,
     3            203220100,0,0,245000000,
     4            203220100,0,0,495000000,
     5            203220100,0,0,995000000,
     6            203221000,0,0,        0,
     7                    0,0,0,        0,
     8            213220100,0,0,950052000,
     9            213220100,0,0,955051000,
     A            213220100,0,0,245000000,
     B            213220100,0,0,495000000,
     C            213220100,0,0,995000000,
     D            213221000,0,0,        0/
      DATA TABLE/          0., .000,
     1                    .01, .095,
     2                    .10, .245,
...
C
      IER=0
CD     WRITE(KFILDO,100)NX,NY
CD100  FORMAT(/' AT 100 IN SCLQ06--NX,NY',2I6)
C
...
C        IS IN ID(3).
C
CD     WRITE(KFILDO,101)NCAT,
CD    1              (((ITABLE(I,J,L),I=1,4),J=1,NCAT),L=1,2)
CD101  FORMAT(/' AT 101 IN SCLQ06--NCAT,',
CD    1        '(((ITABLE(I,J,L),I=1,4),J=1,NCAT),L=1,2)',/,
CD    2         I6,/,(4I11))
CD     WRITE(KFILDO,102)((TABLE(I,J),I=1,2),J=1,NCAT-1)
CD102  FORMAT(/' AT 102 IN SCLQ06--',
CD    1        '((TABLE(I,J),I=1,2),J=1,NCAT-1)',/,(2F10.3))
C
      DO 105 L=1,2
C
...
C
 ...
 110  FORMAT(/' ****VARIABLE ',I9.9,I10.9,I10.9,I4.3,' NOT',
     1        ' ACCOMMODATED IN SUBROUTINE SCLQ06.  IER =',I3)
      GO TO 900
C
...
C
 ...
 112     FORMAT(/,' ****NCAT =',I3,' DOES NOT EQUAL IDCAT-1 IN SCLQ06.',
...
CD1135 FORMAT(' AT 1135 IN SCLQ06--NVAL,FACTOR,(XDATA(M),M=1,NVAL)',
...
C              THE ABOVE MAY BE A POSSIBILITY FOR HAWAII OR PUERTO RICO.
            ISTOP(1)=ISTOP(1)+1
            ISTOP(3)=ISTOP(3)+1
         ENDIF
C
...
CD117  FORMAT(/' AT 116--J,L,(ITABLE(M1,J,L),M1=1,4)',
CD    1          6I12)
C
...
C
 ...
 120     FORMAT(/' ****COULD NOT FIND PROBABILITY RECORD',
     1           3I10.9,I10,'.  FATAL ERROR IN SCLQ06 AT 120.')
         GO TO 900
      ENDIF
C
...
C 
 ...
 146           FORMAT(/' ****MAP PROJECTION NUMBER NPROJ =',I3,
     1           ' NOT 3, 5, OR 7.  FATAL ERROR IN SCLQ06 AT',
     2           ' 146.')
         IER=777
         GO TO 900
      ENDIF
...
C
 ...
 148           FORMAT(/' ****NX AND NXI =,',2I6,' OR NY AND NYI =',2I6,
     1           ' DO NOT AGREE AT 148 IN SCLQ06.  FATAL ERROR.')
         IER=777
         GO TO 900
      ENDIF
C
CD           WRITE(KFILDO,149)MESHI,MESH,NXI,NYI,NX,NY
CD149        FORMAT(/' AT 149 IN SCLQ06--MESHI,MESH,NXI,NYI,NX,NY',
CD    1        6I6)
C
...
CD1495    FORMAT(' IN SCLQ06 AT 1495--XDATA(K),DATA(K),XMAX,XMIN',
...
CD160  FORMAT(/,' IN SCLQ06 AT 160--XMAX,XMIN,RANGE,',
...
CD225  FORMAT(/,' IN SCLQ06 AT 225',/,(15F8.2))
CD230  CONTINUE
C
 900  RETURN
      END
