...
C                       J=4 IS HH
C                       (INPUT)
C            XDATA(J) = DATA TO ANALYZE (J=1,NSTA).  (INPUT)
...
C                       SETPT AND BY CORBC5.  THE USABLE ORIGINAL DATA
C                       VALUES HAVE A ZERO IN LTAGPT( ) AND THE
C                       AUGMENTED ONES HAVE A 9999.  (INPUT)
C                IAUG = 0 BEFORE AUGMENTATION IS DONE, AND SET TO 1
C                       AFTER.  THIS IS TO SIGNAL WHETHER TO USE
C                       LTAG( ) OR LTAGPT( ) IN CALL TO SETPNT.
C                       (INPUT)
...
C                       GTHRES.  THIS IS FOR DISTANCE GRAID IN SPOTRM.
...
C                       (INPUT)
C              NPASSP = THE NUMBER OF THE PASS TO START SPOTRM.
C                       (INPUT)
...
C              NSMNUM = THE NUMBER OF CALLS TO SMOTHG AFTER SPOTRM.
C                       (INPUT)
...
C                       (INPUT)
C               DIFFA = THE MAXIMUM DIFFERECE IN ELEVATION IN METERS
...
C                       IN THE ANALYSIS.  WHEN THIS IS 1., NO DISTINCTION
C                       IS MADE IN THE DATA; WHEN WTAUG = 0., ONLY
C                       THE BASE (REAL) DATA ARE USED.  (INPUT)
C                 RAY = THE MULTIPLIER OF THE DISTANCE TO THE CLOSEST
C                       STATION TO USE AS THE RADIUS OF THE SMOOTHING
C                       CIRCLE (OR MAJOR AXIS OFELIPSE).  (INPUT)
...
C           NAMMAX(M) = MAME OF THE STATION ASSOCIATED WITH
C                       DIFMAX(M) (M=1,3).  INTERNAL)
C              ISETPT = 0 UNTIL LTAGPT( ) IS SET TO INDICATE BASE
C                       DATA, THEN SET = 1.
...
C
     ...
     3          QUALST(NSTA),XLAPSE(NSTA),LTAGPT(NSTA),VRAD(ND1,6),
     4          XYP(ND1,2),SDATA(ND1)
...
C
      IER=0
D     CALL TIMPR(KFILDO,KFILDO,'START BCD5          ')
C
C
CCCC      WRITE(KFILDO,130)NPASSF,NSMNUM,DIFFA,NOCEAN,SQUEZE,WTAUG,RAY
CCCC 130  FORMAT(' IN BCD5--NPASSF,NSMNUM,DIFFA,NOCEAN,SQUEZE,WTAUG,RAY',
CCCC     1       2I4,F6.0,I4,3F8.3)
C
C        SAVE IVRAD SO THAT IT CAN BE RESTSORED UPON EXIT.  OTHERWISE,
...
C
      ...
      IOPTGR(8)=IOPT(8)
      ISETPT=0
C        ISETPT INDICATES LTAGPT( ) HAS NOT BEEN FILLED TO INDICATE
C        WHICH ARE BASE DATA.
C 
...
D    1        'MESH,NX,NY,NSTA',I4,2F10.5,5I6)
C
 ...
 150  FORMAT(/' STARTING PASS',I3,' FOR ',A16,
     ...
     2        '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
C
C        BEFORE PASS NPASSF, INTERPOLATION IS MADE INTO THE 
C        CURRENT ANALYSIS FOR THE FULL SET OF STATIONS.  THE
C        STATIONS WITHOUT DATA GET THE INTERPOLTED VALUES FOR
C        THE REST OF THE RUN.  LTAG( ) THEN INDICATES THE STATIONS
C        TO USE IN THE ANALYSIS (VIRTUALLY ALL), AND LTAGPT( )
C        INDICATES WHICH ARE BASE AND WHICH ARE INTERPOLATED
C        (AUGMENTATION POINTS).
C
      IF(NPASSF.NE.0.AND.LP.GE.NPASSF)THEN
         IAUG=1
C           IAUG SET TO 1 TO INDICATE AUGMENTATION HAS BEEN DONE.
         ISETPT=1
C           ISETPT SET = 1 TO INDICATE LTAGPT( ) HAS BEEN PROPERLY
C           INITIALIZED. 
         IVRAD=0
C           FOR THE REST OF THE RUN, THE CONSTANT RADII ARE USED.
         CALL INTRPB(KFILDO,P,NX,NY,XYP,ND1,NSTA,SDATA)
C           INTRPB INTERPOLATES TO ALL STATIONS.
C           MIGHT HAVE TO USE OTHER INTERPOLATIONS FOR SOME VARIABLES.
C
         DO 155 K=1,NSTA
C
         IF(LP.EQ.NPASSF)THEN
C              CAN'T DO THIS BUT ONCE.  AFTER THE FIRST TIME,
C              ALL XDATA( ) VALUES WILL BE LT 9998, SOME OF THEM
C              AUGMENTED.
CCCCD           WRITE(KFILDO,151)CCALL(K),LTAG(K),LTAGPT(K),XDATA(K)
CCCCD151        FORMAT(/' AT 151--CCALL(K),LTAG(K),LTAGPT(K),XDATA(K)  ',
CCCCD    1              A8,2I6,F10.3)
C
            IF(XDATA(K).LT.9998.)THEN
C                 ON THE FIRST TIME THROUGH HERE, XDATA(K) LT 9999.
C                 MEANS THE ORIGINAL DATUM WAS PRESENT.  IF SPOTRM
C                 WERE USED WITH VARIABLES THAT HAVE ERROR CHECKING
C                 AND A POINT COULD HAVE BEEN TOSSED BEFORE SPOTRM
C                 WAS CALLED,THEN LTAGPT( ) COULD BE LEFT 9999 
C                 INDICATING THIS STATION IS NOT USED ON THIS PASS.
C                 EVEN IF IT WERE LATER ACCEPTED, IT IS PROBABLY
C                 QUESTIONABLE AND MAY NOT WANT TO FIT CLOSELY.
C                 CEILING AND VISIBILITY VALUES ARE ALWAYS ACCEPTED.
               LTAGPT(K)=0
            ELSE
               LTAG(K)=0
C                 THE LTAG(K) VALUE IS NOT MODIFIED WHEN THERE IS
C                 A BASE DATUM PRESENT, BECAUSE IT COULD HAVE BEEN
C                 SET NON ZERO BECAUSE OF ERROR CHECKING.
            ENDIF
C
CCCCD           IF(CCALL(K).EQ.'KARB    '.OR.CCALL(K).EQ.'KDCA    ')THEN
CCCCD              WRITE(KFILDO,152)CCALL(K),LTAG(K),LTAGPT(K),XDATA(K)
CCCCD152           FORMAT(/' AT 152--CCALL(K),LTAG(K),LTAGPT(K),XDATA(K)  ',
CCCCD    1                 A8,2I6,F10.3)
CCCCD           ENDIF
C
         ENDIF
C
         IF(LTAGPT(K).GT.9998.)THEN
            XDATA(K)=SDATA(K)
C              INTERPOLATED DATA ARE INSERTED INTO XDATA( ).
         ENDIF
C
CCCCD        WRITE(KFILDO,153)K,CCALL(K),XYP(K,1),XYP(K,2),
CCCCD    1                    XDATA(K),SDATA(K),LTAG(K),LTAGPT(K)
CCCCD153     FORMAT(' AT 153, BCD5--K,CCALL(K),XYP(K,1),XYP(K,2),',
CCCCD    1          'XDATA(K),SDATA(K),LTAG(K),LTAGPT(K)',
CCCCD    2          I7,1X,A8,4F8.2,2I7)
 155     CONTINUE
C
      ELSEIF(NAPSSP.NE.0.AND.LP.EQ.NPASSP.AND.ISETPT.EQ.0)THEN
         ISETPT=1
C           ISETPT = 1 TO KEEP THIS FROM BEING DONE AGAIN.
C           NOTE THAT IF A DATUM WERE TOSSED, THIS WOULD NOT
C           BE CHANGED TO REFLECT THAT.
C
         DO 160 K=1,NSTA
C
         IF(XDATA(K).LT.9998.)THEN
...
C
      ENDIF
C
      IF(NPASSR.NE.0.AND.LP.GE.NPASSR)IVRAD=0
...
C         NOTE THIS CHECKS ONLY ORIGINAL DATA VIA LTAG( )
C
...
CD     CALL TIMPR(KFILDO,KFILDO,'START CORBC5        ')
 190  CALL CORBC5(KFILDO,IP14,IP20,CCALL,XDATA,XP,YP,XPL,YPL,LTAG,
     ...
     2            ILS,LNDWAT(LP),WTWTL,WTLTW,WTAUG,IAUG,LTAGPT,
...
C 
         ...
         IF(IAUG.GT.0.AND.LTAGPT(K).GT.9998)GOTO 263
C           THIS TEST PUT IN TO VERIFY ONLY BASE STATIONS, NOT ANY
C           AUGMENTED ONES.
C
...
C          
         ...
         SUM=SUM+ADIF
         NSUM=NSUM+1
CCCC         WRITE(KFILDO,2585)CCALL(K),BB,XDATA(K),DIF,NSUM,SUM
CCCC 2585    FORMAT(' AT 2585--CCALL(K),BB,XDATA(K),DUFMNSUM,SUM ',
CCCC     1            A8,3F8.2,I6,F8.2)
C
...
C        IS NOT TO BE DONE, AND (3) ISETP NE 0.  WHEN IAUG NE 0, 
C        INDICATING AUGMENTATION, THEN ONLY THE ORIGINAL STATIONS
C        ARE INVOLVED IN SETTING GRIDPOINT VALUES.  NOTE THAT IF
C        SCALING ON THE ORIGINAL VALUES HAS BEEN DONE, THEN IT IS
C        THESE SCALED VALUES THAT ARE USED.  THE SCALED VALUE
C        WILL, WHEN TRUNCATED TO AN INTEGER, HAVE THE ORIGINAL
C        INTEGER VALUE.
C
 2646 IF(LP.EQ.NPASS.AND.B(LP).EQ.0..AND.ISETP.NE.0)THEN
C
         IF(IAUG.EQ.0)THEN
C              AUGMENTATION WITH AUGMT3 HAS NOT BEEN DONE.
            CALL SETPNT(KFILDO,XDATA,LTAG,XP,YP,NSTA,P,FD2,NX,NY,
     1                  ISETP,IER)
C              THERE IS CURRENTLY NO NON ZERO IER RETURN.
         ELSE
C              AUGMENTATION WITH AUGMT3 HAS BEEN DONE.
            CALL SETPNT(KFILDO,XDATA,LTAGPT,XP,YP,NSTA,P,FD2,NX,NY,
     1                  ISETP,IER)
C              THERE IS CURRENTLY NO NON ZERO IER RETURN.
         ENDIF
C
...
C
      ELSEIF(NSMTYP.EQ.8.AND.RAY.GT.0..AND.LP.GE.NPASSP)THEN
C
         WRITE(KFILDO,274)NPASSP,NSMNUM,NOPTN,DIFFA,NOCEAN,SQUEZE,
     1                    DPOWER,WTAUG,RAY,R(1)
 274     FORMAT(/' NPASSP =',I2,' NSMNUM ',I3,
     2           '  NOPTN =',I2,'  DIFFA =',F5.0,'  NOCEAN =',I2,
     3           '  SQUEZE =',F5.2,'  DPOWER =',F5.2,'  WTAUG =',F7.3,
     4           '  RAY =',F5.2,'  RMAX =',F5.1,/)
C
         PLAINT='NEAREST DISTANCE TO STATION     '
         CALL SPOTRM(KFILDO,KFILOG,IP16,
     1               CCALL,XP,YP,LNDSEA,LTAGPT,NSTA,
     2               ID,IDPARS,P,COUNT,MESH,NX,NY,
     3               TELEV,SEALND,CPNDFD,NXE,NYE,MESHE,
     4               IPACK,DATA,IWORK,ND5,
     5               MODNO,NDATE,
     6               ALATL,ALONL,NPROJ,ORIENT,XLAT,ISCALD,
     7               DIFFA,NOCEAN,SQUEZE,DPOWER,RAY,R(1),
     8               IS0,IS1,IS2,IS4,ND7,
     9               JTOTBY,JTOTRC,PLAINT,IPLANT,
     A               L3264B,L3264W,MINPK,ISTOP(1),IER) 
C           IF IER NE 0, A DIAGNOSTIC IS PRODUCED IN SPOTRM WITH ISTOP(1)
C           INCREMENTED.  THIS DOES NOT STOP PROCESSING. 
C
C           FOLLOW THE RAY SMOOTHER WITH TWO TERRAIN FOLLOWING
C           PASSES TO SMOOTH OUT NOISE.
C
         IF(NSMNUM.NE.0)THEN
...
C
 ...
 2754    FORMAT(/' ****NSMTYP =',I4,' NOT A CORRECT VALUE OF',
     1           ' 1, 2, 3, ,4, 5, 6, 7 OR 8 (ZERO DEFAULTS TO 1),'/
     2           '     RAY LE 0, OR B(LP) INDICATES SMOOTHING',
     3           ' BEFORE NPASSP FOR SPOTRM.',
     4           '  NO SMOOTHING WILL BE DONE FOR THIS VARIABLE.')
         ISTOP(1)=ISTOP(1)+1 
         GO TO 2799
      ENDIF   
C
...
C 
         ...
         IF(IAUG.GT.0.AND.LTAGPT(K).GT.9998)GOTO 278
C           THIS TEST PUT IN TO VERIFY ONLY BASE STATIONS, NOT ANY
C           AUGMENTED ONES.
C
C           FIND INTERPOLATED VALUE OR NEAREST NEIGHBOR VALUE IN
...
C        PROVIDED A CLOSER STATION TO THE GRIDPOINT DOES NOT EXIST.
C        THIS IS DONE HERE IF IT IS (1) THE LAST PASS, (2) SMOOTHING
C        IS DONE, AND (3) ISETP NE 0.  WHEN IAUG NE 0, INDICATING
C        AUGMENTATION, THEN ONLY THE ORIGINAL STATIONS ARE INVOLVED
C        IN SETTING GRIDPOINT VALUES.  NOTE THAT IF SCALING ON
C        THE ORIGINAL VALUES HAS BEEN DONE, THEN IT IS THESE SCALED
C        VALUES THAT ARE USED.  THE SCALED VALUE WILL, WHEN TRUNCATED
C        TO AN INTEGER, HAVE THE ORIGINAL INTEGER VALUE.
C
...
C              AUGMENTATION WITH AUGMT3 HAS BEEN DONE.
            CALL SETPNT(KFILDO,XDATA,LTAGPT,XP,YP,NSTA,P,FD2,NX,NY,
     1                  ISETP,IER)
C              NOTE LTAGPT( ) IN CALL VICE LTAG( ).
C              THERE IS CURRENTLY NO NON ZERO IER RETURN.
         ENDIF
C
      ENDIF
...
C
D     CALL TIMPR(KFILDO,KFILDO,'END BCD5            ')
      RETURN
      END
