      SUBROUTINE LAPSUA(KFILDO,KFIL10,IP14,ID,IDPARS,NCEPNO,
     1                  CCALL,NAME,LNDSEA,
...
C                                 DATA; ADDED FL174
C        DECEMBER  2008   GLAHN   MODIFIED FORMAT 1275
C        MARCH     2008   GLAHN   OMITTED CHECK ON 2ND WORD AT 105
C        AUGUST    2009   GLAHN   ADDED DO 100 LOOP
C        AUGUST    2009   GLAHN   ADDED PRINT TO IP14 WHEN LAPFG = 2
C        AUGUST    2009   GLAHN   CHANGED ISTOP=ISTOP+1 TO
C                                 ISTOP(1)=ISTOP(1)+1 IN ONE PLACE
C        AUGUST    2009   GLAHN   MODIFIED TEST ON ELEVHI( ) AND ELEV( )
C                                 TO INTEGER
C        SEPTEMBER 2009   GLAHN   REVISED THE LAPSE CALCULATIONS A BIT
C        JULY      2010   GLAHN   ADDED NCEPNO TO CALL; USED NCEPNO AS
C                                 DD FOR UA DATA VICE IDPARS(4)
C        SEPTEMBER 2010   GLAHN   MODIFIED FOR ONLY 6 UP LEVELS FOR
C                                 EKDMOS VICE 12 FOR MOS
C        OCTOBER   2012   ENGLE   MODIFIED FORMAT STATEMENT
C                                 205 FOR INTEL COMPILER
C
C        PURPOSE
...
C            SPECIAL TREATMENT.
C
C            THE LAPSE CAN BE COMPUTED IN ONE OF TWO WAYS DEPENDING
...
C              ELEVHI( ).  HOWEVER, IF THIS LEVEL IS ABOVE THE 2ND
C              UPPER LEVEL, THE LEVEL BELOW IS USED.  THIS MAKES THE
C              LAPSE COMPUTED OVER A RANGE OF ELEVATIONS ENCOMPASING
C              THE STATION ELEVATION.  IT DOES NOT USE THE DATA TO
C              ANALYZE.  THIS MAY NOT WORK WELL IN INTENSE INVERSIONS.
C
...
C            WITH MAX AND MIN, BECAUSE THEY COVER A PERIOD.  THE
...
C
C            *******************************************************
C            THIS VERSION FOR ALASKA WHEN UA DATA ARE ON THE ORIGINAL
C            ARCHIVED 95 OR 190 KM GRID.  HOWEVER, THE INTERPOLATION
C            TO STATIONS CAN BE DONE FROM ANY GRID THAT COVERS THE
C            STATIONS WHETHER OR NOT IT MATCHES THE GRID OF THE
C            ANALYSIS.  THIS FLEXIBILITY IS DUE TO THE STATION
C            LOCATIONS BEING CARRIED IN DIR( , , ) FOR EACH GRID
C            READ.  NOTE THAT THE FIRST GUESS GRID, IF ONE IS
C            NEEDED, MUST BE ON THE SAME MAP PROJECTION AS THE
C            ANALYSIS.
C            *******************************************************
C
...
C                       UPPER AIR DATA FOR LAPSE.  (INPUT)
...
C                       CCALL(K) (K=1,NSTA).  (INPUT)  (NOT ACTUALLY
C                       USED)
C           ELEVHI(K) = THE HIGH ELEVATION ASSOCIATED WITH STATION
C                       CCALL(K) (K=1,NSTA).  THE HIGHEST GRIDPOINT
C                       ELEVATION TO BE AFFECTED BY THIS STATION K.
C                       (INPUT)
...
C                       GUESS WHEN MGUESS = 3.  IBACKL = 1 MEANS 
C                       CURRENT (MOST RECENT) CYCLE PLUS THE ONE
C                       6 HOURS BEFORE).  NORMALLY, THIS IS 0 FOR
C                       DEVELOPMENT; MAY BE OTHERWISE FOR OPERATIONS.
C                       (INPUT)
...
C                       (INTERNAL)
C                JDIM = NUMBER OF LEVELS OF DATA NEEDED.  DETERMINED
C                       BY WHETHER THE ENTRY IS MOS OR EKDMKOS.
C                       (INTERNAL)
C                LDIM = MAXIMUM OF LDIM LEVELS OF UP DATA PROVIDED. 
C                       SET BY PARAMETER.  (INTERNAL)
C         JTABLE(M,L) = THE LEVELS OF UPPER AIR DATA THAT CAN
C                       BE ACCESSED FOR COMPUTING THE LAPSE RATE
C                       (M=1,LDIM) FOR GMOS (L=1) OR EKDMOS (L=2)
C                       (INTERNAL)
...
C                       THE LAPSE IS BEING COMPUTED FOR EACH 
C                       STATION (K=1,NSTA), EACH LEVEL  (J=1,LDIM),
...
C                       THE LAPSE IS BEING COMPUTED CORRESPONDING TO
C                       VDATA(K, ,J) (K=1,NSTA) (J=1,LDIM).  (INTERNAL)
C                       (ALLOCATED) (SAVED)
...
c
      DATA JTABLE/
     1   2,1000,975,950,925,900,850,800,750,700,600,500,
     2   2,1000,        925,    850,        700,    500,0,0,0,0,0,0/
C
D     CALL TIMPR(KFILDO,KFILDO,'START LAPSUA        ')
      IER=0
C
C        SET XLAPSE( ) = 0. IN CASE IT CAN'T BE COMPUTED.
C
      DO 100 K=1,NSTA
      XLAPSE(K)=0.
 100  CONTINUE
C
C        DETERMINE WHETHER THIS IS GFS MOS OR GEFS EKDKMOS.
C
      IF(IDPARS(4).EQ.8)THEN
         JDIM=12
C           THIS IS GFS MOS.
      ELSEIF(IDPARS(4).EQ.61)THEN
         JDIM=6
C           THIS IS GEFS EKDMOS.
      ELSE
         WRITE(KFILDO,101)IDPARS(4)
 101     FORMAT(/' ****IDPRS(4) =',I3,' NOT 8 FOR GFS OR 61 FOR',
     1           ' EKDMKOS IN LAPSUA AT 101.  RETURN WITH IER = 777.')
         IER=777
         GO TO 250
      ENDIF
C
...
C
CD     WRITE(KFILDO,101)NDATE,ND5,ND7,ND10,ND9,ND1,NSTA,
CD    1                 NPROJ,ORIENT,XLAT,BMESH,L3264B
CD102  FORMAT(/,' AT 102 IN LAPSUA--',
CD    1         'NDATE,ND5,ND7,ND10,ND9,ND1,NSTA,',
CD    2         'NPROJ,ORIENT,XLAT,L3264B',/,
CD    3         I12,7I8,2F10.4,F10.6,I4)
C
...
C
      IF(JFIRST.EQ.0)THEN
         ALLOCATE (VDATA(NSTA,3,LDIM),HDATA(NSTA,LDIM),XSAVE(NSTA),
     1             STAT=IOS)
C
         IF(IOS.EQ.1)THEN
C
...
C
      DO 105 L=1,NDIM
C
      IF(ID(1)/100.EQ.ITABLE(1,L)/100)GO TO 108
C        THIS DEFINES L.  THE DD IS NOT CHECKED.
C        ID(2) IS NOT CHECKED BECAUSE OF EKDMOS.
C
 105  CONTINUE
C
...
C
 108  IF(L.EQ.5.OR.L.EQ.8)THEN
      ...
      ENDIF
C
CD     WRITE(KFILDO,1085)NDATE,(NDATSV(J),J=1,3),
CD    1                  ID(1),(IDSAV(J),J=1,3),
CD    2                  IDPARS(12),(NTAUSV(J),J=1,3),
CD    3                  N2,(IDVSAV(J),J=1,3),
CD    4                  LAPSV
CD1085 FORMAT(/' AT 1085--NDATE,(NDATSV(J),J=1,3),',
CD    1        'ID(1),(IDSAV(J),J=1,3),',
CD    2        'IDPARS(12),NTAUSV(J),J=1,3)',
CD    3        'N2,(IDVSAV(J),J=1,3),LAPSV'/,  
CD    4        (4I12))
C
...
C
CD     WRITE(KFILDO,126)ID(1),IDVSAV(1),IDVSAV(2),
CD    1                       NDATSV(1),NDATSV(2),
CD    2                       NTAUSV(1),NTAUSV(2)
CD126  FORMAT(/' AT 126 IN LAPSUA--ID(1)',
CD    1    'IDSAV(1),IDSAV(2),NDATSV(1),NDATSV(2),NTAUSV(1),NTARSV(2)',
CD    2     /,7I11)           
C
...
C
 ...
 1275       FORMAT(/' UPPER AIR WIND SPEED BEING COMPUTED FROM',
     1              ' U AND V COMPONENTS.')
C
            GO TO 150
         ELSE
...
C   
      LD(1)=ITABLE(3,L)+NCEPNO
C
C        READ THE HEIGHTS IF NECESSARY.  THIS ASSUMES THAT IF
C        ANY ANALYSIS HAS BEEN MADE FOR THIS PROJECTION, THE HEIGHTS
C        EXIST.
C
CD     WRITE(KFILDO,1285)(NDATSV(JJ),JJ=1,3),(NTAUSV(JJ),JJ=1,3),
CD    1                  (IDVSAV(JJ),JJ=1,3),(ID(JJ),JJ=1,3),
CD    2                  ITABLE(3,L),NDATE,IDPARS(12)
CD1285 FORMAT(/' AT 1285--(NDATSV(JJ),JJ=1,3),(NTAUSV(JJ),JJ=1,3),',
CD    1        '(IDVSAV(JJ),JJ-1,3),(ID(JJ),JJ=1,3)',
CD    2        'ITABLE(3,L),NDATE,IDPARS(12)',/,
CD    3         (3I12))
C
CD     WRITE(KFILDO,1286)N2,IDVSAV(N2),L,ITABLE(3,L)
CD1286 FORMAT(/' AT 1286--N2,IDVSAV(N2),L,ITABLE(3,L)',4I12)
C
...
C
         IF(IDPARS(4).EQ.8)THEN
            LD(2)=JTABLE(J,1)
C              THIS IS FOR GFS MOS.
         ELSE
            LD(2)=JTABLE(J,2)
C              THIS IS FOR GEFS EKDMOS.
         ENDIF
C
      ENDIF
C
...
C
 ...
 130     FORMAT(/,' ****GRID CHARACTERISTICS IN NGRIDC( , ) DO NOT',
     ...
     7            'NGRIDC(6,NSLAB) =',I8,'    IS2(6) =',I8)
         ...
         GO TO 220
      ELSE
CD        WRITE(KFILDO,131)NSLAB
CD131     FORMAT(' GRID RETRIEVED IN LAPSUA WITH NSLAB =',I3)
      ENDIF
C
...
C
 ...
 140     FORMAT(/,' ****GRID CHARACTERISTICS IN NGRIDC( , ) DO NOT',
     ...
     7            'NGRIDC(6,NSLAB) =',I8,'    IS2(6) =',I8)
         ...
         GO TO 220
      ELSE
CD        WRITE(KFILDO,142)NSLAB
CD142     FORMAT(' GRID RETRIEVED IN LAPSUA WITH NSLAB =',I3)
      ENDIF
C
C        INTERPOLATE TO EACH STATION.
C
...
C
      IF(NINT(ELEVHI(K)).EQ.9999.OR.
     1     NINT(ELEV(K)).EQ.9999.OR.
C             NOTE THAT THE ELEVATIONS CAN EXCEED 9999.
...
C
      ...
      ENDIF 
C
C        THE LAPSE IS COMPUTED FROM ONLY UA DATA ABOVE THE STATION
C        WHEN LAPFG = 2, BUT FROM THE STATION UP TO THE FIRST
C        LEVEL ABOVE THE STATION WHEN LAPFG = 3.
C
      HLOW=9999.
      HHIGH=9999.
C
...
C
         DO 156 J=1,JDIM
C
D        IF(CCALL(K).EQ.'516198 ')THEN
D           WRITE(KFILDO,155)K,CCALL(K),NAME(K),J,HDATA(K,J),ELEV(K),
D    1                       VDATA(K,N2,J),AVXDAT(K)
D155        FORMAT(/' AT 155 IN LAPSUA--K,CCALL(K),NAME(K),J,',
D    1              'HDATA(K,J),ELEV(K),VDATA(K,N2,J),AVXDAT(K)',/,
...
C
            IF(J.GT.2)THEN
C                 UNLESS THE ELEVATIONS ARE QUITE LOW, THE LAPSE
C                 WILL BE COMPUTED OVER A RANGE OF ELEVTIONS THAT
C                 INCLUDES THE STATION.
               HLOW=HDATA(K,J-1)
               VLOW=VDATA(K,N2,J-1)
               JLOW=J-1
               GO TO 157
...
C
      ELSE
C           HLOW AND VLOW ARE NOT USED WHEN LAPFG = 3.  SET JLOW
C           FOR LOOP BELOW.
         JLOW=0
         GO TO 157
C
...
C
      WRITE(KFILDO,1569)
 1569 FORMAT(/,' ****ERROR IN LAPSUA AT 1569.',
...
C
CD     IF(CCALL(K).EQ.'516198 ')THEN
CD        WRITE(KFILDO,1575)K,CCALL(K),NAME(K),J,HDATA(K,J),ELEV(K),
CD    1                    VDATA(K,N2,J),AVXDAT(K),ELEVHI(K)
CD1575    FORMAT(/' AT 1575 IN LAPSUA--K,CCALL(K),NAME(K),J,HDATA(K,J),',
CD    1           'ELEV(K),VDATA(K,N2,J),AVXDAT(K),ELEVHI(K)'
CD    1           ,/,I5,2X,A8,2X,A20,I3,5F8.1)
D     ENDIF
C
...
C        HAS NOT BEEN EXCEEDED.  USE THE 500 MB LEVEL.
      HHIGH=HDATA(K,JDIM)
      VHIGH=VDATA(K,N2,JDIM)
C
C        COMPUTE LAPSE.
C
 162  IF(LAPFG.EQ.2)THEN
C
         IF(NINT(HHIGH).EQ.9999.OR.NINT(HLOW).EQ.9999)THEN
            WRITE(KFILDO,1621)CCALL(K),NAME(K)
 1621       FORMAT(/,' ****LAPSE COULD NOT BE COMPUTED FOR STATION  ',
     1             A8,2X,A20,' AT 1621 IN LAPSUA.  SET IT TO ZERO.')
            XLAPSE(K)=0.
         ELSE
C
            IF(HHIGH-HLOW.GT.130.)THEN
C                 DON'T COMPUTE OVER A SMALL RANGE OF HEIGHTS.  THE
C                 130 AGREES WITH WHAT IS USED FOR COMPUTING THE LAPSE
C                 WITH STATION DATA IN U174.
...
C
         IF(NINT(HHIGH).EQ.9999.OR.NINT(ELEV(K)).EQ.9999)THEN
            WRITE(KFILDO,1621)CCALL(K),NAME(K)
            XLAPSE(K)=0.
         ELSE       
C
            IF(HHIGH-ELEV(K).GT.130.)THEN
C                 DON'T COMPUTE OVER A SMALL RANGE OF HEIGHTS.  THE
C                 130 AGREES WITH WHAT IS USED FOR COMPUTING THE LAPSE
C                 WITH STATION DATA IN U174.  AVXDAT(K) HAS BEEN CHECKED
C                 FOR NON MISSING ABOVE.
...
C
      ...
      ENDIF
C
CD     WRITE(KFILDO,1624)K,CCALL(K),AVXDAT(K),ELEV(K),
CD    1                 (VDATA(K,N2,J),J=1,JDIM),
CD    2                 (HDATA(K,J),J=1,JDIM) 
CD1624 FORMAT(/' AT 1624--K,CCALL(K),AVXDAT(K),ELEV(K),',
CD    1       '(VDATA(K,N2,J),J=1,JDIM),',
CD    2       '(HDATA(K,J),J=1,JDIM)',/,
CD    3         I6,2X,A8,2F8.1/,(12F9.2))
CC        FORMAT ABOVE SET FOR JDIM = 12. 
C    
CD     WRITE(KFILDO,1625)K,CCALL(K),AVXDAT(K),ELEV(K),
CD    1                  ELEVLO(K),ELEVHI(K),
CD    2                  VHIGH,VLOW,HHIGH,HLOW,XLAPSE(K)
CD1625 FORMAT(' AT 1625--K,CCALL(K),AVXDAT(K),ELEV(K),',
CD    1       'ELEVLO(K),ELEVHI(K),',
CD    2       'VHIGH,VLOW,HHIGH,HLOW,XLAPSE(K)',/,
CD    3       I11,2X,A8,F7.1,F9.1,F11.0,F6.0,4F7.1,F8.4)
C
 200  CONTINUE
C
...
C
 ...
 205  FORMAT(/,    I7,' = NUMBER OF COMPUTED LAPSE RATES OUTSIDE',
     1                ' RANGE',F6.3,' AND,',F6.3,' AND SET TO ZERO',/,
...
C
      IF(IP14.NE.0.AND.IFIRST.EQ.0.AND.(LAPFG.EQ.2.OR.LAPFG.EQ.3))THEN
         IFIRST=1
...
C
      ...
      SV174=FL174
C
CD     WRITE(KFILDO,217)N2,(IDSAV(M),M=1,3),(NDATSV(M),M=1,3),
CD    1                    (NTAUSV(M),M=1,3)
CD217  FORMAT(/' LAPSUA AT 217--N2,IDSAV(1-3),NDATSV(1-3),',
CD    1        'NTAUSV(1-3),',I2,1X,3I10,3I11,3I3)
C
      GO TO 250
C     
