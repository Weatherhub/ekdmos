      SUBROUTINE SCLEQ6(KFILDO,KFIL10,NDATE,ID,IDPARS,JD,
     1                  XDATA,FD2,ND1,NVAL,
     ...
     8                  L3264B,ISTOP,IER)
C
C        MAY       2007   GLAHN   TDL   MOS-2000
C                                 ADAPTED FROM SCLSNO
C        JUNE      2007   SMB     CHANGED FLOATS TO REALS.  ADDED
C                                 COMMA FOR IBM COMPILE.
C
C        PURPOSE
...
C            WAS WRITTEN FOR 6-HQPF, AND ITABLE( , , ) AND TABLE( , , )
...
C                       SCLEQ6 CAN BE USED FOR EITHER VECTOR DATA
...
C                       (I=1,2), (J=1,NCAT-1).  (SEE ITABLE( , , , ))
C                       FOR MORE EXPLANATION.  (INTERNAL)     
C        1         2         3         4         5         6         7 X
C
...
C
      DATA ITABLE/        0,000000000,0,        0,
     1            203220100,003000000,0,950052000,         
     2            203220100,003000000,0,495051000,         
     3            203220100,003000000,0,950051000,
     4            203220100,003000000,0,149500000,
     5            203220100,003000000,0,195000000,
     6            203220100,003000000,0,245000000,
     7            203220100,003000000,0,295000000,
     8            203220100,003000000,0,395000000,
     9            203220100,003000000,0,495000000,
     X            203220100,003000000,0,749500000,
     A            203220100,003000000,0,995000000,
     B            203220100,003000000,0,149501000,
     C            203221000,003000000,0,        0,
     D                    0,0,0,        0,
     E            213220100,0,0,950052000,
     F            213220100,0,0,495051000,
     G            213220100,0,0,950051000,
     H            213220100,0,0,149500000,
     I            213220100,0,0,195000000,
     J            213220100,0,0,245000000,
     K            213220100,0,0,295000000,
     L            213220100,0,0,395000000,
     M            213220100,0,0,495000000,
     N            213220100,0,0,749500000,
     O            213220100,0,0,995000000,
     P            213220100,0,0,149501000,
     Q            213221000,0,0,        0/
      DATA TABLE/          0., .000,
     1                    .01, .0495,
     2                    .05, .095,
...
C
      IER=0
D     WRITE(KFILDO,100)NX,NY
D100  FORMAT(/' AT 100 IN SCLEQ6--NX,NY',2I6)
C
...
D    1              (((ITABLE(I,J,L),I=1,4),J=1,NCAT),L=1,2)
D101  FORMAT(/' AT 101 IN SCLEQ6--NCAT,',
D    1        '(((ITABLE(I,J,L),I=1,4),J=1,NCAT),L=1,2)',/,
D    2         I6,/,(4I11))
D     WRITE(KFILDO,102)((TABLE(I,J),I=1,2),J=1,NCAT-1)
D102  FORMAT(/' AT 102 IN SCLEQ6--',
D    1        '((TABLE(I,J),I=1,2),J=1,NCAT-1)',/,(2F10.3))
C
      DO 105 L=1,2
C
...
C
 ...
 110  FORMAT(/' ****VARIABLE ',I9.9,I10.9,I10.9,I4.3,' NOT',
     1        ' ACCOMMODATED IN SUBROUTINE SCLEQ6.  IER =',I3)
      IER=103
      GO TO 900
C
...
D1135 FORMAT(' AT 1135 IN SCLEQ6--NVAL,FACTOR,(XDATA(M),M=1,NVAL)',
D    1        I12,F8.2,/,(15F8.1))
C
      KOUNT=0
C
...
C
         IF(KOUNT.EQ.0)THEN
            WRITE(KFILDO,116)J
 116        FORMAT(/' ****THERE WERE NO QPF FORECASTS IN CATEGORY',I4,
...
D117  FORMAT(/' AT 116--J,L,(ITABLE(M1,J,L),M1=1,4)',
D    1          6I12)
C
...
C
 ...
 120     FORMAT(/' ****COULD NOT FIND PROBABILITY RECORD',
     1           3I10.9,I10,'.  FATAL ERROR IN SCLEQ6 AT 120.')
         GO TO 900
      ENDIF
C
...
C 
 ...
 146           FORMAT(/' ****MAP PROJECTION NUMBER NPROJ =',I3,
     1           ' NOT 3, 5, OR 7.  FATAL ERROR IN SCLEQ6 AT',
     2           ' 146.')
         ISTOP(1)=ISTOP(1)+1
         IER=777
         GO TO 900
...
C
 ...
 148           FORMAT(/' ****NX AND NXI =,',2I6,' OR NY AND NYI =',2I6,
     1           ' DO NOT AGREE AT 148 IN SCLEQ6.  FATAL ERROR.')
         ISTOP(1)=ISTOP(1)+1
         IER=777
         GO TO 900
      ENDIF
...
D149        FORMAT(/' AT 149 IN SCLEQ6--MESHI,MESH,NXI,NYI,NX,NY',
D    1        6I6)
C
...
D1495    FORMAT(' IN SCLEQ6 AT 1495--XDATA(K),DATA(K),XMAX,XMIN',
...
D160  FORMAT(/,' IN SCLEQ6 AT 160--XMAX,XMIN,RANGE,',
...
D225  FORMAT(/,' IN SCLEQ6 AT 225',/,(15F8.2))
D230  CONTINUE
C
 900  RETURN
      END
