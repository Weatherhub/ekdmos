...
C
      IER=0
C
C        FIND THE MEAN OF THE GFS ENSEMBLE MEMBERS.
C
      IF((IDPARS(4).EQ.62).OR.(IDPARS(4).EQ.63))THEN
         CALL ENSMEAN(KFILDO,KFIL10,NFIRST,
     1                ID,IDPARS,THRESH,JD,NDATE,
...
C
     ...
     5               ISTAV,L3264B,MISTOT,IER)
C
C        LOOK FOR COMPUTATION OF HEAT INDEX
C
      ELSEIF(IDPARS(1).EQ.002.AND.IDPARS(2).EQ.003) THEN
         CALL MODTHTX(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1               NGRIDC,ND11,NSLAB,IPACK,IWORK,DATA,ND5,
     2               LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3               IS0,IS1,IS2,IS4,ND7,
     4               FD1,FD2,ND2X3,
...
C
     ...
     5               ISTAV,L3264B,MISTOT,IER)
C
C        LOOK FOR COMPUTATION OF SURFACE WIND CHILL FROM MODEL DATA IN DEGREES F
C
      ELSEIF(IDPARS(1).EQ.002.AND.IDPARS(2).EQ.221) THEN
         CALL WCHILL(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1               NGRIDC,ND11,NSLAB,IPACK,IWORK,DATA,ND5,
     2               LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3               IS0,IS1,IS2,IS4,ND7,
     4               FD1,FD2,FD3,ND2X3,
