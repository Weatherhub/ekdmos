      SUBROUTINE ADVTWL(KFILDO,KFIL10,IDPARS,ICCCFFF,NDATE,
     1                  SDATA,ND1,NSTA,NGRIDC,ND11,NSLAB,IPACK,IWORK,
     2                  FDAV,ND5,
     3                  LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     4                  IS0,IS1,IS2,IS4,ND7,
     5                  FD1,FD2,FD3,FD4,FDSINS,FDMS,ND2X3,
     6                  ISTAV,L3264B,MISTOT,IER)
C
C	 JANUARY  2005  CHARBA     ADVTWL IS AN EXTENDED VERSION OF 
C				   ADVCTW, WHICH ADDS THE CAPABILITY OF
C			           PERFORMING TIMEWISE INTERPOLATION 
C				   OF GFS WINDS WHEN TAU IS NOT A WHOLE 
C				   MULTIPLE OF 3 HOURS.
C        SEPTEMBER2005  CHARBA     CHANGED TO ACCOMMODATE SWITCH IN 
C				   ID(1) FROM 003509008 TO 003509005.
C        JUNE           CHARBA     MADE COSMETIC CHANGES TO SATISFY CODE
C                                  WALK-THRU.
C
C        PURPOSE
C            TO COMPUTE ADVECTION OF A QUANTITY, Q, FROM U- AND V- WIND
C            COMPONENTS ON AN ISOBARIC, OR A CONSTANT HEIGHT, OR A SIGMA
C            SURFACE.  UNITS ARE Q/SEC.  SINCE THE ACTUAL GRID DIMEN-
C	     SIONS LX AND LY ARE NOT KNOWN UNTIL THE DATA ARE FETCHED, 
C	     THE SPATIAL GRID COMPUTATIONS ARE MADE IN SUBROUTINES 
C	     MAPLAT ADVCTW1.  THIS SUBROUTINE IS UNUSUAL IN THAT THERE 
C	     IS BOTH AN INPUT GRID AND AN OUTPUT GRID (IN FDAV( )).
C
C        DATA SET USE 
...
C              FD1(J) = WORK ARRAY TO HOLD MAP FACTOR (J=1,ND2X3)
C              FD2(J) = WORK ARRAY TO HOLD U WIND (J=1,ND2X3)
C              FD3(J) = WORK ARRAY TO HOLD V WIND (J=1,ND2X3)
...
C                             2 FIELDS.
C                       SEE GFETCH, TIMGRD, AND MAPLAT FOR OTHER VALUES
C                       WHEN IER.NE.0 AND DATA ARE RETURNED AS MISSING. 
C                       (INTERNAL-OUTPUT)
...
C            GFETCH, TIMGRD, MAPLAT, ADVCTW1
C
      ...
      INTEGER LD(4),LDPARS(15),MD(4),MDPARS(15)
      INTEGER I,ICCCFFF,IER,ISTAV,J,KFIL10,KFILDO,L3264B,LITEMS,LX,LY,
     1        MISSP,MISSS,MISTOT,MX,MY,NBLOCK,ND2X3,ND5,ND7,ND9,ND10,
     2        ND11,NDATE,NFETCH,NPACK,NSLAB,NSLABU,NSLABV,NTIMES,
     3        NWORDS,KK,NSTA,ND1
C
      REAL SDATA(ND1)
      REAL FDAV(ND5)
...
C
      ...
      ISTAV=0
C
C        TO OBTAIN U WINDS IN COMPUTING ADVECTION CREATE ID FOR FETCHING
C	 U-WINDS.  IF GFS WINDS ARE TO BE USED AND TAU IS NOT A WHOLE 
C	 MULTIPLE OF 3 HOURS THEN CALL TIMGRD TO PERFORM TIME INTERPO-
C	 LATION.
C
      LD(1)=ICCCFFF
      LD(2)=IDPARS(7)
 ...
 20   CONTINUE
C
      IF(MOD(ICCCFFF,100).EQ.05.OR.MOD(IDPARS(12),3).EQ.0) THEN
C
C          CALL GFETCH OR TIMGRD TO FETCH U-WINDS IN FD2.
C
     ...
     1              SDATA,ND1,NSTA,NSLABU,IPACK,IWORK,FD2,ND2X3,
...
C
 ...
 100     FORMAT(/' ****U-WIND FETCH FAILED IN ADVTWL.  IER=',I3)
	 GOTO 900
      ENDIF
C
      LX=IS2(3)
      ...
      NSLAB=NSLABU
C
C        TO OBTAIN V WINDS IN COMPUTING ADVECTION CREATE ID FOR FETCHING
C	 V-WINDS.  IF GFS WINDS ARE TO BE USED AND TAU IS NOT A WHOLE 
C	 MULTIPLE OF 3 HOURS THEN CALL TIMGRD TO PERFORM TIME INTERPO-
C	 LATION.
C
      MD(1)=ICCCFFF+100000
...
C
      IF(MOD(ICCCFFF,100).EQ.05.OR.MOD(IDPARS(12),3).EQ.0) THEN
C
C          CALL GFETCH OR TIMGRD TO FETCH V-WINDS IN FD3.
C
     ...
     1              SDATA,ND1,NSTA,NSLABV,IPACK,IWORK,FD3,ND2X3,
     2              LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3              IS0,IS1,IS2,IS4,ND7,
     4              FD1,FD4,ND2X3,
     5              ISTAV,L3264B,MISTOT,IER)
...
C
 ...
 200     FORMAT(/' ****V-WIND FETCH FAILED IN ADVTWL.  IER=',I3)
	 GOTO 900
      ENDIF
C
C	 IF LAMP MODEL (MOD(ICCCFFF,100)=05), CHANGE UNITS FROM KT TO 
C	 M/S.
C
C     IF(IDPARS(4).EQ.05) THEN
      IF(MOD(ICCCFFF,100).EQ.05) THEN
        DO 220 J=1,NWORDS
          FD2(J)=FD2(J)*.5148
          FD3(J)=FD3(J)*.5148
 220    CONTINUE
...
C
      ...
      MY=IS2(4)
C
      IF(NSLAB.NE.NSLABV.AND.LX.NE.MX.AND.LY.NE.MY)THEN
C           THE GRID CHARACTERISTICS ARE NOT THE SAME.
 ...
 300     FORMAT(/' ****DIFFERENT GRID CHARACTERISTICS.  PREDICTOR NOT ',
     1           'COMPUTED IN ADVTWL.  VALUES FROM NGRIDC( , ) AND X*Y',
