      SUBROUTINE MOSTDPB(KFILDO,KFIL10,IDPARS,JD,NDATE,
...
C
C        PURPOSE 
C            TO MAKE QUASI-BINARY PREDICTORS OUT OF MOS TEMPERATURE AND
C            DEWPOINT, WHICH REPRESENT THE HOW LIKELY PRECIPITATION WOULD
C            FALL AS EITHER FROZEN (SNOW) OR FREEZING (SLEET/FREEZING RAIN)
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C               202 502 - MOS WEIGHTED TEMPERATURE BIN. (FRZN)
C               202 503 - MOS WEIGHTED TEMPERATURE BIN. (FRZG)
C               203 502 - MOS WEIGHTED DEWPOINT BIN. (FRZN)
C               203 503 - MOS WEIGHTED DEWPOINT BIN. (FRZG)
C
C        DATA SET USE 
...
C              FD1(J) = WORK ARRAY TO HOLD MOS TDP FROM GFETCH (J=1,ND1) (INTERNAL)
...
C               MD(J) = WORK ARRAY HOLDING THE 4 ID WORDS OF MOS TDP
...
C
      IENTER=IENTER+1
C
C        CHECK IF THIS CODE ACCOMMODATES MOS TDP BIN
C       
      DO 105 JJ=1,NDIM 
...
C
 ...
 107  FORMAT(/,' ****MOSTDPB ENTERED FOR VARIABLE',
...
C
C        CONSTRUCT THE ID FOR MOS TEMPERATURE OR DEWPOINT.  
C        DATA WILL BE RETURNED IN FD1( ).  ALTHOUGH ND2X3 IS BEING USED,
C        TO DIMENSION ARRAY, THE DATA WILL BE IN VECTOR FORMAT
C
...
C
 ...
 405        FORMAT(/,' ****ERROR IN MOSTDPB, FOR FIRST PROCESS DATE,',
     1               ' MOS TEMPERATURE OR DEWPOINT IS MISSING',
...
C         THE APPROPRIATE VALUE.  IF MOS TEMP/DEWPT IS MISSING, SET
...
C
      ENDIF
C
C        FILL IN THE MOS TDP BIN ARRAY WITH MISSING VALUES
C
 800  DO 810 J=1,ND1
         SDATA(J)=9999.
 810  CONTINUE
