...
C            BOUND HARDWIRED IN THIS ROUTINE.
C
...
C              ITABLE(1,II)   ITABLE(2,II)  RBOUNDS(1,II)  RBOUNDS(2,II)
C
C            AS ANY NEW CCCFFF IS REMOVED OR ADDED TO ITABLE( , ) AND 
C            RBOUNDS( , ), CHANGE THE PARAMETER NDIM APPROPRIATELY.
C
...
C        RBOUNDS(I,J) = TABLE CONTAINING THE LOWER(I=1) AND UPPER(I=2) BOUNDS
C                       TO USE TO CONSTRAIN THE GRIDDED FIELD.  PLACEMENT OF
C                       FIELD IN RBOUNDS MUST MATCH PLACEMENT OF CCCFFFS IN 
C                       ITABLE. (INTERNAL) 
...
C
        IF(FD1(J).LT.RBOUNDS(1,JJ).AND.(NINT(FD1(J)).NE.9999))THEN
           DATA(J)=RBOUNDS(1,JJ)
        ELSEIF((FD1(J).GT.RBOUNDS(2,JJ)).AND.(NINT(FD1(J)).NE.9999))THEN
           DATA(J)=RBOUNDS(2,JJ)
        ELSE
          DATA(J)=FD1(J)
        ENDIF
