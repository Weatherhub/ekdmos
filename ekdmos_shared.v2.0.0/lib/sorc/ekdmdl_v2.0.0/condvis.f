      SUBROUTINE CONDVIS(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                   SDATA,ND1,NSTA,IPACK,IWORK,FD1,ND2X3,
     2                   LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3                   IS0,IS1,IS2,IS4,ND7,
     4                   ISTAV,L3264B,IER)
C
C        MAY      2005   RUDACK   MDL   MOS-2000 (ADAPTED FROM SUBROUTINE
C                                                 OBSPOPO)
C        FEBRUARY 2006   RUDACK   MADE COSMETIC CHANGES TO CODE.  
C                                 DELETED UNUSED ARRAY "FD2( )".
C
C        PURPOSE 
C            THIS SUBROUTINE WILL DETERMINE CONDITIONAL VISIBILITY BASED
C            UPON THE PRESENT WEATHER REPORTS OF PRECIPITATION OCCURRENCE
C            AT THE HOUR.  IF PRECIPITATION IS OCCURRING AT THE HOUR,
C            THE VISIBILITY VALUE IS RETAINED, OTHERWISE THE VISIBILITY
C            IS ASSIGNED A VALUE OF '9999.' 
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C               708145 - CONDITIONAL VISIBILITY AT A SINGLE HOUR                
C
...
C                       (INPUT).
C            SDATA(K) = DATA TO RETURN (CONDITIONAL OBSERVED VISIBILITY)
C                       (K=1,NSTA).  (OUTPUT) 
...
C                       PRESENT WEATHER AND OBSERVED VISIBILITY.
C                       (INTERNAL)
C               ND2X3 = DIMENSION OF IPACK( ),IWORK( ), FD1( ). (INPUT)
...
C
      IF((IDPARS(1).NE.708).OR.(IDPARS(2).NE.145))THEN
         IER=103
...
C
 ...
 120    FORMAT(/,' ****CONDVIS CAN NOT PROCESS DATA BECAUSE OF THE',
...
C
 ...
 140        FORMAT(/,' ****ERROR FROM GFETCH OCCURRED ON',
     ...
     2               '.  CONDVIS CANNOT RUN, PLEASE READ WRITE UP')
            GO TO 900
          ENDIF
C
...
C
 ...
 160        FORMAT(/,' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2               ' IN CONDVIS. DATA SET TO MISSING.',
     3               '  IER=',I3)
            GO TO 900
          ENDIF
C
...
C
 200    CONTINUE
C
C          PLACE THE OBSERVED VISIBILITY ID INTO ARRAY LD( ). 
C
        LD(1)=708100000
        LD(2)=0
        LD(3)=0
        LD(4)=0
C
C          RETRIEVE THE OBSERVED VISIBILITY. 
C
        CALL GFETCH(KFILDO,KFIL10,LD,7777,LSTORE,ND9,LITEMS,
     1              IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FD1,ND2X3,
     2              NWORDS,NPACK,NDATE,NTIMES,CORE,ND10,
...
C
        IF(NWORDS.NE.NSTA)THEN
           IER=52
           WRITE(KFILDO,300)NWORDS,NSTA,IER
...
C
        IF((IER.NE.0).AND.(IENTER.EQ.1)) THEN
          WRITE(KFILDO,355)NDATE
 355      FORMAT(/,' ****ERROR FROM GFETCH OCCURRED ON',
     1             ' 1ST PROCESS DATE ',I12,
     2             '.  CONDVIS CANNOT RUN, PLEASE READ WRITE UP.')
          GO TO 900
        ENDIF
...
C
 ...
 400    CONTINUE
C
C          PLACE THE THE OBSERVED VISIBILITY ID INTO ARRAY LD( ).
C
        LD(1)=708100000
        LD(2)=0
        LD(3)=0
...
C
        IF(NWORDS.NE.NSTA)THEN
           IER=52
           WRITE(KFILDO,460)NWORDS,NSTA,IER
