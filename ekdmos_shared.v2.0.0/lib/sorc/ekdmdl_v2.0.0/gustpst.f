      SUBROUTINE GUSTPST(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                   IDPARS,JD,ITAU,
     ...
     7                   L3264B,L3264W,IER)
C 
C        MAY        2005   WIEDENFELD   MDL   MOS-2000
C        JUNE       2006   WIEDENFELD   MDL   CLEANED UP CODE.
C
C        PURPOSE
C            POST PROCESSED LAMP WIND GUST = LMP GUST DIFFERENCE + 
C            LMP WIND SPD UNDER THE FOLLOWING CONDITIONS.  FIRST 
C            COMBINE CATEGORICAL GUST FORECASTS WITH THE CONTINUOUS 
C            GUST DIFFERENCE FORECAST.  AFTER THAT ENSURE THAT THE GUST
C            FORECAST IS ONLY MADE WHEN THE LAMP WIND SPEED IS AT
C            LEAST 3 KNOTS, THE DIFFERENCE FORECAST IS AT LEAST 4.2 
C            KNOTS AND THE LAMP FORECASTED WIND GUST SPEED IS AT 
C            LEAST 13.5 KNOTS.
C             
C
C            IDPARS(1) & IDPARS(2) OF WIND GUST SPEED ARE MAPPED 
C            INTO      FROM 
C            204355    204350 (WIND GUST DIFF SPEED)
C                      204366 (CATEGORICAL WIND GUST)
C                      204325 (POST PROCESSED INFLATED LAMP WND SPD FCSTS)
C
C        DATA SET USE 
...
C            XDATA(K) = CATEGORICAL WIND GUST BEFORE MODIFCATION, 
C                       LAMP WIND GUST AFTER MODIFICATION
C                       (K=1,NSTA).  (OUTPUT)
...
C                       YDATA( ) AND ZDATA( ) (J=1,4).  (INTERNAL)
C            YDATA(K) = WORK ARRAY; LAMP WIND GUST SPEED (K=1,NSTA).
C                       (AUTOMATIC)
C            ZDATA(K) = WORK ARRAY; LAMP WIND SPEED (K=1,NSTA).
C                       (AUTOMATIC)
C         ITABLE(I,J) = VALUES OF CCCFFF ACCOMMODATED (I=1) AND
C                       THE ASSOCIATED VARIABLES NEEDED (I=2 THROUGH 4)
C                       (J=1,NDIM).  (INTERNAL)
C                NDIM = SECOND DIMENSION OF ITABLE( , ).  SET BY
C                       PARAMETER.  (INTERNAL)
C           LDPARS(J) = PARSED ID FILLED IN SUBROUTINE PRSID1 FOR
C                       CATEGORICAL WIND GUST (J=1,15). (INTERNAL)
C           MDPARS(J) = PARSED ID FILLED IN SUBROUTINE PRSID1 FOR 
C                       LMP WIND GUST SPD AND LMP WIND SPD.(J=1,15). 
C                       (INTERNAL)
C 
...
C
C        YDATA( ) AND ZDATA( )ARE AN AUTOMATIC ARRAYS.
C
      ...
      DIMENSION ITABLE(4,1)
C
      DATA LDPARS/15*0/
      DATA ITABLE/204355, 204366, 204350, 204325/
C
      IER=0
C     
...
C     
 ...
 107  FORMAT(/,' ****GUSTPST ENTERED FOR VARIABLE',
...
C        YDATA( )
C     
 108  LD(1)=ITABLE(2,JJ)*1000+IDPARS(4)
      LD(2)=JD(2)
      LD(3)=JD(3)
C
...
C          
C        FETCH THE LMP BEST CAT WND GUST INTO XDATA( ).
C         
...
C
 ...
 125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN GUSTPST',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
        GOTO 300
      ENDIF
C
      ...
      MD(3)=JD(3)
C          
C        FETCH THE LMP WIND GUST INTO YDATA( ).
C         
...
C
      ...
      ENDIF
C
      MD(1)=ITABLE(4,JJ)*1000+IDPARS(4)
      MD(2)=JD(2)
      MD(3)=JD(3)
C          
C        FETCH THE LMP WIND SPD INTO ZDATA( ).
C         
      MD(4)=IDPARS(13)*100+IDPARS(14)*10+IDPARS(15)
      CALL PRSID1(KFILDO,MD,MDPARS)
      CALL RETVEC(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
...
C
      ...
      ENDIF
C
C        NOW POST PROCESS THE GUSTS.  
C        1.  ONLY FORECAST A GUST IF CAT=2, ELSE SET GUST TO 0 
C        2.  IF GUST DIFF GE 4.2 AND WND SPD GE 3 SET
C            GUST TO GUST DIFF + SPD
C        3.  IF GUST IS STILL LESS THAN 13.5 SET GUST TO 0
C        4.  IF ANY OF THE WND SPD, GUST DIFF, OR CAT GUST 
C            ARE MISSING SET GUST TO MISSING.
C        
      DO 170 K=1,NSTA
C
         ISDATA(K)=NINT(XDATA(K))
         SELECT CASE(ISDATA(K))
C
                CASE(1)
                   XDATA(K)=0.
                CASE(2)
     ...
     1                NINT(ZDATA(K)).EQ.9999)THEN
                      XDATA(K)=9999.
                   ELSEIF(YDATA(K).GE.4.2.AND.ZDATA(K).GE.3.)THEN
                      XDATA(K)=YDATA(K)+ZDATA(K)
                      IF(XDATA(K).LT.13.5)XDATA(K)=0.
                   ELSE
                      XDATA(K)=0.
                   ENDIF
                CASE(9999)
