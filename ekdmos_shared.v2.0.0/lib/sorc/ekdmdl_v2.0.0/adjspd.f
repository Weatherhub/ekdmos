      SUBROUTINE ADJSPD(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
     ...
     7                  L3264B,L3264W,IER)
C 
C        MAR   2003   MCALOON MDL   MOS-2000
C        JAN   2007   RLC     ADDED IDS TO ADJUST WIND GUSTS
C
C        PURPOSE 
C            TO ADJUST THE FORECAST WIND SPEED TO BE CONSISTENT WITH 
C            WIND SPEED MEASURED AT 10M. THIS ROUTINE WAS ORIGINALLY
C            WRITTEN FOR MARINE WIND DEVELOPMENT TO ACCOUNT FOR THE
C            VARIABILITY IN ANEMOMETER HEIGHT AT MARINE STATIONS. 
C            IDPARS (1) AND IDPARS(2) ARE MAPPED: 
C                        INTO        FROM 
C                       204335       204325 (SFC. WIND SPEED)
C                       204380       204375 (SFC. WIND GUST)
C
...
C            NEEDED.  ADJSPD ASSUMES THERE WILL BE NO SECONDARY
C            MISSING VALUES OF 9997; THESE SHOULD HAVE ALREADY BEEN
C            TREATED.
C
C	     THIS ROUTINE IS AN ADAPTATION OF THE ZRONEG ROUTINE. REFER
C            TO ZRONEG FOR THE ORIGINAL DOCUMENTATION.
C
C        DATA SET USE 
...
C                IDHT = THE ID FOR THE ANEMOMETER HEIGHT VARIABLE.
C              HEIGHT = THE ANEMOMETER HEIGHT OBTAINED FROM CONST.
C 
C        NONSYSTEM SUBROUTINES CALLED 
C            PRSID1,RETVEC,CONST
C
      PARAMETER (NDIM=2)
C
...
C
      ...
      DIMENSION HEIGHT(ND1)
C        YDATA( ) IS AN AUTOMATIC ARRAY
      DIMENSION ID(4),IDPARS(15),JD(4)
      DIMENSION LD(4),KFILRA(5),IDHT(4)
      DIMENSION IPACK(ND5),IWORK(ND5),DATA(ND5)
      DIMENSION IS0(ND7),IS1(ND7),IS2(ND7),IS4(ND7)
      DIMENSION LSTORE(12,ND9),ICALLD(L3264W,ND5)
      DIMENSION CORE(ND10)
      DIMENSION ITABLE(2,NDIM),LDPARS(15)
C
      DATA LDPARS/15*0/
      DATA ITABLE/204335, 204325,
     1            204380, 204375/        
      DATA IDHT/400008000,0,0,0/
C
      IER=0
C
...
C
 ...
 115  CONTINUE
C
      WRITE(KFILDO,117)(ID(L),L=1,4)
 117  FORMAT(/,' ****ADJSPD ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 320
C
C        FETCH THE ANEMOMETER HEIGHT
C
 120    LD(1)=IDHT(1)                    
        LD(2)=IDHT(2)
        LD(3)=IDHT(3)
...
C
  ...
  125      FORMAT(/,' ****VARIABLE NOT RETRIEVED BY CONST IN ADJSPD',
     1            2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
           GO TO 320
        ENDIF
C
C        FETCH THE WIND SPEED OR GUST.
C
 220  LD(1)=ITABLE(2,JJ)*1000+IDPARS(4)
      LD(2)=JD(2)
      LD(3)=JD(3)
...
C
  ...
  225    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN ADJSPD',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 320
      ENDIF
C
C         ADJUST THE WIND SPEED. FIRST IF THE HEIGHT IS MISSING FROM
C         THE CONSTANT FILE, SET THE ADJUSTED WIND TO MISSING. IF THE 
C         ORIGINAL WIND SPEED IS MISSING, SET THE ADJUSTED WIND SPEED
C         TO MISSING. 
C
      DO 290 K=1,NSTA
       IF(NINT(HEIGHT(K)).EQ.9999)THEN
         XDATA(K)=9999.
       ELSE IF(NINT(XDATA(K)).EQ.9999)THEN
         XDATA(K)=9999. 
       ELSE
         XDATA(K)=XDATA(K)*(10.0/HEIGHT(K))**0.11
       ENDIF
 290  CONTINUE
C
      GO TO 350
