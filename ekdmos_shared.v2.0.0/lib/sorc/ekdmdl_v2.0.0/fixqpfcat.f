      SUBROUTINE FIXQPFCAT(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                     ID,IDPARS,JD,ITAU,
     ...
     7                     L3264B,L3264W,IER)
C 
C        SEPTEMBER 2005  MALONEY   MDL   MOS-2000
C
C        PURPOSE 
C            TO UNIFY THE 12-H QPF CATEGORIES FROM THE SHORT- AND
C            EXTENDED-RANGE GFS MOS SYSTEMS INTO ONE ID.  THIS IS
C            STRICTLY FOR BUFR.
C
C            IDPARS (1) AND IDPARS(2) 
C            ARE MAPPED  INTO        FROM      AND
C                       203 332    203 331   203 321
C
...
C            XDATA(K) = CATEGORICAL FORECAST TO CHANGE (K=1,NSTA).  (OUTPUT)
...
C
      DATA ITABLE/203332, 203331, 203321/
C
      IER=0
C
...
C
 ...
 107  FORMAT(' ****FIXQPFCAT ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        FETCH THE ORIGINAL CATEGORY.
C
...
C
      IF(IER.NE.0)THEN
C
C           AT THIS POINT, TRY THE SECONDARY CATEGORY.
C
...
C
         IF(IER.NE.0)THEN
C
C              BOTH FETCHES HAVE FAILED.
C
            WRITE(KFILDO,125)(LD(J),J=1,4)
  125       FORMAT(' ****VARIABLE NOT RETRIEVED BY RETVEC IN FIXCAT',
     1             2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
            GO TO 300
         ENDIF
      ENDIF
C
C        CATEGORIES HAVE BEEN FETCHED.  NO PROCESSING IS REQUIRED.
C
      GO TO 350   
C
