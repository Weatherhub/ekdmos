      SUBROUTINE WNDCHL(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C                                  FROM GFETCH
C        SEPTEMBER 1999   GLAHN    MODIFIED DO 211 LOOP; ADDED YDATA( );
C        APRIL     2000   GLAHN    IMPROVED COMMENTS; CHECKED SPELLING;
C                                  ADDED DIMENSION FOR ISDATA( ); ADDED
C                                  /, IN **** COMMENTS 
C        APRIL     2000   RUDACK   ADDED DATA LDPARS/15*0/; ADDED TO 
C                                  ITABLE( , ) AND COMMENTS
C
C        PURPOSE 
C            TO DETERMINE THE WIND CHILL GIVEN TEMPERATURE
C            AND WIND SPEED IN KNOTS.  
C
C            IDPARS(1) & IDPARS(2) OF TEMPERATURE  
C            ARE MAPPED INTO      FROM 
C                       202040    202020 AND 204320 (UNINFLATED WIND SPEED)
C                       202045    202020 AND 204325 (INFLATED WIND SPEED)
C
C        DATA SET USE 
...
C            XDATA(K) = TEMPERATURE IN F (INTERNAL), THEN WIND CHILL
C                       (K=1,NSTA).  (OUTPUT)
...
C                       (INTERNAL)
C            YDATA(K) = WORK ARRAY WHICH STORES WIND SPEED IN KTS (K=1,ND1).  
C                       (AUTOMATIC)
C         ITABLE(I,J) = VALUES OF CCCFFF ACCOMMODATED (I=2) AND
C                       THE ASSOCIATED VARIABLES NEEDED (I=2,3)
C                       (J=1,NDIM).  (INTERNAL)
C                NDIM = SECOND DIMENSION OF ITABLE( , ).  SET BY
C                       PARAMETER.  (INTERNAL)
C              WNDMPH = WIND SPEED IN MILES/HOUR.
C 
...
C  
      DATA LDPARS/15*0/  
      DATA ITABLE/202040,202020,204320,
     1            202040,202020,204325/
C
      IER=0
C
...
C
 ...
 107  FORMAT(/,' ****WNDCHL ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        GET THE TEMPERATURE IN FAHRENHEIT.
C
 120  LD(1)=ITABLE(2,JJ)*1000+IDPARS(4)+10
      LD(2)=JD(2)
      LD(3)=JD(3)
...
C
  ...
  125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN WNDCHL',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        GET THE WIND SPEED IN KNOTS.
C
...
C
  ...
  127    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN WNDCHL',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        COMPUTE THE WIND CHILL IF THE TEMPERATURE AND
C        WIND SPEED ARE NOT MISSING AND MEET THE APPROPRIATE CRITERIA.
C
      DO 211 K=1,NSTA
C
     ...
     1      NINT(YDATA(K)).EQ.9999)THEN
         ...
         ELSE
C
C              CONVERTING WIND TO MILES/HOUR.
C
            IF(YDATA(K).LT.0.)YDATA(K)=0.
            WNDMPH=0.868391*YDATA(K)
C
C              CALCULATING WIND CHILL.
C
            IF(WNDMPH.GT.4.) THEN
               XDATA(K)=.0817*(3.71*SQRT(WNDMPH)+5.81-.25*WNDMPH)*
     1                  (XDATA(K)-91.4)+91.4
            ENDIF
C
         ENDIF
