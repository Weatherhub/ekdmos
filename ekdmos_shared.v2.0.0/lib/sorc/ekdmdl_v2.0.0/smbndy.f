      SUBROUTINE SMBNDY(KFILDO,GRD1,NX,NY,IBNDCT,IBNDPT,
     1                  BNDLAT,BNDLON,IBNSTA,ITAU)
C
C        APRIL    2006  CHARBA  MDL   MOS-2000
C        MAY      2006  CHARBA  MODIFIED TO ALLOW MULTIPLE SMOOTHING
C                               PASSES, TO USE DIFFERENT GRADIENT THRES-
C                               HOLDS FOR DIFFERENT PROJECTIONS, AND TO
C                               APPLY SMOOTHING ONLY FOR PROJECTIONS 
C                               SPECIFIED.
C        MAY      2006  CHARBA  TIGHTENED THE GRADIENT THRESHOLDS FOR
C                               WHICH SMOOTHING ALONG REGIONAL BOUND-
C                               ARIES IS PERFORMED, CHANGED THE INITIAL
C                               PROJECTION FOR WHICH THE SMOOTHING IS
C                               PERFORMED FROM 5 H TO 4 H, AND MADE THE
C                               NUMBER OF SMOOTHING PASSES TO BE A FUNC-
C                               TION OF PROJECTION (RANGES FROM 1 TO 3).
C                               ALSO, THE GRADIENT THRESHOLDS ARE NOW
C                               THE SAME FOR ALL SMOOTHING PASSES.
...
C                               NOTICEABLE.
C        SEPTEMBER2006  CHARBA  MODIFIED SUCH THAT EVEN CYCLES ARE 
...
C               RMISS = MISSING VALUE (=9999.).  (INTERNAL)
...
C  
      DATA MXPASS/3/,FMISS/9998.5/,RMISS/9999./
C
C        SET THE GRADIENT THRESHOLDS ACCORDING TO FORECAST PROJECTION.
C
      THRESH=0.025
      IF(ITAU.EQ.4) THRESH=0.090
      IF(ITAU.EQ.5) THRESH=0.036
      IF(ITAU.EQ.6) THRESH=0.033
      IF(ITAU.EQ.7) THRESH=0.030
      IF(ITAU.EQ.8.OR.ITAU.EQ.9) THRESH=0.027
C
      NPASS=1
C
C        FOR PROJECTIONS 4H AND 5H, PERFORM JUST ONE AND TWO SMOOTHING
C        PASSES, RESPECTIVELY.  OTHERWISE, DO MXPASS SMOOTHING PASSES.
C
 10   IF((ITAU.EQ.4.AND.NPASS.EQ.2).OR.(ITAU.EQ.5.AND.NPASS.EQ.3))
     1    GO TO 900
C
C        PERFORM BOUNDARY SMOOTHING FOR CURRENT PASS.  FIRST, INITIALIZE
C        WORK ARRAYS.
C
      DO 30 J=1,NY
        DO 20 I=1,NX
...
C
 ...
 180          FORMAT(//,' **** IN SMBNDY EXCEEDED THE MAXIMUM OF ',I2,
     1                  ' SMOOTHINGS ALLOWED AT THE POINT ',2I4.4,
