      SUBROUTINE L1D(KFILDO,KFIL10,IP12,IP16,NFIRST,
...
C        AUGUST    1999   GLAHN   ADDED IP12 TO CALL, CALL TO L1D50CFFF
...
C              (1) ENTER THE CALL TO THE APPROPRIATE SUBROUTINE 
C                  IN THE IF-THEN-ELSE CALLING SEQUENCE, SWITCHED
C                  IN THE APPROPRIATE MANNER.  FOR MODEL VARIABLES,
C                  THE SWITCHING CAN BE SOMEWHAT GENERIC.  FOR MODEL
C                  VARIABLES, THE CCCFFF WILL BE 5XCFFF, WHERE
C                    5 = THE 1-D LINEARIZATION KEY,
C                    0 = THE PARTICULAR COMBINATION OF SEASON NUMBER
C                        PROJECTION NUMBER, AND CYCLE,
...
C              (2) WRITE THE SPECIFIC SUBROUTINE ACCORDING TO THE 
C                  PATTERN OF L1D50CFFF.
C              (3) INCLUDE IN THE PLAIN LANGUAGE FILE 'MOS2000PRED' THE
C                  PLAIN LANGUAGE FOR THE PREDICTOR WHEN NECESSARY.
C                  THE PROGRAM WILL RUN WITHOUT THIS, BUT THE PLAIN
C                  LANGUAGE OUTPUT WILL BE DEFICIENT.  NOTE THAT THIS
C                  WILL NOT BE NECESSARY FOR THE ROUTINE MODEL VARIABLES,
C                  BECAUSE ENTRIES SHOULD ALREADY BE THERE. 
C
C        DATA SET USE 
...
C                 IER = STATUS RETURN.
C                        0 = GOOD RETURN.
C                       99 = PREDICTOR NOT DEFINED IN L1D.
C                       SEE CALLED ROUTINES FOR OTHER VALUES.
C                       (OUTPUT)
C               L1DCF = THE CCCFFF OF THE LINEARIZED VARIABLE.  (INTERNAL)
C                         
...
C
C     CALL TIMPR(KFILDO,KFILDO,'START L1D           ')
C
      IER=0
      L1DCF=IDPARS(1)*1000+IDPARS(2)
C
C        FIND PREDICTOR IN LIST.
C
      IF(L1DCF/10000.EQ.50)THEN
         CALL L1D50CFFF(KFILDO,KFIL10,NFIRST,
     1                  ID,IDPARS,THRESH,JD,NDATE,
     2                  KFILRA,RACESS,NUMRA,
     3                  ICALL,CCALL,ICALLD,CCALLD,NELEV,STALAT,STALON,
     ...
     B                  ISTAV,L3264B,L3264W,MISTOT,IER)
C
C           PUT OTHER CALLS HERE OR BEFORE THE CALL TO L1D50CFFF.
C
      ELSE
C
C           DROP THROUGH HERE MEANS VARIABLE COULD NOT BE FOUND IN LIST.
C
         WRITE(KFILDO,121)L1DCF
 121     FORMAT(/' ****COULD NOT FIND PREDICTOR CCCFFF ',I6.6,
     1           ' IN SUBROUTINE L1D AT 121.')
         IER=99
C           IER = 99 INDICATES PREDICTOR IS NOT IDENTIFIED OR CANNOT
C           BE CALLED IN L1D.  ISTOP WILL BE INCREMENTED AND SDATA( ) 
