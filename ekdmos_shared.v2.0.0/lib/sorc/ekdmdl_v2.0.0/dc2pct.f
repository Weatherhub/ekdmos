      SUBROUTINE DC2PCT(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
     ...
     7                  L3264B,L3264W,IER)
C 
C        JANUARY    2006   WIEDENFELD   MDL   MOS-2000
C        JUNE       2006   WIEDENFELD   MDL   MODIFIED CODE NAME.
C                                             CLEANED UP CODE.
C
C        PURPOSE
C            TO CONVERT PROBABILITY FORECASTS TO PERCENT.
C
C            IDPARS(1) & IDPARS(2) ARE MAPPED 
C            INTO      FROM 
C            207514    207510 (TRNC-SM 2H TSTM PROB)
C
C        DATA SET USE 
...
C            XDATA(K) = DECIMAL POINT FORECASTS, I.E. PROBABILITIES BEFORE
C                       MODIFICATION, PERCENT AFTER MODIFICATION.
C                       (K=1,NSTA).  (OUTPUT)
...
C           LDPARS(J) = PARSED ID FROM LD FILLED IN SUBROUTINE PRSID1.
C                       (J=1,15). (INTERNAL)
C 
C        NONSYSTEM SUBROUTINES CALLED 
C            PRSID1,RETVEC,SCALX
C
      PARAMETER (NDIM=1)
C
...
C     
 ...
 107  FORMAT(/,' ****DC2PCT ENTERED FOR VARIABLE',
...
C     
 108  LD(1)=ITABLE(2,JJ)*1000+IDPARS(3)*100+IDPARS(4)
      ...
      LD(4)=ID(4)
C          
C        FETCH THE PROBABILITY FORECAST INTO XDATA( ).
C         
...
C
 ...
 125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN DC2PCT',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
        GOTO 300
      ENDIF
C
C        NOW PERFORM SCALING TO CONVERT PROBABILTY TO PERCENT.
C
      CALL SCALX(KFILDO,XDATA,ND1,CONST,NSCALE,IER)      
C
      GO TO 350  
C     
