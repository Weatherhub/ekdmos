      SUBROUTINE DIVGW(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                 SDATA,ND1,NSTA,NGRIDC,ND11,NSLAB,IPACK,IWORK,
     2                 FDDV,ND5,
     3                 LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     4                 IS0,IS1,IS2,IS4,ND7,
     5                 FD1,FD2,FD3,FD4,FD5,FD6,FDSINS,FDMS,ND2X3,
     6                 ISTAV,L3264B,MISTOT,IER)
C
C        DECEMBER 2004  CHARBA  MDL   MOS-2000
C	 MAY      2005  REMOVED THE OPTION OF COMPUTING GEOSTROPHIC MASS
C			DIVERGENCE, ALTHOUGH THE SUPPORTING CODE WAS 
C			LEFT INTACT.  THUS, THIS VARIABLE COULD BE EASI-
C			LY RESURRECTED.  ALSO, ENHANCED DOCUMENTATION.
C        MAY      2006  MADE COSMETIC CHANGES TO SATISFY CODE WALK-THRU.
C
C        PURPOSE 
C           TO COMPUTE MASS DIVERGENCE (IN UNITS OF /SEC*100000) FROM U
C           AND V WINDS ON AN ISOBARIC, CONSTANT HEIGHT, OR A SIGMA
C           SURFACE.  IN ADDITION, THE ROUTINE CAN COMPUTE MASS DIVER-
C	    GENCE USING LAMP SURFACE WIND AND GEOSTROPHIC WIND ON A 
C	    CONSTANT PRESSURE SURFACE.  FURTHER, DIVGW CAN PERFORM TIME-
C	    WISE INTERPOLATION OF GFS WINDS WHEN TAU IS NOT A WHOLE MUL-
C	    TIPLE OF 3 HOURS.  THE ROUTINE, THUS, IS AN EXTEND-
C	    ED VERSION OF DIVW.
...
C              006 112 - MASS DIVERGENCE FOR LAMP MODEL SFC
C              006 113 - MASS DIVERGENCE ON PRESSURE SFC USING GEOSTRO-
C			 PHIC WIND (OPTION REMOVED BUT SUPPORTING CODE
C                        LEFT INTACT.)
...
C             FDDV(J) = ARRAY TO RETURN WIND DIVERGENCE (S^-1)(J=1,ND5).
C		        (OUTPUT)
...
C                       SEE GWIND, GFETCH, TIMGRD, AND MAPLAT FOR OTHER 
C                       VALUES WHEN IER.NE.0 AND
...
C            GFETCH, MAPLAT, DIVW1, GWIND, PRSID1, TIMGRD
C
      IMPLICIT NONE
C
      INTEGER IPACK(ND5),IWORK(ND5)
      INTEGER IS0(ND7),IS1(ND7),IS2(ND7),IS4(ND7)
      INTEGER LSTORE(12,ND9),NSTA,ND1
      INTEGER NGRIDC(6,ND11)
      INTEGER JD(4),IDPARS(15)
      INTEGER ICCCFFF(5),LD(4),LDPARS(15),MD(4),MDPARS(15)
...
C
     ...
     1     FD6(ND2X3),FDMS(ND2X3),FDSINS(ND2X3)
      ...
      REAL XMESHL
C
      DATA (ICCCFFF(I),I=1,5) /004000,004020,004051,004002,004016/
C
      IER=0
      ISTAV=0
C
...
C
      IF(IDPARS(1).NE.006.OR.(IDPARS(2).NE.110.AND.                     
     1   IDPARS(2).NE.111.AND.IDPARS(2).NE.112.AND.
     2   IDPARS(2).NE.116))THEN                    
         IER=103
...
C
         ISO=2
      ELSE IF(IDPARS(2).EQ.112) THEN
C
C           USE LAMP SURFACE WIND.
C
         ISO=3
      ELSE IF(IDPARS(2).EQ.113) THEN
C
C           USE GEOSTROPHIC WIND ON A CONSTANT PRESSURE SURFACE.
C
         ISO=4
      ELSE 
C
...
C
      ...
      ENDIF
C
C        TO OBTAIN GRID U WINDS IN COMPUTING DIVERGENCE,
C        CREATE ID FOR OBTAINING U WIND.  IF GEOSTROPHIC WIND IS TO BE
C	 USED, IT MUST BE COMPUTED USING GWIND.
C
...
C
      IF(ISO.EQ.4) THEN
C
C          CALL GWIND TO COMPUTE GEOSTROPHIC U-GRID 
C
        CALL PRSID1(KFILDO,LD,LDPARS)
C
        CALL GWIND(KFILDO,KFIL10,LDPARS,LD,NDATE,
     1             NGRIDC,ND11,NSLABU,IPACK,IWORK,FD3,ND2X3,
     2             LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3             IS0,IS1,IS2,IS4,ND7,
     4             FD1,FD2,FD5,FD6,FDSINS,FDMS,ND2X3,
     5             ISTAV,L3264B,MISTOT,IER)
C
        IF(IER.NE.0)GOTO 900
C
      ELSEIF(ISO.EQ.3.OR.MOD(IDPARS(12),3).EQ.0) THEN
C
C          CALL GFETCH TO FETCH U-GRID.
C
...
C
      ...
      ELSE
C
C	   CALL TIMGRD TO OBTAIN INTERPOLATED U-GRID.
C
        CALL PRSID1(KFILDO,LD,LDPARS)
        CALL TIMGRD(KFILDO,KFIL10,LD,LDPARS,LD,NDATE,
     1              SDATA,ND1,NSTA,NSLABU,IPACK,IWORK,FD3,ND2X3,
     2              LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3              IS0,IS1,IS2,IS4,ND7,
     4              FD1,FD2,ND2X3,
     5              ISTAV,L3264B,MISTOT,IER)
...
C
      ...
      NSLAB=NSLABU
C
C        TO OBTAIN GRID V WINDS IN COMPUTING DIVERGENCE,
...
C
      IF(ISO.EQ.4) THEN
C
C          CALL GWIND TO COMPUTE GEOSTROPHIC V-GRID 
C
        CALL PRSID1(KFILDO,MD,MDPARS)
C
        CALL GWIND(KFILDO,KFIL10,MDPARS,MD,NDATE,
     1             NGRIDC,ND11,NSLABU,IPACK,IWORK,FD4,ND2X3,
     2             LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3             IS0,IS1,IS2,IS4,ND7,
     4             FD1,FD2,FD5,FD6,FDSINS,FDMS,ND2X3,
     5             ISTAV,L3264B,MISTOT,IER)
...
C
      ...
      ELSE
C
C	   CALL TIMGRD TO OBTAIN INTERPOLATED V-GRID.
C
        CALL PRSID1(KFILDO,MD,MDPARS)
        CALL TIMGRD(KFILDO,KFIL10,MD,MDPARS,MD,NDATE,
     1              SDATA,ND1,NSTA,NSLABV,IPACK,IWORK,FD4,ND2X3,
     2              LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3              IS0,IS1,IS2,IS4,ND7,
     4              FD1,FD2,ND2X3,
     5              ISTAV,L3264B,MISTOT,IER)
...
C
 ...
 200     FORMAT(/' ****DIFFERENT GRID CHARACTERISTICS.  PREDICTOR NOT ',
     ...
     3           '  IER=',I3)
      ...
      ENDIF
C
C	 IF LAMP SFC WINDS, CONVERT FROM KT TO M/S.
C
      IF(ISO.EQ.3) THEN
	DO 300 J=1,NWORDS
          FD3(J)=FD3(J)*.5148
