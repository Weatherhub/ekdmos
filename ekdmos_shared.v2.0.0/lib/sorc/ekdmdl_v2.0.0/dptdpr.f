      SUBROUTINE DPTDPR(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                  NGRIDC,ND11,NSLAB,IPACK,IWORK,FDDD,ND5,
...
C                                AND FDDV( ) = ND5;  MODIFIED LINES
C                                IN CALL TO DETPT AND GFETCH
C        MAY      2003   GALHN   REARRANGED TYPE STATEMENTS
C
C        PURPOSE 
C            TO COMPUTE GRIDDED DEW POINT DEPRESSION USING TEMPERATURE
C            (DEG K) AND DEW POINT TEMPERATURE (DEG K) ON EITHER AN
C            ISOBARIC, OR A CONSTANT HEIGHT, OR A SIGMA SURFACE.
C  
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C               003 170 - DEW POINT DEPRESSION ON PRESSURE SFC
C               003 171 - DEW POINT DEPRESSION ON HEIGHT SFC
C               003 176 - DEW POINT DEPRESSION ON SIGMA SFC
C
C        DATA SET USE 
...
C                       OF LX*LY (OUTPUT).
C              FD1(J) = WORK ARRAY TO HOLD TEMPERATURE (DEG K)
...
C             FDDD(J) = DATA ARRAY TO HOLD DEW POINT DEPRESSION (DEG K)
C                       (J=1,ND5) (OUTPUT)
...
C                       1 IS FOR DEW POINT TEMPERATURE IN DEG K, ROW 2
C                       IS FOR TEMPERATURE IN DEG K. (INTERNAL)
...
C                       DEW POINT DEPRESSION ON.  (1 FOR ISOBARIC, 2
...
C                       FILE (INPUT)
C               ND2X3 = DIMENSION OF FD1( ), FD2( ), FD3( ), AND FD4( )
C                       (INPUT)
C                 ND5 = DIMENSION OF IPACK( ), IWORK( ), AND FDDD( ).
C                       (INPUT)
...
C                       DEW POINT TEMPERATURE (INTERNAL)
...
C
      ...
      REAL CORE(ND10)
C
      DATA ((ICCCFFF(I,J),J=1,3),I=1,2) /003100,003101,003106,
     1                                   002000,002001,002006/
C
      IER=0
      ISTAV=0
C
C        CHECK IF THIS CODE ACCOMMODATES DEW POINT DEPRESSION
C 
      IF(IDPARS(1).NE.003.OR.(IDPARS(2).NE.170.AND.
     1   IDPARS(2).NE.171.AND.IDPARS(2).NE.176))THEN
         IER=103
         WRITE(KFILDO,100)(JD(J),J=1,4),IER
 100     FORMAT(/' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE DEW',
     1           ' POINT DEPRESSION.',
     2          /'     PREDICTOR ',I9.9,I10.9,I10.9,I4.3,
     3           ' NOT ACCOMMODATED IN DPTDPR.  IER=',I3)
	 GOTO 900
      ENDIF
C
C        DETERMINE IF THE DEW POINT DEPRESSION IS TO BE COMPUTED ON AN
C        ISOBARIC, OR A CONSTANT HEIGHT, OR A SIGMA SURFACE
 
      IF (IDPARS(2).EQ.170)THEN
C
C           THIS IS DATA FOR A CONSTANT PRESSURE SURFACE.
C
         ISO=1
      ELSE IF (IDPARS(2).EQ.171)THEN  
C
...
C
      ...
      END IF
C 
C        TO OBTAIN DEW POINT IN COMPUTING DEW POINT DEPRESSION
C 
C        CREATE IDS FOR OBTAINING DEW POINT TEMPERATURE
C
...
C
C        CALL DEWPT TO OBTAIN DEW POINT TEMPERATURE FDDD (THE DATA ARRAY
C        FDDD IS BEING BORROWED HERE TO USE AS A WORK ARRAY)
C
      CALL DEWPT(KFILDO,KFIL10,LDPARS,JD,NDATE,
     1           NGRIDC,ND11,NSLABL,IPACK,IWORK,FDDD,ND5,
     2           LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3           IS0,IS1,IS2,IS4,ND7,
     4           FD1,FD2,FD3,FD4,ND2X3,
     5           ISTAV,L3264B,MISTOT,IER)
...
C
      ...
      NSLAB=NSLABL
C
C        TO OBTAIN TEMPERATURE IN COMPUTING DEW POINT DEPRESSION
C
C        CREATE IDS FOR OBTAINING TEMPERATURE
C
      ...
      MD(4)=0
C
C        CALL GFETCH TO FETCH TEMPERATURE FD1
C
      CALL GFETCH(KFILDO,KFIL10,MD,7777,LSTORE,ND9,LITEMS,
     1        IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FD1,ND2X3,
     3        NWORDS,NPACK,NDATE,NTIMES,CORE,ND10,
     4        NBLOCK,NFETCH,NSLABM,MISSP,MISSS,L3264B,
     5        1,IER)
      IF(MISSP.NE.0)MISTOT=MISTOT+1
C
      IF(IER.NE.0)THEN
C***         WRITE(KFILDO,300)IER
C*** 300     FORMAT(/' ****TEMPERATURE FETCH FAILED IN DPTDPR.  IER=',I3)
         GOTO 900
      ENDIF
C
      MX=IS2(3)
...
C
 ...
 400     FORMAT(/' ****DIFFERENT GRID CHARACTERISTICS.  PREDICTOR NOT ',
     1           'COMPUTED IN DPTDPR.  VALUES FROM NGRIDC( , ) AND X*Y',
     2           ' ARE: ',2(/2X,3I10.9,I4.3,4X,6I10,4X,I3,'*',I3),
     3           '  IER=',I3)
         GO TO 900
      ENDIF
C
C        COMPUTE DEW POINT DEPRESSION FROM TEMPERATURE AND DEW POINT
C 
      DO 500 I=1,LX*LY
         IF(FDDD(I).NE.9999..AND.FD1(I).NE.9999.)FDDD(I)=FD1(I)-FDDD(I)
	 IF(FDDD(I).LT.0.)FDDD(I)=0.
 500  CONTINUE
      GO TO 999
C
C        FILL IN THE DEW POINT DEPRESSION DATA ARRAY WITH MISSING VALUES
C
 900  DO J=1,ND2X3
         FDDD(J)=9999.
      ENDDO
