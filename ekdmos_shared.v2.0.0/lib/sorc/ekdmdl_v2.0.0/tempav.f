      SUBROUTINE TEMPAV(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C                                  PROPER PROGRAM FLOW.  ADDED 'ELSEIF'
C                                  TEST WITHIN THE DO LOOP OF 211.
C
C        PURPOSE 
C            TO AVERAGE TEMPERATURE OR DEWPOINT AT THE SAME PROJECTION. 
C	
C	     IDPARS(1) & IDPARS(2) OF DEWPOINT TEMPERATURE
C            ARE MAPPED:   INTO        FROM                    
C                          202010      202000 AND 202005     
C                          203010      203000 AND 203005     
C
C        DATA SET USE 
...
C            XDATA(K) = THE FIRST OF THE TWO TEMPERATURES OR DEW POINTS
C                       FOR AVERAGING (INTERNAL); THEN THE AVERAGE
C                       (K=1,NSTA).  (OUTPUT)
...
C                       (INTERNAL)
C            YDATA(K) = WORK ARRAY WHICH STORES 2ND TEMPERATURE (K=1,ND1).  
C                       (AUTOMATIC)
...
C
      DATA LDPARS/15*0/  
      DATA ITABLE/202010,202000,202005,
     1 	          203010,203000,203005/
C     
      IER=0
      IERF=0
C
C        FIND THE CCCFFF OF ID(1) IN ITABLE(1, ).
C
      DO 105 JJ=1,NDIM
...
C
 ...
 107  FORMAT(/,' ****TEMPAV ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        GET THE TWO TEMPERATURES FOR AVERAGING.
C
...
C 
  ...
  125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN TEMPAV',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
      ENDIF
C
...
C
      IF(IER.NE.0)THEN
         WRITE(KFILDO,128)(LD(J),J=1,4)
  128    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN TEMPAV',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
      ENDIF
C      
C        AVERAGE TEMPERATURES WHEN BOTH ARE NOT MISSING.
C        IF ONE IS MISSING, XDATA( ) IS SET TO THE NON
C        MISSING ONE. IF BOTH XDATA( ) AND YDATA( ) ARE MISSING,
C        SET XDATA( ) TO MISSING.
C
...
C
         ELSEIF((NINT(XDATA(K)).EQ.9999).AND.
     1          (NINT(YDATA(K)).EQ.9999))THEN
            XDATA(K)=9999.
C
         ELSEIF(NINT(YDATA(K)).NE.9999)THEN
            XDATA(K)=YDATA(K)
C
         ENDIF
C      
