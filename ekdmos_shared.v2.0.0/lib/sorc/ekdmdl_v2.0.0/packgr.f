      SUBROUTINE PACKGR(KFILDO,KFILX,CFILX,ID,IDPARS,
     ...
     5                  NWORDS,NTOTGB,NTOTGR,
     6                  L3264B,L3264W,ISTOP,IER)
C
C        OCTOBER   2004   GLAHN   TDL   MOS-2000
C                                 MODIFED FROM PACKG
C        APRIL     2007   GLAHN   REMOVED /D FROM 158; ADDED ID TO
C                                 DIAGNOSTIC; PUT STATEMENTS IN
C                                 NUMERICAL ORDER; 158 BECAME 388
C        AUG       2007   SMB     ADDED COMMAS TO FORMAT STATEMENT
C                                 388 FOR IBM COMPILE
...
C
      IER=0
D     WRITE(KFILDO,100)ID,IDPARS
D100  FORMAT(/,' **** ENTERING PACKGR, ID( ) =',4I12,/,(15I8))
D     WRITE(KFILDO,101)ND2X3,NX,NY,ND5,MINPK,XMISSP,L3264B
D101  FORMAT(/' AT 101 IN PACKGR--',
D    1        'ND2X3,NX,NY,ND5,MINPK,XMISSP,L3264B',10I10)
C
C        PACK THE DATA.  ELIMINATE THE "MODEL NUMBER" FROM 
C        THE "CONSTANT" VARIABLES.
C
C        VERIFY UNIT NUMBER IS IN RANGE 42-43.
C
      IF(KFILX.LT.42.OR.KFILX.GT.43)THEN
         WRITE(KFILDO,105)KFILX,(ID(J),J=1,4)
 105     FORMAT(/,' ****UNIT NUMBER =',I4,' IN PACKGR IS NOT 42 OR 43.',
...
C        IS1(2) = 1 SIGNIFIES GRIDDED DATA.
      ...
      IS1(22)=32
C     
D     WRITE(KFILDO,371)NX,NY,ND2X3
D371  FORMAT(/' AT 371 IN PACKGR--NX,NY,ND2X3',3I10)
D     WRITE(KFILDO,372)IS1
D372  FORMAT(/,' AT 372 IN PACKGR--IS1( )',/,(10I10))
C
...
C
D     WRITE(KFILDO,385)(IS2(J),J=1,ND7)
D385  FORMAT(/,' IN PACKGR AT 385--IS2( )',/,(10I10))
C
C        WRITE THE DATA.
C      
...
C
      IF(IER.EQ.0)THEN
         ...
         NTOTGR=NTOTGR+1
C
         WRITE(KFILDO,388)KFILX,(ID(J),J=1,4),NTOTGB,NTOTGR
 388     FORMAT(/,' WRITING DATA IN PACKGR ON UNIT NO.',I4,
     1             I12.9,3I10,/,
     2        '    NTOTGB =',I10, '   TOTAL BYTES   WRITTEN TO RANDOM',
     3                            ' ACCESS FILE',/,
     4        '    NTOTGR =',I10, '   TOTAL RECORDS WRITTEN TO RANDOM',
     5                            ' ACCESS FILE')
      ELSE
         ISTOP=ISTOP+1
      ENDIF
