      SUBROUTINE RDLVRB(KFILDO,IP6,IP7,IP9,KFILP,KFILCP,
...
C            SOME INSERTIONS MADE FOR COMPUTED PREDITORS BY ROUTINE
C            SETPLN.  RDLVRB IS FOR U150; A SIMILIAR ROUTINE RDPRED IS
C            IS USED FOR U201, AND RDVRBL IS USED FOR PROGRAMS SUCH
C            AS U600.  ALL GRID BINARIES ARE GIVEN THE ISCALD( )
C            VALUE = 2, AND ALL POINT BINARIES ARE GIVEN THE ISCALD( )
C            VALUE = 0 BY SETPLN. HOWEVER, THIS CAN BE CHANGED FOR
C            SPECIFIC VARIABLES BY INSERTING AN EXACT CCCFFFB MATCH 
C            IN THE PLAIN LANGUAGE FILE.
C
C            THE DIFFERENCES BETWEEN RDLVRB AND RDPRED ARE THAT RDPRED
C            ORDERS VARIABLES LOW TO HIGH AND RDLVRB DOES NOT, 
C            RDVRBL DOES NOT CALL CKIDS; AND SMULT, SADD, ORIGIN,
C            CINT, AND UNITS FROM SETPLN ARE NOT PRINTED BY RDLVRB.
C
C        DATA SET USE 
...
C
 ...
 1030 FORMAT(/,' VARIABLES AS READ BY RDLVRB')
      IF(IP6.NE.0)WRITE(IP6,1031)(ITEMP(J),J=1,4),
     1            TEMP,ITEMP(5),ITEMP(6),(JTEMP(J),J=1,3)
 1031 FORMAT(' ',I9.9,1X,I9.9,1X,I9.9,1X,I3.3,1X,A1,1X,I4,
     1       1X,I3,4X,3I2)
      GO TO 120
C
 104  NERR=NERR+1
...
C
 ...
 122  FORMAT(/,' ****ND4 = ',I4,' TOO SMALL IN RDLVRB.')
      GO TO 180
C
