      SUBROUTINE SOLAMT(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C                                   ON THE IBM SP
C        APRIL  2000   GLAHN    ADDED DIMENSION FOR ISDATA( ); IMPROVED 
C                               COMMENTS; ADDED /, FOR **** COMMENTS
C
C        PURPOSE 
C            TO CALCULATE THE SOLAR ENERGY AMOUNT IN (KWH/M^2).  
C            CONVERSION USED:  (FRACTION OF SOLAR ENERGY)*
C            (EXTRATERRESTIAL RADIATION).  THE FORMER IS CALCULATED
C            IN SOLENG.  IDPARS(1) & IDPARS(2) 
C            ARE MAPPED  
C               INTO     FROM 
C               209210   209200
C
...
C            XDATA(K) = THE FRACTION OF SOLAR ENERGY (INTERNAL); THEN,
C                       THE SOLAR ENERGY AMOUNT (KHW/M^2) (K=1,ND1).
C                       (OUTPUT) 
C                 ND1 = MAXIMUM NUMBER OF STATIONS THAT CAN BE DEALT 
C                       WITH.  DIMENSION OF XDATA( ), ISDATA( ) (INPUT)
...
C             DATA(J) = WORK ARRAY (J=1,ND5) WHICH STORES THE EXTRATERRESTRIAL
C                       RADIATION.  (INTERNAL)
...
C
 ...
 107  FORMAT(/,' ****SOLAMT ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        GET THE ID FOR THE FRACTION OF SOLAR ENERGY.
C        THE FRACTION OF SOLAR ENERGY IS CALUCLATED IN SOLENG.
C
...
C
 ...
 125     FORMAT(/,' ****VARIABLE NOT RETRIEVED BY SOLENG IN SOLAMT',
     1            2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C    
C        CALCULATE THE SOLAR ENERGY AMOUNT.
C
      DO 211 K=1,NSTA
C
         IF(NINT(XDATA(K)).EQ.9999.OR.
     1      NINT(DATA(K)).EQ.9999)THEN
            XDATA(K)=9999.
         ELSE
            XDATA(K)=XDATA(K)*DATA(K)
         ENDIF
C
 211  CONTINUE
C 
