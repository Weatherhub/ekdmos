...
C        GFS PRIOR TO 10/1/98 
       IF((IDPARS(2).EQ.156).OR.(IDPARS(2).EQ.154))THEN
          LEVEL(1)=1000
          LEVEL(2)=925
          LEVEL(3)=850
          LEVEL(4)=700
...
C        AND FOR THE "MRF VERSION" USE THE 12-HR.  IF IT IS NON-ZERO,
C        SET THE POP FLAG TO 1.  NOTE:  IN FALL 2002, THERE CEASED TO
C        BE 12-HR MRF PCP SO ROUTINE WAS CHANGED TO COMPUTE A 12 FROM
C        TWO 6S IF A 12 CAN NOT BE FOUND.
C
      SELECT CASE(IDPARS(2))
C        
C           SET UP MD1(1) TO HOLD THAT MODEL'S PRECIP AMT, OR
C           IN THE CASE OF THE "AVN VERSION", CALL TPCP3 INSTEAD OF 
C           GFETCH
C
C              ETA
C
