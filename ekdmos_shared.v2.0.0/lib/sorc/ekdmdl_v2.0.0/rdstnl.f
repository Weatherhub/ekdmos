      SUBROUTINE RDSTNL(KFILDO,IP4,IP5,KFILD,NEW,CCALL,
     1                  NAME,NELEV,IWBAN,STALAT,STALON,IFOUND,
     2                  NGP,GPNAME,ND1,KGP,NSTA,IER)
C 
C        MARCH     1998   GLAHN   TDL   MOS-2000 
...
C            LIST IN THE ORDER READ BY GROUP.  FOR EACH GROUP,
...
C            INITIALIZED.
C
C            NOTE:  THE DIFFERENCE BETWEEN THIS RDSTNL AND RDSTNA IS 
C                   THAT RDSTNA ALPHABETIZES STATIONS, WHILE RDSTNL
C                   DOES NOT.  RDSTAL IS USED FOR A SINGLE STATION
C                   LIST WITH NO NAME; RDSTGN IS USED FOR MULTIPLE 
C                   LISTS WITHOUT NAMES; RDSTNL IS USED FOR MULTIPLE
C                   LISTS WITH NAMES.
C
C        DATA SET USE 
...
C                       NELEV( ), IWBAN( ), STALAT( ), STALON( ) IFOUND( ), 
C                       AND NGP( ).  THIS IS THE MAXIMUM NUMBER 
C                       OF STATIONS IN THE LIST TO BE RETURNED.  (INPUT)
C                 KGP = THE NUMBER OF VALUES RETURNED IN NGP( ).
C                       (OUTPUT) 
C                NSTA = THE TOTAL NUMBER OF STATIONS RETURNED IN CCALL( , )
C                       AND OTHER OUTPUT ARRAYS.  (OUTPUT)
...
C                       SO THAT ALL VALUES ARE WEST.  (CHARACTER*1) 
C                       (INTERNAL)
...
C        THAT IS, 'OKC     ' COULD BE 'OKC' OR 'OKC '.
     ...
     1      WRITE(KFILDO,110)NSTA1,KGP,(CCALL(K,1),K=NSTA+1,NSTA+NSTA1)
...
C        IP4.
 ...
 116  FORMAT(/,' ****TOO MANY GROUPS IN RDSTNL FOR DIMENSION ND1 =',
     1       I7)
      IER=34
      KGP=KGP-1
      GO TO 160
...
C
 ...
 124  FORMAT(/,' ****ERROR READING STATION DIRECTORY IN RDSTNL.',
     1         '  IOSTAT =',I5)
      IER=33
      GO TO 1405
C
