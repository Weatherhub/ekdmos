      SUBROUTINE DBTAIL(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C 
C        SEPTEMBER 2005   WEISS    MDL   MOS-2000
C                                  BASED ON MONPRB
C        SEPTEMBER 2005   WEISS    ADDED IDS FOR CONDITIONAL CEILING HT
C                                  AND CONDITIONAL VISIBILITY
C                                  NDIM INCREASED TO 2
C        DECEMEBR  2006   RUDACK   FORCED ALL "9997" FORECASTS TO EQUAL ZERO.
C                                  THIS MODIFICATION WAS NEEDED TO ENSURE THAT
C                                  CUMULATIVE PROBABILITY FORECASTS
C                                  (E.G. CONDITIONAL CEILING HEIGHT), WHICH
C                                  ARE CHANGED TO DISCRETE FORECASTS IN
C                                  OPERATIONS, ALWAYS REMAIN POSITIVE. 
C                                  THIS MODIFICATION WAS NOT NECESSARY FOR 
C                                  DEVELOPMENT BUT RATHER FOR OPERATIONS.
C
C        PURPOSE:
...
C            (DOUBLE TAIL DISTRIBUTION). WRITTEN ESPECIALLY FOR
C            CONDITIONAL CEILING HEIGHT AND VISIBILITY.  DBTAIL ALSO
...
C
      ...
      LOGICAL ZERO_FLAG,NONZERO_FLAG,DBTAIL_FLAG(ND1)
C
...
C     
      DATA ITABLE/2081552,2080552/
      DATA JTABLE/208145,208045/
C
      IER=0
      ISTAB=1
C      
...
C     
 ...
 107  FORMAT(/,' ****DBTAIL ENTERED FOR VARIABLE',
...
C
 ...
 110     FORMAT(/,' ****NCAT =',I4,' NOT CORRECT IN DBTAIL.')
         IER=102
         GO TO 300
      ENDIF
C
...
C
 ...
 145        FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN DBTAIL',
     1               2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
            GO TO 300
         ENDIF
 150     CONTINUE
C
         SCALE=10.**REAL(IS1(17))
C 
...
C
      IF(ITABLE(JJ).EQ.2080552.OR.
     1   ITABLE(JJ).EQ.2081552) GOTO 195
C
      DO 190 J=1,NCAT              
         DO 189 K=1,NSTA   
C
...
C        MONOTONIC CHECK.
C
C           STEP 1. CHECK FOR ZERO PROBABILITIES
C           FLANKED BY ATLEAST ONE HIGHER CATEGORY
C           NON-ZERO PROBABILITY
C
      DO 206 K=1,NSTA
         DBTAIL_FLAG(K)=.FALSE.
         ZERO_FLAG=.FALSE.
...
C
         DO 205 J=1,NCAT
            IF(NINT(XDATA(K,J)).NE.9999) THEN
               IF(NINT(XDATA(K,J)*SCALE).EQ.0)ZERO_FLAG=.TRUE.
               IF(NINT(XDATA(K,J)*SCALE).NE.0)THEN
                  NONZERO_FLAG=.TRUE.
                  ZERO_FLAG=.FALSE.
               ENDIF
               IF(ZERO_FLAG.AND.NONZERO_FLAG)THEN 
                  DBTAIL_FLAG(K)=.TRUE.
               ENDIF
            ENDIF
 205     CONTINUE
 206  CONTINUE
C
C           STEP 2. FOR .FALSE. DBTAIL_FLAG STATIONS, PROCESS
C           AS A NORMAL MONPROB SET OF PROBABILITIES
C
       DO 250 K=1,NSTA 
         IF(.NOT.DBTAIL_FLAG(K))THEN
            DO 215 J=NCAT,2,-1
C
               IF(NINT(XDATA(K,J-1)).NE.9999) THEN
                  IF(XDATA(K,J-1).GT.XDATA(K,J)) THEN
...
C
 215        CONTINUE
C
C           STEP 3. FOR .TRUE. DBTAIL_FLAG STATIONS,
C           PROCESS USING THE DOUBLE TAIL RARE EVENT
C           CORRECTION FOR THE PROBABILITIES
C
         ELSEIF(DBTAIL_FLAG(K))THEN
C
C              STEP 3A. CHECK FOR CATEGORICAL ZEROES (2 to 7)
C              AND FILL IN ZEROES WITH THE LOWER CATEGORY
C              NON-ZERO VALUE
            DO 225 J=1,NCAT
C 
               IF(NINT(XDATA(K,J)).NE.9999) THEN
                  IF((J.LE.NCAT).AND.(J.GE.2))THEN
                     IF(NINT(XDATA(K,J)*SCALE).EQ.0)THEN
                        IF(NINT(XDATA(K,J-1)*SCALE).NE.0)THEN
                           XDATA(K,J)=XDATA(K,J-1)
                        ENDIF
                     ENDIF
                  ENDIF
               ENDIF
C
 225        CONTINUE
C
C              STEP 3B. AFTER FILLING IN THE PATHOLOGICAL 
C              ZEROES. PROCESS AS A NORMAL MONPROB SET OF 
C              PROBABILITIES. (7 to 2)
            DO 235 J=NCAT,2,-1
C
               IF(NINT(XDATA(K,J-1)).NE.9999) THEN
                  IF(XDATA(K,J-1).GT.XDATA(K,J)) THEN
