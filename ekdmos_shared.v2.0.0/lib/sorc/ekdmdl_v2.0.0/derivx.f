      SUBROUTINE DERIVX(KFILDO,NPRJCT,XMESHL,ORIENT,XLAT,
     1                  VAR,XDER,NX,NY,XNP,YNP,IER)
C
...
C
C        PURPOSE
C            TO COMPUTE SPATIAL DERIVATIVE IN X-DIRECTION FOR VARIABLE
C            IN VAR( , ).  INCORPORATES POLAR STEREOGRAPHIC MAP FACTOR.
C            OUTPUT IS IN THE UNITS OF VAR( , ) PER METER.
C
C        DATA SET USE
...
C              NPRJCT = CODE FOR MAP PROJECTION.  (INPUT)
C              XMESHL = MESH LENGTH OF GRID AT XLAT DEG N LATITUDE IN 
C			KM.  (INPUT)
C              ORIENT = ORIENTATION OF GRID, W LONGITUDE, IN DEGREES.
C                       GRID COLUMNS ARE PARALLEL TO THIS LONGITUDE.  
C			(INPUT)
C                XLAT = LATITUDE AT WHICH XMESHL APPLIES.  (INPUT)
C          VAR(IX,JY) = VARIABLE FOR WHICH TO GET X-DERIVITIVE
C                       (IX=1,NX) (JY=1,NY).  (INPUT)
C         XDER(IX,JY) = DERIVITIVE OF VARIABLE IN VAR( , ) IN X-DIREC-
C			TION.  RESULTS IN UNITS OF VAR( , ) PER 
C			METER (IX=1,NX) (JY=1,NY).  (OUTPUT)
...
C            NONE
C
      DIMENSION VAR(NX,NY),XDER(NX,NY)
C
...
C
 ...
 104  FORMAT(' **** IN DERIVX, INCORRECT MAP PROJECTION AND GRID',
...
C
        DO 120 IX=2,NX-1
          ...
          XDER(IX,JY)=(RMAPF/DGX2)*(VAR(IX+1,JY)-VAR(IX-1,JY))
D         WRITE(KFILDO,119)IX,JY,DXSQ,DYSQ,SINPHI,RMAPF,VAR(IX+1,JY),
D    1                     VAR(IX-1,JY),XDER(IX,JY),RMAPF/DGX2
 119      FORMAT(' ',2I4,8F14.8)
 120    CONTINUE
C
        DXSQ=(XNP-1.)**2
        SINPHI=(PTOEQ2-(DXSQ+DYSQ))/(PTOEQ2+(DXSQ+DYSQ))
        RMAPF=OPSLAT/(1.+SINPHI)
        XDER(1,JY)=(RMAPF/DG)*(VAR(2,JY)-VAR(1,JY))
D       WRITE(KFILDO,125)JY,DXSQ,DYSQ,SINPHI,RMAPF
D125    FORMAT('     ',I4,4F14.6)
C
        DXSQ=(XNP-NX)**2
        SINPHI=(PTOEQ2-(DXSQ+DYSQ))/(PTOEQ2+(DXSQ+DYSQ))
        RMAPF=OPSLAT/(1.+SINPHI)
        XDER(NX,JY)=(RMAPF/DG)*(VAR(NX,JY)-VAR(NX-1,JY))
D       WRITE(KFILDO,125)JY,DXSQ,DYSQ,SINPHI,RMAPF
C
 130  CONTINUE
C
