...
C            STATIONS IN THE SEPARATE LIST WILL NOT BE KEPT, AND A 
...
C
C            NOTE:  THE DIFFERENCE BETWEEN THIS RDSTDL AND RDSTDD IS 
C                   THAT RDSTDD ALPHABETIZES STATIONS, WHILE RDSTDL
C                   DOES NOT.  RDSTDL IS USED FOR A SINGLE STATION
C                   LIST PER CALL; RDSTGN IS USED FOR MULTIPLE LISTS.
C
C                   THE DIFFERENCE BETWEEN RDSTAL AND RDSTDL AND 
C                   BETWEEN RDSTAD AND RDSTDD IS THAT RDSTDL AND RDSTDD
C                   ELIMINATE DUPLICATES, WHILE RDSTAL AND RDSTAD DO NOT.
C    
C
C        DATA SET USE 
...
C                       (INTERNAL)
C                LSTA = NUMBER OF STATIONS IN THE LIST AFTER ANY
C                       DUPLICATES HAVE BEEN REMOVED.
C        1         2         3         4         5         6         7 X
C 
...
C
      ...
      DATA CCALLT/6*' '/
C
D     CALL TIMPR(KFILDO,KFILDO,'START RDSTDL        ')
C
      IER=0
      JER=0
      MSTA=0
...
C
 ...
 124  FORMAT(/' ****ERROR READING STATION DIRECTORY IN RDSTDL.',
     1        '  IOSTAT =',I5)
      IER=33
...
C        *************************************************************  
C
C        DIRECTORY IS TO BE USED FOR LIST.  THIS SHOULD NOT CONTAIN
C        DUPLICATES.
C
C        *************************************************************  
C
...
C           LEFT OVER IN CCALL( , ).
         K=K+1
C           THE ITEM IS LEFT IN THE LIST, SO UPDATE K.
      ELSE
         CCALL(K,1)=CCALLS(K)
...
C
            JER=1
            IF(K.EQ.LSTA)K=K+1
C              THE SAME ITEM NUMBER MUST BE CONSIDERED, BUT K
C              MUST BE UPDATED WHEN THE ITEM IS THE LAST ONE TO
C              GET OUT OF THE LOOP.
C
         IF(K.LT.LSTA)THEN
C              CALL ELIMDS TO ELIMINATE THE DUPLICATE.
            CALL ELIMDS(KFILDO,CCALL,CCALLS,NAME,NELEV,IWBAN,
     1                  STALAT,STALON,ITIMEZ,IFOUND,K,ND1,LSTA)
         ELSE
C              THIS IS THE LAST STATION; JUST REDUCE LSTA.
            LSTA=LSTA-1
         ENDIF         
C
...
C
 ...
 143           FORMAT(/' ****STATION ',A8,' IS A DUPLICATE;',
     1                 ' IT IS ELIMINATED.  MAY BE CAUSED BY A LINK',
     2                 ' TO ALTERNATE STATION.')
            ELSE
...
C
 146  IF(IER.EQ.33.OR.IER.EQ.34)GO TO 148
C
...
C
      ENDIF
C
D     CALL TIMPR(KFILDO,KFILDO,'END RDSTDL          ')
C
 160  RETURN
      END
