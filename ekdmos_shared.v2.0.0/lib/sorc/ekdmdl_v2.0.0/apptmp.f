      SUBROUTINE APPTMP(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C        SEPTEMBER 1999   GLAHN    CHANGED DO 211 LOOP; NO TEST ON 9997;
C                                  ADDED YDATA( )
C        THIS PREDICTOR CANNOT BE COMPUTED.  SET XDATA( ) TO MISSING.
C        THIS IS FOR SAFETY; XDATA( ) SHOULD ALREADY BE SET TO MISSING.
C        APRIL     2000   RUDACK   CORRECTED IDS FOR OPERATIONAL USE;
C                                  COMMAS IN FORMATS FOR IBM
C        APRIL     2000   GLAHN    IMPROVED COMMENTS; INSURED /, IN 
C                                  **** COMMENTS
C        APRIL     2000   RUDACK   ADDED DATA LDPARS/15*0/ 
C
C        PURPOSE 
C            TO DETERMINE THE APPARENT TEMPERATURE GIVEN TEMPERATURE
C            AND DEW POINT IN FAHRENHEIT.  
C
C            IDPARS(1) & IDPARS(2) OF TEMPERATURE  
C            ARE MAPPED INTO      FROM 
C                       202030    202020 AND 203020
C
C        DATA SET USE 
...
C            XDATA(K) = TEMPERATURE (INTERNAL), THEN APPARENT
C                       TEMPERATURE (K=1,NSTA).  (OUTPUT)
C                 ND1 = MAXIMUM NUMBER OF STATIONS THAT CAN BE DEALT 
C                       WITH.  DIMENSION OF XDATA( ), YDATA( ) AND
C                       ISDATA( ).  (INPUT)
...
C  
      DATA LDPARS/15*0/
      DATA ITABLE/202030,202020,203020/
C
      IER=0
C
...
C
 ...
 107  FORMAT(/,' ****APPTMP ENTERED FOR VARIABLE',
...
C
  ...
  125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN APPTMP',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF

C        GET THE DEW POINT TEMPERATURE IN FAHRENHEIT.
C
...
C
  ...
  127    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN APPTMP',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        COMPUTE THE APPARENT TEMPERATURE IF BOTH THE
C        TEMPERATURE AND DEW POINT ARE NOT MISSING.
C
      DO 211 K=1,NSTA
C
     ...
     1      NINT(YDATA(K)).EQ.9999) THEN
            XDATA(K)=9999.
         ELSE
            XDATA(K)=1.03*XDATA(K)+((XDATA(K)*
     1               (EXP((YDATA(K)-59.)/17.)-1.))/19.)-3.
         ENDIF
C
 211  CONTINUE
C 
