...
C                             /, IN **** COMMENTS
C        APRIL 2000   RUDACK  ADDED COMMENTS; ADDED DATA LDPARS/15*0/ 
C        AUGUST 2002  MCALOON UPDATED ID'S TO ACCOMMODATE NGM WIND
C                             FCST BY MOD. PERFECT PROG.
C        MARCH  2006  RLC     CHANGED TESTS FOR CALM WIND FROM
...
C                    004200     004010, 004110 (U-V COMP. OF WINDS ON P-SURFACE)
C                    004201     004011, 004111 (U-V COMP. OF WINDS ON CONST. HT. SFC.)
C                    004202     004012, 004112 (GEOSTROPHIC U-V COMP. ON P-SURFACE)
C                    704200     704010, 704110 (U-V WIND COMPONENT (KTS) - EARTH ORIENTED)
C                    204200     204010, 204110 (SFC. WIND U-V COMPONENTS)
C                    224200     224010, 224110 (SFC. WIND U-V COMPONENTS - NGM MOS FCST)
C
C        DATA SET USE 
...
C                       XDATA( ) FOR THE U COMPONENT (J=1,4).  (INTERNAL)
C               MD(J) = HOLDS THE 4 ID WORDS OF THE DATA RETRIEVED INTO
C                       YDATA( ) FOR THE V COMPONENT (J=1,4).  (INTERNAL)
C           LDPARS(J) = PARSED ID FILLED IN SUBROUTINE PRSID1 FOR U-COMPONENT.
C                       (J=1,15). (INTERNAL)
C           MDPARS(J) = PARSED ID FILLED IN SUBROUTINE PRSID1 FOR V-COMPONENT.
C                       (J=1,15). (INTERNAL)
C         ITABLE(I,J) = CCCFFF OF THE DIRECTION (I=1) AND THE NEEDED
C                       U COMPONENT (I=2) AND V COMPONENT (I=3) (J=1,NDIM).
C                       (INTERNAL)
...
C
 ...
 105  CONTINUE
C
      WRITE(KFILDO,110)(JD(L),L=1,4),IDPARS(2)
 110  FORMAT(/,' ****DIRFUV ENTERED FOR PREDICTOR',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' WITH PROCESSING IDPARS(2) = ',I3,
     3       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
