      SUBROUTINE ATLCL(KFILDO,KFIL10,KFILRA,RACESS,NUMRA,IDPARS,JD,
     1                 NDATE,NGRIDC,ND11,NSLAB,IPACK,IWORK,TLCL,ND5,
     2                 LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3                 IS0,IS1,IS2,IS4,ND7,
     4                 FDELV,FDTMP,FDDEW,ND2X3,
...
C        SEPTMBER 2005  CHARBA  CORRECTED ERROR IN ARGUMENT LIST.  THE
C				ERROR RESULTED IN AN INCORRECT COMPUTED
C				FIELD. 
...
C
C        PURPOSE 
C            TO COMPUTE SIMPLE APPROXIMATION OF LCL TEMPERATURE FROM
C            LAMP SURFACE TEMPERATURE AND DEWPOINT.  TERRAIN ELEVATION 
C	     IS USED TO FIRST REDUCE THE TEMPERATURE AND DEWPOINT TO 
C	     MSL.  A VERY LIGHT WEIGHTED NINE-POINT SMOOTHER IS APPLIED 
C	     TO COMPUTED FIELD.
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C	         003  162   LAMP MODEL APPROX LCL TEMP (DEG K)
C
C        DATA SET USE 
...
C		  ND5 = DIMENSION OF IPACK( ),IWORK( ), AND TLCL( ).
C			INPUT
C            IPACK(J) = WORK ARRAY (J=1,ND5).  (INTERNAL)
C            IWORK(J) = WORK ARRAY (J=1,ND5).  (INTERNAL)
C             TLCL(K) = CALCULATED LCL TEMPERATURE USING TERRAIN 
...
C          ICCCFFF(J) = ID(1)'s OF VARIABLES TO FETCH (J=1,3).  
C			(INTERNAL)
...
C      
      ...
      DIMENSION FDELV(ND2X3),FDTMP(ND2X3),FDDEW(ND2X3)
      DIMENSION TLCL(ND5),IPACK(ND5),IWORK(ND5)
...
C
      IF(IDPARS(1).EQ.003.AND.IDPARS(2).EQ.162) GO TO 30
C
      WRITE(KFILDO,20) JD
 20   FORMAT(/,' **** VARIABLE ',I9.9,2I10.9,I4.3,' CANNOT BE COMPU',
     1         'TED IN ATLCL ...SET IER=103 AND SUPPLY MISSING VALUES')
      IER=103
      GO TO 800   
C
C        ID(1) FOR TERRAIN, TEMP, AND DEWPOINT.
C
 30   ICCCFFF(1)=409020                              
      ICCCFFF(2)=002301
      ICCCFFF(3)=003301
C
D     CALL TIMPR(KFILDO,KFILDO,'START TLCL         ')
      IER=0
      ISTAV=0
C
...
C	 FILE. 
C
C     LD(1)=ICCCFFF(1)*1000+IDPARS(4)
      LD(1)=ICCCFFF(1)*1000
      LD(2)=0
      LD(3)=0
      LD(4)=0
...
C
 40   LD(1)=ICCCFFF(2)*1000+IDPARS(4)
      LD(2)=JD(2)
...
C
     ...
     3            NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,1,IER)
      ...
      IF(IER.NE.0)GO TO 800
C
C	 COMPUTE TLCL ...NO CHECKS FOR MISSING.
C
      F=5./9.
C
      DO 100 J=1,NWORDS
...
C            TIONS FOR CONSISTENCY.
 ...
 100  CONTINUE
C
C	 SMOOTH THE TLCL GRID.
C
      CALL SMTH9V(TLCL,IS2(3),IS2(4),0.4,IWORK,9998.5)
C
      GO TO 900
C
