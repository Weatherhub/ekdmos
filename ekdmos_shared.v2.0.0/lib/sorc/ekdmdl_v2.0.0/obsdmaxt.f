      SUBROUTINE OBSDMAXT(KFILDO,KFIL10,ID,IDPARS,JD,NDATE,
...
C        MAY       2003   GLAHN   WHITE SPACE; INDENTION; SPELL CHECK
C        MAY       2003   WEISS   PRINT FORMAT CHANGES; GFETCH FOR 
C                                 ARRAY HRLY; VARIABLE NDATES TAKEN
C                                 OUT
...
C                                 IS NOT 6; CHANGED ND2X3 TO ND1 IN
C                                 ALL CALLS TO GFETCH
C         DECEMBER 2003   WEISS   REINSERTED INITIALIZE STATEMENT
C                                 FOR ARRAY HRLY
C
C        PURPOSE 
C            THIS SUBROUTINE WILL COMPUTE THE DAYTIME MAXIMUM 
C            TEMPERATURE (DAYMAX) FROM OBSERVATION VECTOR DATA.
C            INPUT DATA AVAILABLE PRIOR TO DECEMBER 1, 1996 INCLUDES 
C            HOURLY OBS, AND BOTH 12 AND 24 HOUR MAXIMUM TEMPERATURE.
C            FOR DECEMBER 1, 1996 AND LATER, HOURLY OBS AND 6 HOUR 
C            MAXIMUM TEMPERATURE ARE THE INPUTS. DUE TO THESE INPUT
C            DATA DIFFERENCES, TWO ALGORITHMS ARE USED TO ESTIMATE
C            DAYMAX FOR EACH PERIOD OF TIME RESPECTIVELY. 
C            CALCULATIONS ARE DONE FOR IDPARS(7) = 0.
C
C            THE MAXIMUM TEMPERATURE WILL BE CALCULATED ONLY FOR 
C            06Z.  NO RECORD IS WRITTEN FOR ANY OTHER TIMES.
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C               702 001 - DAYTIME MAXIMUM TEMPERATURE 
C
C        DATA SET USE 
...
C              FD1(M) = WORK ARRAY (M=1,ND2X3).  IF HOUR OF FIRST 
C                       DATE IS NOT EQUAL TO 18 THEN FD1( ) IS USED
C                       FOR DUMMY READS WHICH ARE NECESSARY TO
C                       INITIALIZE U201'S MASS STORAGE SYSTEM.
C              FD2(M) = WORK ARRAY (M=1,ND2X3).  NOT ACTUALLY USED. 
...
C                       MAX ESTIMATES, WHERE N=1,ND1 AND K=MAXHRL
C                       (INTERNAL).
C                   I = INTEGER COUNTER (INTERNAL).
C              IBADHR = COUNTER OF NON-SYNOPTIC PROCESS DATES, DEFINED
C                       AS DATES NOT ENDING AS 00Z, 06Z 12Z, OR 18Z.
C                       (INTERNAL). 
C          ID_HRLY(J) = THE 4 WORD ID OF HOURLY TEMPERATURE, WHERE
C                       J=1,4 (INTERNAL).
C        ID_06MXHR(J) = THE 4 WORD ID OF 6 HOUR MAXIMUM TEMPERATURE,
C                       WHERE J=1,4 (INTERNAL).
C        ID_12MXHR(J) = THE 4 WORD ID OF 12 HOUR MAXIMUM TEMPERATURE,
C                       WHERE J=1,4 (INTERNAL).
C        ID_24MXHR(J) = THE 4 WORD ID OF 24 HOUR MAXIMUM TEMPERATURE,
C                       WHERE J=1,4 (INTERNAL).
C            IDUMYRR  = OFFSET USED TO INSERT ENOUGH DATA TO PROPERLY
C                       INITIALIZE U201'S DATA MASS STORAGE (INTERNAL).
C                 IHR = THE HOUR EXTRACTED FROM THE FULL CALENDAR 
C                       DATE (INTERNAL). 
...
C                       OR MAXMINB, EQUAL TO 1 FOR DAYMAX PROCESSING
C                       (INTERNAL). 
...
C          NEWPROCESS = LOGICAL USED TO CHOOSE MAX TEMPERATURE 
...
C                       GFETCH (INTERNAL). 
C                NPOS = FLAG FOR NEWPROCESS DUMMY READS (INTERNAL).
...
C              NWORDS = NUMBER OF WORDS RETURNED BY GFETCH (INTERNAL).
C          OLDPROCESS = LOGICAL USED TO CHOOSE MAX TEMPERATURE
C                       PROCESSING PRIOR TO 12/1/1996 (INTERNAL).
C      RMAX_06HR(N,J) = 6 HOUR MAXIMUM TEMPERATURES WHERE N=1,ND1
C                       AND J=1,4 (INTERNAL).
C        RMAX_12HR(N) = 12 HOUR MAX TEMPERATURE FOR EACH STATION
C                       WHERE N=1,ND1 (INTERNAL).
C        RMAX_24HR(N) = 24 HOUR MAX TEMPERATURE FOR EACH STATION
C                       WHERE N=1,ND1 (INTERNAL).
C             PROCESS = TRUE WHEN THE HOUR OF THE DATE IS O6Z.
C                       (INTERNAL)  (LOGICAL)
C********
C        INTERNAL SUBROUTINES
C            DATEFOUR = GENERATES FOUR DATES FOR MASS STORAGE READ 
C                       PURPOSES.
C              MAXMIN = SUBROUTINE TO GENERATE DAYTIME MAX VALUES
C                       FROM 12/1/1996 AND LATER.
C             MAXMINB = SUBROUTINE TO GENERATE DAYTIME MAX VALUES
...
C
      ...
      INTEGER ID_HRLY(4),ID_06MXHR(4),ID_12MXHR(4),ID_24MXHR(4),
     1        NDATE_FOUR(4)
      INTEGER KFILDO,KFIL10,NBLOCK,NFETCH,NPACK,NTIMES,NWORDS,
     1        MISSP,MISSS,NDATE1,IHR,IRR,IDUMYRR,IENTER,
     2        IBADHR,NPOS,NSLAB,J,I,JJ,N,MXMN,NDATE,ND1,NSTA,
     3        ND2X3,ND7,ISTAV,L3264B,IER,ND9,LITEMS,ND10,NOOUT
C
      REAL SDATA(ND1)
      REAL HRLY(ND1,MAXHRL),RMAX_06HR(ND1,4),
     *     RMAX_24HR(ND1),RMAX_12HR(ND1)
C             HRLY( , ), RMAX_06HR( , ), RMAX_24HR( ), AND RMAX_12HR( )
...
C
      ...
      ISTAV=1
C
C        STEP 1A. VERIFY THE PROCESSING OF DAYTIME MAXIMUM TEMPERATURE
C
      IF(JD(1).NE.702001000) THEN
        WRITE(KFILDO,150)(JD(J),J=1,4)
 150    FORMAT(/' ****OBSDMAXT WAS ENTERED WITH THE INCORRECT',
     1          ' PREDICTOR ID ',I9.9,2I10.9,I4.3)
        IER=103
        GO TO 800
      ENDIF
C
C        INFORM THE USER IS A TIME OTHER THAN 06 IS ENCOUNTERED.
C
      IF(MOD(NDATE,100).NE.06.AND.NOOUT.EQ.0)THEN
         WRITE(KFILDO,151)
 151     FORMAT(/' A TIME OTHER THAN 06 IS BEING PROCESSED IN',
     1           ' SUBROUTINE OBSDMAXT.  THERE WILL BE NO OUTPUT',
     2           ' FOR ANY TIME OTHER THAN 06.')
         NOOUT=1
      ENDIF
C
...
C        AND IT IS THE FIRST PROCESS DATE.  OBSDMAXT WILL NOT WORK,
C        AND THE SUBROUTINE WILL BE EXITED WITH IER = -1.
C
      IF(MOD(MOD(NDATE,100),6).NE.0.AND.IENTER.EQ.1) THEN
        WRITE(KFILDO,160)NDATE
 160    FORMAT(/' ****ERROR IN OBSDMAXT, FIRST PROCESS DATE IS',
     1          ' A NON-SYNOPTIC TIME.  NDATE=',I12,
     2         /'     OBSDMAXT CANNOT RUN, PLEASE READ WRITE UP.',
     3          '  ALL VALUES OF DAYTIME MAXIMUM TEMPERATURE',
     4          ' ARE MISSING.'
...
C        OBSDMAXT WILL NOT (CANNOT) BE CALCULATED,
C        AND THE SUBROUTINE WILL BE EXITED WITH IER = -1.
...
C
 ...
 165      FORMAT(/' ****IN OBSDMAXT, ONE OR MORE NON-SYNOPTIC DATES',
...
C        BASED ON THE CUTOFF DECEMBER 1, 1996 (06Z)
C
      IF(IENTER.EQ.1)THEN
C         GENERATE FOUR DATES FOR DUMMY READS (IF NEEDED)
        CALL DATEFOUR(NDATE,NDATE_FOUR,24)
        IF(NDATE.LE.1996120106) OLDPROCESS=.TRUE.
	IF(NDATE.GT.1996120106) NEWPROCESS=.TRUE.
C
C         SPECIAL FOR DECEMBER 1996 PROCESSING ONLY
C
        IF(NDATE.EQ.1996120100) THEN
           OLDPROCESS=.FALSE.
           NEWPROCESS=.TRUE.
        ENDIF
C
...
C
        IF(OLDPROCESS.AND.(NDATE.GT.1996120106)) THEN
          WRITE(KFILDO,170) NDATE
 170      FORMAT(/' ****ATTEMPTING OLDDATA PROCESS IN',
     1            ' OBSDMAXT, NDATE =',I12,' IS > 1996120106'
     2           /'     NO RECORD IS WRITTEN FOR THIS DATE')
          IER=-1
          GO TO 800
        ENDIF
...
C        PRECEDING DATE.
C        (IF(NEWPROCESS.AND.(NDATE.LE.1996120106)) THEN)
C
...
C
      IF(NEWPROCESS) THEN
C        CONSTRUCT THE ARRAY OF 6 HOUR MAXIMUM TEMPERATURE
	ID_06MXHR(1)=702100000
        ID_06MXHR(2)=IDPARS(7)
        ID_06MXHR(3)=IDPARS(9)*1000000+IDPARS(12)
        ID_06MXHR(4)=0
      ELSEIF(OLDPROCESS)THEN
C        CONSTRUCT THE ARRAYS OF 12 AND 24 HOUR MAXIMUM TEMPERATURE
        ID_12MXHR(1)=702004000
        ID_12MXHR(2)=IDPARS(7)
        ID_12MXHR(3)=IDPARS(9)*1000000+IDPARS(12)
        ID_12MXHR(4)=0
        ID_24MXHR(1)=702005000
        ID_24MXHR(2)=IDPARS(7)
        ID_24MXHR(3)=IDPARS(9)*1000000+IDPARS(12)
        ID_24MXHR(4)=0
      ENDIF
C
C        ****  MAIN IF BLOCK FOR DAYMAX ESTIMATES  ****
C        ****  IF HOUR = 06Z  ****
C
      IF(PROCESS)THEN
C
...
C
 ...
 195        FORMAT(/' ****ERROR IN OBSDMAXT, FOR FIRST DATE,',
     1              ' HOURLY TEMPERATURE IS MISSING FOR DATE =',I12,
     2             /'     POSSIBLE DATA GAP.  ALL VALUES OF DAYTIME',
     3              ' MAXIMUM TEMPERATURE ARE MISSING',
...
C
 ...
 200          FORMAT(/,' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2               ' TEMPERATURE IN OBSDMAXT.  DATA SET TO MISSING.')
              IER=52
              GO TO 800
            ENDIF
C
	  ENDIF   
C
C           STEP 2B. FILL IN THE ARRAYS RMAX_12HR AND RMAX_24HR
C                    IF OLDPROCESS
C           2. FETCH THE 12HR AND 24 MAX TEMPERATURES 
C              (ID_12MXHR AND ID24_MXHR) IF OLDPROCESS
C
          IF(OLDPROCESS) THEN
C
...
c
            IF(IHR.EQ.0) THEN 
              CALL GFETCH(KFILDO,KFIL10,ID_12MXHR,7777,LSTORE,ND9,
     1             LITEMS,IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,RMAX_12HR,ND1,
     2             NWORDS,NPACK,NDATE1,NTIMES,CORE,ND10,
     3             NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,0,
     4             IER)
C
              IF((IENTER.EQ.1).AND.(IER.NE.0)) THEN
                WRITE(KFILDO,300) NDATE1,(JD(J),J=1,4),IER
 300            FORMAT(/' ****ERROR IN OBSDMAXT, FOR FIRST DATE,',
     1                  ' HOURLY TEMPERATURE IS MISSING FOR DATE =',
     2                    I12,'.',
     3                 /'     POSSIBLE DATA GAP.  ALL VALUES OF',
     4                  ' DAYTIME MAXIMUM TEMPERATURE ARE MISSING',
     5                 /'     FOR VARIABLE ',I9.9,2I10.9,I4.3,
     6                  '.  IER =',I3,
     7                 /'     YOU MAY NEED A DUMMY CCC = 799 RECORD',
     8                  ' WITH AN RR = 24 AND DD = 00.')
...
C
 ...
 310              FORMAT(/,' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2                     ' TEMPERATURE IN OBSDMAXT.  DATA SET TO',
     3                     ' MISSING.')
                  IER=52
                  GO TO 800
                ENDIF
...
C
            ELSEIF((IHR.EQ.6).and.(I.GT.1)) THEN
              CALL GFETCH(KFILDO,KFIL10,ID_24MXHR,7777,LSTORE,ND9,
     1             LITEMS,IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,RMAX_24HR,ND1,
     2             NWORDS,NPACK,NDATE1,NTIMES,CORE,ND10,
     3             NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,0,
     4             IER)
C
 ...
 340            FORMAT(/' ****ERROR IN OBSDMAXT, FOR FIRST DATE.  24',
     1                  ' HOUR TEMPERATURE IS MISSING FOR DATE =', I12,
     2                 /'     POSSIBLE DATA GAP.  ALL VALUES OF',
     3                  ' DAYTIME MAXIMUM TEMPERATURE ARE MISSING',
     4                 /'     FOR VARIABLE ',I9.9,2I10.9,I4.3,
     5                  '.  IER =',I3,
     6                 /'     YOU MAY NEED A DUMMY CCC = 799 RECORD',
     7                  ' WITH AN RR = 24 AND DD = 00.')

                GO TO 800
              ELSEIF((IENTER.GT.1).AND.((IER.NE.0).AND.(IER.NE.47)))
     *                THEN
                GO TO 800
              ENDIF
C
...
C
            ENDIF
C
C             STEP 2C. FILL IN THE ARRAYS RMAX_06HR
C                     IF NEWPROCESS
C
          ELSEIF(NEWPROCESS) THEN
            NPOS=0
C
C              06HR (NORMAL PROCESSING AND SPECIAL TEST)
C
            IF(IHR.EQ.12) NPOS=1
            IF(IHR.EQ.18) NPOS=2
            IF(IHR.EQ.0)  NPOS=3
            IF((IHR.EQ.6).AND.(I.GT.1)) NPOS=4
C
            IF(NPOS.GE.1) THEN
              CALL GFETCH(KFILDO,KFIL10,ID_06MXHR,7777,LSTORE,ND9,
     1                 LITEMS,IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,
     2                 RMAX_06HR(1,NPOS),ND1,
     3                 NWORDS,NPACK,NDATE1,NTIMES,CORE,ND10,
     4                 NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,0,
     5                 IER)
C
              IF((IENTER.EQ.1).AND.(IER.NE.0)) THEN
                WRITE(KFILDO,400) NDATE1,(JD(J),J=1,4),IER
 400            FORMAT(/' ****ERROR IN OBSDMAXT, FOR FIRST DATE.  6',
     1                  '-HOUR TEMPERATURE IS MISSING FOR DATE =',I12,
     2                 /'     POSSIBLE DATA GAP.  ALL VALUES OF',
     3                  ' DAYTIME MAXIMUM TEMPERATURE ARE MISSING',
     4                 /'     FOR VARIABLE ',I9.9,2I10.9,I4.3,
...
C
 500    CONTINUE
C
C         STEP 3. CALCULATE THE DAYTIME MAXIMUM TEMPERATURE
C                 BY CALLING EITHER SUBROUTINE MAXMIN OR MAXMINB.
        MXMN=2
C
        IF(NEWPROCESS) THEN
          CALL MAXMIN(KFILDO,HRLY,RMAX_06HR,ITIMEZ,CCALL,SDATA,
     *                ND1,4,MAXHRL,MXMN,0,NSTA)
        ELSEIF(OLDPROCESS) THEN
          CALL MAXMINB(KFILDO,HRLY,RMAX_24HR,RMAX_12HR,ITIMEZ,CCALL,
     *                 SDATA,ND1,NDATE,MAXHRL,MXMN,NSTA)
        ENDIF
C
      ELSE
C
C        STEP 4. IF THE HOUR IS NOT 06Z AND THE SUBROUTINE IS 
C                BEING CALLED FOR THE FIRST PROCESS DATE
C                (REMEMBER, THE HOUR MUST BE 00Z, 06Z, 12Z, 
C                OR 06Z), "DUMMY" READS OF INPUT PARAMETERS
C                ARE CONDUCTED TO INITIALIZE U201'S MASS
C                STORAGE FOR THE PROPER STORAGE OF DATA.
C                NOTE: A LOOKBACK OF AT LEAST 24 HOURS MUST BE
C                      IMPLEMENTED.
C
...
C
 ...
 590          FORMAT(/' ****ERROR IN OBSDMAXT ON READ',
...
C
 ...
 595            FORMAT(/' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     1                  ' NOT EQUAL TO NSTA =',I6,' FOR DUMMY HOURLY',
     2                  ' TEMPERATURE IN OBSDMAXT.  DATA SET TO',
     3                  ' MISSING.')
                IER=52
                GO TO 800
...
C
            DO 610 N=1,4
              IF(MOD(NDATE_FOUR(N),100).EQ.6) NDATE1=NDATE_FOUR(N)
 610        CONTINUE
C
            CALL GFETCH(KFILDO,KFIL10,ID_24MXHR,7777,LSTORE,ND9,LITEMS,
     1                  IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FD1,ND1,
     2                  NWORDS,NPACK,NDATE1,NTIMES,CORE,ND10,
     3                  NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,0,
     4                  IER)
CC
 ...
 620          FORMAT(/' ****ERROR IN OBSDMAXT ON READ',
...
C
 ...
 621            FORMAT(/' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     1                  ' NOT EQUAL TO NSTA =',I6,' FOR DUMMY 24-',
     2                  ' HOUR TEMPERATURE IN OBSDMAXT.  DATA SET TO',
     3                  ' MISSING.')
                IER=52
                GO TO 800
C
...
C
            DO 625 N=1,4
              IF(MOD(NDATE_FOUR(N),100).EQ.0) NDATE1=NDATE_FOUR(N)
 625        CONTINUE
C
            CALL GFETCH(KFILDO,KFIL10,ID_12MXHR,7777,LSTORE,ND9,LITEMS,
     1                  IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FD1,ND1,
     2                  NWORDS,NPACK,NDATE1,NTIMES,CORE,ND10,
     3                  NBLOCK,NFETCH,NSLAB,MISSP,MISSS,L3264B,0,
     4                  IER)
C
 ...
 635          FORMAT(/' ****ERROR IN OBSDMAXT ON READ',
...
C
 ...
 640            FORMAT(/' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     1                  ' NOT EQUAL TO NSTA =',I6,' FOR DUMMY 12-HOUR',
     2                  ' TEMPERATURE IN OBSDMAXT.  DATA SET TO',
     3                  ' MISSING.')
                IER=52
                GO TO 800
...
C
 ...
 660            FORMAT(/' ****ERROR IN OBSDMAXT ON READ',
...
C
 ...
 665              FORMAT(/' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     1                    ' NOT EQUAL TO NSTA =',I6,' FOR DUMMY 6-HOUR',
     2                    ' TEMPERATURE IN OBSDMAXT.  DATA SET TO',
     3                    ' MISSING.')
                  IER=52
                  GO TO 800
...
D815  FORMAT(/,' ****ERROR IN OBSDMAXT, IER=',I5,' FOR VARIABLE',
D    *       4I12)
 850  RETURN
      END
