      SUBROUTINE OBSPOPCLMP(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                      SDATA,ND1,NSTA,IPACK,IWORK,FD1,ND2X3,
     2                      LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3                      IS0,IS1,IS2,IS4,ND7,ISTAV,L3264B,IER)
C
C        AUGUST   2005   SCALLION MDL MOS-2000
C                                 MODIFIED FROM OBSPOPC FOR NEW
C                                 CATEGORIES FOR LAMP DEVELOPMENT
C
C        PURPOSE 
C           THIS SUBROUTINE WILL CONVERT THE OBSERVATIONAL PRESENT
C           WEATHER REPORTS INTO THEIR RESPECTIVE PRECIPITATION 
C           CHARACTERISTIC CATEGORIES.  THESE CATEGORIES HAVE BEEN
C           ALTERED FROM THE CATEGORIES IN OBSPOPC TO ACCOMODATE FOR 
C           LAMP DEVELOPMENT OF PRECIP CHARACTERISTICS.  THE HOURLY DATA 
C           ARCHIVE CHANGED FROM SAO TO METAR ON DECEMBER 1, 1996, SO 
C           THERE IS ONE SET OF PRESENT WEATHER NUMBERS FOR THE SAO
...
C
C              708 602 - CATEGORY OF PRECIPITATION CHARACTERISTIC      
C 
C           THE CATEGORIES FOR PRECIPITATION CHARACTERISTIC ARE:
C              1 = CONVECTIVE, SHOWERY PRECIPITATION
C              2 = DRIZZLE
C              3 = STRATIFORM, STEADY PRECIPITATION
C           9999 = NO CASE (NO PRECIPITATION), MISSING DATA 
C                  OR UNDETERMINED WEATHER CHARACTERISTIC
C
...
C
      ...
      INTEGER I,J,KFILDO,KFIL10,NDATE,ND1,NSTA,ISTAV,IENTER,
     1        L3264B,ND2X3,IER,ND9,LITEMS,ND10,NBLOCK,NFETCH,
     2        IPWX,NSLAB,NTIMES,NWORDS,ND7,MISSP,MISSS,NPACK
C
      REAL SDATA(ND1)
      REAL FD1(ND2X3)
      REAL CORE(ND10)
...
C        VISIBILITY TYPE 
C
      IF((IDPARS(1).NE.708).OR.(IDPARS(2).NE.602))THEN
         IER=103
         WRITE(KFILDO,100)(JD(J),J=1,4),IER
 100     FORMAT(/,' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE ',
     1            'OBSERVATION OF PRECIPITATION CHARACTERISTIC.',  
     2          / '     PREDICTOR ',I9.9,2I10.9,I4.3,' NOT',
     3            ' ACCOMMODATED IN SUBROUTINE OBSPOPCLMP,  IER=',I3)
         GOTO 900
      ENDIF
C
...
C
 ...
 120     FORMAT(/,' ****OBSPOPCLMP CAN NOT PROCESS DATA BECAUSE OF THE',
...
C        A 'CUMULATIVE' PRECIPITATION CHARACTERISTIC.
C
         DO 200 I=1,3
C
...
C
 ...
 140           FORMAT(/,' ****ERROR FROM GFETCH OCCURRED ON',
     ...
     2                 ' OBSPOPCLMP CANNOT RUN. PLEASE READ WRITE UP')
               GOTO 900
            ENDIF
C
...
C
 ...
 160           FORMAT(/,' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2                  ' IN OBSPOPCLMP. DATA SET TO MISSING.',
     3                  ' IER=',I3)
               GOTO 900
            ENDIF
C
...
C                 WEATHER GROUP FALLS IN.  THEN YOU MUST CHECK TO SEE 
C                 IF A PREVIOUS PRESENT WEATHER GROUP HAD CAUSED SDATA 
C                 TO ALREADY BE SET TO A CATEGORY
C
               IPWX = NINT(FD1(J))
C
               SELECT CASE(IPWX)
C
C                    NO CASE, CASE MISSING OR UNDETERMINED PRECIPITATION TYPE
C
...
C  
               CASE(51,53,55:57,76,156)
                  SOBS=2.
C
C                    STRATIFORM, STEADY PRECIPITATION
C
               CASE(58,59,61,63,65:69,71,73,75,
     1              77,79,166,174,176)
                  SOBS=3.
C
C                    CONVECTIVE, SHOWERY PRECIPITATION
C
               CASE(80,81,83:86,88,90,95,96,97,
     1              183,187,196)
                  SOBS=1.
C
               END SELECT
...
C
            IF(NINT(SDATA(J)).EQ.9999) SDATA(J)=SOBS
C
 170     CONTINUE 
C
 200  CONTINUE
...
C           30, 1996.
C
C           SET UP THE LD ARRAY TO HOLD THE ID FOR THE PRESENT WEATHER.
C
         ...
         LD(4)=0
C
C          CALL GFETCH TO GET THE PRESENT WEATHER AND STORE IT IN FD1.
C
        CALL GFETCH(KFILDO,KFIL10,LD,7777,LSTORE,ND9,LITEMS,
     1              IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FD1,ND2X3,
     2              NWORDS,NPACK,NDATE,NTIMES,CORE,ND10,
