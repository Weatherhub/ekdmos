      SUBROUTINE KWHTOJ(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C        AUGUST 1999   RUDACK   CHANGED CALL AND CALL TO RETVEC FROM GFETCH
...
C
C        PURPOSE 
C            TO CHANGE UNITS FROM KWH TO JOULES.
C            CONVERSION USED:  1 KWH = 3.6E+06 J.
...
C            XDATA(K) = VARIABLE IN KWH, THEN JOULES (K=1,NSTA).  (OUTPUT)
...
C
 ...
 107  FORMAT(' ****KWHTOJ ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        GET THE VARIABLE IN KWH.
C
...
C
  ...
  125    FORMAT(' ****VARIABLE NOT RETRIEVED BY RETVEC IN KWHTOJ',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        CHANGE UNITS.  IF VALUE IS MISSING, LEAVE IT.
C
      DO 211 K=1,NSTA
C
         IF(NINT(XDATA(K)).EQ.9999) GO TO 211
C     
         XDATA(K)=(XDATA(K))*3.6E+6
C
 211  CONTINUE
C 
      GO TO 350   
