      SUBROUTINE GRIDBM(KFILDO,ID,IDPARS,THRESH,P,FD,NX,NY,IER)
C
...
C            VARIABLE IS GE THRESH AND 0 OTHERWISE.  NOTE THAT GRIDBM
C	     CHECKS FOR MISSING (9999) GRIDPOINT VALUES.  WHEN FOUND 
C	     THE MISSING IS RETAINED.  GRIDBM WAS ADAPTED FROM GRIDB.
C 
C        DATA SET USE
...
C                       ACTUALLLY BE USED, BUT IS PROVIDED IN CASE IT
...
C
 ...
 101  FORMAT(/' ****THRESHOLD =',F7.0,' FOR BINARY PARAMETER IDPARS =',
     ...
     2        '     IS INDICATED AS MISSING IN SUBROUTINE GRIDBM.',
...
C
      DO 160 JY=1,NY
      DO 159 IX=1,NX
      IF(NINT(P(IX,JY)).EQ.9999)GOTO 159                              
...
C
 ...
 801  FORMAT(/' ****THE BINARY PARAMETER IDPARS =',I3,
     ...
     2        '     WAS NOT FOUND IN SUBROUTINE GRIDBM.',
