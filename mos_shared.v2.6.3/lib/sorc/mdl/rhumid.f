      SUBROUTINE RHUMID(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
     ...
     7                  L3264B,L3264W,IER)
C
C        MAY       1999   RUDACK   TDL   MOS-2000
...
C                                      ON THE IBM SP
C        JUNE      2005   RLC      IN PREPARATION TO USE THIS ROUTINE
C                                  MODIFIED ALGORITHM TO SET TO MISSING
C                                  IF EITHER T OR TD WERE MISSING, AND
C                                  CONSTRAIN RH TO BE BETWEEN 2 AND 100.
C                                  CHANGED TEMP/DEWPT IDS TO USE CHECKED
C                                  IDS NOT RAW ONES.
C
C        PURPOSE 
C            TO DETERMINE THE RELATIVE HUMIDITY GIVEN TEMPERATURE
C            AND DEW POINT IN FARENHEIT.  CONVERSION USED:
C            RH=VAPOR PRESSURE/SATURATION VAPOR PRESSURE.
C            THIS ROUTINE ACTS ON STATION DATA.
C
C            (1) & IDPARS(2) OF TEMPERATURE  
C            ARE MAPPED INTO      FROM 
C                       203060    202020 AND 203020
C
C        DATA SET USE 
...
C            XDATA(K) = TEMPERATURE IN F (INTERNAL), THEN RELATIVE
C                       HUMIDITY (K=1,NSTA).  (OUTPUT)
...
C                       (INTERNAL)
C            YDATA(K) = WORK ARRAY WHICH STORES DEW POINTS (K=1,ND1).  
C                       (AUTOMATIC)
...
C               TCEL  = TEMPERATURE IN CELSIUS.
C               DPCEL = DEW POINT TEMPERATURE IN CELSIUS.
C               VP    = VAPOR PRESSURE.
C               SVP   = SATURATION VAPOR PRESSURE.
C 
...
C
      DATA ITABLE/203060, 202020, 203020/
C
      IER=0
C
...
C
 ...
 107  FORMAT(' ****RHUMID ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        GET THE TEMPERATURE IN FARENHEIT.
C
...
C
  ...
  125    FORMAT(' ****VARIABLE NOT RETRIEVED BY RETVEC IN RHUMID',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        GET THE DEW POINT TEMPERATURE IN FARENHEIT.
C
...
C
  ...
  127    FORMAT(' ****VARIABLE NOT RETRIEVED BY RETVEC IN RHUMID',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        COMPUTE THE RELATIVE HUMIDITY IF THE TEMPERATURE AND DEW POINT
C        ARE NOT MISSING.
C
      F=5./9.
C
...
C
         IF(XDATA(K).LT.2.0)XDATA(K)=2.0
         IF(XDATA(K).GT.100.0)XDATA(K)=100.0
