...
C        JANUARY   2006   DAGOSTARO   DISABLED SECTION OF CODE THAT SET
C                                 PLAIN LANGUAGE FOR VARIABLES CXC,
C                                 WHERE X = 8 OR 9, BECAUSE THE PLAIN
C                                 LANGUAGE WAS TOO GENERIC.  INSTEAD,
C                                 ADDED SPECIFIC ENTRIES IN THE MOS2000        
C                                 ID TABLE.  ALSO ADDED CODE TO FILL IN
C                                 MODELS "HPC" AND "AEV".
C
C        PURPOSE 
...
C            AND THE DEFAULT WILL CONTINUE TO BE 0.  WHILE ALL CALLING
C            PROGRAMS MAY NOT NEED ALL OF THIS INFORMATION, THAT NOT
C            NEEDED CAN BE DISCARDED, AND SETPLN WILL DO THE JOB FOR
C            ALL PROGRAMS.
C            
...
C           VARIABLES ARE COMPUTED ON THE FLY IN U602, U700, U900.
      ...
      ENDIF
C
C        THE FOLLOWING SECTION WAS DISABLED IN JANUARY 2006:
C        EXEMPT MATCHING VARIABLE X = 8 OR 9 IN CXC. 
C
C     ICXC=(IDPARS(1,N)-(IDPARS(1,N)/100)*100)/10
C
C     IF(ICXC.EQ.8.OR.ICXC.EQ.9)THEN
C        NFOUND=NFOUND+1
C        JDD(N)=1
C        PLAIN(N)='     MATCHING VARIABLE          '
C        GO TO 210
C     ENDIF
C
...
C
 218  IF(IDPARS(1,N).GE.500.AND.IDPARS(1,N).LE.599)THEN
      ...
      ELSEIF(IDPARS(1,N).GE.600.AND.IDPARS(1,N).LE.699)THEN
         PLAIN(N)(26:27)='D2' 
C
C        INSERT "HPC", "NDF", "AEV", "LMP", "NGM", "ETA", "AVN", OR
C        "MRF" FOR MODEL NUMBERS DD = 78, 79, 80-82, 5, 6, 7, 8, OR 9,
C        RESPECTIVELY, FOR MOS AND NDFD FORECASTS CCC = 2XX.
C
      ELSEIF(IDPARS(1,N).GE.200.AND.IDPARS(1,N).LE.299)THEN
C
         IF(IDPARS(4,N).EQ.78)THEN
            PLAIN(N)(2:4)='HPC'
         ELSEIF(IDPARS(4,N).EQ.79)THEN
            PLAIN(N)(2:4)='NDF'
         ELSEIF(IDPARS(4,N).GE.80.AND.IDPARS(4,N).LE.82)THEN
            PLAIN(N)(2:4)='AEV'
