...
C        MARCH    2013   SHAFER  NEW ID ADDED FOR ECMWF WHICH HAS
C                                7 VERTICAL LEVELS;
C                                ADDED ENTRIES TO ILIM(), ZLVL(),
C                                IFFF(), AND IDD() AND INCREASED
C                                DIMENSION OF EACH ARRAY TO 5.
C
C        PURPOSE
...
C                          FOR THE GEFS.  LEVELS ARE 1000,925,850,
C                          700,500 MB.
...
C                002 050 - THE FREEZING LEVEL HEIGHT RETURNED
C                          FOR THE ECMWF.  LEVELS ARE 1000,925,850,
C                          700,500,400,300 MB.
C
C        DATA SET USE
...
C                       (INTERNAL-OUTPUT)
C             IFFF(J) = THE FFF'S FOR FIVE DIFFERENT MODELS IN ORDER 
C                       NGM, MRF, ETA, AVN, AND ECM (J=1,5).  THIS MUST
C                       MATCH IDPARS(2). (INTERNAL)
C              IDD(J) = THE DD'S FOR FIVE DIFFERENT MODELS IN ORDER
C                       NGM, MRF, ETA, AVN, AND ECM (J=1,5).  THIS MUST
C                       MATCH IDPARS(4). (INTERNAL)
C             ILIM(J) = NUMBER OF ISOBARIC LEVELS FOR FIVE DIFFERENT
C                       MODELS IN ORDER NGM, MRF, ETA, AVN, AND ECM 
C                       (J=1,5). (INTERNAL)
...
C
      ...
      INTEGER IFFF(6),ILIM(6),MDT1(6),IDD(6)
...
C
     ...
     B     (ZLVL(I,5),I=1,7)
     C           /1000.0,925.0,850.0,700.0,500.0,400.0,
     D             300.0/
     B     (ZLVL(I,6),I=1,5)
     C           /1000.0,925.0,850.0,700.0,500.0/
      DATA IFFF/049,046,047,048,050,045/
      DATA IDD/6,9,7,8,1,62/
...
C
     ...
     2   IDPARS(2).NE.048.AND.IDPARS(2).NE.050.AND.
     3   IDPARS(2).NE.045))THEN
...
C
        IER=103
        WRITE(KFILDO,105)(JD(J),J=1,5),IER
