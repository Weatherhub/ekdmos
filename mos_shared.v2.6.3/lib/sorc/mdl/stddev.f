      SUBROUTINE STDDEV(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     ...
     7                  L3264B,L3264W,IER)
C 
C        FEBRUARY    2013   RUDACK   MDL   MOS-2000
C
C        PURPOSE 
C            TO CALCULATE THE STANDARD DEVIATION OF THE ENSEMBLE 
C            MEAN MOS FORECASTS.  THIS CODE IS DESIGNED TO 
C            PROCESS THE 51 MEMBERS OF THE ECWMF ENSEMBLE MOS.  
C            THE FOLLOWING WEATHER ELEMENTS ARE ACCOMMODATED: 
C
C            (1) 2-M MOS TEMPERATURE FORECASTS
C            (2) 2-M MOS DEWPOINT FORECASTS
C            (3) 2-M MOS DAYTIME MAXIMUM TEMPERATURE FORECASTS
C            (4) 2-M MOS NIGHTTIME MINIMUM TEMPERATURE FORECASTS
C            (5) 10-M MOS WIND SPEED FORECASTS
C            (6) 06-H MOS POP FORECASTS
C            (7) 12-H MOS POP FORECASTS
C            (8) 12-H MOS POP FORECASTS
C
C        DATA SET USE 
...
C               ISTAB = SET TO 1 TO KEEP FROM ENTERING ROUTINE "BINFUL."
C                       (OUTPUT)
...
C            XDATA(K) = STANDARD DEVIATION OF THE ENSEMBLE MEMBERS 
C                       (K=1,NSTA).  (OUTPUT)
...
C                       (INTERNAL)
C           ITABLE(I) = VALUES OF CCCFFF ACCOMMODATED (I=1) 
C                       (J=1,NDIM).  (INTERNAL)
...
C
      ...
      DIMENSION YDATA(ND1),SDSUM(ND1),JSUM(ND1)
C        YDATA( ), JSUM( ) AND SDSUM( ) ARE AUTOMATIC ARRAYS.
...
C
      DO 105 JJ=1,NDIM
         IF(ITABLE(JJ).EQ.IDPARS(1)*1000+IDPARS(2)) GO TO 120
 105  CONTINUE
C
      WRITE(KFILDO,107)(JD(L),L=1,4)
 107  FORMAT(/,' ****STDDEV ENTERED FOR VARIABLE',
...
C
 ...
 108  CONTINUE
C
C        FILL LD( ) AND LDPARS( ) WITH THE ID OF THE MEAN OF 
C        THE VARIABLE TO BE PROCESSED.
C
      LD(1)=ITABLE(JJ)*1000+100*IDPARS(3)+IDPARS(4)
      LD(2)=003000000
      LD(3)=JD(3)
      LD(4)=ID(4)
C
...
C
      IF(IER.NE.0)THEN
         WRITE(KFILDO,115)(LD(J),J=1,4)
 115     FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN STDDEV',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GOTO 300
      ENDIF
C
C        FILL LD( ) AND LDPARS( ) WITH THE EACH MEMBER'S
C        VARIABLE ID TO BE PROCESSED.  
C
      JDD=0
C
      DO 200 L=1,NPERT
C
         LD(1)=ITABLE(JJ)*1000+100*IDPARS(3)+JDD
         LD(2)=0
C
         CALL PRSID1(KFILDO,LD,LDPARS)
         CALL RETVEC(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
...
C
 ...
 125        FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN STDDEV',
     1             2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         ENDIF
C
C           ADD THE VALUES FOR THE CALCULATION OF THE STANDARD DEVIATION.  
C           ONLY COUNT MEMBERS THAT HAVE NON-MISSING FORECASTS.
C
         DO 150 K=1,NSTA
            IF((NINT(XDATA(K)).NE.9999).AND.
     1         (NINT(YDATA(K)).NE.9999)) THEN
               SDSUM(K)=SDSUM(K)+(YDATA(K)-XDATA(K))**2
               JSUM(K)=JSUM(K)+1
            ENDIF
 150     CONTINUE
C
...
C
 200  CONTINUE              
C
C        FIND THE STANDARD DEVIATION.  GUARD FOR THE DENOMINATOR
C        NOT BEING ZERO.
C
      DO 211 K=1,NSTA
         IF(JSUM(K).GE.2) THEN
            XDATA(K)=SQRT(SDSUM(K)/(FLOAT(JSUM(K))-1.))
         ELSE
            XDATA(K)=9999.
         ENDIF
 211  CONTINUE
C
C        RESET ERROR TO ZERO IN CASE THE LAST PERTURBATION
C        WAS MISSING BUT OTHER PERTURBATIONS WERE NOT.
C
      IER=0
C 
      GO TO 350   
