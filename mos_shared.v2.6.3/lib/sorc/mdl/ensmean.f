      SUBROUTINE ENSMEAN(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
...
C
C        PURPOSE 
C            TO CALCULATE THE ENSEMBLE MEAN OF MOS FORECASTS.
C            THIS CODE IS DESIGNED TO PROCESS THE 51 MEMBERS OF
C            THE ECWMF ENSEMBLE MOS.  THE FOLLOWING WEATHER 
C            ELEMENTS ARE ACCOMMODATED: 
C
...
C            XDATA(K) = ENSEMBLE MEAN (K=1,NSTA).  (OUTPUT)
...
C
      ...
      DIMENSION YDATA(ND1),JSUM(ND1)
C        YDATA( ) AND JSUM( ) ARE AUTOMATIC ARRAYS.
...
C
 ...
 107  FORMAT(/,' ****ENSMEAN ENTERED FOR VARIABLE',
...
C
 ...
 125        FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN ENSMEAN',
     1             2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         ENDIF
C
C           ADD THE VALUES FOR THE CALCULATION OF THE MEAN.  ONLY COUNT
C           MEMBERS THAT HAVE NON-MISSING FORECASTS.
C
         DO 150 K=1,NSTA
            IF(NINT(XDATA(K)).NE.9999) THEN
               YDATA(K)=YDATA(K)+XDATA(K)
               JSUM(K)=JSUM(K)+1
            ENDIF
 150     CONTINUE
C
...
C
 200  CONTINUE              
C
C        FIND THE ENSEMBLE MEAN.
C
      DO 211 K=1,NSTA
         IF(JSUM(K).NE.0) THEN
            XDATA(K)=YDATA(K)/FLOAT(JSUM(K))
         ELSE
            XDATA(K)=9999.
         ENDIF
 211  CONTINUE
