...
C         AUG 2007   ANTOLIK                ADDED CAPABILITY TO COMPUTE
C                                           ADJUSTED TEMPERATURE BY USING
C                                           ETA/NMM TERRAIN.  CORRECTED SOME
C                                           DIAGNOSTIC MESSAGES.
C         MAY 2015   ENGLE                  ADDED CAPABILITY TO SET TERRAIN ID
C                                           ACCORDING TO MAP PROJECTION (FROM 
C                                           NGRIDC(1,1) WHEN DD=07.
C
C        PURPOSE
...
C
C               DD=08   - USE GFS TERRAIN ON 95-KM GRID (CCCFFF=409020) 
C               DD=07   - IF MAP PROJ = 3 (LC), USE ETA/NMM TERRAIN ON 
C                         32-KM GRID (CCCFFF=409370).
C                         IF MAP PROJ = 5 (PS), USE NAM TERRAIN ON 33-KM
C                         GRID (CCCFFF=409400).
C
C        DATA SET USE
...
C                             THE LAPSE RATE, OR IDPARS(4) DOES NOT 
C                             INDICATE A SUPPORTED MODEL.
...
C
 ...
 101     FORMAT(/' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE ',
     ...
     3           ' ACCOMMODATED IN ADJTMP.  IER =',I4)
         IER=103
         GOTO 800
      END IF
C
C       GET THE APPROPRIATE MODEL (GFS/NAM) TERRAIN USING CONST1.
C
      IF(IDPARS(4).EQ.07)THEN
         IF(NGRIDC(1,1).EQ.3)THEN
C             HERE WOULD BE THE "OLD" ETA (AWIPS 221) LC GRID.
           LD(1)=409370000
         ELSEIF(NGRIDC(1,1).EQ.5)THEN
C             HERE WOULD BE THE "NEW" NAM (AWIPS 151) PS GRID.
           LD(1)=409400000
         ELSE
            WRITE(KFILDO,1010)IDPARS(4)
 1010       FORMAT(/' ****ONLY LAMBERT CONFORMAL OR POLAR ',
     1              'STEREOGRAPHIC GRIDS SUPPORTED WHEN ',
     2              'IDPARS(4) = ',I2) 
            IER=107
            GOTO 800
         ENDIF
      ...
      ELSE
         WRITE(KFILDO,101)(JD(J),J=1,4)
 102     FORMAT(/' ****IDPARS(4) DOES NOT INDICATE A SUPPORTED ',
     1           'MODEL.  NO TERRAIN CAN BE FOUND.',
     2          /'     PREDICTOR ',I9.9,I10.9,I10.9,I4.3,' NOT',
     3           ' ACCOMMODATED IN ADJTMP.  IER =',I4)
        IER=103
        GOTO 800
...
C
 ...
 110    FORMAT(/,'****CALL TO CONST1 READING MODEL TERRAIN ',
     1         'FAILED.  IER = ',I3)
        GOTO 800
      END IF
      MCX=IS2(3)
...
C
 ...
 120    FORMAT(/' ****THE GRID CHARACTERISTICS OF THE ',
     1          ' 2-M TEMP AND THE MODEL TERRAIN HEIGHT',
     2          '.',I3,2X,I3,
