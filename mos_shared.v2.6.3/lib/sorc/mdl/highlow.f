      SUBROUTINE HIGHLOW(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
...
C
C        PURPOSE 
C            TO CALCULATE THE MAXIMUM OR MINIMUM VALUE OF A SET OF
C            ENSEMBLE MOS FORECASTS.  THIS CODE IS DESIGNED TO 
...
C            XDATA(K) = ENSEMBLE MAXIMUM OR MINIMUM VALUE (K=1,NSTA).  (OUTPUT)
...
C
      ...
      DIMENSION YDATA(ND1,NPERT),JSUM(ND1)
C        YDATA( , ) AND JSUM( ) ARE AN AUTOMATIC ARRAYS.
...
C
 ...
 107  FORMAT(/,' ****HIGHLOW ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        INITIALIZE ARRAYS PRIOR TO USE.
C
 120  DO 122 K=1,ND1
         XDATA(K)=9999.
         JSUM(K)=0
 122  CONTINUE
C
C        FILL LD( ) AND LDPARS( ) FOR THE VARIABLE TO BE PROCESSED.
C
      LD(2)=0
      LD(3)=JD(3)
      LD(4)=ID(4)
...
C
 ...
 135        FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN HIGHLOW',
     1             2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         ENDIF
C
         JDD=JDD+1
C
 200  CONTINUE              
C
C        FLAG THOSE STATIONS THAT ALWAYS HAVE A MISSING VALUE.  
C        IN THIS CASE, JSUM( ) SHOULD EQUAL ZERO.  THE MAXVAL 
C        FUNCTION CANNOT DEAL WITH THAT SITUATION IF YOU SET 
C        THE MASK TO THAT SAME VALUE.
C
      DO 215 K=1,NSTA
         DO 210 L=1,NPERT
            IF(NINT(YDATA(K,L)).NE.9999) THEN
               JSUM(K)=JSUM(K)+1
            ENDIF
 210     CONTINUE
 215  CONTINUE
C
C        FIND THE MAXIMUM VALUE OF THE ENSEMBLE MOS FORECASTS.
C
      IF(IDPARS(6).EQ.400) THEN 
C
         DO 225 K=1,NSTA
            DO 224 L=1,NPERT
            IF(JSUM(K).GT.0) THEN
               XDATA(K)=MAXVAL(YDATA(K,1:NPERT),
     1                  MASK=NINT(YDATA(K,1:NPERT)).NE.9999)
            ENDIF
 224        CONTINUE
 225     CONTINUE 
C
      ELSE
C
C           FIND THE MINIMUM VALUE OF THE ENSEMBLE MOS FORECASTS.
C
         DO 235 K=1,NSTA
            IF(JSUM(K).GT.0) THEN
               XDATA(K)=MINVAL(YDATA(K,1:NPERT),
     1                  MASK=NINT(YDATA(K,1:NPERT)).NE.9999)
            ENDIF
 235     CONTINUE
C
