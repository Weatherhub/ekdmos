      SUBROUTINE OBSPOPC(KFILDO,KFIL10,IDPARS,JD,NDATE,
...
C                                 USED DESIGN OF ROUTINE 'OBSOBVIS'
C                                 FOR 'OBSPOPC'
...
C        MAY      2003   GLAHN    MODIFIED FORMAT 100
C
C        PURPOSE 
C           THIS SUBROUTINE WILL CONVERT THE OBSERVATIONAL PRESENT
C           WEATHER REPORTS INTO THEIR RESPECTIVE PRECIPITATION 
C           CHARACTERISTIC CATEGORIES.  THE HOURLY DATA ARCHIVE CHANGED 
C           FROM SAO TO METAR ON DECEMBER 1, 1996, SO THERE IS
...
C
C              708 605 - CATEGORY OF PRECIPITATION CHARACTERISTIC      
C 
C           THE CATEGORIES FOR PRECIPITATION CHARACTERISTIC ARE:
C              1 = DRIZZLE
C              2 = STRATIFORM, STEADY PRECIPITATION
C              3 = CONVECTIVE, SHOWERY PRECIPITATION
...
C        VISIBILITY TYPE 
C
      IF((IDPARS(1).NE.708).AND.(IDPARS(2).NE.605))THEN
         IER=103
 ...
 100     FORMAT(/,' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE ',
     ...
     3            ' ACCOMMODATED IN SUBROUTINE OBSPOPC,  IER=',I3)
         GOTO 900
      ENDIF
C
...
C
 ...
 120     FORMAT(/,' ****OBSPOPC CAN NOT PROCESS DATA BECAUSE OF THE',
...
C
 ...
 140        FORMAT(/,' ****ERROR FROM GFETCH OCCURRED ON',
     ...
     2              ' OBSPOPC CANNOT RUN. PLEASE READ WRITE UP')
            GOTO 900
         ENDIF
C
...
C
 ...
 160        FORMAT(/,' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2               ' IN OBSPOPC. DATA SET TO MISSING.',
     3               ' IER=',I3)
            GOTO 900
         ENDIF
C
