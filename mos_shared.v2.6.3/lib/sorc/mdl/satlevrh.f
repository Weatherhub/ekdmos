...
C        FEBRUARY 2014   RUDACK  ADDED 47 KM ECMWF MODEL TERRAIN HEIGHT.
C                                ADDED LOGIC TO SKIP THE LOWEST 30 MB
C                                RH SINCE THAT DATA IS UNAVAILABLE IN
C                                THE ECMWF MODEL ARCHIVE.
C        JUNE     2015   ENGLE   ADDED CHECK WHEN IDPARS(2) ARE ETA/NAM                                        
C                                ADDED LOGIC TO SKIP THE LOWEST 30 MB                                          |    48 C                                SPECIFIC AND MPROJ=5 (NPS) TO FETCH FOR                                       
C                                RH SINCE THAT DATA IS UNAVAILABLE IN                                          |    49 C                                TERRAIN ID 409400.                    
C
C        PURPOSE
...
C                 003 314 - ECMWF      ( 5 LEVEL), THRESHOLD 85 PERCENT
...
C                 003 324 - ECMWF      ( 5 LEVEL), THRESHOLD 90 PERCENT
...
C                 003 334 - ECMWF      ( 5 LEVEL), THRESHOLD 95 PERCENT
...
C
      SELECT CASE(IDPARS(2))
        CASE(312,322,332)
          ...
          LEVEL(9)=500
          NUMLEV=9
          IF(NGRIDC(1,1).EQ.3)THEN
             KD(1)=409030000
          ELSEIF(NGRIDC(1,1).EQ.5)THEN
             KD(1)=409400000
          ENDIF
          KD(2)=0
          KD(3)=0
...
C
      SELECT CASE(IDPARS(2))
        CASE(314,324,334)
          ...
          LEVEL(5)=500
          NUMLEV=5
          KD(1)=409510000 ! ECMWF 47 KM TERRAIN
          KD(2)=0
          KD(3)=0
          KD(4)=0
      END SELECT
C
      IF(IDPARS(4).EQ.1)THEN
C
        IF(MOD(MOD(NDATE,100),3).NE.0)THEN
          WRITE(KFILDO,148)NDATE
 148      FORMAT(/' ****NDATE, ',I10,', FOR ECMWF SATLEVRH MUST',
     &            ' BE A MULTIPLE OF 3.')
          IER=187
          GO TO 800
        ENDIF
...
C
 ...
 152      CONTINUE
C
C            SINCE THE LOWEST 30 MB LEVEL IS NOT AVAILABLE IN THE
C            ECMWF MODEL ARCHIVE, SKIP THAT PORTION OF THE CODE.
          IF(IDPARS(4).EQ.1) GOTO 1615
          GO TO 161
        ENDIF
C
 160  CONTINUE
...
C
 ...
 161  MD_RH30(1) = ICCC_RH*1000000 + IFFF_RH30*1000 + IDPARS(4)
      MD_RH30(1) = ICCC_RH*1000000 + IFFF_RH30*1000 + IDPARS(4)
      MD_RH30(2) = 925
      MD_RH30(2) = 970
