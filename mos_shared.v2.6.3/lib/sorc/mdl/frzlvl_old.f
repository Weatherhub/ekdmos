      SUBROUTINE FRZLVL_OLD(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                NGRIDC,ND11,NSLAB,IPACK,IWORK,
     2                FDFZ,ND5,LSTORE,ND9,LITEMS,CORE,ND10,
     3                NBLOCK,NFETCH,IS0,IS1,IS2,IS4,ND7,FD1,
     4                FD2,FD3,FD4,ND2X3,ISTAV,L3264B,
     5                MISTOT,IER)
C
C         OCT    1998   SFANOS     TDL-MOS 2000
C         NOV    1998   SFANOS     RENAMED WORK ARRAYS,
C                                  PUT IN MISSP IF TEST,
C                                  AND CHANGED SOME COMMENTS
C         OCT    1999   SFANOS     RESTRUCTERED PROGRAM TO
...
C         OCT    2002   WEISS      CHANGED ND5 TO ND2X3
C         JULY   2003   COSGROVE   THERE IS AN ERROR IN THIS ROUTINE 
C                                  (FDFZ IS NOT INITIALIZED) SO WE
C                                  HAVE A CORRECTED VERSION WITH CORRECTED
C                                  IDS.  THIS IS STILL NEEDED FOR 
C                                  OPERATIONS FOR NOW.
C                                  I HAD TO ADD THE INITIALIZATION FOR
C                                  CONTINUITY IN OPERATIONS.  TRUST ME.
C
C        PURPOSE
...
C                OF THE TWO LEVELS. (J=1,ND2X3)
C                
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C                   002 042  - THE FREEZING LEVEL HEIGHT RETURNED
C                              FOR THE MRF(OLD AVN).  LEVELS ARE
C                              1000,925,850,700,500,300 MB. 
C                   002 043  - THE FREEZING LEVEL HEIGHT RETURNED
C                              FOR THE ETA.  LEVELS ARE 1000,950,900,
C                              850,800,750,700,600,500,400,300 MB. 
C                   002 044  - THE FREEZING LEVEL HEIGHT RETURNED
C                              FOR THE AVN. LEVELS ARE 1000,975,950,925,
C                              900,850,800,750,700,600,500,300 MB. 
C                   002 045  - THE FREEZING LEVEL HEIGHT RETURNED
C                              FOR THE NGM.  LEVELS ARE 100,950,900,850,
C                              800,750,700,500,300 MB.
C
C        DATA SET USE
...
C                       (INPUT)
C             FDFZ(J) = ARRAY TO HOLD RETURNED FDFZ WHEN THE DATA ARE
C                       AT GRIDPOINTS. IN THIS CASE,THE FREEZING LEVEL
C                       HEIGHT.(J=1,ND2X3). (OUTPUT)
...
C                       (INTERNAL-OUTPUT)
C             IFFF(J) = THE IDS FOR DIFFERENT MODELS.  THIS ID IS
C                       IDPARS(2). (J=1,4) (INTERNAL)
C             ILIM(J) = NUMBER OF ISOBARIC LEVELS IN THE TEMPLATES. (J=1,4)
C                       (INTERNAL)
...
C        MAKE SURE THIS SUBROUTINE DEALS WITH THE PREDICTOR.
      IF(IDPARS(1).NE.002.OR.(IDPARS(2).NE.045.AND.
     1   IDPARS(2).NE.042.AND.IDPARS(2).NE.043.AND.
     2   IDPARS(2).NE.044))THEN
         WRITE(KFILDO,101)(JD(J),J=1,4)
 101     FORMAT(/' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE ',
     1           'THE FREEZING LEVEL. ',
     2           I9.9,2I10.9,
     3           I4.3, 'NOT COMPUTED IN FRZLVL. ')
         IER=103
         GOTO 800
      END IF
C
...
C
      IF(IDPARS(2).EQ.IFFF(1))THEN
        IDTMPL=1
      ELSE IF(IDPARS(2).EQ.IFFF(2))THEN
        IDTMPL=2
      ELSE IF(IDPARS(2).EQ.IFFF(3))THEN
        IDTMPL=3
      ELSE IF(IDPARS(2).EQ.IFFF(4))THEN
        IDTMPL=4
      ELSE
        WRITE(KFILDO,105)IDPARS(2)
 105    FORMAT(/' ****THE IDPARS(2) USED IS INCORRECT FOR ',
     1          'THE FREEZING LEVEL.  THE IDPARS ENTERED ',
     2          '= ',I3)
        IER=103
        GOTO 800
      END IF
...
C
      ...
      DO I=1,ILIMJ
C
        IDPARS(7)=ZLVL(I,IDTMPL)
C        CHECK IF AT HIGHEST LEVEL; IF SO, GO TO 
C        CALCULATIONS; IF NOT, CONTINUE.
        IF(IDPARS(7).EQ.300)GOTO 200
C  
C        CREATE ID FOR TEMPERATURE OF FIRST LEVEL
C
        MDT1(1)=ICCCFFF(1)*1000+IDPARS(4)
        MDT1(2)=IDPARS(7)
        MDT1(3)=IDPARS(9)*1000000+IDPARS(12)
        MDT1(4)=0
C
C        FD1 IS A WORK ARRAY DIMENSIONED ND2X3 BUT PASSED
C        FROM GFETCH AS A DATA ARRAY WITH DIMENSION ND2X3.
C
...
C        COMPUTATION OF THE LAPSE RATE
     ...
     1             ZLVL(I+1,IDTMPL))    
C
C        TERM IN THE EQUATION.  THIS TERM WILL BE RAISED TO THE 
C        EXP FUNCTION.
