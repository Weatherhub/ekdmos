      SUBROUTINE SNOWFL(KFILDO,KFIL10,IDPARS,JD,NDATE,
     1                  SDATA,ND1,NSTA,IPACK,IWORK,FDSNOW,FDFLAG,ND2X3,
     2                  LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3                  IS0,IS1,IS2,IS4,ND7,
     4                  ISTAV,L3264B,IER)
C
C        JANUARY   1999   SHIREY   TDL   MOS-2000
C        DECEMBER  2000   RUDACK   MODIFIED CODE TO CONFORM WITH MDL 
C                                  FORMAT SPECIFICATIONS
C        APRIL     2003   GLAHN    CHANGED ND5 TO ND2X3 TO CONFORM
C                                  WITH OTHER ROUTINES
C
C        PURPOSE 
C            THIS SUBROUTINE WILL COMPUTE THE SPECIFIED SNOWFALL AMOUNT
C            (CONTAINED IN IDPARS(2)), FROM VECTOR DATA.  THE AMOUNT 
C            WILL BE CALCULATED FROM 6 HR SNOWFALL AMOUNTS WITH THE 
...
C            THE SNOWFALL DATA HAS BEEN QC'D ELSEWHERE, SO NO ERROR 
C            CHECKING IS DONE ON THIS INPUT VARIABLE.  A TRACE WILL 
C            EQUAL .04.  IF ONE OF THE 6 HR SNOWFALL AMOUNTS IS MISSING,
...
C            WHERE IDPARS(1) = 799, AND RR = (TOTAL SNOWFALL AMOUNT-6). 
C            (FOR EXAMPLE RR=18 FOR 24-HR SNOWFALL CALCULATED TOTAL).
C            ON THE FIRST PROCESS DATE, DATA AT THE TIME (NDATE-RR) MUST
C            BE PRESENT OR THE SUBROUTINE WILL NOT WORK.  FOR EXAMPLE IF
C            YOU ARE CALCULATING 12-HR SNOWFALL TOTAL (RR=6) AND  
C            STARTING AT 5/6/12Z, THEN SNOWFALL DATA FOR 5/6/06Z MUST BE
C            PRESENT.  PLEASE READ WRITE UP FOR MORE DETAILS.
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C               708 402 - 12-HR SNOWFALL AMOUNT
C               708 405 - 24-HR SNOWFALL AMOUNT
C
C        DATA SET USE 
...
C           FDSNOW(J) = WORK ARRAY (J=1,ND2X3) FDSNOW CONTAINS
C                       6-HR PRECIP AMOUNT RETURNED FROM GFETCH.  
C                       (INTERNAL). 
C           FDFLAG(J) = TRACE FLAG (=-10. FOR TRACE, =0. FOR NOT TRACE)
C                       (J=1,ND2X3) (INTERNAL).
C               ND2X3 = USED AS DIMENSION OF IPACK( ),IWORK( ).
C                       ACTUAL DIMENSION OF FDSNOW( ), AND FDFLAG( ).
C                       (INPUT).
...
C                       CORRESPONDING HRLY SNOWFALL AMOUNTS THEY
...
C                       FROM NDATE TO GET THE 6HR SNOW DATA.(INTERNAL).
...
C              NWORDS = NUMBER OF WORDS RETURNED IN FDSNOW( ).
C                       (INTERNAL).
C        1         2         3         4         5         6         7 X
C 
...
C
      ...
      INTEGER IDTBL(2,2),LD(4)
      INTEGER ENDLP,I,IBADHR,IENTER,IER,ISTAV,J,KFIL10,KFILDO,
     1        L3264B,LITEMS,MISSP,MISSS,N,NBLOCK,ND1,ND2X3,ND7,
     2        ND9,ND10,NDATE,NFETCH,NPACK,NSLAB,NSTA,NTIMES,
     3	      NWORDS 
C
      REAL SDATA(ND1)
      REAL FDSNOW(ND2X3),FDFLAG(ND2X3)
      REAL CORE(ND10)
C
...
C
	 ...
	 SDATA(J)=0.
      ...
      END DO
C
C        INITIALIZE IDTABLE WHERE IDTBL(1, )=IDPARS(2) AND
C        IDTBL(2, )=THE CORRESPONING SNOWFALL AMOUNT IN HRS.
C
      IDTBL(1,1)=402
      IDTBL(2,1)=12
      IDTBL(1,2)=405
...
C        IN THIS SUBROUTINE.
C
      IF(IDPARS(1).NE.708.OR.
     1  (IDPARS(2).NE.402.AND.IDPARS(2).NE.405)) THEN
	 IER=103
	 WRITE(KFILDO,100)(JD(J),J=1,4),IER
 100     FORMAT(/' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE',
     1           ' SNOW AMOUNT CALCULATED FROM 6-HR AMOUNTS.'/
     2           '     PREDICTOR ',I9.9,I10.9,I10.9,I4.3,' NOT',
     3           ' ACCOMMODATED IN SNOWFL.  IER=',I3)
         GO TO 900
      ENDIF
C
...
C
 ...
 120     FORMAT(/' ****ERROR, FIRST PROCESS DATE IS ON A',
     ...
     2           '.  SNOWFL CANNOT RUN, PLEASE READ WRITE UP.') 
	 GO TO 910
      ENDIF
C                                                                       
C        IF NDATE DOES NOT END AT 00Z, 06Z, 12Z, OR 18Z,
C        SNOWFALL AMOUNT WILL NOT (CANNOT) BE CALCULATED,  
C        AND THE SUBROUTINE WILL BE EXITED WITH IER= -1.
...
C
 ...
 130     FORMAT(/' ****REMINDER, ONE OR MORE NON-SYNOPTIC',
     1           ' PROCESS DATES HAVE BEEN DETECTED IN SNOWFL.'/ 
     2           '     PLEASE REMEMBER THAT SNOWFL WILL NOT',
     3           ' PRODUCE OUTPUT ON NON-SYNOPTIC PROCESS DATES.')
	 IBADHR=1
	 IER=-1
	 GO TO 910
      ENDIF
...
C        TOTAL AMOUNT OF SNOWFALL.
C
      DO 230 I=1,ENDLP
C
C           CONSTRUCT THE ARRAY LD FOR SNOWFALL AMOUNT
C           AND FETCH THE PARAMETER.  
C
         LD(1)=IDPARS(1)*1000000+401000+IDPARS(4)
         LD(2)=IDPARS(7)
         LD(3)=(IDPARS(9)+N)*1000000+IDPARS(12)
         LD(4)=0
C
         CALL GFETCH(KFILDO,KFIL10,LD,7777,LSTORE,ND9,LITEMS,
     1               IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FDSNOW,ND2X3,
...
C
C***D
C***D        IF(IER.NE.0) THEN
C***D           WRITE(KFILDO,155)IER,NDATE,N
C***D155        FORMAT(/' 6-HR SNOWFALL FETCH FAILED IN PCPX6 AT 155.',
C***D    1              '  IER=',I4,' FOR NDATE=',I12,'AND RR=',I4)  
C***D           GO TO 900
C***D        ENDIF
C***D
...
C
 ...
 160        FORMAT(/' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     1              ' AT 160, NOT EQUAL TO NSTA =',I6,
     2              ' IN SNOWFL.  DATA SET TO MISSING.  IER=',I4) 
	    GO TO 900
         ENDIF
C
C           START THE SNOWFALL CALCULATION PROCESS.
C
         DO 220 J=1,NSTA
C 
C              IF SNOWFALL AMOUNT IS MISSING...DO NOT 
C              EXECUTE...AND SET VALUE TO MISSING.
C
            IF(FDSNOW(J).NE.9999..AND.SDATA(J).NE.9999.)THEN
C
C                 IF NEEDED, SET TRACE FLAG (FDFLAG(J)=-10. FOR TRACE)
C                 AND SKIP CALCULATION.              
C
               IF((FDSNOW(J).LT..05).AND.(FDSNOW(J).GT..03)) THEN
	          FDFLAG(J)=-10. 
C
C                 IF FDSNOW(J) NOT A TRACE THEN ADD AMOUNT TO 
C                 TOTAL.
C
               ELSE
	          SDATA(J)=SDATA(J)+FDSNOW(J)
               ENDIF
            ELSE 
               SDATA(J)=9999.
            ENDIF
...
C
            IF (I.EQ.ENDLP) THEN
	       IF (FDFLAG(J).LT.-1..AND.SDATA(J).LT..02) THEN
	          SDATA(J)=.04
               ENDIF	
            ENDIF
 220     CONTINUE
...
C
      GO TO 910 
C
C        FILL IN THE SNOWFL ARRAY WITH MISSING VALUES
C
 900  DO J=1,ND1
	 SDATA(J)=9999.
      END DO
