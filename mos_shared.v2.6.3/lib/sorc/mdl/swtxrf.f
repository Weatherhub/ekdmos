      SUBROUTINE SWTXRF(KFILDO,KFIL10,IP12,IP16,IDPARS,JD,NDATE,
...
C        SEPT   2010    SHAFER  MODIFIED CODE TO HANDLE MONTHLY RELATIVE
C                               FREQUENCY ID SCHEME 2.  PREVIOUS VERSION
C                               WAS WRITTEN TO HANDLE SCHEME 1.
C        NOV    2010    SHAFER  REARRANGED ARGUMENTS TO BE MORE CONSISTENT
C                               WITH OTHER MOS-2000 ROUTINES.
C
C        PURPOSE
...
C           FREQUENCY (F).  THE SWEAT INDEX IS CALCULATED BY SUBROUTINE
C           SWEATI.F. F IS OBTAINED BY LINEAR INTERPOLATION BETWEEN THE
C           MONTHLY CLIMATOLOGICAL VALUES OF SEVERE THUNDERSTORM FREQUENCY
C           STORED ON DISK.  THE SWEAT INDEX AND SEVERE THUNDERSTORM
C           FREQUENCY INPUT FILES ARE STORED ON DISK IN TDLPACK
C           FORMAT.  THE FREQUENCY FILES ARE IN THE FORM OF RANDOM-ACCESS
C           (CONSTANT) FILES ON DISK.  THE CONSTANT FILES ARE CREATED BY
C           U350 AND U351. SWTXRF VALUES ARE RETURNED TO THE CALLING PROGRAM
C           (OPTION) IN ARRAY SDATA.  PRIMARY USE OF THE SWTXRF PREDICTOR IS
C           TO PREDICT SEVERE THUNDERSTORM ACTIVITY.
C            
C        DATA SET USE
...
C        007361 = SWEATI * PREVIOUS 3 HR SEVERE WX
C        007366 = SWEATI * SUBSEQUENT 3 HR SEVERE WX
C        007371 = SWEATI * PREVIOUS 6 HR SEVERE WX
C        007376 = SWEATI * SUBSEQUENT 6 HR SEVERE WX
C        007381 = SWEATI * PREVIOUS 12 HR SEVERE WX
C        007386 = SWEATI * SUBSEQUENT 12 HR SEVERE WX
C        007391 = SWEATI * PREVIOUS 24 HR SEVERE WX
C
      IF((IDPARS(1).NE.007).OR.((IDPARS(2).NE.361).AND.
     1 (IDPARS(2).NE.366).AND.(IDPARS(2).NE.371).AND.
     2 (IDPARS(2).NE.376).AND.(IDPARS(2).NE.381).AND.
     3 (IDPARS(2).NE.386).AND.(IDPARS(2).NE.391))) THEN
      WRITE(12,101)(JD(J),J=1,4)
 101  FORMAT(/,'****IDPARS(1) AND IDPARS(2) DO NOT INDICATE SWTXRF',
     1 ' PREDICTOR. ',I9.9,2I10.9,I4.3,' NOT COMPUTED IN SWTXRF. ')
      ENDIF
C
...
C                CONSTANT FILE.  THE IDS USED ARE FOR
C                RELATIVE FREQUENCY SCHEME 2.
C
C     LD(1)=437352000 IF ID IS 007361000, CSVRWX FOR THE PREVIOUS 3 HOURS
C     LD(1)=437352000 IF ID IS 007366000, CSVRWX FOR THE SUBSEQUENT 3 HOURS
C     LD(1)=437353000 IF ID IS 007371000, CSVRWX FOR THE PREVIOUS 6 HOURS
C     LD(1)=437353000 IF ID IS 007376000, CSVRWX FOR THE SUBSEQUENT 6 HOURS
C     LD(1)=437354000 IF ID IS 007381000, CSVRWX FOR THE PREVIOUS 12 HOURS
C     LD(1)=437354000 IF ID IS 007386000, CSVRWX FOR THE SUBSEQUENT 12 HOURS
C     LD(1)=437355000 IF ID IS 007391000, CSVRWX FOR THE PREVIOUS 24 HOURS
C
C
