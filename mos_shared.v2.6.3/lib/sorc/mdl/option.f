...
C       JUNE       2007    JCM    ADDED 003301 TO DEWPT, ADDED ROUTINE 
C                                 MODELKTOF FOR AK GRIDDED MOS FIRST GUESS
C       AUG        2007    RLC    ADDED ID TO CALL TO GRDCNST FOR 5-DAY NORMALS
C       SEPT       2007    RLC    ADDED ANOTHER ID TO CALL TO MODELFTOK
C       DECEMBER   2007    RLC    ADDED 12-HR MAX/MIN IDS IN F TO CALL TO MODELMXMN
C       APRIL      2008    SMB    ADDED OPERATIONAL CODE TIMTRPGOE
C       OCT        2008    SMB    ADDED NEW VERSION OF SWTXRF
C       OCT        2009    SMB    MODIFIED CALL TO GRDCNST FOR GRIDDED
C                                 PRECIP TYPE RELATIVE FREQUENCIES, 50%
C                                 LOGIT VALUES, AND SNOWFALL CLIMATOLOGY.  
C                                 ADDED CALL TO LOGITTRANS AND PCPEFF.
C       NOV        2010    MNB    MODIFIED CALL TO SWTXRF.
C       DEC        2011    SDS    ADDED CALL TO KINXRF2 AND REQUIRED BYPASS
C                                 FOR TIMTRP CALL.
C       JUNE       2012    PES    MODIFIED CALL TO KINXRF2 AND REQUIRED
C                                 BYPASS FOR TIMTRP TO ACCOMMODATE 2-H TSTMS.
C       JUNE       2012    SDS    ADDED GRIDDED POPO/POPO3 IDS BEFORE 
C                                 CALL TO GRDCNST.
C       AUG        2013    SDS    ADDED OBSQCLD.
C       SEP        2013    PS     MODIFIED CALL TO GRDCNST FOR GRIDDED
C                                 POPO/POPO3 RELATIVE FREQUENCIES.
C       SEPTEMBER  2014    SDS    CHANGED ENTRY TO TIMTRPGOE TO BE AFTER 240.
C       OCTOBER    2014    SDS    ADDED CALL TO TGINTRP
C       MARCH      2015    SDS    MERGED WITH ECMWF-MOS OPTION, TO ALLOW ECMOS 
C                                 IDS TO CALL FRZLVL, ZRPRED, SATLEVNUM, 
C                                 SATLEVRH AND LOGITTRANS.
C       MARCH      2016    SDS    MODIFIED CALL TO TIMTRPGOE TO AVOID 
C                                 ENTERING FOR LATE PROJECTION FORIER IDS.
C
C        PURPOSE 
...
C            FTOKGRD, CKBOUNDS, CMPDPGR, CMPRHGR, OBSGUST, GUSTRAT,
C            GRDCNST, MODELKTOF, TIMTRPGOE, LOGITTRANS, PCPEFF, KINXRF2,
C            OBSQCLD, TGINTRP
C
...
C
C        TIME INTERPOLATE CERTAIN FORECASTS TO HOURLY VALUES.
C        TIMTRPGOE RETURNS VECTOR DATA OR GRIDPONT DATA, DEPENDING
C        ON ISTAV.  THIS WORKS FOR 12-HR FIELDS. (FOR THE GFS
C        THAT MEANS BEYOND 240 HRS).  ONLY OPERATE ON MODEL FIELDS
C        (CCC=0xx).
C
      ELSEIF(IDPARS(1).LT.100.AND.
     1      (IDPARS(2).LT.201.OR.IDPARS(2).GT.208).AND.
     2       IDPARS(12).GT.240.AND.MOD(IDPARS(12),12).NE.0)THEN
         CALL TIMTRPGOE(KFILDO,KFIL10,NFIRST,
     1               ID,IDPARS,THRESH,JD,NDATE,
     2               KFILRA,RACESS,NUMRA,
     3               ICALL,CCALL,ICALLD,CCALLD,NAME,
     4               NELEV,STALAT,STALON,
...
C        (5) GRIDDED POPO/POPO3 RELATIVE FREQUENCIES
C
     ...
     6                               (IDPARS(2).EQ.540).OR.
     7                               (IDPARS(2).EQ.541))).OR.
     8      ((IDPARS(1).EQ.438).AND.(IDPARS(2).EQ.545)).OR.
     9      ((IDPARS(1).EQ.461).AND.((IDPARS(2).EQ.541).OR.
     A                               (IDPARS(2).EQ.542))).OR.
     B      ((IDPARS(1).EQ.462).AND.((IDPARS(2).EQ.541).OR.
     C                               (IDPARS(2).EQ.542).OR.
     D                               (IDPARS(2).EQ.543))))THEN
...
C
     ...
     3                             IDPARS(2).EQ.049.OR.
     4                             IDPARS(2).EQ.050)) THEN
...
C
     ...
     3                               (IDPARS(2).EQ.156).OR.
     4                               (IDPARS(2).EQ.157))) THEN
...
C
     ...
     3                               (IDPARS(2).EQ.314).OR.
     4                               (IDPARS(2).EQ.312).OR.
     5                               (IDPARS(2).EQ.322).OR.
     6                               (IDPARS(2).EQ.332).OR.
     7                               (IDPARS(2).EQ.324).OR.
     8                               (IDPARS(2).EQ.313).OR.
     9                               (IDPARS(2).EQ.323).OR.
     A                               (IDPARS(2).EQ.333).OR.
     B                               (IDPARS(2).EQ.334)))THEN
...
C
     ...
     8                               (IDPARS(2).EQ.363).OR.
     9                               (IDPARS(2).EQ.344).OR.
     A                               (IDPARS(2).EQ.354).OR.
     B                               (IDPARS(2).EQ.364)))THEN
...
C
     ...
     4                               (IDPARS(2).EQ.040).OR.
     5                               (IDPARS(2).EQ.047).OR.
     6                               (IDPARS(2).EQ.050))))THEN
