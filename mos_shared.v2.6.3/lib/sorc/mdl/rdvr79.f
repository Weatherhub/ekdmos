      SUBROUTINE RDVR79(KFILDO,IP6,IP7,IP8,IP9,KFILP,KFILCP,
     1                  ID,IDPARS,TRESHL,TRESHU,JD,JP,PLAIN,
     2                  ITAU,IWDTH,IPREC,CFMT,ISCALD,HEAD,NCAT,ND4,
     3                  NVRBL,ISTOP,IER)
C 
C        JUNE    1997   GLAHN   TDL   MOS-2000 
C        OCTOBER 1999   GLAHN   ADDED CHECK FOR PROBABILITY FRACTILES
C        APRIL   2000   DALLAVALLE   MODIFIED FORMAT STATEMENTS TO
C                               CONFORM TO FORTRAN 90 STANDARDS
C                               ON THE IBM SP
C        JUNE    2000   GLAHN   ADDED PRINT OF NCAT AT STATEMENT 182
C        NOV.    2000   DALLAVALLE   ADDED DAVE RUDACK'S CORRECTED
C                               IF-TEST TO HANDLE PROBABILITY
C                               FRACTILES FOR QPF.  AT THIS TIME,
C                               NO FRACTILES ARE BEING CALCULATED.
C                               WHEN THE CALCULATION IS REQUIRED,
C                               ADDITIONAL CHECK-OUT WILL BE NEEDED.
C
C        PURPOSE 
C            TO READ FOR U710 THROUGH INT710 A VARIABLE LIST AND 
...
C            USED FOR U710 AND ALSO U910; A SIMILIAR ONE RDPRED IS
C            USED FOR U201 AND RDVRBL FOR U600.
C
C        DATA SET USE 
...
C                       THAT, SINCE THE VARIABLES ARE NOT SORTED BY U710
...
C                       CAN BE MADE DIRECTLY IN U710, WHICH IS ONLY FORMING
C                       BINARIES.  THE "G" VARIABLE HAS NO MEANING IN U710,
...
C
 ...
 1030 FORMAT(/,' VARIABLES AS READ BY RDVR79')
...
C
      IF(NERR.GE.100)THEN
         WRITE(KFILDO,110)
 110     FORMAT(/,' ****READING ERRORS IN RDVR79 HALTED PROGRAM',
     1            ' AT 110.')
         STOP 110
      ENDIF
C
...
C
 ...
 122  FORMAT(/,' ****ND4 = ',I4,' TOO SMALL IN RDVR79.')
      GO TO 180
C
...
C        COPIED FROM A U201 CONTROL FILE TO A U710 CONTROL FILE,
C        THE USER MIGHT FORGET TO REMOVE THE MODEL NUMBER.
C********THIS IS PROBABLY NO LONGER DESIRABLE.  11/14/99 
...
C        OUTPUT (E.G., U710), JD( , ) = ID( , ) EXCEPT IN ID(1, ) 
...
D151  FORMAT(/,' AT 151 IN RDVR79--NCAT',10I4)
C
      NCAT(1)=1
      M=0
C
...
D155  FORMAT(' AT 155 IN RDVR79--ID( ,N),ID( ,N-1)',4I12,/,
D    1       '                                    ',4I12,/,
D    2       '                                    '15I4,/,
D    3       '                                    '15I4)
...
C              FRACTILES AND NEED TO BE PART OF A SEQUENCE
D           WRITE(KFILDO,156)N,M
D156        FORMAT(/,' AT 156 IN RDVR79--N,M',2I4)
C
...
D157              FORMAT(/,' AT 157 IN RDVR79--N,M',2I4)
C
                  IF(M.EQ.0)THEN
                     M=N-1
                  ENDIF
...
D158              FORMAT(/,' AT 158 IN RDVR79--N,M',2I4)
                  M=0
                  NCAT(N)=1
            ENDIF
C
...
D161  FORMAT(/,' AT 161 IN RDVR79--NCAT',10I4)
C
...
C
 ...
 231  FORMAT(/,' ',I4,' VARIABLES AND INFORMATION FROM VARIABLE',
     ...
     2            TRESHL(N),TRESHU(N),ITAU(N),NCAT(N),N=1,NVRBL)
 235  FORMAT('  NO.     ID(1)     ID(2)     ID(3)      ID(4)',
     1       '       PLAIN LANGUAGE ID           ISCALD  JP( )',
     2       '    L--THRESHOLDS--U    ITAU NCAT',/,
     3      (' ',I4,1X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,2X,
     4       A32,I7,1X,3I2,F11.3,F11.3,I5,I5))
 250  RETURN
      END 
