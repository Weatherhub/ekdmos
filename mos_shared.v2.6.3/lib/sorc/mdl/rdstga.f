      SUBROUTINE RDSTGA(KFILDO,IP4,IP5,KFILD,NEW,CCALL,CCALLD,
     1                  NAME,NELEV,IWBAN,STALAT,STALON,IFOUND,
     2                  NGP,NGPRUN,ND1,KGP,NSTA,IER)
C 
...
C        NOVEMBER  2001   GLAHN   CHANGED IP4 TO IP5 ABOVE 1412
...
C            LIST IN ALPHABETICAL ORDER BY GROUP.  THE STATION LIST
...
C            THE DIRECTORY.  THE CALL TO RDC IS SET TO READ UP TO NT = 7
C            VALUES PER RECORD ACCORDING TO THE FORMAT (7(A8,1X));
C            HOWEVER, THE RECORD NEED NOT HAVE THAT MANY VALUES.
C            THE STATIONS IN THE LIST RETURNED IN CCALL( ,1) WILL
C            BE IN ALPHABETICAL ORDER BY GROUP PROVIDED THE
...
C            SINCE THE ENTRY IN THE DIRECTORY CCALLT(1) IS SEARCH FOR
C            ALL STATIONS BEFORE CCALLT(2), ETC., THE DUPLICATE
C            STATIONS MAY NOT BE PRINTED IN THE ORDER IN THE INPUT
C            LIST IN CCALLD( ).
C
C            NOTE:  THE DIFFERENCE BETWEEN THIS RDSTGA AND RDSTGN IS 
C                   THAT RDSTGA ALPHABETIZES STATIONS, WHILE RDSTGN
C                   DOES NOT.  RDSTAD IS USED FOR A SINGLE STATION
C                   LIST; RDSTGA IS USED FOR MULTIPLE LISTS.
C
C        DATA SET USE 
...
C
      ...
      DATA NAMET/' '/
C
D     CALL TIMPR(KFILDO,KFILDO,'START RDSTGA        ')
C
      IER=0
      KGP=1
      NSTA=0
...
C
         IF(KGP.EQ.1)THEN
            WRITE(KFILDO,110)NSTA1,KGP,(CCALLD(K),K=NSTA+1,NSTA+NSTA1)
 110        FORMAT(/,' ',I5,' STATIONS INPUT FOR GROUP NO.',I5,1X,
     1             10(1X,A8),/,(14X,13(1X,A8)))
         ELSE
            WRITE(KFILDO,111)NSTA1,KGP,(CCALLD(K),K=NSTA+1,NSTA+NSTA1)
...
C
      ...
      ELSE
         WRITE(IP4,111)NSTA1,KGP,(CCALLD(K),K=NSTA+1,NSTA+NSTA1)
      ENDIF
C
 115  IF(KGP.LE.ND1)GO TO 117
      WRITE(KFILDO,116)ND1
 116  FORMAT(/,' ****TOO MANY GROUPS IN RDSTGA FOR DIMENSION ND1 =',I4)
      IER=34
      KGP=KGP-1
      GO TO 160
C
...
C
 ...
 124  FORMAT(/,' ****ERROR READING STATION DIRECTORY IN RDSTGA.',
     1         '  IOSTAT =',I5)
      IER=33
      GO TO 1405
C
...
C
 ...
 158  CONTINUE
C
D     CALL TIMPR(KFILDO,KFILDO,'END RDSTGA          ')
C
 160  RETURN
      END
