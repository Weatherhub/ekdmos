      SUBROUTINE LMSTR8(KFILDO,NDATE,NRRDAT,LSTORE,LITEMS,
...
C        SEPTEMBER 2000   GLAHN   SHORTENED COMMENTS TO 72 COLUMNS
C        JANUARY   2001   GLAHN   ADDED COMMENT BETWEEN  1141 AND 1143
C        JUNE      2001   GLAHN   ADAPTED FROM LMSTR1 TO WORK FOR
C                                 UNORDERED VARIABLES
C        AUGUST    2001   RUCACK  ADDED TEST ON LSTORE(8,L) AS PART OF
C                                 TEST BEFORE ADDING 7777 JUST ABOVE 2ND
C                                 CHANGE TO LMSTR1
C        DECEMBER  2002   GLAHN   MODIFIED FORMATS 204 AND 1881
C
C        PURPOSE
C           TO INITIALIZE MSTORE AND PREPARE LSTORE FOR GCPAC FOR U203
C           AND U860.  ADAPTED FROM LMSTR1 (WRITTEN FOR ORDERED
C           INPUT VARIABLES) TO WORK FOR UNORDERED VARIABLES.  THERE
C           ARE ONLY TWO CHANGES MARKED BY C***********.
C   
C        DATA SET USE
...
C                               MSTORE(6, )
C                       NOTE THAT MSTORE IN U201 AND LMSTR1 IS NOT
...
C                       USED IN LMSTR8 TO ASSURE THAT DATA ARE SAVED FOR
C                       CYCLES AT THIS FREQUENCY.
C                 IER = STATUS RETURN.  (OUTPUT)
C                        0 = GOOD RETURN.
C                       70 = DID NOT COMPUTE MAXIMUM TIME OFFSET.
C              MINDAT = MINIMUM DATE FOR WHICH THIS VARIABLE MAY BE
C                       NEEDED.  (INTERNAL)
...
D100  FORMAT(/' LSTORE STARTING LMSTR8'/('  '3I10,I11,3I8,I12,3I8,I12))
C
...
D109  FORMAT(' LSTORE AT 109 IN LMSTR8, J,L,LUSE ='3I6/
...
C***********************
C              THE ADDITION OF 7777 TO THE ABOVE STATEMENT IS
C              ONE OF TWO DIFFERENCES BETWEEN LMSTR1 AND LMSTR8               
C***********************
            ELSE
               MSTORE(5,M)=MST5+7777
            ENDIF
...
C
 ...
 1140    FORMAT(/,' ****NO SLOTS AVAILABLE IN MSTORE( , ).',
     ...
     4         (' ',4I11,I6,I11,I6))
         WRITE(KFILDO,1141)
 1141    FORMAT(' ****STOP IN LMSTR8 AT 1141.')
         STOP 1141
      ENDIF
C
...
C
         IF(LSTORE(8,L).EQ.NDATE.AND.LSTORE(8,L).LT.7777)THEN
            MSTORE(5,MITEMS)=LSTORE(11,L)+7777
C***********************
C              THE ADDITION OF 7777 TO THE ABOVE TWO STATEMENTS IS
C              ONE OF TWO DIFFERENCES BETWEEN LMSTR1 AND LMSTR8               
C***********************
         ELSE
...
c
            ...
            ELSE
               MSTORE(5,MITEMS)=LSTORE(11,L)+7777
C                 BECAUSE OF HAVING ITEMS IN MSTORE( , ) WHEN
C                 ENTERED, WHICH ARE ITEMS THAT COULD NOT BE 
C                 FOUND OR COMPUTED FOR DAY 1, MORE
C                 VARIABLES HAVE TO BE STORED IN LSTORE( , )
C                 IN CASE THEY ARE NEEDED ON SUBSEQUENT DAYS.
            ENDIF
C
         ELSE
            MSTORE(5,MITEMS)=7777
...
C              ANY TIME THERE IS A TIME OFFSET, THE VARIABLE MUST BE
C              STORED.
            GO TO 180
         ENDIF
 160     CONTINUE
...
C
      WRITE(KFILDO,179)
 179  FORMAT(' ****DID NOT COMPUTE MAXIMUM TIME OFFSET IN LMSTR8')
      IER=70
 180  CONTINUE
C
...
C
 ...
 1880    FORMAT(/,' ****NO SLOTS AVAILABLE IN MSTORE( , ).',
     ...
     4         (' ',4I11,I6,I11,I6))
         WRITE(KFILDO,1881)
 1881    FORMAT(' ****STOP IN LMSTR8 AT 1881.')
         STOP 1881
      ENDIF
C
...
C           THERE IS A MATCH.
D     WRITE(KFILDO,204)M,L,(LSTORE(J,L),J=1,4),MSTORE(6,M),
D    1                 LSTORE(8,L),MSTORE(7,M),LSTORE(12,L)
C
...
D     WRITE(KFILDO,204)M,L,(LSTORE(J,L),J=1,4),MSTORE(6,M),
D    1                 LSTORE(8,L),MSTORE(7,M),LSTORE(12,L)
D204  FORMAT(' LMSTR8 AT 204'2I5,8I12)
C
 205  CONTINUE
C
 210  CONTINUE
...
D220  FORMAT(/' LSTORE ENDING LMSTR8'/('  '3I10,I11,3I8,I12,3I8,I12))
 230  RETURN
      END
