      SUBROUTINE FTOKGRD(KFILDO,KFIL10,
...
C
C        PURPOSE
C            TO CHANGE UNITS FROM FAHRENHEIT TO KELVIN FOR GRIDDED DATA.
C            CONVERSION USED:  K=(F-32)*5/9 + 273.15.
...
C        05-07       COSGROVE   ADDED ID FOR DEWPOINT COMPUTED FROM T AND RH.
C
C        DATA SET USE
...
C                       (INPUT)
C             DATA( ) = THE GRIDDED TEMPERATURE OF DEWPOINT FIELDS IN
...
C               RATIO = VARIABLE TO HOLD THE RATIO 5/9 USED IN THE CONVERSION
C                       FROM F TO K. (INTERNAL)
...
C
      ...
      RATIO=5./9.
C
C        MAKE SURE THE REQUESTED VARIABLE IS A TEMPERATURE OR DEWPOINT GRIDDED
C        FIELD IN DEGREES K.
C
...
C
 ...
 110  FORMAT(/,' ****FTOKGRD ENTERED FOR VARIABLE',
...
C
      IF(IER.NE.0) GOTO 800
         ...
         NY=IS2(4)
C
C        CONVERT THE TEMPERATURE FROM DEG F TO DEG K.
C
 300   DO 350 J=1,NX*NY
           IF(NINT(DATA(J)).NE.9999) THEN
              DATA(J)=((DATA(J)-32.0)*RATIO)+273.15
           ENDIF
 350   CONTINUE
       GO TO 900
C
