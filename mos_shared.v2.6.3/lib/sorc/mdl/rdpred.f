...
C        JULY      2003   GLAHN   ADDED NPRED = N-1 AT 122
C        SEPTEMBER 2003   GLAHN   ADDED CCC = 799 DIAGNOSTICS AT 1295
C        OCTOBER   2003   SMB     CORRECTED FORMAT STATEMENTS 103,   
C                                 1031, 107, 122, 1296, 181, 182, 191,
C                                 230, AND 235 FOR THE IBM
C        OCTOBER   2012   GHIRARDELLI  MODIFIED FORMAT STATEMENT
...
C
 ...
 1030 FORMAT(/' VARIABLES AS READ BY RDPRED')
     ...
     1            TEMP,ITEMP(5),ITEMP(6),(JTEMP(J),J=1,3)
 1031 FORMAT(' ',I9.9,1X,I9.9,1X,I9.9,1X,I3.3,1X,A1,1X,
     1       I4,1X,I3,4X,3I2)
      GO TO 120
C
 104  NERR=NERR+1
...
C
 ...
 122  FORMAT(/' ****ND4 = ',I4,' TOO SMALL IN RDPRED.')
      NPRED=N-1
C        PROGRAM PROCEEDS WITH ND4 PREDICTORS.
      GO TO 180
C
...
C
 129  CONTINUE
C
      IF(IDPARS(1,N).EQ.799)THEN
C
         IF(IDPARS(4,N).EQ.0)THEN
            WRITE(KFILDO,1295)IDPARS(9,N)
 1295       FORMAT(/' YOU ARE USING A CCC = 799 VARIABLE FOR VECTOR',
     1              ' DATA WITH RR =',I3,'.  IS IT NECESSARY?',
     2              '  IT CAN CAUSE INCREASED CP AND CLOCK TIME.')
         ELSE
            WRITE(KFILDO,1296)IDPARS(4,N),IDPARS(9,N)
 1296       FORMAT(/' YOU ARE USING A CCC = 799 VARIABLE FOR MODEL NO.',
     1              I3,' WITH RR =',I3,'.  IS IT NECESSARY?',
     2              '  IT CAN CAUSE INCREASED CP AND CLOCK TIME.')
         ENDIF
C
      ENDIF
...
C
      DO 185 N=1,NPRED
      JD(4,N)=IDPARS(14,N)*100+
     1         IDPARS(3,N)*10+
     2         IDPARS(8,N)
 185  CONTINUE
C
      CALL SORTEM(KFILDO,JD,ISCALD,THRESH,NPRED)
C
...
C
      CALL XCHANG(ID,     4,ISCALD,SMULT,NPRED)
      CALL XCHANG(IDPARS,15,ISCALD,SMULT,NPRED)
      CALL XCHANG(JP,     3,ISCALD,SMULT,NPRED)
C
...
C
 ...
 187  CONTINUE
C      
      IF(IP8.NE.0)WRITE(IP8,191)NPRED
 191  FORMAT(/' ',I4,' VARIABLES AND PARSED IDS',
     1        ' (ORDERED BY BASIC VARIABLE',
     2        '--SANS PROCESSING PARAMETERS)')
      IF(IP8.NE.0)WRITE(IP8,182)(N,(ID(J,N),J=1,4),
     1                          (IDPARS(J,N),J=1,15),
     2                          THRESH(N),THRESH(N),
     3                          (JP(J,N),J=1,3),N=1,NPRED)
C
