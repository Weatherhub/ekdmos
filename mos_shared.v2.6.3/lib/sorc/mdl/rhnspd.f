...
C                       TO CONSTRUCT A PREDICTOR OF COMBINING RELATIVE HUMIDITY AND
C                       WIND SPEED USED IN FORECASTING VISIBILITY
...
C        AUGUST 2013    RUDACK - MODIFIED CODE TO ACCOMMODATE THE VERTICAL LEVELS 
C                       ASSOCIATED WITH THE ECMWF MODEL.
C              
C        PURPOSE
C            TO COMPUTE FOG POTENTIAL.  THIS IS USED IN THE REGRESSION
...
C                       LOWER LAYER (J=1,ND2X3). (OUTPUT)
...
C
     ...
     4        NSLABD,NSLABT,NSLABH,NSLABS,NTIMES,NWORDS
      ...
      REAL CORE(ND10)
      REAL FFOG(ND2X3),UWIND(ND2X3),VWIND(ND2X3)
      REAL FD1(ND2X3),FD2(ND2X3)
      REAL FD3(ND2X3),FD4(ND2X3),FD5(ND2X3),FD6(ND2X3)
      REAL FD7(ND2X3),FDRV(ND2X3)
C        10M GRIDDED WIND SPEED ID
      ICCCFFF(2)=004211
C
C        CHECK IF ECMWF MODEL IS BEING PROCESSED.  IF SO,
C        USE 925 MB.  AS OF AUGUST 2013 MDL DOES NOT RECEIVE
C        MODEL FIELDS AT 970 MB.
C
      IF((IDPARS(4).EQ.1).OR.(IDPARS(4).EQ.6)) THEN 
C           BOUNDARY LAYER RELATIVE HUMIDITY ID (ECM)
         ICCCFFF(1)=003041
         IUUUU(1)=925
      ELSE
C           BOUNDARY LAYER RELATIVE HUMIDITY ID (NON-ECM)
         ICCCFFF(1)=003007
         IUUUU(1)=970
      ENDIF
C
...
C
 ...
 101     FORMAT(/' ****IDPARS(1) AND IDPARS(2) DO NOT INDICATE ',
     ...
     2           I4.3, 'THEREFORE, NOT COMPUTED IN RHNSPD. ')
      ...
      END IF
C
      IF((IDPARS(4).NE.1).AND.(IDPARS(4).NE.6)) THEN
C           NOT USING THE ECMWF MODEL
C
C           CONSTRUCT ID FOR THE MEAN PBL (30 MB) RH (00300700X 000000970
C           000000000)
C
...
C 
      ELSE
C           PROCESSING THE ECMWF MODEL
C
C           MODIFIED TO USE THE 1000-925 MB MEAN RH.  THE ECMWF MODEL DOES NOT HAVE
C           A MEAN 970 MB RH.
...
C
         ...
         CALL MEANRH(KFILDO,KFIL10,LDPARS,LD,NDATE,
     1               NGRIDC,ND11,NSLAB,IPACK,IWORK,FD1,ND5,
     2               LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3               IS0,IS1,IS2,IS4,ND7,
     4               FD2,FD3,FD4,FD5,FD6,FD7,FDRV,ND2X3,
     5               ISTAV,L3264B,MISTOT,IER)
C
         IF(MISSP.NE.0)MISTOT=MISTOT+1
         IF(IER.NE.0)GOTO 800
