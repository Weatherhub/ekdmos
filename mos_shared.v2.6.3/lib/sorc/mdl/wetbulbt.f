...
C                               AND SPECHUM.
C        APRIL   2013   SHAFER  ADDED CALL TO GFETCH TO CHECK IF DEWPOINT
C                               IS AVAILABLE IN STORAGE BEFORE COMPUTING IT
C                               IN DEWPT.  THIS WAS NECESSARY TO ALLOW 2-M
C                               WET BULB TO BE COMPUTED FOR THE ECMWF.
C        MAY     2013   SHAFER  FOR 2-M WET BULB ONLY - IF SFC PRESSURE
C                               OR B.L. PRESSURE CANNOT BE FETCHED THEN
C                               WET BULB IS APPROXIMATED AS THE MIDPOINT
C                               BETWEEN T AND TD.    
C
C
C        PURPOSE 
...
C      
C        FIRST ATTEMPT TO FETCH DEWPOINT FROM MOS-2000 STORAGE.
C        GFETCH WILL RETURN IER = 47 IF THE DATA CANNOT BE FOUND.
C        IF FOUND DEWPOINT IS RETURNED IN ARRAY FD1( ).
C
...
C
      IF(IER.EQ.0) GOTO 250
      IF(IER.EQ.47) GOTO 150
C
      WRITE(KFILDO,FMT='(/1X,''****ERROR IN GFETCH (IER='',I3,
     1      '') WHEN ATTEMPTING TO FETCH DEWPOINT '', 
     2      I9.9,I10.9,I10.9,I4.3)') IER, (LD(J),J=1,4)
      GOTO 900
C
C        IF DEWPOINT CANNOT BE FETCHED FROM STORAGE:
C        CALL DEWPT TO OBTAIN DEW POINT TEMPERATURE IN FD1( ).
C
 150  CALL DEWPT(KFILDO,KFIL10,LDPARS,LD,NDATE,
...
C
 ...
 200     FORMAT(/' ****ATTEMPT TO OBTAIN DEW POINT TEMPERATURE FAILED ',
     +           'IN WETBULBT.  IER =',I4)
         GOTO 900
      ENDIF
C
...
C
 ...
 300     FORMAT(/,' ****TEMPERATURE FETCH FAILED IN WETBULBT. IER =',I4)
         GOTO 900
      ENDIF
C
      MX=IS2(3)
...
C
              ELSE
C
C              IF THE 0-30MB BL PRESSURE IS NOT AVAILABLE THEN AS 
C              A LAST RESORT WE WILL APPROXIMATE THE 2-M WET BULB
C              AS THE MIDPOINT BETWEEN THE TEMPERATURE AND DEWPOINT.
C              VALUE FOR IER IS SET TO 0 FOR RETURN.
C

c                 write(*,*) 'Twet set to midpoint ',IDPARS(12)

                 WRITE(KFILDO,500) NDATE
 500             FORMAT(/' ****SURFACE PRESSURE OR B.L. PRESSURE ',
     +                   'COULD NOT BE FETCHED IN WETBULBT ',
     +                   'FOR DATE ',I10,
     +                  /' ****2-M WET BULB TEMPERATURE IS SET TO ',
     +                   'MIDPOINT BETWEEN T AND TD.')
C
                 DO I=1,LX*LY
...
C
      END IF
C
C           COMPUTE WET BULB FROM DEW POINT TEMPERATURE, TEMPERATURE,
