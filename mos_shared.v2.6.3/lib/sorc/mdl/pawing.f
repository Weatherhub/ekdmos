      SUBROUTINE PAWING(KFILDO,KFIL10,NDATE,
...
C        NOVEMBER  2000   GLAHN   REMOVED INEFFECTIVE TEST ON IER;
C                                 ADDED MESHB TO CALL
C        NOVEMBER  2000   GLAHN   ADDED TEST FOR ND5 GE NX*NY AT 105
C        FEBRUARY  2001   GLAHN   INSERTED GRIDPOINT IN PURPOSE
C        OCTOBER   2001   GLAHN   ADDED DIAGNOSTIC FOR UNPKBG ERROR
C        JANUARY   2002   GLAHN   REMOVED COMMENT ABOUT NSLAB IN GSTORE
C        DECEMBER  2002   RUDACK  MODIFIED FORMAT STATEMENTS TO ADHERE
C                                 TO THE F90 COMPILER STANDARDS FOUND ON 
C                                 THE IBM SYSTEM
C        DECEMBER  2002   GLAHN   ADDED /D COMMENT AFTER GSTORE
C        JUNE      2004   GLAHN   ADDED TEST ON KFIL10 = 0 TO NOT WRITE
...
C                       IT IS OUTPUT IN THE ARGUMENT LIST OF PAWING IN
...
C
      IER=0
D     CALL TIMPR(KFILDO,KFILDO,'START PAWING        ')
D     WRITE(KFILDO,100)(ID(J),J=1,4),MESH,ALAT,ALON,ORIENT,XLAT,NX,NY
D100  FORMAT(/' IN PAWING--ID(J),MESH,ALAT,ALON,ORIENT,XLAT,NX,NY'/
...
C
 ...
 105     FORMAT(/' ****ND5 = ',I8,'  MUST BE GE NX*NY =',I8,
     1           ' IN PAWING.  DATA NOT PACKED AND WRITTEN.'/
...
C        IS2(7) IS THE GRID ORIENTATION*10000.
      CALL MSHXMS(KFILDO,MESH,XMESHN,XMESH)
...
C        BE WRITTEN.  THIS ALLOWS PAWING TO BE USED FOR PROGRAMS
...
D150  FORMAT(' IN PAWING--(IS1(J),J=9,12),NSTORE,IOCTET,MINPK,ISCALD',
D    1       4I11,4I6)
...
C***D     CALL TIMPR(KFILDO,KFILDO,'END   PAWING        ')
D     WRITE(KFILDO,151)(IS2(J),J=1,20)
D151        FORMAT(' IN PAWING--(IS2(J),J=1,20)--IS2(J),J=1,20)',
D    1              /(10I10))
 900  RETURN
C        
