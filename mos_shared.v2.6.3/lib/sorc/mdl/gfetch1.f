      SUBROUTINE GFETCH1(KFILDO,KFIL,ID,LOCPRD,LSTORE,ND9,LITEMS,
...
C
C            THE DIFFERENCE BETWEEN THIS GFETCH1 AND GFETCH IS THAT
C            GFETCH USES ONLY ID( ) TO IDENTIFY THE RECORD WANTED,
C            WHILE GETCH1 USES ID( ) AND NSLAB.  NSLAB IS INPUT
C            TO GFETCH1 AND OUTPUT TO GFETCH.
C
C        DATA SET USE 
...
C                KFIL = UNIT NUMBER FOR READING THE DATA INTO DATA( )
C                       FROM DISK.  (INPUT)
C               ID(J) = THE 4-WORD ID OF THE VARAIBLE TO FETCH
C                       (J=1,4).  (INPUT)
C              LOCPRD = THE LOCATION OF THE PREDICTOR ID( ) IN THE
C                       PREDICTOR LIST WHEN GFETCH1 IS ACCESSED FROM PRED1.
C                       WHEN GFETCH1 IS ACCESSED FROM OPTION OR
...
C                              UPON RETRIEVAL BY GFETCH1.
C                       L=10 --NSLAB, THE NUMBER OF THE SLAB IN DIR( , ,L)
C                              DEFINING THE DIRECTORY FOR THIS INPUT UNIT
C                              NUMBER.
...
C                       IS USED ONLY WITHIN GSTORE AND GFETCH1.  (INPUT)
...
C              NFETCH = INCREMENTED EACH TIME GFETCH1 IS ENTERED.  IT IS
...
C               NSLAB = GFETCH1 IS USED IN U662.  NSLAB IS THE UNIT NUMBER
C                       OF THE SOURCE OF THE DATA, AND IS USED AS ONE
C                       ELEMENT OF SEARCH FOR RETURNING DATA.
C                       (SEE LSTORE(10, ).  (INPUT) 
...
C
 106  CONTINUE
D     WRITE(KFILDO,107)(JD(J),J=1,4),NSLAB,LDATE
D107  FORMAT(/' IN GFETCH1,'26X' LOOKING FOR'2XI9.9,2I10.9,I11.3,I4,
D    1        '  FOR DATE'I11)   
C
...
C
 ...
 110  CONTINUE
C
      IF(LSTORE(10,J).NE.NSLAB)GO TO 170
C
...
C***D114  FORMAT(/' GFETCH1 AT 114  '3I10.9,I10.3,I8,I8,I2,I11,2I4,I5,I11/
C***D    1       ('                 '3I10.9,I10.3,I8,I8,I2,I11,2I4,I5,I11)) 
C***D     WRITE(KFILDO,115)((LSTORE(M,N),M=1,12),N=1,LITEMS)
C***D115  FORMAT(/' GFETCH1 AT 115  '3I10.9,I10.3,I8,I8,I2,I11,2I4,I5,I11/
C***D    1       ('                 '3I10.9,I10.3,I8,I8,I2,I11,2I4,I5,I11)) 
      NWORDS=LSTORE(6,J)
C
...
C        SIZE OF DATA( ) IN RDDISK, MUST BE AN EVEN MULTILPE OF
...
C***D155  FORMAT(/' IN GFETCH1 AT 155--IS1( )'/(10I12))
C***D     WRITE(KFILDO,156)(IS2(JJ),JJ=1,9)
C***D156  FORMAT(/' IN GFETCH1 AT 156--IS1( )'/(9I12))
C        SUBROUTINE UNPACK WILL NOT OVERFLOW DATA.
      IF(IER.NE.0)GO TO 180
C        IER NE 0 SHOULD BE TREATED AS A FATAL ERROR.  A DIAGNOSTIC
C        WILL HAVE BEEN PRINTED BY UNPACK.
...
C***D165  FORMAT(/' GFETCH1  '2I4,I11.9,2I10.9,I10.3,I8,I8,I2,I11,2I4,I5,I10) 
      GO TO 200
C
 170  CONTINUE
C
...
D     WRITE(KFILDO,175)(JD(J),J=1,4),NSLAB,LDATE,IER
D175  FORMAT(' ****DATA NOT FOUND BY GFETCH1.',
D    1       '        LOOKING FOR'2XI9.9,2I10.9,I11.3,I4,
...
C
 ...
 178  FORMAT(/,' ****DIMENSION NDX =',I6,' NOT LARGE ENOUGH TO HOLD',
     ...
     2         '     DATA NOT RETURNED BY GFETCH1.  ID( ) =',
...
C***D201  FORMAT(/' GFETCH1  'I4,I11.9,2I10.9,I10.3,I8,I8,I2,I11,2I4,I5,I11/
