...
C        THAT MEANS BEYOND 180 HRS).  ONLY OPERATE ON MODEL FIELDS
C        (CCC=0xx).
C
      ELSEIF(IDPARS(1).LT.100.AND.
     1       IDPARS(12).GT.180.AND.MOD(IDPARS(12),12).NE.0)THEN
...
C
C        LOOK FOR COMPUTATION OF NORMAL MAX OR NORMAL MIN.
C
         IF(((IDPARS(1).EQ.432).OR.(IDPARS(1).EQ.422)).AND.
     1                                ((IDPARS(2).EQ.540).OR.
     2                                (IDPARS(2).EQ.541)))THEN
       CALL GRDCNST(KFILDO,IDPARS,JD,NDATE,
