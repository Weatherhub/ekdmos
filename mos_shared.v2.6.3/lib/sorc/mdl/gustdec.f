      SUBROUTINE GUSTDEC(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                   IDPARS,JD,ITAU,
...
C        MAY      2006   MALONEY  MINOR FIX TO DD FOR MOS WINDS
C                                 FOR OPERATIONS
C        JANUARY  2007   RUDACK   CORRECTED DOCUMENTATION BLOCK.
C
C        PURPOSE 
C            TO DETERMINE IF A WIND GUST FORECAST IS TO BE MADE.
C            "GUSTDEC" ENSURES THAT A WIND GUST FORECAST DOES NOT
C            EXCEED AN UNREALISTIC VALUE.
C
C        DATA SET USE 
...
C            XDATA(K) = POST PROCESSED WIND GUSTS IN KTS (K=1,NSTA).
C                       (OUTPUT)
...
C
C            ZDATA(K) = MOS WIND SPEED FORECASTS IN KTS (K=1,NSTA).  
C                       (INTERNAL) (AUTOMATIC ARRAY)
C            FDATA(K) = PRE-PROCESSED MOS WIND GUST FORECASTS IN
C                       KTS (K=1,NSTA).  (INTERNAL) (AUTOMATIC ARRAY)
C            YDATA(K) = GUST/NO GUST FORECASTS AS OUTPUT FROM CATGR1.
C                       (K=1,NSTA). (INTERNAL) (AUTOMATIC ARRAY)
C               MD(J) = HOLDS THE 4 ID WORDS OF THE DATA RETRIEVED INTO
C                       ZDATA( ) (MOS WIND SPEED FORECASTS), 
C                       FDATA( ) (MOS WIND GUST FORECASTS) AND YDATA( ) 
C                       CATGEGORICAL FORECASTS OF GUST-NO GUST. (J=1,4).
C                       (INTERNAL)
C           MDPARS(J) = PARSED VALUES CORRESPONDING TO MD( ) (J=1,15)
C                       (INTERNAL)
C           ITABLE(J) = VALUES OF CCCFFF ACCOMMODATED (J=1) AND
C                       THE ASSOCIATED VARIABLE NEEDED (J=2,3,4).
C                       (INTERNAL)
C 
...
C
      ...
      DIMENSION YDATA(ND1),ZDATA(ND1),FDATA(ND1)
C               (AUTOMATIC ARRAYS)
...
C
      DATA MDPARS/15*0/  
      DATA ITABLE/204375, 204325, 204370, 204366/
C
      IER=0
C
...
C
 ...
 107  FORMAT(/,' ****GUSTDEC ENTERED FOR VARIABLE',
...
C
 120  MD(1)=ITABLE(2)*1000+MOD(IDPARS(4),10)
      MD(2)=JD(2)
      MD(3)=JD(3)
      MD(4)=0
C
C        RETRIEVE THE MOS WIND SPEED FORECASTS.
C
...
C
  ...
  125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN GUSTDEC',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
...
C
      ...
      MD(4)=0
C
C        RETRIEVE THE MOS WIND GUST FORECASTS.
C
      CALL PRSID1(KFILDO,MD,MDPARS)
      CALL RETVEC(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1            MD,MDPARS,JD,ITAU,
...
C
  ...
  135    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN GUSTDEC',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        FILL MD( ) AND MDPARS( ).
C
      MD(1)=ITABLE(4)*1000+IDPARS(4)
      MD(2)=JD(2)
      MD(3)=JD(3)
      MD(4)=0
C
C        RETRIEVE THE CATGR1 WIND GUST FORECASTS.
C
      CALL PRSID1(KFILDO,MD,MDPARS)
      CALL RETVEC(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1            MD,MDPARS,JD,ITAU,
...
C
      IF(IER.NE.0)THEN
         WRITE(KFILDO,148)(MD(J),J=1,4)
  148    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN GUSTDEC',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        IF A MOS WIND SPEED IS NOT MISSING AND A WIND GUST IS
C        ALSO NOT MISSING, PROCESS THE WIND GUST FORECAST.  
C        NOTE:  IF NO GUST IS ANTICIPATED FOR A STATION OR THE 
C        DIFFERENCE BETWEEN THE WIND GUST AND WIND SPEED DOES 
C        NOT EXCEED 3 KTS, SET THE WIND GUST FORECAST TO ZERO.  
C        IF EITHER THE WIND SPEED OR WIND GUST FORECAST IS 
C        MISSING, THE WIND GUST FORECAST IS ALSO SET TO 
C        MISSING.
C    
      DO 211 K=1,NSTA
         IF((NINT(ZDATA(K)).NE.9999).AND.
     1      (NINT(FDATA(K)).NE.9999)) THEN
            IF((NINT(YDATA(K)).EQ.2).AND.
     1        ((FDATA(K)-ZDATA(K)).GE.3.)) THEN
               XDATA(K)=FDATA(K)
            ELSE
               XDATA(K)=0.0
            ENDIF
 ...
 211  CONTINUE
C
C        POST PROCESS WIND GUSTS TO ENSURE THAT THEY DO NOT
C        EXCEED UNREALISTIC VALUES.
C
      DO 250 K=1,NSTA
         IF((NINT(ZDATA(K)).NE.9999).AND.
     1      (NINT(XDATA(K)).NE.9999)) THEN
            IF((ZDATA(K).GE.9.).AND.(ZDATA(K).LE.39.)) THEN
               IF((XDATA(K)).GT.(2.5*ZDATA(K)))XDATA(K)=2.5*ZDATA(K)
            ELSEIF(ZDATA(K).GE.40.) THEN
               IF((XDATA(K)).GT.(2.0*ZDATA(K))) XDATA(K)=2.0*ZDATA(K)
            ENDIF 
        ENDIF
 250  CONTINUE
C
