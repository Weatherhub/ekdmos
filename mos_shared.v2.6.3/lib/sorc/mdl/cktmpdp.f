      SUBROUTINE CKTMPDP(KFILDO,KFIL10,
...
C
C
C        MARCH    2005   ANTOLIK  MDL MOS-2000. 
C
C        PURPOSE
C            TO CHECK AN INPUT GRID OF TEMPERATURE VALUES (222 020) AGAINST
C            THE GRIDDED DEWPOINTS (223 020) TO MAKE SURE THAT THERE ARE NO
C            CASES WHERE T < TD.  IF SO, THEN BOTH THE TEMPERATURE AND DEWPOINT
C            ARE SET TO THE VALUE (T + TD)/2.         
C
...
C
C                222 030 - CONSISTENCY CHECKED GRIDDED TEMPERATURE IN DEG F.
C                223 030 - CONSISTENCY CHECKED GRIDDED TEMPERATURE IN DEG F.
C
C        HISTORY:   
C        05/03       ANTOLIK	NEW SUBROUTINE                  
C
C        DATA SET USE
...
C                       GRIDDED DEWPROINT IN DEGREES F BY EITHER
C                       GFETCH OR CONST1. (INTERNAL)
...
C        DATA ID TABLE
C
      DATA ITABLE/222030, 222020, 223020,
     1            223030, 222020, 223020/
C
C
      IER=0
      ISTAV=0
C
C        MAKE SURE THE REQUESTED PREDICTOR IS A GRIDDED TEMPERATURE
C        OR DEWPOINT FIELD IN DEGREES F.
C
...
C
 ...
 110  FORMAT(/,' ****CKTMPDP ENTERED FOR VARIABLE',
...
C        ENFORCE T, TD CONSISTENCY
C
 400  DO 450 J=1,NX(1)*NY(1)
C
        IF(IDPARS(1).EQ.222)THEN
          DATA(J)=FD1(J)
        ELSEIF(IDPARS(1).EQ.223)THEN
          DATA(J)=FD2(J)
        ELSE
          GOTO 800
        ENDIF
