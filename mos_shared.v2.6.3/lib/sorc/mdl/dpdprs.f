      SUBROUTINE DPDPRS(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                  ID,IDPARS,JD,ITAU,
...
C        SEPTEMBER 1999   GLAHN    ADDED XDATA(K) = 9999 IN DO 211 LOOP;
C                                  ADDED YDATA( )
C        APRIL     2000   RUDACK   CORRECTED ID FOR OPERATIONAL USE
C        APRIL     2000   GLAHN    CORRECTED COMMENTS, CHECKED SPELLING;
C                                  INSURED /, IN **** COMMENTS
C        APRIL     2000   RUDACK   ADDED DATA LDPARS/15*0/ 
C
C        PURPOSE 
C            TO DETERMINE THE DEW POINT DEPRESSION GIVEN TEMPERATURE
C            AND DEW POINT.   IF EITHER THE TEMPERATURE OR DEW POINT
C            IS MISSING FOR A STATION, THE DEPRESSION IS SET TO MISSING.
C
C            IDPARS(1) & IDPARS(2) OF TEMPERATURE  
C            ARE MAPPED INTO      FROM 
C                       203040    202020 AND 203020
C
C        DATA SET USE 
...
C            XDATA(K) = TEMPERATURE (INTERNAL), THEN DEW POINT
C                       DEPRESSION (K=1,NSTA).  (OUTPUT)
...
C                       (INTERNAL)
C            YDATA(K) = WORK ARRAY FOR HOLDING DEW POINTS (K=1,ND1).  
C                       (AUTOMATIC)
...
C  
      DATA LDPARS/15*0/  
      DATA ITABLE/203040,202020,203020/
C
      IER=0
C
...
C
 ...
 107  FORMAT(/,' ****DPDPRS ENTERED FOR VARIABLE',
...
C
  ...
  125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN DPDPRS',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        GET THE DEW POINT TEMPERATURE.
C
...
C
  ...
  127    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN DPDPRS',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
C        COMPUTE THE DEW POINT DEPRESSION IF THE TEMPERATURE AND
C        DEW POINT ARE NOT MISSING.  THEY WILL NEVER BE 9997.
C
      DO 211 K=1,NSTA
C
