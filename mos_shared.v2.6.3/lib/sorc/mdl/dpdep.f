      SUBROUTINE DPDEP(KFILDO,KFIL10,ID,IDPARS,JD,NDATE,
     1                   SDATA,ND1,NSTA,IPACK,IWORK,FD1,FD2,ND2X3,
     2                   LSTORE,ND9,LITEMS,CORE,ND10,NBLOCK,NFETCH,
     3                   IS0,IS1,IS2,IS4,ND7,
     4                   ISTAV,L3264B,IER)
C
C        GLAHN    2004   GLAHN   MOS-2000
C                                MODIFIED FROM OBSDPTD
C
C        PURPOSE 
...
C
C            NOTE:  THIS IS THE SAME AS OBSDPTD MODIFIED BY GLAHN
C                   NOVEMBER 2004.  IT WAS PUT IN MOSLIB BECAUSE
C                   THE LOADING SEQUENCE GAVE TROUBLE WHEN IT WAS
C                   IN U201LIB.  (USUALLY, U201LIB IS LOADED BEFORE
C                   MOSLIB, BUT OPTX IS IN MOSLIB AND CALLS DPDEP.
C
...
C
 ...
 150    FORMAT(/,' ****DPDEP ENTERED FOR INCORRECT PREDICTOR ',
     1          I9.9,2I10.9,I4.3)
	IER=103
	GO TO 800
      ENDIF
...
C         
 ...
 205    FORMAT(/,' ****ERROR IN DPDEP, FOR FIRST PROCESS DATE,',
...
C
 ...
 210    FORMAT(/,' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2         ' IN DPDEP. DATA SET TO MISSING.')
        IER=52
        GO TO 800
      ENDIF
C
...
C
      IF(IER.EQ.47)THEN
C           IF THE RECORD WASN'T FOUND, TRY THE CORRECTED
C           DEW POINT.
         MD(1)=703102000
         CALL GFETCH(KFILDO,KFIL10,MD,7777,LSTORE,ND9,LITEMS,
     1               IS0,IS1,IS2,IS4,ND7,IPACK,IWORK,FD2,ND2X3,
...
C
 ...
 212    FORMAT(/,' ****ERROR IN DPDEP, FOR FIRST PROCESS DATE,',
...
D815  FORMAT(/,' ****ERROR IN DPDEP, IER =',I5,' FOR VARIABLE',4I12)
C
 850  RETURN
      END     
