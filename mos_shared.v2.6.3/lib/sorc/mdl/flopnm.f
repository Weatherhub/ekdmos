...
C        JUNE     2006   RLC        TOOK FILENAME OUT OF OPEN 
C                                   STATEMENT FOR OPERATIONAL VERSION
...
C               KFILX = UNIT NUMBER FOR MOS-2000 FILE.  (INPUT)
C               CFILX = THE FILE NAME OF THE FILE TO OPEN.  (CHARACTER*60)
C                       (INPUT)
...
C
      IF(NRW.EQ.1)THEN
C200  IF(NRW.EQ.1)THEN
D        WRITE(KFILDO,2002)KFILX,NBYTES,CFILX
D2002    FORMAT(' OPENING FILE ON UNIT NO. =',I3,
D    1          ' FOR READ ONLY IN FLOPNM, PHYSICAL RECORD',
D    2          ' SIZE IN BYTES =',I6/'    FILE NAME = ',A60)
         OPEN(UNIT=KFILX,CONVERT=CONVERTX,
     1      STATUS='OLD',ACCESS='DIRECT',RECL=NBYTES, 
     2      ACTION='READ',IOSTAT=IOS,ERR=890)
C
      ELSEIF(NRW.EQ.2)THEN
D        WRITE(KFILDO,2003)KFILX,NBYTES,CFILX
D2003    FORMAT(' OPENING FILE ON UNIT NO. =',I3,
D    1          ' FOR READWRITE IN FLOPNM, PHYSICAL RECORD',
D    2          ' SIZE IN BYTES =',I6/'    FILE NAME = ',A60)
         OPEN(UNIT=KFILX,CONVERT=CONVERTX,
     1      STATUS='OLD',ACCESS='DIRECT',RECL=NBYTES,
