      SUBROUTINE KTOF(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1                ID,IDPARS,JD,ITAU,
...
C        AUGUST 1999   RUDACK   REPLACED GFETCH WITH RETVEC
C        APRIL  2000   GLAHN    IMPROVED COMMENTS; SPELLING CHECKED;
C                               INSURED /, IN **** COMMENTS 
C        APRIL  2000   RUDACK   ADDED DATA LDPARS/15*0/ 
C        JUNE   2001   DAGOSTARO
C                      GORLINE  CHANGED FTOKEL TO KELTOF
C        AUGUST 2002   DAGOSTARO   ADDED ABILITY TO CONVERT NDFD
C                                  MAX/MIN, TEMPERATURE, AND DEW
C                                  POINT FROM K TO F.  
C        APRIL  2006   ANTOLIK  INCORPORATED ALL IDS FROM OLD ROUTINE 
C                               KTOF. CHANGED ROUTINE NAME TO "KTOF",
C                               BUT RETAINED ORIGINAL KELTOF PROGRAM
C                               STRUCTURE.
C
C        PURPOSE 
C            TO CHANGE UNITS FROM KELVIN TO FAHRENHEIT.
C            CONVERSION USED:  F = (K - 273.15)*1.8 + 32.  
C            SEE ITABLE( , ) FOR THE MAPPING IDS.  IF 
...
C
 ...
 107  FORMAT(/,' ****KELTOF ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 300
C
C        FILL LD( ) AND LDPARS( ) AND GET THE TEMPERATURE IN KELVIN.
C
...
C
  ...
  125    FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN KELTOF',
     1          2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3)
         GO TO 300
      ENDIF
C
