...
C        OCT       2009    SMB    ADDED IDS TO CALL TO GRDCNST FOR GRIDDED
C                                 PRECIP TYPE RELATIVE FREQUENCIES AND 50%
C                                 LOGIT VALUES, AND SNOWFALL CLIMATOLOGY.
C        NOV       2010    MNB    MODIFIED CALL TO SWTXRF.
C        DEC       2011    SDS    ADDED CALL TO KINXRF2.
C        JUNE      2012    PES    MODIFIED CALL TO KINXRF2 FOR 2H TSTMS.
C        MARCH     2015    SDS    MERGED WITH ECMWF-MOS OPTN2 TO INCLUDE ECMOS
C                                 IDS BEFORE CALLS TO FRZLVL, ZRPRED, SATLEVRH,
C                                 SATLEVNUM AND LOGITTRANS. ADDED CALL TO 
C                                 GUSTRAT.
C
C        PURPOSE 
...
C            KINXRF, KINXRF2,  OBSTCLD,  OBSCIGHT, RHVV, FRZLVL, SSR, 
...
C
     ...
     6      ((IDPARS(1).EQ.438).AND.(IDPARS(2).EQ.545)).OR.
