      SUBROUTINE RDSTNA(KFILDO,IP4,IP5,KFILD,NEW,CCALL,CCALLD,
     1                  NAME,NELEV,IWBAN,STALAT,STALON,IFOUND,
     2                  NGP,GPNAME,NGPRUN,ND1,KGP,NSTA,IER)
C 
C        MARCH     1997   GLAHN   TDL   MOS-2000 
...
C                                 BY ADDING LINK( ), LINKNO( ), JSTA( )
C                                 NGPTOT( ), LOC( );
C                                 ADDED J=J-1 AND NSTA=J ABOVE 135
C                                 TO MATCH RDSTAL AND BE CORRECT AT 148;
C                                 ADDED TO FORMATS 142 AND 1420;
C                                 SHORTENED SOME COMENTS TO 72 COLUMNS;
C                                 NEW USE FOR IFOUND( );
C                                 CORRECTED FORMAT 132; DIAGNOSTIC 
C                                 WRITTEN IF KFILD(1) = KFILD(2)
C        APRIL     2001   SFANOS  MODIFIED FORMAT STATEMENTS TO CONFORM
...
C            LIST IN ALPHABETICAL ORDER BY GROUP.  FOR EACH GROUP,
C            A GROUP NAME IS READ.  THE STATION LIST
...
C            FILE, BUT THE STATION LIST CANNOT BE ON THE SAME FILE AS THE 
C            DIRECTORY.  THE CALL TO RDC IS SET TO READ UP TO 
C            NT = 7 VALUES PER RECORD ACCORDING TO THE FORMAT 
C            (7(A8,1X)); HOWEVER, THE RECORD NEED NOT HAVE THAT MANY 
C            VALUES.  THE STATIONS IN THE LIST RETURNED IN CCALL( ,1)
C            WILL BE IN ALPHABETICAL ORDER BY GROUP PROVIDED THE
C            DIRECTORY IS ALPHABETICAL.  THE CURRENT DIRECTORY
C            IS ALPHABETICAL BY THE NEW ICAO CALL LETTERS;
C            SO, IF THE OLD CALL LETTERS ARE USED, THE STATIONS MAY
C            NOT BE IN ALPHABETICAL ORDER.  THIS ROUTINE IS USED FOR
C            READING A COMPLETE LIST COMPOSED OF ONE OR MORE GROUPS.
C            READING STOPS WHEN AN EMPTY SET IN ENCOUNTERED. 
C            DUPLICATE STATIONS WILL BE KEPT, BUT A DIAGNOSTIC IS
C            OUTPUT WHICH PERTAINS TO THE TOTAL LIST, NOT JUST WITHIN
C            A GROUP.  THE ARRAYS TO BE FILLED ARE INITIALIZED.
C
C            NOTE:  THE DIFFERENCE BETWEEN THIS RDSTNA AND RDSTNL IS 
C                   THAT RDSTNA ALPHABETIZES STATIONS, WHILE RDSTNL
C                   DOES NOT.  RDSTAD IS USED FOR A SINGLE STATION
C                   LIST; RDSTNA IS USED FOR MULTIPLE LISTS.
C
C        DATA SET USE 
...
C                       (OUTPUT)
C            IP5      - UNIT NUMBER FOR OUTPUT OF DIRECTORY
...
C                       (OUTPUT)
C           CCALLD(K) = CALL LETTERS AS READ.  (CHARACTER*8)  (INTERNAL)
...
C                           ALTERNATE STATION.  AN ALTRNATE STATION IS
...
C           NGPRUN(J) = HOLDS STARTING POINT OF EACH GROUP (J=1,KGP).
C                       LATER IS ONE PAST THE POINT TO WHICH THE GROUP
C                       IS FILLED.  (INTERNAL)
C                 ND1 = SIZE OF ARRAYS CCALL( , ), CCALLD( ), NAME( ),
C                       NELEV( ), IWBAN( ), STALAT( ), STALON( ) 
C                       IFOUND( ), NGP( ), AND NGPRUN( ).  THIS IS
...
C                       FOR EACH STATION (K=1,NSTA)  (AUTOMATIC)
C                       (INTERNAL)
C             JSTA(K) = KEEPS COUNT OF ENTRIES IN EACH GROUP (K=1,KGP)
C                       (AUTOMATIC)  (INTERNAL)
C           NGPTOT(K) = THE TOTAL STATIONS IN ALL GROUPS UP TO AND
C                       INCLUDING GROUP K.  (AUTOMATIC)  (INTERNAL)
...
C
     ...
     1          IFOUND(ND1),NGP(ND1),NGPRUN(ND1)
      DIMENSION LINK(ND1),LINKNO(ND1),JSTA(ND1),NGPTOT(ND1),LOC(ND1)
C        LINK( ), LINKNO( ), JSTA( ), LOC( ), AND NGPTOT( ) ARE 
C        AUTOMATIC ARRAYS.
      DIMENSION KFILD(2)
C
      DATA BLANK8/' '/
...
C     
 105  CALL RDC(KFILDO,IP4,KFILD(1),CCALLD(NSTA+1),ND1-NSTA,CTEMP,NT,
...
C        THAT IS, 'OKC     ' COULD BE 'OKC' OR 'OKC '.
      IF(IER.NE.0.AND.NSTA1.NE.0)
     1      WRITE(KFILDO,110)NSTA1,KGP,(CCALLD(K),K=NSTA+1,NSTA+NSTA1)
 110  FORMAT(/,' ',I4,' STATIONS INPUT FOR GROUP NO.',I4,/,
     1      (14(1X,A8)))
...
C        AN EMPTY SET TERMINATES READING.
      ...
      WRITE(IP4,110)NSTA1,KGP,(CCALLD(K),K=NSTA+1,NSTA+NSTA1)
...
C        IP4.
 ...
 116  FORMAT(/,' ****TOO MANY GROUPS IN RDSTNA FOR DIMENSION ND1 =',
     1       I7)
      IER=34
      KGP=KGP-1
      GO TO 160
...
C
 120  KGP=KGP-1
C
C        FILL NGPRUN( ) WITH STARTING POINT OF EACH GROUP.
C
      NGPRUN(1)=1
      IF(KGP.EQ.1)GO TO 121
C
      DO 1205 L=2,KGP
      NGPRUN(L)=NGPRUN(L-1)+NGP(L-1)
 1205 CONTINUE
C
...
C
 ...
 124  FORMAT(/,' ****ERROR READING STATION DIRECTORY IN RDSTNA.',
     1         '  IOSTAT =',I5)
      IER=33
      GO TO 1405
C
C        A STATION LIST RESIDES IN CCALLD( ).  DETERMINE WHETHER THIS
C        DIRECTORY ENTRY MATCHES A STATION IN CCALLD( ) AND
C        IS TO BE KEPT.  THE FOLLOWING PROCEDURE WILL CAUSE
C        THE STATIONS TO BE KEPT ALPHABETICALLY BY GROUP, PROVIDED 
C        THE DIRECTORY IS ALPHABETICAL.  DUPLICATE STATIONS WILL BE KEPT.
C        STATIONS NOT IN THE DIRECTORY WILL APPEAR AT THE END OF EACH
C        GROUP'S LIST.
C
 130  JP1=0
...
C
      ...
      IF(IFOUND(KL).GT.0.AND.LINK(KL).EQ.0)GO TO 139
...
C
      IF(CCALLT(L).NE.CCALLD(KL))GO TO 139
C
      IF(LINK(KL).NE.0)THEN
C
...
C
         LINKK=LINK(KL)+1
         MMEND=NGPTOT(M)+NGP(M)
C
C           LINKS FOR STATIONS GT KL IN THE LIST NEED TO BE DECREASED
C           BY 1 TO ACCOUNT FOR THE DELETION BELOW.
C
         DO 1388 MM=LINKK,MMEND
C
         IF(LINK(MM).NE.0)THEN
C
            IF(LINK(MM).GT.LINK(KL))THEN
               LINK(MM)=LINK(MM)-1
            ENDIF
C
         ENDIF
C
 1388    CONTINUE
C
         DO 1315 MM=LINKK,NGPRUN(M)-1
C           IN CASE LINKK GT JSTA(M), THIS LOOP WILL NOT BE EXECUTED.
         CCALL(MM-1,1)=CCALL(MM,1)
         CCALL(MM-1,2)=CCALL(MM,2)
         CCALL(MM-1,3)=CCALL(MM,3)
         CCALL(MM-1,4)=CCALL(MM,4)
         CCALL(MM-1,5)=CCALL(MM,5)
         CCALL(MM-1,6)=CCALL(MM,6)
         NAME(MM-1)(1:17)=NAME(MM)(1:17)
         NAME(MM-1)(19:20)=NAME(MM)(19:20)
         IWBAN(MM-1)=IWBAN(MM)
         NELEV(MM-1)=NELEV(MM)
         STALAT(MM-1)=STALAT(MM)
         STALON(MM-1)=STALON(MM)
 1315    CONTINUE
C
         J=NGPRUN(M)-1
      ELSE
...
C        DIRECTORY) MUST BE ADDED TO CCALLD( ).  DUPLICATE
C        STATIONS AND STATIONS NOT FOUND IN THE DIRECTORY
C        ARE LISTED ACCORDING TO THE ORDER INPUT.
C
 1405 JER=0
      ISTART=0
C
...
C
         IF(JER.EQ.0)THEN
            WRITE(KFILDO,141)CCALLD(KL)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,141)CCALLD(KL)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,141)CCALLD(KL)
 141        FORMAT(/,' ****STATION ',A8,' NOT FOUND IN DIRECTORY')
         ELSE
            WRITE(KFILDO,1410)CCALLD(KL)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,1410)CCALLD(KL)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,1410)CCALLD(KL)
...
C           EARLIER.)
         WRITE(KFILDO,132)ND1
         IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,132)ND1
         IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)WRITE(IP4,132)ND1
         IER=34
         GO TO 145
C      
 1412    CCALL(NSTX,1)=CCALLD(KL)
         CCALL(NSTX,2)=BLANK8
         CCALL(NSTX,3)=BLANK8
         CCALL(NSTX,4)=BLANK8
         CCALL(NSTX,5)=BLANK8
         ...
         STALON(NSTX)=0.
C
      ELSEIF(IFOUND(KL).EQ.7)THEN
C
         IF(JER.EQ.0)THEN
            WRITE(KFILDO,142)CCALL(LOC(KL),1)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,142)
     1                                    CCALL(LOC(KL),1)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,142)CCALL(LOC(KL),1)
 142        FORMAT(/,' ****STATION ',A8,' IS A DUPLICATE.')
         ELSE
            WRITE(KFILDO,1420)CCALL(LOC(KL),1)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,1420)
     1                                    CCALL(LOC(KL),1)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,1420)CCALL(LOC(KL),1)
...
C
         IF(JER.EQ.0)THEN
            WRITE(KFILDO,143)CCALL(LOC(KL),1)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,143)
     1                                    CCALL(LOC(KL),1)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,143)CCALL(LOC(KL),1)
 143        FORMAT(/,' ****STATION ',A8,' IS A DUPLICATE;',
     1              ' MAY BE CAUSED BY A LINK TO ALTERNATE STATION.')
         ELSE
            WRITE(KFILDO,1430)CCALL(LOC(KL),1)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,1430)
     1                                    CCALL(LOC(KL),1)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,1430)CCALL(LOC(KL),1)
...
C
         IF(JER.EQ.0)THEN
            WRITE(KFILDO,144)CCALL(LOC(KL),1),IFOUND(KL)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,144)
     1                                    CCALL(LOC(KL),1),IFOUND(KL)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,144)CCALL(LOC(KL),1),IFOUND(KL)
 144        FORMAT(/,' ****STATION ',A8,' REQUIRED A LINK NO. =',I3)
         ELSE
            WRITE(KFILDO,1440)CCALL(LOC(KL),1),IFOUND(KL)
            IF(IP4.NE.0.AND.IP4.NE.KFILDO)WRITE(IP4,1440)
     1                                    CCALL(LOC(KL),1),IFOUND(KL)
            IF(IP5.NE.0.AND.IP5.NE.KFILDO.AND.IP4.NE.IP5)
     1             WRITE(IP5,1440)CCALL(LOC(KL),1),IFOUND(KL)
...
C
 ...
 153           FORMAT(/,' GROUP',I4,',',I5,' STATIONS   ',A30)
               WRITE(IP5,154)CCALL(J,1),NAME(J),IWBAN(J),
     1              STALAT(J),STALON(J),NELEV(J),(CCALL(J,I),I=2,6)
 154           FORMAT(/,' ',10X,A8,1X,A20,1X,I6,2F8.2,I6,2X,5(1X,A8))
            ELSE
...
C
         IF(J.EQ.ISTART+1.AND.KGP.NE.NSTA)THEN
            ...
            WRITE(KFILDO,154)CCALL(J,1),NAME(J),IWBAN(J),
     1           STALAT(J),STALON(J),NELEV(J),(CCALL(J,I),I=2,6)
         ELSE
