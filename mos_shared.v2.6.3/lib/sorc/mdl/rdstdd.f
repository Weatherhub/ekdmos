      SUBROUTINE RDSTDD(KFILDO,IP4,IP5,KFILD,NEW,CCALL,CCALLD,
...
C        OCTOBER   2003   GLAHN   CHANGED CCALLS TO CCALLD IN CALL
C                                 TO ELIMDS
C        NOVEMBER  2003   GLAHN   CHANGED IP4 TO IP5 ABOVE 132
C
C        PURPOSE 
...
C                       LETTERS (K=1,NSTA).  LATER USED TO INDICATE
C                       WHICH STATIONS TO ELIMINATE.  (AUTOMATIC)  (INTERNAL)
...
C
      ...
      DATA BLANK/' '/
C
D     CALL TIMPR(KFILDO,KFILDO,'START RDSTDD        ')
C
      IER=0
      NSTX=0
      JSTA=0
...
C
 ...
 124  FORMAT(/' ****ERROR READING STATION DIRECTORY IN RDSTDD.',
     1        '  IOSTAT =',I5)
      IER=33
...
C
         JER=1
         LINK(LOC(K))=0
C           THIS STATION IS LATER ELIMINATED.
C
...
C
 145  CONTINUE      
C 
C        TOSS DUPLICATES INDICATED BY 0 IN LINK(K).
C
      NSTA=NSTX
      LSTA=NSTA
      K=1
C 
 146  IF(K.GT.LSTA)THEN
         NSTA=LSTA
         GO TO 1469
      ENDIF
C
...
C              CALL ELIMDS TO ELIMINATE THE DUPLICATE.
            CALL ELIMDS(KFILDO,CCALL,CCALLD,NAME,NELEV,IWBAN,
     1                  STALAT,STALON,ITIMEZ,LINK,K,ND1,LSTA)
         ELSE
...
C    
      ELSE
C           UPDATE K UNLESS AN ENTRY WAS ELIMINATED ABOVE,
C           IN WHICH CASE THE SAME K MUST BE CONSIDERED.
         K=K+1
      ENDIF 
C
      GO TO 146            
...
C
 1469 IF(IER.EQ.33.OR.IER.EQ.34)GO TO 148
C
...
C
      ENDIF
C
D     CALL TIMPR(KFILDO,KFILDO,'END RDSTDD          ')
C
 160  RETURN
      END
