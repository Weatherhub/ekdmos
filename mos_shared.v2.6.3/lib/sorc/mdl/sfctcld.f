      SUBROUTINE SFCTCLD(KFILDO,KFIL10,ID,IDPARS,JD,NDATE,
...
C        FEBRUARY 1999   WEISS  THIS CODE IS NEW TO U201 AND IS
C                               DERIVED FROM OBSTCLD.
C        OCTOBER  2001   WEISS  IF STATEMENTS CONTAINING REAL VALUES
C                               HAVE BEEN CHANGED TO INTEGERS.
C        OCTOBER  2001   GLAHN  INSERTED /D TIMPR
...
C        MAY      2003   GLAHN  CHANGED MAIN DO LOOPS FROM 1,NWORDS
C                               TO 1,NSTA; WHITE SPACE; REARRANGED
C                               TYPE STATEMENTS; INCLUDED KSTOP( ) IN
C                               CALL; DIAGNOSTIC FORMATS
C
C        PURPOSE 
C            THIS SUBROUTINE WILL COMPUTE THE AMOUNT OF TOTAL CLOUD
C            COVERAGE FROM OBSERVATION VECTOR DATA USING ONLY SURFACE 
C            OBSERVATIONS, FOR PERIODS BEFORE OR AFTER 1996120100.
C            THE GENERATED PARAMETER WILL BE REFERRED TO AS THE
C            SURFACE TOTAL CLOUD COVERAGE WITH A FIRST WORD VALUE
C            OF 708311000. FOR DATES PRECEDING 1996120100 THE SURFACE 
C            TOTAL CLOUD WILL SIMPLY BE THE VALUES OF OLD SURFACE TOTAL
C            CLOUD (708310) READ FROM ARCHIVE AND CONVERTED TO METAR 
C            CODED VALUES (SEE TABLE BELOW). FOR DATES 1996120100 00Z 
C            AND LATER, SURFACE TOTAL CLOUD WILL BE ESTIMATED FROM THE
C            SURFACE OBSERVATIONS OF CLOUD AMOUNT FROM DIFFERENT LEVELS.
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C               708 311 - TOTAL CLOUD COVERAGE
C 
...
C
D     CALL TIMPR(KFILDO,KFILDO,'START SFCTCLD       ')
C
      IER=0
      ISTAV=1
C
...
C
      IF(JD(1).NE.708311000) THEN
        WRITE(KFILDO,150)(JD(J),J=1,4)
 150    FORMAT(/' ****SFCTCLD ENTERED FOR INCORRECT PREDICTOR.  ',
     1           I9.9,2I10.9,I4.3,' NOT COMPUTED IN SFCTCLD.')
        IER=103
        GO TO 800
      ENDIF
C
...
C
 ...
 170      FORMAT(/,' ****ERROR IN SFCTCLD ATTEMPTING OLDPROC',
...
C
 ...
 320        FORMAT(/' ****ERROR FROM GFETCH IN SFCTCLD AT 320 FOR',
...
C
 ...
 325        FORMAT(/' ****NWORDS =',I6,' RETURNED FROM GFETCH',
     ...
     2              ' COVERAGE (LEVEL ',I1,') IN SFCTCLD.',
...
C        MANUAL
C        ASOS WITH FAA MANUAL AUGMENTATION --| 
C                                            -- ASOS ALONE 
C        ASOS WITH SCP AUGMENTATION ---------|
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
...
C
 ...
 450      FORMAT(/' ****ERROR FROM GFETCH IN SFCTCLD FOR FIRST PROCESS',
...
C
      ENDIF
C
D     CALL TIMPR(KFILDO,KFILDO,'END   SFCTCLD       ')
C
      GO TO 850 
C
...
D815  FORMAT(/' ****ERROR IN SFCTCLD, IER =',I5,' FOR VARIABLE',4I12)
C
 850  RETURN
      END
