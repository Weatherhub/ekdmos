      SUBROUTINE WXTSVR(KFILDO,KFIL10,IP12,KFILRA,RACESS,
...
C
C        PURPOSE
C           SUBROUTINE WXTSVR EVALUATES 3- AND 12-HR SEVERE
C           THUNDERSTORM PROBABILITES TO DETERMINE SEVERE POTENTIAL
C           INDEX (SPI), THEN COMPARES SPI AGAINST USER-DEFINED
C           CATEGORICAL THRESHOLDS AND DETERMINES THUNDERSTORM 
C           CATEGORIES. CONSISTENCY CHECKS ARE PERFORMED TO
C           ENSURE THE SPI DOES NOT EXCEED ITS 12-HR SEVERE 
C           THUNDERSTORM COUNTERPART, NOR THUNDERSTORM POTENTIAL INDEX
C           (TPI) NOR PRECIPITATION POTENTIAL INDEX (PPI).
C    
...
C           RETURNED IN GSPI( ) AND NXTSW( ). CALLED BY GENWX.
C
C        DATA SET USE
...
C           THRSVR(I) = SEVERE POTENTIAL THRESHOLDS, EXPRESSED 
C                       IN PERCENT, OF THE CATEGORICAL SEVERE 
C                       THUNDERSTORM PROBABILTY FORECASTS. ELEMENTS 
C                       WITHIN THRSVR MUST BE BETWEEN 0 AND 100 PERCENT, 
...
C                       THUNDERSTORMS. THE SECOND DIGIT HAS THE FOLLWING
C                       VALUES FOR SEVERE THUNDERSTORMS:
C                       MISSEL - MISSING
C                            5 - DEFINITE
C                            3 - NUMEROUS
C                            2 - SCATTERED
...
C           ITABLE(I) = CCCFFF OF WEATHER (I=1) AND THE 3-H TSVR (I=2),
C                       6-H TSVR (I=3), 12-H TSVR (I=4), AND 
C                       TEMPERATURE (I=5) FORECASTS.  (INTERNAL)
C            IDO(K,M) = 4-WORD MOS ID USED TO OBTAIN GSVR3. 
C                       (K=1,4) (M=1,4) (INTERNAL)
C               LD(M) = 4-WORD MOS ID USED TO OBTAIN GSVR12. 
C                       (M=1,4)(INTERNAL)
C               MD(M) = 4-WORD MOS ID USED TO OBTAIN TEMPERATURE. 
C                       (M=1,4)(INTERNAL)
...
C              ISVR12 = 1 SIGNIFIES GSVR12 DATA ARE CORRECT.  
C                       IF NOT, IT WILL BE SET TO MISSEL. (INTERNAL)
C            ISVR3(K) = 1 SIGNIFIES GSVR3 DATA ARE CORRECT.  
C                       IF NOT, IT WILL BE SET TO MISSEL. 
C                       (K=1,4) (INTERNAL)
C            ITEMP(K) = 1 SIGNIFIES GTEMP DATA ARE CORRECT.  
C                       IF NOT, IT WILL BE SET TO MISSEL. 
C                       (K=1,4) (INTERNAL)
C          GSVR3(K,J) = ARRAY CONTAINING SVR3 GRIDPOINT FORECASTS. 
C                       (K=1,4) (J=1,ND2X3) (INTERNAL)
C           GSVR12(J) = ARRAY CONTAINING SVR12 GRIDPOINT FORECASTS. 
C                       (J=1,ND2X3) (INTERNAL)
C            GTEMP(J) = GRIDDED MOS TEMPERATURE GRIDPOINT FORECASTS. 
C                       (J=1,ND2X3) (INTERNAL)
C              FACTOR = FACTOR USED TO INFLATE SVR3 FORECASTS TO 
C                       APPROPRIATE SCALE FOR SPI. (INTERNAL)
C               FD(K) = WORK VARIABLE CONTAINING EITHER SVR3, 
C                       OR SVR12 FORECASTS.  (K=1,4) (INTERNAL)
C               SVR12 = WORK VARIABLE. (INTERNAL)
C             FCSTMAX = MAXIMUM FORECAST IN THIS FORECAST PERIOD. 
C                       (INTERNAL)
C              SPI(J) = WORK VARIABLE CONTAINING THE FOUR SPI FORECASTS
C                       FOR THIS 12-HOUR PERIOD. (J=1,4) (INTERNAL)
C             IER_SVR = INTEGER ERROR FLAG.  IF THE ELEMENTS IN 
C                       INPUT VECTOR SVRTRW ARE OUTSIDE THE RANGE 
C                       0 TO 100 PERCENT, INCLUSIVE, OR IF THE ORDER OF 
C                       SVRTRW IS NOT MONOTONICALLY INCREASING, 
C                       IER_SVR, NOMINALLY 0, IS SET TO 1, AND 
C                       DEFAULT THRESHOLDS ARE USED INSTEAD. (OUTPUT)
C                ISPI = FLAG FOR METHOD TO DETERMINE SPI. 
C                       1 - USE ONLY SVR3
C                       2 - USE SVR3 AND SVR12
C                       (INTERNAL)
C
...
C
     ...
     1        JSTA(NSTA),MISSEL
C
      REAL    GPPI(ND2X3),GTPI(ND2X3),GSPI(ND2X3),THRSVR(4),RRMIS,RRMIS2
C
C        DECLARE WEATHER GRID VARIABLES (INTERNAL):
C
      INTEGER LDPARS(15),IDO(4,4),LD(4),MD(4),ISVR3(4),
     1        ISPI,IDUMMY(2),ISVR12,NSIZE,ISTOP,
     2        IER_WORD3,IER_SVR,I,J,K,L,M
C
      REAL    GSVR3(4,ND2X3),GSVR12(ND2X3),GTEMP(ND2X3),
     1        FD(4),SPI(4),FACTOR,FCSTMAX,SVR12
C
C     INTEGER ITABLE(5) / 228500,227165,227275,227375,222030/ ! CONDITIONAL
      INTEGER ITABLE(5) / 228500,227170,227280,227380,222030/ ! UNCONDITIONAL
C                          WX     3H     6H     12H    TEMP
C
...
C***D WRITE(KFILDO,100)(ID(J),J=1,4)
 100  FORMAT(' *********** IN WXTSVR *************'/' ',4I10)
C
C        VERIFY THE PROCESSING INDICATOR, IDPARS(1) AND IDPARS(2).
C
      IF(ITABLE(1).EQ.IDPARS(1)*1000+IDPARS(2).AND.
     1                              (IDPARS(7).EQ.0.OR.
     2                               IDPARS(7).EQ.6))GO TO 108
C
      WRITE(KFILDO,102)(ID(L),L=1,4)
  102 FORMAT(/,' ****WXTSVR ENTERED FOR VARIABLE',
     1        2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2        ' NOT ACCOMMODATED.')
      IER=103
      GO TO 340
C  
C        THE SEVERE POTENTIAL INDEX ALGORITHM IS DESIGNED TO 
C        CALCULATE SPI WITHIN A FORECAST PERIOD.  A FORECAST PERIOD IS 
C        DEFINED AS THE 12-HOUR PERIOD ENDING 00Z OR 12Z.  SEVERE 
C        POTENTIAL INDEX (SPI) USES THE SVR12 FORECAST THAT SPANS THE  
C        FORECAST PERIOD AND THE FOUR SVR3 FORECASTS MADE FOR PROJECTIONS  
...
C        SET UP THE THIRD WORD OF THE IDS TO BE FETCHED.
      CALL WXWRD3(IDPARS(12),IDO(1:4,3),IDUMMY(1:2),LD(3),IER_WORD3)
C
...
C***********************************************************************
C
C        NOW THAT THE IDS ARE SET UP FOR SVR3 AND SVR12,
C        GET THE SVR3 FORECASTS.
C 
      DO K=1,4
         CALL PRSID1(KFILDO,IDO(K,1:4),LDPARS)
         JD=IDO(K,1:4)
         ISVR3(K)=1
C           ISVR3(K) = 1 SIGNIFIES DATA ARE CORRECT.  IF NOT, IT
C           WILL BE SET TO MISSEL.
     ...
     6               IPACK,IWORK,GSVR3(K,1:ND2X3),ND2X3,
...
C
  ...
  127       FORMAT(/' ####VARIABLE = ',3I10.9,I11.3,
     1              ' NOT RETRIEVED BY GTVEGR IN WXTSVR.')
            ISVR3(K)=MISSEL
            GSVR3(K,1:ND2X3)=RRMIS
  ...
  130       FORMAT(/,' ****GRID CHARACTERISTICS FOR VARIABLE ',4I11,
     ...
     4               ' THIS VARIABLE NOT USED.')
      ...
      ENDDO
C
C        GET THE SVR12 FORECAST
C 
      CALL PRSID1(KFILDO,LD,LDPARS)
      ISVR12=1
      JD=LD
C        ISVR12 = 1 SIGNIFIES DATA ARE CORRECT.  IF NOT, IT
C        WILL BE SET TO MISSEL.
      CALL GTVEGR(KFILDO,KFIL10,IP12,
...
C
      IF((ISVR3(1).EQ.MISSEL).AND.(ISVR3(2).EQ.MISSEL).AND.
     1   (ISVR3(3).EQ.MISSEL).AND.(ISVR3(4).EQ.MISSEL).AND.
     2   (ISVR12.EQ.MISSEL))THEN
         WRITE(KFILDO,110) IDPARS(12)
  110    FORMAT(/,' ****NO SVR12 OR SVR3 FORECASTS ARE ',
     1            'AVAILABLE. CANNOT MAKE SEVERE POTENTIAL ',
     2            'FOR IDPARS(12)=',I5,'.')
         GO TO 340
      ENDIF
C
C        VERIFY THAT THE USER-DEFINED SEVERE THRESHOLDS
C        ARE MONOTONICALLY INCREASING AND WITHIN THE RANGE 0 TO 100,
C        INCLUSIVE:
C
      CALL WXVCAT(THRSVR,IER_SVR)
C
      IF(IER_SVR.NE.0)THEN
         WRITE(KFILDO,115) 
  115    FORMAT(/,' ****DEFAULT VALUES USED FOR SEVERE ',
     1            'THRESHOLDS. CHECK VALUES OF THRSVR.')
      ENDIF
C
C        NOW THAT FETCHING IS DONE, GET SEVERE POTENTIAL 
C        INDEX (SPI).  THE ALGORITHM FOR SPI CONSISTS OF THREE STEPS:
C           1) GET INFLACTION FACTOR FOR EACH GRIDPOINT AND FORECAST 
C              PERIOD.  FACTOR IS THE SVR12 FORECAST DIVIDED BY THE 
C              MAXIMUM SVR3 OR SVR6 FORECAST
C           2) INFLATE SVR3 OR SVR6 FORECASTS BY INFLATION FACTOR TO 
C              GET FIRST-GUESS SPI
C           3) CONSISTENCY CHECKS AND SMOOTHING.
C
C        DETERMINE WHAT ELEMENTS ARE AVAILABLE, MAKING THE ASSUMPTION
C        THAT THERE IS NO TIME WHEN SVR6 AVAILABLE.
C
      IF(((ISVR3(1).NE.MISSEL).OR.(ISVR3(2).NE.MISSEL).OR.
     1   (ISVR3(3).NE.MISSEL).OR.(ISVR3(4).NE.MISSEL)).AND.
     2   (ISVR12.EQ.MISSEL))THEN
         ISPI=1
      ELSEIF((ISVR3(1).NE.MISSEL).OR.(ISVR3(2).NE.MISSEL).OR.
     1   (ISVR3(3).NE.MISSEL).OR.(ISVR3(4).NE.MISSEL).AND.
     2   (ISVR12.NE.MISSEL))THEN
         ISPI=2
      ENDIF
C
...
C        FROM 0 TO 100.
C
      CALL SCALX(KFILDO,GSVR12,ND2X3,1.,2,IER)
      CALL SCALX(KFILDO,GSVR3(1,1:ND2X3),ND2X3,1.,2,IER)
      CALL SCALX(KFILDO,GSVR3(2,1:ND2X3),ND2X3,1.,2,IER)
      CALL SCALX(KFILDO,GSVR3(3,1:ND2X3),ND2X3,1.,2,IER)
      CALL SCALX(KFILDO,GSVR3(4,1:ND2X3),ND2X3,1.,2,IER)
C
C        LOOP THROUGH ALL GRIDPOINTS IN PARALLEL
C
!$OMP PARALLEL DO 
!$OMP& SHARED(GSVR3,GSVR12,GPPI)
!$OMP& PRIVATE(I,J,FD,SVR12,FACTOR,FCSTMAX,SPI)
      DO 200 J=1,ND2X3
         FD(1:4)=RRMIS
         SPI(1:4)=RRMIS
         FACTOR=1.
...
C
            CASE(1)
               ...
               FD(4)=GSVR3(4,J)
C
C        SINCE SVR12 IS MISSING, USE A DERIVED INFLATION FACTOR.
               FACTOR=1.84 
C
               DO I=1,4
               ...
               ENDDO
C
C        SVR3 AND SVR12 ARE AVAILABLE.
C
            CASE(2)
               DO I=1,4
                  IF(GSVR3(I,J).EQ.RRMIS)THEN
                     FD(I)=RRMIS2
                  ELSE
                     FD(I)=GSVR3(I,J)
                  ENDIF
...
C
               IF(FCSTMAX.GE.0.1)THEN
                  FACTOR=SVR12/FCSTMAX
               ELSE
                  FACTOR=-1.
               ENDIF
C
C        WHEN SVR12 IS MISSING, SPI WILL BE MISSING.
C
               DO I=1,4
                  IF(SVR12.EQ.RRMIS)THEN
                     SPI(I)=RRMIS2
               ...
               ENDDO
C
C        IF NO SVR3, SVR6, SVR12, SET SPI TO MISSING.
C
            CASE DEFAULT
               SPI(1:4)=RRMIS2
C
...
C        ENFORCE UPPER LIMIT OF INDEX.
            IF(SPI(I).GT.100.)SPI(I)=100.
C        ENFORCE TPI LIMIT ON INDEX.
            IF(GTPI(J).EQ.RRMIS)THEN
               SPI(I)=RRMIS2
            ELSEIF(SPI(I).GT.GTPI(J))THEN
               SPI(I)=GTPI(J)
            ENDIF
C
C        SET MISSING DATA BACK TO RRMIS
            IF(SPI(I).EQ.RRMIS2)SPI(I)=RRMIS
C
C        GET THE SPI FOR THE RELEVANT TAU.
            IF(IDO(I,3).EQ.IDPARS(12))GSPI(J)=SPI(I)
         ENDDO
C
C        ENFORCE PPI LIMIT ON INDEX.
         IF(GSPI(J).NE.RRMIS.AND.GSPI(J).GT.GPPI(J))GSPI(J)=GPPI(J)
C
 200  CONTINUE
!$OMP END PARALLEL DO
C
C        SMOOTH SPI
         CALL HSMTH(KFILDO,GSPI,NX,NY,4,1,ISTOP,IER)

      DO 300 J=1,ND2X3
C
C        NOW THAT WE'VE SMOOTHED, DO ANOTHER CONSISTENCY CHECK.
C
         IF(GSPI(J).NE.RRMIS.AND.GSPI(J).GT.GSVR12(J))GSPI(J)=GSVR12(J)
         IF(GSPI(J).NE.RRMIS.AND.GSPI(J).GT.GTPI(J))GSPI(J)=GTPI(J)
C
C        WHERE SPI FORECASTS ARE NOT MISSING, ASSIGN CATEGORICAL 
C        SEVERE FORECASTS BASED ON THE USER-DEFINED THRESHOLDS 
C        IN THRSVR INTO THE NXTSW COUNTERPART.
C
C        MISSING DATA
         IF(NPCAT(1,J).EQ.RRMIS)THEN
            NXTSW(J)=RRMIS
         ELSEIF(GSPI(J).EQ.RRMIS)THEN
            NXTSW(J)=NXTSW(J)
C        NO THUNDER SO NO SEVERE
         ELSEIF(NXTSW(J).EQ.0)THEN
            NXTSW(J)=NXTSW(J)
C        DEFINITE SEVERE
         ELSEIF(GSPI(J).GE.THRSVR(4))THEN
            NXTSW(J)=NXTSW(J)+5000
C        NUMEROUS
         ELSEIF(GSPI(J).GE.THRSVR(3))THEN
            NXTSW(J)=NXTSW(J)+3000
C        SCATTERED
         ELSEIF(GSPI(J).GE.THRSVR(2))THEN
            NXTSW(J)=NXTSW(J)+2000
C        ISOLATED
         ELSEIF(GSPI(J).GE.THRSVR(1))THEN
            NXTSW(J)=NXTSW(J)+1000
C        NO SEVERE
         ELSE
            NXTSW(J)=NXTSW(J)
         ENDIF
...
C        USED TO CALCULATE THAT STATION'S SPI.
C
D     WRITE(KFILDO,554)
D 554 FORMAT(/,'CCALL',6X,'J',5X,'   GPPI(J)',1X,'   GTPI(J)',1X,
D    1         'TSVR3(1,J)',1X,'TSVR3(2,J)',1X,'TSVR3(3,J)',1X,
D    2         'TSVR3(4,J)',1X,' TSTM12(J)',1X,'   GSPI(J)',1X,
D    3         '  NXSTW(J)')
D     DO K=1,NSTA
...
D    1      GTPI(JSTA(K)),GSVR3(1:4,JSTA(K)),GSVR12(JSTA(K)),
D    2      GSPI(JSTA(K)),NXTSW(JSTA(K))
D        ELSE
