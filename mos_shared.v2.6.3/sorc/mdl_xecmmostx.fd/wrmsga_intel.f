...
C        COSGROVE  MAR 2006     - THE TEST TO REMOVE THE BLANK CHARACTER
C                                 IN THE FIRST COLUMN WAS NOT WORKING FOR
C                                 THE DAY LINE IN THE 12Z MESSAGE.  FIXED
C                                 THAT.
...
C         3/2006 - THE 12Z MESSAGE HAS A DAY LINE WITH SEVERAL BLANKS
C                  AND THEN THE DAYS.  FIXED IF TEST TO HANDLE THIS
        DO 650 J=1,LINES                                                
          IF (KOUNT(J,K).NE.0) THEN                                     
           IF(((MSG(1,(K-1)*LINES+J+NOHEAD).EQ.' ').AND.
     1         (MSG(2,(K-1)*LINES+J+NOHEAD).NE.' ')).OR.((MHG.EQ.12)
     2          .AND.(MSG(1,(K-1)*LINES+J+NOHEAD).EQ.' ').AND.
     3          (MSG(13,(K-1)*LINES+J+NOHEAD).EQ.'|')))THEN
