...
C   15-03-16  DER/SDS -  EXTENDED Q12, Q24, AND SNOWFALL TO PRINT THROUGH
C                        192 HOURS.
C                                                                       
C USAGE:                                                                
C                                                                       
...
C$$$ 
      ...
      PROGRAM MDL_ECMMOSTX

      PARAMETER(NOCHAR=70,MAXSTA=25000,LINE=21,NUMDAY=8)
...
C        24,36,48,60,72,84,96,108,120,132,144,156,168,180,192
     ...
     *     NHEAD/0/,NESTA/0/,IOPT/0/,NMOSTA/0/,ISW/1/,IDYWK/0/
      ...
      CALL W3TAGB('MDL_GFSMEXTX',2002,0011,0016,'OST22')                  
...
C        BUILD 1ST LINE
      ...
      CALL PUTCHAR('ECMX MOS GUIDANCE',MSG(9,(K-1)*LINE+2),17)
     ...
     *       2,2,'99',KX)
      KOUNT(1,K)=1
C        BUILD 1ST LINE 
C      CALL PUTCHAR(MREC,MSG(1,(K-1)*LINE+2),1)
C      CALL PUTCHAR(CCALL(K,1),MSG(2,(K-1)*LINE+2),4)
C      CALL PUTCHAR('ECMWFX MOS GUIDANCE',MSG(9,(K-1)*LINE+2),19)
C      CALL PUTCHAR('/  /',MSG(30,(K-1)*LINE+2),4)
C      CALL PUTMOS('MOG',FLOAT(MOG),0.,1.,1,12,MSG(28,(K-1)*LINE+2),
C     *       2,0,'99',KX)
C      CALL PUTMOS('MDG',FLOAT(MDG),0.,1.,1,31,MSG(31,(K-1)*LINE+2),
C     *       2,2,'99',KX)
C      CALL PUTMOS('MYG',FLOAT(MYGC),0.,1.,19,99,MSG(34,(K-1)*LINE+2),
C     *       2,2,'99',KX)  
C      CALL PUTMOS('MYG',FLOAT(MYGA),0.,1.,0,99,MSG(36,(K-1)*LINE+2),
C     *       2,2,'99',KX)
C      CALL PUTCHAR('00 UTC',MSG(42,(K-1)*LINE+2),6)
C      CALL PUTMOS('MHG',FLOAT(MHG),0.,1.,0,12,MSG(40,(K-1)*LINE+2),
C     *       2,2,'99',KX)
C      KOUNT(1,K)=1
C
...
C
        DO 130 LL=3,20
          DO 120 I=1,7
           ...
           IF(MHG.EQ.12)CALL PUTCHAR(VBAR,MSG(LOC12(I),(K-1)*LINE+LL),1)
 120      CONTINUE
 130    CONTINUE
        KOUNT(LINE,K)=-1
...
C
      ...
      INC=12
      DO 210 K=1,15
C       IF(K.GE.7)ID(1)=203320101
C       IF(K.GE.6)KD(1)=203320101
        ID(3)=ID(3)+INC
        CALL READ_MOSDA(KFILDO,KFILX,IPTST,ID,LDATE,CCALL,
     *       NMOSTA,FCST,MAXSTA,ND5,ND7,IS0,IS1,IS2,IS4,INDEX,IER)
        IF(IER.LT.2) KUT=KUT+1
...
C        (10/2003 - CHANGED TO LOOK FOR 331 THROUGH 84 HRS)
C
      ID(1)=203331001
      ID(2)=0
      ID(3)=12
      ...
      INC=12
      DO 230 K=1,15
        IF(K.GE.7) ID(1)=203331001
        IF(K.GE.6) KD(1)=203331001
        ID(3)=ID(3)+INC
        CALL READ_MOSDA(KFILDO,KFILX,IPTST,ID,LDATE,CCALL,
     *       NMOSTA,FCST,MAXSTA,ND5,ND7,IS0,IS1,IS2,IS4,INDEX,IER)
        IF(IER.LT.2) KUT=KUT+1
...
C        (10/2003 -CHANGED ID FROM 203421)
C
      ID(1)=203431001
      ID(2)=0
      IF(MHG.EQ.0)ID(3)=24
...
C        (10/2003 - CHANGED TO LOOK FOR 330 THROUGH 84 HRS)
C        (03/2006 - CHANGED TO USE 330 THROUGHOUT)
C
      ID(1)=207330101
      ID(2)=0
      ID(3)=12
...
C
      ...
      ID(2)=0
      IF(MHG.EQ.0)ID(3)=12
      IF(MHG.EQ.12)ID(3)=24
      ID(4)=0950000000
      INC=24
C        THE PLACEMENT OF THE FORECASTS VARIES BETWEEN THE CYCLES
C        USE THE 220 LOOP INDICES TO CONTROL THIS
       IF(MHG.EQ.0)ISTART=2
       IF(MHG.EQ.12)ISTART=3
      DO 250 K=ISTART,ISTART+12,2 
        ID(3)=ID(3)+INC
        CALL READ_MOSDA(KFILDO,KFILX,IPTST,ID,LDATE,CCALL,
     *       NMOSTA,FCST,MAXSTA,ND5,ND7,IS0,IS1,IS2,IS4,INDEX,IER)
        IF(IER.LT.2) KUT=KUT+1
...
C        AND SNOWFALL FORECASTS IF DATE IS AFTER MAY 31 i
C        AND BEFORE SEP 01.
      JDATE=(MOG*100)+MDG
...
C
 ...
 280  CONTINUE
C
C        PLACE 24H SNOWFALL FORECASTS -- USE 54, 78, 102, 126 PROJS 00Z
C        THE FORECASTS ARE STORED AT TAU+6
C          ADDED NOV/DEC 2003
C          DEC 2009 - ADDED ONE MORE PROJECTION TO BULLETIN
C
      ID(1)=208461001
      ID(2)=0
      IF(MHG.EQ.0)ID(3)=30
      IF(MHG.EQ.12)ID(3)=18
      ID(4)=0
      INC=24
C        THE PLACEMENT OF THE FORECASTS VARIES BETWEEN THE CYCLES
C        USE THE 220 LOOP INDICES TO CONTROL THIS
...
C
 369  IF (KUT.LE.0) THEN
        WRITE(6,370)
 370    FORMAT(//'0NO GFSX MOS FORECASTS FOUND.')
      CALL W3TAGE('MDL_GFSMEXTX') 
        STOP 370
      ENDIF
...
C              WRITEUPS FOR GETNORM AND GETMONTH 
      IF(MHG.EQ.0)ID(3)=108
      IF(MHG.EQ.12)ID(3)=96
      ID(4)=0
...
C
      ID(1)=422006000
      ID(2)=0
      IF(MHG.EQ.0)ID(3)=120
      IF(MHG.EQ.12)ID(3)=108
      ID(4)=0
      CALL READ_MOSDA(KFILDO,KFILC,IPTST,ID,LDATE,CCALL,
     *     NMOSTA,FCST,MAXSTA,ND5,ND7,IS0,IS1,IS2,IS4,INDEX,IER)
C
...
C
      ...
      ID(4)=400
      CALL READ_MOSDA(KFILDO,KFILX,IPTST,ID,LDATE,CCALL,
     *     NMOSTA,FCST2,MAXSTA,ND5,ND7,IS0,IS1,IS2,IS4,INDEX,IER)
C
C       NOW MERGE THE NCDC AND PRISM NORMALS, ONLY KEEPING NCDC NORMAL
C       IF THERE WASN'T A PRISM NORMAL
C
      DO 381 J=1,NMOSTA
      IF(NINT(FCST2(J)).NE.9999)FCST(J)=FCST2(J)
 381  CONTINUE
C
C       NOW PUT THE NORMALS INTO THE MESSAGE
C
