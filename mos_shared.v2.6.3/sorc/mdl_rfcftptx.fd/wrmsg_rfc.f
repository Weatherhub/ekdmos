      SUBROUTINE WRMSG_RFC(MSG,N1,N2,NBSTA,NESTA,LINES,NCAT,KOUNT,
     *     M1,M2,NAME,JA,MDG,MHG,NOHEAD,LUNIT,BFILE,NHEAD,AWIPPIL)
C                                                                       
...
C                                 ARE NOW IN ASCII
C          WEISS   MAR 1996     - CUSTOMIZED VERSION FOR THE RFC 
C                                 BULLETIN (WRMSG_RFC)
C         GILBERT  JAN 1997     - CHANGED HEXIDECIMAL CODE FOR A PLUS SI
C                                 FROM FC TO 7C.
C         GILBERT  APR 1997     - CHANGED FORTRAN STOP TO EXIT
C         SHIREY   JUL 1998     - REMOVED W3LOG
C         ALLEN    MAY 2001     - RAISED THE BULLETIN SIZE TO 14080 FOR
C                                 THE NEW AVN AND MRF RFC MESSAGES.
C                                 THIS NUMBER IS 1280*11, OR AS CLOSE TO
C                                 THE 15000 LIMIT AS WE COULD GET WITH 
C                                 A MULTIPLE OF 1280.  CURRENTLY, IF THE
C                                 MESSAGE IS BROKEN INTO MORE THAN ONE, THE
C                                 AWIPS PIL DOESN'T GET PUT INTO THE SECOND
C                                 PART, AND IT ISN'T BEING INGESTED INTO
C                                 AWIPS (TO BE CONTINUED..)
C         ALLEN     MAY 2001    - ADDED VARIABLE AWIPPIL TO THE CALL.  
C                                 CHANGED CODE SO THAT IT WILL INCLUDE
C                                 THIS AWIPS HEADER IN THE HEADER SECTION,
C                                 NOT IN THE BODY OF THE MESSAGE (SEE 
C                                 PREVIOUS NOTE)
C        COSGROVE   AUG 2005    - CONVERTED FROM THE FORMAT REQUIRED TO
C                                 DISSEMINATE VIA THE STATFILE TO VIA NTC
C        COSGROVE   AUG 2006    - INCREASED THE BULLETIN SIZE LIMIT (FROM 
C                                 1280*11) TO ALLOW FOR A MUCH BIGGER NWRFC
C                                 MESSAGE WITHOUT BREAKING IT UP.
...
C            MISSION FILE.
C        VARIABLES
C             AWIPPIL=SECOND LINE OF WMO HEADER (AWIPS PIL)
...
C                NPOS=POSITION IN MSG ARRAY FOR EACH REGIONS HEADER
C                     NOTE: THE HEADER FOR EACH REGION IS UNIQUE
...
C            BULLETIN. IN 5/2001, WE UPPED THE LENGTH OF THE BULLETIN
C            TO 14080 BECAUSE SUPPOSEDLY ANYTHING LESS THAN 15000 BYTES
C            DOESN'T NEED TO BE BROKEN UP.  AT THIS TIME, ALL OF THE 
C            RFC MESSAGES ARE LESS THAN THIS LIMIT                                      
...
C               NOTE:  AUGUST 2006 - THE NWRFC IS MUCH BIGGER THAN 15000
C            AND WE ALREADY SEND THE COOP MESSAGE OUT IN A BIGGER SIZE,
C            SO WE STOPPED WORRYING ABOUT THE 15000 LIMIT.
C                                                                       
      PARAMETER (MAXSTA=2500,LRECL=1280)
      DIMENSION KOUNT(M1,M2),ICNT(MAXSTA)
      INTEGER IREC,NSTART,NHEAD,NPOS
      CHARACTER*11 NAME
      CHARACTER*10 AWIPPIL
      CHARACTER*80 BFILE,ENVVAR,FILEO
      CHARACTER*5  NCAT
      CHARACTER*3  MCRCRLF
      CHARACTER*1 MSG(N1,N2),NETB,NFIL,PLUS,
     *            MT(42*LRECL),TMPBUL(MAXSTA),
     *            MCRR,MLNF,MRECS

      DATA TMPBUL/2500*' '/
      DATA NSTART/0/,IREC/0/,NPOS/1/
...
C        8/2005-CHANGED PLUS FROM 124 TO 43
      NETB=CHAR(62)
      NFIL=' '
      PLUS=CHAR(43)
      MCRR=CHAR(13)
      ...
      MCRCRLF(3:3)=MLNF
C
C       SET THE MAXIMUM LENGTH OF THE BULLETIN.  THIS SHOULD ALSO
C       BE THE DIMENSION OF ARRAY MT()
C
      LENBUL=42*LRECL
      DO 100 I=1,LENBUL
        MT(I)=NFIL
...
C        PLACES SECOND LINE OF WMO HEADER(AWIPS PIL)
      CALL PUTCHAR(AWIPPIL,MT(JA+30),6)

C        INSERT CR CR LINE-FEED CONTROL CHARACTERS
      CALL PUTCHAR(MCRCRLF,MT(JA+36),3)

      NOC=JA+39
C        NOC IS PLACE TO PUT NEXT CHARACTER (71)                        
C        WRITE HEADER RECORDS TO MT( )
C        IF HEADER CONTAINS MORE THAN ONE RECORD
C        START THE COUNTER AT 3 BECAUSE WE ALREADY PUT THE
C        AWIPS PIL IN ABOVE
C
C        NPOS IS THE SECOND INDEX OF MSG, OR THE LINE NUMBER.
C        WE NEED TO SKIP OVER THE LINE THAT HAS THE AWIPS
C        PIL OR IT WILL PRINT IT OUT TWICE IN THE TRAN FILE
C
        NPOS=NPOS+1
      DO 200 I=3,NOHEAD
        NPOS=NPOS+1
          DO 205 II=1,N1
             IF(MSG(II,NPOS).EQ.'+') MSG(II,NPOS)=PLUS
 205      CONTINUE
        CALL PUTQ(MSG(1,NPOS),MT(NOC),NOC,N1)
 200  CONTINUE

      NCHHD=NOC
C        COPY EACH LINE FOR STATION K TO TMPBUL (IF FOR
      ...
      DO 400 K=NBSTA,NESTA
        ...
        DO 300 J=1,LINES
          IF (KOUNT(J,K).NE.0) THEN
            CALL PUTQ(MSG(1,(K-1)*LINES+J+NOHEAD*NSTART),TMPBUL(NOC),
     *      NOC,N1)
          ENDIF
 300    CONTINUE
        ICNT(K)=NOC
...
C
C        5/2001 - MOVED THE FOLLOWING LINE TO THE FRONT OF THE CODE
C        TO MAKE IT EASIER TO MODIFY THE CODE
C      LENBUL=11*LRECL
...
C        WRMSG_RFC GENERATES A TX BULLETIN FOR EVERY NSTAPB STATIONS
      NOC=NCHHD
      KK=0
C     
...
C        HAVING IT HAPPEN MYSTERIOUSLY IN THE GATEWAY SOMEWHERE.
        DO 650 J=1,LINES                                                
          IF (KOUNT(J,K).NE.0) THEN                                     
           IF((MSG(1,(K-1)*LINES+J+NOHEAD*NSTART).EQ.' ').AND.
     1         (MSG(2,(K-1)*LINES+J+NOHEAD*NSTART).NE.' '))THEN
            CALL PUTQ(MSG(2,(K-1)*LINES+J+NOHEAD*NSTART),MT(NOC),NOC,N1)       
           ELSE
            CALL PUTQ(MSG(1,(K-1)*LINES+J+NOHEAD*NSTART),MT(NOC),NOC,N1)       
           ENDIF
          ENDIF                                                         
 650    CONTINUE
...
C           CALL PUTCHAR('%',MT(NOC),1)
           WRITE(6,651) NAME,AWIPPIL,NOC
 651       FORMAT(' BULLETIN ',A11,1X,A10,' CONTAINS ',I6,' BYTES.')
...
C       MUST INCREMENT NPOS ONE MORE TIME
      NPOS=NPOS+1

      GO TO 600

 99    WRITE(6,305)IOS
 305   FORMAT(1X,'OPEN IN WRMSG_RFC FAILED: IOS=',I5)
       CALL EXIT(115)
