      SUBROUTINE INT910(KFILDI,KFILDO,KFILIO,IP,
...
C
C SUBPROGRAM: INT910
C   PRGMMR: GLAHN          ORG: W/OSD21          DATE: 1999-07-01
C
C ABSTRACT: INT910 PERFORMS MUCH OF THE INITIALIZATION FOR U910.
C
C PROGRAM HISTORY LOG:
C   99-07-01  GLAHN
C   00-05-17  DREWRY  ADDED NCEP DOCBLOCK. CHANGED CODE SO THAT
C                     THE DATE IS READ FROM THE NCEP DATE FILE WITH
C                     A CALL TO GET_NCEPDATE. ADDED CALLS TO W3TAGE.
C   00-06-30  MCE     SUBSTITUTED ITEMP FOR IDUM IN RDSNAM CALL 
C                     BEFORE LINE 143. 
C   04-12-06  RLC     CHANGED DICTIONARY READERS FROM GROUPED
...
C   09-02-05  RLC     CHANGED DO LOOP 178 TO LOOP 1,NUMRA INSTEAD OF
C                     1,5.  ON CIRRUS THE 1,5 WAS CAUSING PROBLEMS
C                     BECAUSE IT LOOPED INTO UNINITIALIZED PARTS OF 
C                     THE ARRAY.
C
C USAGE:  CALLED BY U910
C
C        DATA SET USE
C        INPUT FILES:
...
C                        THE COLLATED DATA (N=1,ND4).  ISCALD COMES FROM
C                        THE VARIABLE CONSTANT FILE, MODIFIED TO BE 2 FOR
C                        GRID BINARIES, AND 0 FOR BINARIES.  ZERO WHEN 
C                        NOT FOUND IN THE FILE.  NO BINARY SCALING IS 
...
C
      ...
      CHARACTER*60 DIRNAM(2),PRENAM,CONNAM,DATNAM,OUTNAM,CFILX
      CHARACTER*72 RUNID/' '/
C
...
C     
      STATE='105 '
C     OPEN(UNIT=KFILDI,FILE='U910.CN',STATUS='OLD',IOSTAT=IOS,ERR=900)
C
...
C
 ...
 111  CONTINUE
C
 112  CALL TIMPR(IP(J),IP(J),'START U910          ')
 113  CONTINUE
C
...
C
     ...
     2     NPRINT,ICHARS,LNGTH
 125  FORMAT(10(I10/),I10)
      GO TO 1255
...
C
 ...
 1258    FORMAT(/' ****LNGTH =',I3,' TOO SMALL, SET TO 25')
      ...
      ENDIF
C
      WRITE(KFILDO,127)KSKIP,KWRITE,JSTOP,INCCYL,NEW,NALPH,
     1      NREPLA,NCHECK,NPRINT,ICHARS,LNGTH,L3264B
...
C
 ...
 131     FORMAT(/' ****ERROR: CAN NOT READ NCEP DATE FILE - ',
     1           'CATASTROPHIC ERROR IN INT910. STOP AT 131.')
         CALL W3TAGE('INT910')
         STOP 131
      ENDIF
      NDATES=1
...
C     IF(IER.NE.0)THEN
C        WRITE(KFILDO,134)
C134     FORMAT(/' ****ERROR IN DATE LIST.  STOP IN U910 AT 134.')
C        CALL W3TAGE('INT910')
C        STOP 134
C      ENDIF
C
...
C           THE ABOVE PRINT IS FOR THE EMPTY SET.
	...
	print *, 'numra,kfilra', numra, kfilra
C
...
C
      CALL RDVR79(KFILDO,IP(6),IP(7),IP(8),IP(9),KFILP,KFILCP,
     1            ID,IDPARS,TRESHL,TRESHU,JD,JP,PLAIN,
     2            ITAU,IWDTH,IPREC,CFMT,ISCALD,HEAD,NCAT,ND4,
     3            NVRBL,ISTOP,IER)
C
D     WRITE(KFILDO,1575)(HEAD(1:30,J),J=1,NVRBL)
D1575 FORMAT(/' INT910, HEAD(J)'/(30A1))
      IF(IER.EQ.42)THEN
         WRITE(KFILDO,158)
 158     FORMAT(/' ****NO VARIABLE LIST PROVIDED.',
     1           '  STOP IN INT910 AT 158.')
         CALL W3TAGE('INT910')
         STOP 158
C
      ENDIF
...
C 
 ...
 159  FORMAT(/' ****PROGRAM STOP AT 159 IN INT910 BECAUSE OF ERROR IN',
...
C
 ...
 165  FORMAT(/' ****INCONSISTENCY IN INPUT UNIT NUMBERS IN KFILRA( )',
     ...
     2        ' KFILCP, KFILIO, OR KFILDO'/'     STOP IN INT910 AT 165')
      CALL W3TAGE('INT910')
      STOP 165
C
 170  CONTINUE
...
C     IF(KFILIO.EQ.0)GO TO 180
 ...
 175  FORMAT(/' ****INCONSISTENCY OF INPUT UNIT NUMBER FOR KFILIO',
     1        ' WITH EITHER KFILDT, KFILD( ), KFILP,',
     2        ' OR KFILCP'/'     STOP IN INT910 AT 175')
      CALL W3TAGE('INT910')
      STOP 175
C
...
C        ERROR STOP BELOW IS FOR ERRORS OF CONTROL INFORMATION INPUT.
 900  CALL IERX(KFILDO,KFILDO,IOS,'INT910',STATE)
      CALL W3TAGE('INT910')
      STOP 9999
      END
