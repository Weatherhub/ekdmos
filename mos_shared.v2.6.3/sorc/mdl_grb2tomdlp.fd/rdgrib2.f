...
C   APRIL    2014   ENGLE     MDL   CODE NOW USES THE GRIB2 PRODUCT DISCIPLINE
C                                   READ FROM ELF AND STORED IN ID68(2,JJ).
C                                   THE VALUE IS PUT INTO JDISC AND PASSED INTO
C                                   GETGB2; ADDED JDISC TO COMMENTS. THE PRODUCT
C                                   DISCIPLINE, ALONG WITH THE PDS ARE USED TO FIND
C                                   A GRIB2 MESSAGE
C  JANUARY   2015   ENGLE     MDL   CHANGED RANK OF IC ARRAY FROM 2D TO 1D. ITS IS
C                                   ALLOCATED BY NX*NY.
C
C DATA SET USE:
C         INPUT FILES:
...
C                         (J=1,NX;K=1,NY).
C                 IC(K) = WORK ARRAY FOR 'PACK2D' (DYNAMICALLY ALLOCATED ARRAY)
C                         (K=1,NX*NY).
...
C                         FILE.  (INTERNAL)
C                 JDISC = HOLDS THE GRIB2 PRODUCT DISCIPLINE READ FROM ELF INTO
C                         ID68(2,K). THIS VARIABLE IS PASSED INTO GETGB2.
C                   IER = STATUS RETURN. 
...
C
               IF(ID68(2,JJ).EQ.-9999)THEN
C                    SET TO A VALUE OF -1 (ANY DISCIPLINE).
                  JDISC=-1
               ELSE
C                    SET TO A VALUE HAS SPECIFIED IN ELF.
                  JDISC=ID68(2,JJ)
               ENDIF
C
