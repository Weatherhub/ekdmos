      SUBROUTINE SCLQ12(KFILDO,KFIL10,NDATE,ID,IDPARS,JD,
...
C        JUNE      2007   SMB     CHANGED FLOATS TO REAL.  ADDED
...
C            WAS WRITTEN FOR 12-H QPF, AND ITABLE( , , ) AND TABLE( , )
...
C                       SCLQ12 CAN BE USED FOR EITHER VECTOR DATA
...
C
      DATA ITABLE/        0,0,0,        0,
     1            203330100,0,0,950052000,
     2            203330100,0,0,950051000,
     3            203330100,0,0,245000000,
     4            203330100,0,0,495000000,
     5            203330100,0,0,995000000,
     6            203330100,0,0,199501000,
     7            203332000,0,0,        0,
     8                    0,0,0,        0,
     A            213330100,0,0,950052000,
     B            213330100,0,0,950051000,
     C            213330100,0,0,245000000,
     D            213330100,0,0,495000000,
     E            213330100,0,0,995000000,
     G            213330100,0,0,199501000,
     H            213332000,0,0,        0/
      DATA TABLE/          0., .000,
     1                    .01, .095,
     2                    .10, .245,
...
CD100  FORMAT(/' AT 100 IN SCLQ12--NX,NY',2I6)
C
...
CD101  FORMAT(/' AT 101 IN SCLQ12--NCAT,',
CD    1             '(((ITABLE(I,J,L),I=1,4),J=1,NCAT),L=1,2)',/,
CD    2         I6,/,(4I11))
CD     WRITE(KFILDO,102)((TABLE(I,J),I=1,2),J=1,NCAT-1)
CD102  FORMAT(/' AT 102 IN SCLQ12--',
...
C
 ...
 110  FORMAT(/' ****VARIABLE ',I9.9,I10.9,I10.9,I4.3,' NOT',
     1        ' ACCOMMODATED IN SUBROUTINE SCLQ12.  IER =',I3)
      IER=103
      GO TO 900
C
 ...
 112     FORMAT(/,' ****NCAT =',I3,' DOES NOT EQUAL IDCAT-1 IN SCLQ12.',
...
CD1135 FORMAT(' AT 1135 IN SCLQ12--NVAL,FACTOR,(XDATA(M),M=1,NVAL)',
...
C
 ...
 120     FORMAT(/' ****COULD NOT FIND PROBABILITY RECORD',
     1           3I10.9,I10,'.  FATAL ERROR IN SCLQ12 AT 120.')
         GO TO 900
      ENDIF
C
...
C 
 ...
 146           FORMAT(/' ****MAP PROJECTION NUMBER NPROJ =',I3,
     1           ' NOT 3, 5, OR 7.  FATAL ERROR IN SCLQ12 AT',
     2           ' 146.')
         ISTOP(1)=ISTOP(1)+1
         IER=777
         GO TO 900
...
C
 ...
 148           FORMAT(/' ****NX AND NXI =,',2I6,' OR NY AND NYI =',2I6,
     1           ' DO NOT AGREE AT 148 IN SCLQ12.  FATAL ERROR.')
         ISTOP(1)=ISTOP(1)+1
         IER=777
         GO TO 900
      ENDIF
...
CD149        FORMAT(/' AT 149 IN SCLQ12--MESHI,MESH,NXI,NYI,NX,NY',
CD    1        6I6)
C
...
CD1495    FORMAT(' IN SCLQ12 AT 1495--XDATA(K),DATA(K),XMAX,XMIN',
...
CD160  FORMAT(/,' IN SCLQ12 AT 160--XMAX,XMIN,RANGE,',
...
CD225  FORMAT(/,' IN SCLQ12 AT 225',/,(15F8.2))
C
 900  RETURN
      END
