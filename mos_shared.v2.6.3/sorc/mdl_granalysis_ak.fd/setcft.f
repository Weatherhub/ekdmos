      SUBROUTINE SETCFT(KFILDO,P,DSAV,NX,NY,
...
C            IN XDATA( ).  ITABLE IS FOR CEILING HEIGHT.
C
C        DATA SET USE
...
C                       THE GRID IN P( , ).  (INPUT)
C            XDATA(K) = CATEGORICAL CEILING HEIGHT FORECASTS (K=1,NVAL).
C                       (INPUT)
...
C
     ...
     7           120.0,999.0/
C
CD     CALL TIMPR(KFILDO,KFILDO,'START SETCFT        ')
      IER=0
      ICOUNT=0
C
C        INITIALIZE DSAV( , ).
...
CD    1                ' CLOSEST GRIDPOINT VALUE OF 'F8.2,'  TO ',F7.1,
CD    2                ' PLUS 0.1')
CD           ELSE
...
C              PACKING TO TENTHS OF HUNDREDS, THE UNITS IN P( , ).
            ICOUNT=ICOUNT+1
C
         ELSEIF(P(IX,JY).GT.TABLE(2,NCAT))THEN
C         
CD           IF(DSAV(IX,JY).GT.9998.)THEN         
CD              WRITE(KFILDO,127)CCALL(K),NAME(K),NCAT,P(IX,JY),
CD    1                          TABLE(2,NCAT)
CD127           FORMAT(' STATION 'A8,1X,A20,' OF',I3, ' ADJUSTED',
CD    2                ' CLOSEST GRIDPOINT VALUE OF 'F8.2,'  TO ',F7.1,
CD    3                ' MINUS 0.1')
CD           ELSE
...
C
 ...
 205     FORMAT(/,I6,' VALUES IN THE CEILING HEIGHT GRID MODIFIED',
