...
C                                 FOR ALL CALLS TO PACKGR.
C        DECEMBER  2012   HUNTEMANN  ADDED LOGIC FOR FGPOST FOR AK POPO3
C
C
C        PURPOSE
...
C                       FURNISH TO CAKSNO.  EQUIVALENCED TO IPLANT.
C                       (INTERNAL)
...
C
 ...
 2886    FORMAT('     FATAL ERROR IN FSTGS5')
      ...
      ENDIF
C
C     IF ICOMPT=2 WAS USED TO LOAD A FIRST GUESS, SKIP TO 3465 AFTER
C     THE FIRST GUESS FIELD IS LOADED IN.
      IF(ICOMPT(M).EQ.2)GO TO 3465
C
...
C              ONLY ONE COMPUTATION ROUTINE IS ALLOWED HERE.
C
         ELSEIF(POSTAR(NN).EQ.'FGPOST')THEN
C
C              FGPOST IS NOT ACTUALLY A ROUTINE, BUT A KEYWORD
C              USED TO ALLOW POST-PROCESSORS TO WORK DIRECTLY
C              ON A FIRST GUESS GRID.  SKIPS DIRECTLY TO THE
C              ANALYZED GRID POST-PROCESSING CALLS AFTER ADJUSTING
C              MESH, NX, AND NY.
C
            MESH=MESHB
            NX=NXL
...
C           TO KEEP DIAGNOSTIC BELOW FROM OCCURRING.
         ELSEIF(POSTAR(NN).EQ.'FGPOST')THEN
C           THE POSTPROCESSING KEYWORD "FGPOST" HAS BEEN USED
C           TO ALLOW DIRECT POST-PROCESSING OF THE FIRST GUESS
C           GRID.  THIS TEST IS HERE TO KEEP DIAGNOSTIC BELOW
C           FROM OCCURRING.
         ELSE
C
...
C
         CALL PACKGR(KFILDO,KFILRA(JJ),RACESS(JJ),ID(1,M),IDPARS(1,M),
