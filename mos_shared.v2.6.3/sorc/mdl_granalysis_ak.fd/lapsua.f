...
C        OCTOBER   2009   GAW     ADDED MODNUM, ND6 TO CALL FROM U405A.
...
C*********************************************************************
     ...
     B             500/
C
CD     CALL TIMPR(KFILDO,KFILDO,'START LAPSUA        ')
      IER=0
C
C        SAVE IDPARS(12).
C
...
C
      DO 156 J=1,JDIM
C
CD     IF(CCALL(K).EQ.'PAKP   ')THEN
CD        WRITE(KFILDO,155)K,CCALL(K),NAME(K),J,HDATA(K,J),ELEV(K),
CD    1                    VDATA(K,N2,J),AVXDAT(K)
CD155     FORMAT(/' AT 155 IN LAPSUA--K,CCALL(K),NAME(K),J,HDATA(K,J),'
CD    1           'ELEV(K),VDATA(K,N2,J),AVXDAT(K)',/,
CD    2           I5,2X,A8,2X,A20,I3,4F8.1)
CD     ENDIF
C
...
CD    1                    VDATA(K,N2,J),AVXDAT(K),ELEVHI(K),HHIGH,VHIGH
CD1575    FORMAT(/' AT 1575 IN LAPSUA--K,CCALL(K),NAME(K),J,HDATA(K,J),'
CD    1           'ELEV(K),VDATA(K,N2,J),AVXDAT(K),ELEVHI(K),HHIGH,VHIGH'
CD    1           ,/,I5,2X,A8,2X,A20,I3,7F8.1)
CD     ENDIF
C
...
CD    1       'ELEVLO(K),ELEVHI(K),',
CD    2       'VHIGH,VLOW,HHIGH,HLOW,XLAPSE(K)',/,
CD    3       I6,2X,A8,2F7.1,2F6.0,4F7.1,F8.4)
C
 200  CONTINUE
C
