      SUBROUTINE RDSTR7(KFILDO,KFIL10,KFILIN,MODNUM,NAMIN,JFOPEN,
...
C                                 ADAPTED FROM RDSTR6 FOR U155
C        JULY      2004   GLAHN   CHANGED LE TO GE ABOVE 1085
C        JULY      2004   GLAHN   ADDED 2 TO MINMOD HOURS TO SAVE
C        OCTOBER   2004   GLAHN   ADDED PRINT OF LSTORE AT 4018
C        OCTOBER   2004   GLAHN   MODIFIED FOR DATES TO SAVE; ELIMINATED
C                                 MODELX
C        FEBRUARY  2007   GLAHN   SWITCHED FILES WHEN TOO MANY READING
C                                 ERRORS AT 1103
C        JUNE      2008   GLAHN   ADDED NCEPNO TO CALL AND IN NCEPNO
C                                 VICE IDPARS(4,N) IN TEST
...
C           LSTORE( , ).  RDSTR7 PERFORMS ABOUT THE SAME PURPOSE FOR 
C           ALL DAYS AFTER DAY 1 THAT RDSTR1 PERFORMS FOR DAY 1.
C           VECTOR DATA ARE UNPACKED AND ORDERED BY STATION;
C           GRIDPOINT DATA ARE STORED PACKED
C   
C        DATA SET USE
...
C                       RDSTR7 IS NOT EXACTLY LIKE THAT IN U600 AND 
...
C                       FIELDS READ FOR THIS DAY WILL BE PRINTED TO UNIT
C                       IP10 ALONG WITH THE FIRST AND LAST DATE/TIME
...
C                             DIRCMP VIA GRCOMB).
C                       127 = ALL INPUT DATA EXHAUSTED.
...
C              ICOUNT = COUNT OF FILES NOT NEEDED AND CLOSED.
C                       (INTERNAL)
...
C              IFIRST = COUNTS TIMES WRITING OF DATES NEEDED. 
C                       (INTERNAL)
C              IFRTOT = TOTAL TIMES WRITING OF DATES NEEDED WILL BE
C                       DONE.  (INTERNAL)
C        1         2         3         4         5         6         7 X
C
...
C
      IER=0
CD     WRITE(KFILDO,100)NDATE,NUMIN,(JFOPEN(IN),IN=1,NUMIN)  
CD100  FORMAT(/' IN RDSTR7 AT 100--NDATE,NUMIN,(JFOPEN(IN),IN=1,NUMIN)',
CD    1        I12,(50I2))      
C
...
C
         ...
         ENDIF
C
C           THIS SAVES A MINIMUM OF MINVEC HOURS OF VECTOR DATA
C           FROM ANY SOURCE.  A ZERO DD WILL BE PRESENT IN ID(1, )
C           FOR OBS.  PRESUMABLY, RR WILL NOT PLAY A ROLE HERE,
C           ALTHOUGH THE ID COULD CONTAIN A NON-ZERO VALUE.
C  
      ELSE
...
C        DATE/TIME.
 105  CONTINUE
C
C        THE BEGINNING AND ENDING DATES NEEDED FOR EACH MODEL WILL
C        BE PRINTED A MAXIMUM OF IFRTOT TIMES.
C
      ICOUNT=0
C
      DO 108 IN=1,NUMIN
C
         IF(IFIRST.LT.IFRTOT*NUMIN)THEN
C
            IF(JFOPEN(IN).EQ.0)GO TO 108
C
            IF(ICOUNT.EQ.0)THEN
C
               IF(IP10.NE.0)THEN
                  WRITE(IP10,106)NDATE
 106              FORMAT(/' BEGINNING AND ENDING DATES FOR EACH MODEL',
     1                    ' FOR DATE',I11)
               ENDIF
C
               ICOUNT=ICOUNT+1
...
C        
            IF(IP10.NE.0)THEN
               WRITE(IP10,107)MODNUM(IN),KFILIN(IN),LDATB(IN),
     1                        LDATE(IN)
 107           FORMAT('    MODEL NO.',I3,'  ON UNIT NO.',I3,2I12)
               IFIRST=IFIRST+1
            ENDIF
C
         ENDIF
C
 108  CONTINUE
C
      IF(IFIRST.EQ.IFRTOT*NUMIN.AND.IP10.NE.0)THEN
         WRITE(IP10,1085)
 1085    FORMAT('    THIS DIAGNOSTIC WILL NOT PRINT AGAIN.')
         IFIRST=IFIRST+100
C           IFIRST INCREASED MORE THAN 1 TO KEEP FROM PRINTING AGAIN.
      ENDIF
C
...
C
 ...
 1102 FORMAT(/' ****ERROR READING PACKED RECORD ON UNIT NO.',I3,
     1        ' PROCESSING DATE',I11,' IN RDSTR7 AT 1102, IOSTAT =',I5/
     2        '     FILE = ',A60)
      ISTOP=ISTOP+1
      LSTOPC=LSTOPC+1
...
C        THIS CHECK IS TO STOP AN INFINITE LOOP THAT MIGHT OCCUR.
 ...
 1103 FORMAT('     A TOTAL OF',I6,' READING ERRORS HAVE OCCURRED.',
     1       '  SWITCHING FILES IN RDSTR7 AT 1103.')
      IER=138
      LSTOPC=0
C        THIS ALLOWS ROUTINE TO PROCEED AND SWITCH RECORDS.
      GO TO 1108
C
...
C
 ...
 1105    FORMAT(/' ****ERROR IN RDSTR7 AT 1105.',
...
C        CHANGE MADE HERE 8/19/01 FROM GO TO 400 TO GO TO 800      
C        ISTOP INCREMENTED IN SWITCH ON ERROR.
      IF(IRD.EQ.0)GO TO 400
C        IRD = 1 WHEN DIRECTORY RECORD MUST BE READ.
...
C
 ...
 1111    FORMAT(/' ****ERROR CLOSING FILE ON UNIT NO.',I3,
     1           ' PROCESSING DATE',I11,' IN RDSTR7 AT 1111,',
     2           ' IOSTAT =',I5/
     3           '     FILE = ',A60)
         ISTOP=ISTOP+1
 1114    LKHERE(IN+1)=0
...
CD     WRITE(KFILDO,1156)KFILIN(IN),NDATE
CD1156 FORMAT(' BACKSPACING INPUT FILE ON UNIT NO.',I3,
CD    1       ' PROCESSING DATE',I11,' IN RDSTR7 AT 1156')
         GO TO 400
      ENDIF
C
...
C
C***            WRITE(KFILDO,1158)(IPACK(JJJ),JJJ=6,9),JCYL
C*** 1158       FORMAT(' AT 1158 IN RDSTR7--(IPACK(JJJ),JJJ=6,9),JCYL',
C***     1            4I10,I16)
C
     ...
     4         JCYL.EQ.MSTORE(6,M))THEN
               N=M
C
               IF(INDEX(N).EQ.IDATE)THEN
                  WRITE(KFILDO,1159)(MSTORE(J,M),J=1,4),NDATE
 1159             FORMAT(/' ****VARIABLE ',I9.9,1X,I9.9,1X,I9.9,1X,
     1                    I10.3,' ENCOUNTERED ON INPUT MORE THAN ONCE',
     2                    ' FOR DATE',I11)
                  GO TO 116
               ENDIF
C
...
C           AN ERROR IN GRCOMB WILL HAVE CREATED A DIAGNOSTIC. 
         GO TO 800
C           IER NE 0 IS TREATED AS FATAL IN RDSTR7 WITH RETURN TO CALLING
...
C
 ...
 1182    FORMAT(/' ****ND5 =',I6,' TOO SMALL FOR DATA ARRAY',
     1           ' IN RDSTR7 AT 1182.  INCREASE TO GE',I6/
...
C
 ...
 397  FORMAT(/' ****ERROR IN UNPKBG IN RDSTR7.  IER =',I4,
     1        ', PROCESSING DATE',I11)
      ISTOP=ISTOP+1
...
C
 ...
 401     FORMAT(/' NO DATA AVAILABLE FOR READING FOR FILE ON',
     1           ' UNIT NO.',I4,' PROCESSING DATE',I11,
     2           '.  PROBABLY THE DATA WERE PREVIOUSLY READ.'/
     2           '     FILE = ',A60)
...
C
      IF(IP10.NE.0)THEN
         WRITE(IP10,4018)LITEMS,NDATE,((LSTORE(J,K),J=1,12),K=1,LITEMS)
 4018    FORMAT(/' ',I4,' FIELDS IN LSTORE FOR NDATE  ',I10,/,
