      SUBROUTINE BPROB(KFILDO,KFILX,NSTA,CCALL,RDATA,KDATA,ID,LPROJ,
...
C   92-07-15  GILBERT
C   01-01-26  WEISS   REVISED FOR MODIFIED BUFR MOS. GTMOS IS 
...
C
 ...
 95       FORMAT('READ_MOSDA ERROR: DATE OR UNIT# INCORRECT',
     *           ' IER=',I3,/,
     *           'BGENER: PARAMETER ID = ',4I10)
          GO TO 250
        ENDIF
...
C        DATA VALUE                                                     
        DO 200 I=1,NSTA
          IRDATA=NINT(RDATA(I))
	  IF (IRDATA.NE.9999) THEN
	    IF (IRDATA.EQ.9997) RDATA(I)=0.0
