...
C   06-04-13  SMB        ADDED LOGIC TO CALL NEW SUBROUTINE CAT2
C                        TO DO PARTIAL INFLATION OF FORECASTS, I.E.,
C                        INFLATE ONLY WHEN THE FORECAST VALUE EXCEEDS
C                        THE AVERAGE VALUE.
C
...
C                       ITAU,IUSED,CONST,AVG,CORR,COEF,ND2,ND3,IDTAND,IDTPAR,
...
D1010 FORMAT(' IN FCST90 AT 101,L,KK,NN,K,CONST(L,NN)',
D    1       4I6,F10.4)
 102  CONTINUE
C
 103  CONTINUE
...
C
         IF(NWORDS.NE.NSTA)THEN
            WRITE(KFILDO,131)NWORDS,NSTA
 131        FORMAT(/' ****ERROR IN FCST90 RETURNING FROM GFETCH.',
