      SUBROUTINE CKWSPDGR(KFILDO,KFIL10,
...
C
C        PURPOSE
C            TO ENSURE GRIDDED WIND SPEED FORECASTS ARE SET TO CALM
C            WHEN THE SPEED IS LESS THAN 1/10 KNOT, OR IF THE DIRECTION
C            HAS BEEN SET TO CALM IN CMPWDRGR.
C
C            THIS ROUTINE WAS WRITTEN AS PART OF THE GRIDDED MOS PROJECT AND
C            IS INTENDED TO BE USED IN U202.
C
C            IF THE DATUM = 9999., IT IS LEFT UNCHANGED.  OUTPUT DATA VALUES 
C            ARE ALSO SET TO 9999 IF THE SPECIFICATIONS OF THE INPUT GRIDS ARE 
C            INCONSISTENT. THIS CODE WILL ACCOMODATE TDLPACKED SEQUENTIAL AND
C            RANDOM ACCESS FILES AND HENCE SHOULD WORK FOR OPERATIONAL USE.
C
C            THE FOLLOWING IDPARS(1) AND IDPARS(2) ARE ACCOMMODATED:
C
C                224 370 - GRIDDED WIND SPEED (KTS) CHECKED FOR CALM WINDS
C
C        HISTORY:   
C        03/21/06    COSGROVE     NEW SUBROUTINE                  
C        01/08/08    WAGNER       MODIFIED IF TEST IN 450 LOOP TO SET
C                                 DATA TO MISSING IN CASE WHERE BOTH DIR
C                                 AND SPEED ARE MISSING.
C
C        DATA SET USE
...
C                       GRIDDED WIND DIRECTION BY EITHER
C                       GFETCH OR CONST1. (INTERNAL)
...
C        WIND SPEED.
C
      IF((IDPARS(1).NE.224).AND.(IDPARS(2).NE.370))THEN
C
      WRITE(KFILDO,110)(JD(L),L=1,4)
 110  FORMAT(/,' ****CKWSPDGR ENTERED FOR VARIABLE',
     ...
     2       ' NOT ACCOMMODATED.')
      ...
      ENDIF
C
C        FILL LD( ) AND LDPARS( ) AND GET THE ANALYZED  
C        WIND SPEED IN FD1
C
        LD(1)= 224 * 1000000 + 360 * 1000 + IDPARS(4)
        LD(2)=JD(2)
        LD(3)=JD(3)
...
C
 140    MSLAB(1)=NSLAB
        ...
        NY(1)=IS2(4)
C
C        FILL LD1( ) AND LDPARS1( ) AND GET THE  
C        COMPUTED WIND DIRECTION IN FD2
C
        LD1(1)= 224 * 1000000 + 250 * 1000 + IDPARS(4)
        LD1(2)=JD(2)
        LD1(3)=JD(3)
