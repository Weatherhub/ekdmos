      SUBROUTINE CKGPOP(KFILDO,KFIL10,
...
C
C        MAY      2007   MALONEY  MDL MOS-2000. (FROM CKWNDGST)
C        MARCH    2010   MALONEY  DISABLED 6-H CHECK FOR PROJECTIONS BEYOND F192.
C
C        PURPOSE
C            TO CHECK AN INPUT GRID OF 12-HR POP (223 330) AGAINST THE CORRESPONDING
C            PAIR OF 6-H POPS (223 250) TO ENSURE STATISTICAL CONSISTENCY:
C       
C               12-H POP >= LARGER OF TWO 6-H POPS
C               12-H POP <= SUM OF TWO 6-H POPS
C               0 <= 12-H POP <= 1.
C
...
C
C                223 350 - GRIDDED 12-H POPS.
C
C        HISTORY:   
C        2007/05/08      MALONEY	NEW SUBROUTINE (FROM CKWNDGST)
C
C        DATA SET USE
...
C                       (INPUT)
C             DATA( ) = THE GRIDDED CONSISTENCY-CHECKED AND BOUNDED 12-H POP.
C                       (OUTPUT)
...
C                       GRIDDED 12-H POPS BY EITHER GFETCH OR CONST1. (INTERNAL)
C              FD2( ) = WORK ARRAY (J=1,ND2X3) USED TO FETCH THE 
C                       GRIDDED 6-H POPS BY EITHER GFETCH OR CONST1. (INTERNAL)
C              FD3( ) = WORK ARRAY (J=1,ND2X3) USED TO FETCH THE 
C                       GRIDDED 6-H POPS BY EITHER GFETCH OR CONST1. (INTERNAL)
...
C        DATA ID TABLE
C                 OUT     IN      IN    
C                 12hPOP  12hPoP  6hPoPs
      DATA ITABLE/223350, 223330, 223250/
C
C
      IER=0
      ISTAV=0
...
C
 ...
 110  FORMAT(/,' ****CKGPOP ENTERED FOR VARIABLE',
     1         2X,I9.9,1X,I9.9,1X,I9.9,1X,I10.3,
     2       ' NOT ACCOMMODATED.')
      IER=102
      GO TO 800
C
C        FILL LD( ) AND LDPARS( ) AND GET THE 12-H POP AND
C        THE FIRST 6-H POP
C
 120  DO 150 II=2,3
         LD(1)=ITABLE(II,JJ)*1000+IDPARS(3)*100+IDPARS(4)
         LD(2)=JD(2)
         LD(3)=JD(3)
CCC         LD(4)=IDPARS(13)*100+IDPARS(14)*10+IDPARS(15)
         LD(4)=ID(4)
...
C           COPY FD2 TO FD1.
C           IF TAU > 192, LOAD DATA DIRECTLY TO DATA( ) ARRAY AND 
C           EXIT CODE.
C
         IF((II-1).EQ.1) THEN
            IF (IDPARS(12).GT.192) THEN
               DO I=1,ND2X3
                  DATA(I)=FD2(I)
               ENDDO
               GOTO 900
 ...
 150  CONTINUE
C
C        NOW FETCH THE 6-H POP FROM 6 HOURS PRIOR.  
C
...
C
     ...
     2            NAME,STALAT,STALON,ISDATA,SDATA,DIR,
     3            ND1,NSTA,NGRIDC,NGRID,ND11,NSLAB,
     4            IPACK,IWORK,FD3,ND5,LSTORE,ND9,LITEMS,CORE,
     5            ND10,LASTL,NBLOCK,LASTD,NSTORE,NFETCH,
     6            IS0,IS1,IS2,IS4,ND7,
     7            ISTAV,L3264B,L3264W,IER)
C
...
C
      IF ((MSLAB(1).NE.MSLAB(2)).OR.(MSLAB(2).NE.MSLAB(3)).OR.
     1    (MSLAB(1).NE.MSLAB(3))) THEN
         WRITE(KFILDO,200)
         WRITE(KFILDO,201)
         GOTO 800
      ENDIF
      IF ((NX(1).NE.NX(2)).OR.(NX(2).NE.NX(3)).OR.(NX(1).NE.NX(3)).OR.
     1    (NY(1).NE.NY(2)).OR.(NY(2).NE.NY(3)).OR.(NY(1).NE.NY(3))) THEN
         WRITE(KFILDO,200)
         WRITE(KFILDO,202)
         GOTO 800
      ENDIF
 200  FORMAT(/,' ****MISMATCH FOUND IN INPUT 12-H POP ',
     1       'AND 6-H POP GRIDS:')
 201  FORMAT('  NSLAB VALUES NOT EQUAL')
 202  FORMAT('  GRID DIMENSIONS NOT EQUIVALENT')
C
C        GRIDS ARE IDENTICAL. NOW LOAD DATA ARRAY;
C        ENFORCE 6-H AND 12-H POP CONSISTENCY AND BOUNDS.
C
 400  DO 450 J=1,NX(1)*NY(1)
         DATA(J)=FD1(J)
         IF(NINT(FD1(J)).EQ.9999.OR.NINT(FD2(J)).EQ.9999) GOTO 450
C
C               IF 12-H POP < THE GREATER OF THE TWO 6-H POPS, SET
C               12-H POP = THE MAX 6-H POP
C
         IF(FD1(J).LT.MAX(FD2(J),FD3(J))) THEN
            DATA(J)=MAX(FD2(J),FD3(J))
C           write(77,404) FD1(J),FD2(J),FD3(J),DATA(J)
C404        format(/,' %%%%12H POP WAS ',F5.3,' 6H POPS WERE ',F5.3,1X,
C    +             F5.3,' NOW 12H POP IS ',F5.3)
         ENDIF
C
C               IF 12-H POP > THE SUM OF THE TWO 6-H POPS, SET
C               12-H POP = THE SUM OF THE 6-H POPS
C
         IF(FD1(J).GT.(FD2(J)+FD3(J))) THEN
            DATA(J)=FD2(J)+FD3(J)
C           write(77,414) FD1(J),FD2(J),FD3(J),DATA(J)
C414        format(/,' %%%%12H POP WAS ',F5.3,' 6H POPS WERE ',F5.3,1X,
C    +             F5.3,' NOW 12H POP IS ',F5.3)
         ENDIF
C
C               IF 12-H POP < 0, SET TO 0.
C               IF 12-H POP > 1, SET TO 1.
C
         IF(DATA(J).LT.0) DATA(J)=0.
         IF(DATA(J).GT.1) DATA(J)=1.
C
 450  CONTINUE
      GOTO 900
C
