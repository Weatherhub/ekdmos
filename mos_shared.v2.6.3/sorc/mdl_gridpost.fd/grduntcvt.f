...
C        03-08-11  SHAFER     ADDED ID FOR 2-H CONVECTION
C        11-18-11  SHAFER     ADDED IDS FOR PRECIPITATION TYPE
C                             INCREASED SIZE OF NDIM
C        11-21-11  SHAFER     ADDED ID FOR PTYPE BEST CATEGORY
C                             (CONVERT TO WMO GRIB2 STANDARD)
C                             INCREASED SIZE OF NDIM
C        10-29-13  SHAFER     ADDED ID FOR POPO (CONVERT TO %)
C                             INCREASED SIZE OF NDIM
C
C
C        PURPOSE
...
C              6 - RECODE PTYPE BEST CATEGORY TO WMO STANDARD  
C
...
C                227 564 - GRIDDED TRUNCATED 2-HR CONV IN %
...
C                228 544 - GRIDDED UNCOND PTYPE PRB IN %
C                228 547 - GRIDDED PTYPE BEST CATEGORY (WMO)
C                228 549 - GRIDDED COND PTYPE PRB IN %
C                228 624 - GRIDDED POPO IN %
C
C        DATA SET USE
...
C
 ...
 354      CONTINUE
          GO TO 900
C
         ELSEIF(ITABLE(3,JJ).EQ.6)THEN
C
C        OPTION 6:CONVERT PTYPE BEST CAT TO WMO VALUES
C
          DO 355 J=1,NX*NY
           IF(NINT(DATA(J)).NE.9999) THEN
	      IF(DATA(J).EQ.1.) THEN
	         DATA(J)=3.
	      ELSEIF(DATA(J).EQ.2.) THEN
	         DATA(J)=5.
	      ELSEIF(DATA(J).EQ.3.) THEN
	         DATA(J)=1.
	      ENDIF
           ENDIF
 355      CONTINUE
