...
CCC      PACK THE DATA FROM THE SEQUENTIAL FILE(S) INTO KFILGO.
CCC
CCC      CALL PACKG(KFILDO,KFILGO,ID(1,N),IDPARS(1,N),
CCC  1           ISCALD(N),0,NGRIDG,
CCC  2           IPLAIN(1,1,N),PLAIN(N),IDATE,NYR,NMO,NDA,NHR,
CCC  3           ISDATA,GDATA,ND2X3,NX,NY,IPACK,IWORK,ND5,
CCC  4           MINPK,IS0,IS1,IS2,IS4,ND7,XMISSP,XMISSS,
CCC  5           NWORDS,NTOTBG,NTOTRG,
CCC  6           L3264B,L3264W,ISTOP(1),IER)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C        RANDOM ACCESS GRIDDED PACKING
C
         CALL PACKGR_OPER(KFILDO,KFILRA,RACESS,ID(1,N),IDPARS(1,N),
     1                    ISCALD(N),0,NGRIDG,
     2                    IPLAIN(1,1,N),PLAIN(N),IDATE,NYR,NMO,NDA,NHR,
     3                    ISDATA,GDATA,ND2X3,IS2(3),IS2(4),IPACK,IWORK,
     +                    ND5,
     4                    MINPK,IS0,IS1,IS2,IS4,ND7,
     5                    XMISSP,XMISSS,NWORDS,NTOTBG,NTOTRG,
