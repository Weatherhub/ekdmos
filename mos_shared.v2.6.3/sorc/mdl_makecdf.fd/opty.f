...
C        FEBRUARY  2013   VEENHUIS  ADDED RANKDIST TO COMPUTE CDF
C                                   BY RANK SORTING MEMBERS. 
C
C        PURPOSE 
C            TO CALL VARIOUS COMPUTATIONAL ROUTINES.
C
C        DATA SET USE 
...
C
     ...
     9               L3264B,L3264W,ISTOP,IER)
        ...
        ENDIF
C
      ELSEIF(KER(NN).EQ.5)THEN
         CALL RANKDIST(KFILDO,KFIL10,KFILAO,IP12,IP15,
     1                  KFILRA,RACESS,NUMRA,
     2                  ID,IDPARS,JD,TRESHL,TRESHU,ITAU,NVRBL,
     3                  NDATE,CCALL,ISDATA,SDATA,ND1,NSTA,
     4                  XDATA,SDDATA,ND2,KER,NN,M,
