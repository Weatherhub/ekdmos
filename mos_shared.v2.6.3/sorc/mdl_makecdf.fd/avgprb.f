      SUBROUTINE AVGPRB(KFILDO,KFIL10,KFILAO,IP12,IP15,
...
C
C        AUGUST    2010   VEENHUIS    MDL   MOS-2000
C                                     NEW SUBROUTINE BASED ON A COMBINATION OF
C                                     DISTF AND WIENDEFIELD'S ORIGINAL AVGPRB
C                                     CODE. THE ORIGINAL AVGPRB CODE WAS A
C                                     U710 SUBROUTINE.  THIS CODE HAS BEEN
C                                     ADAPTED TO WORK WITH U715 AND HANDLE
C                                     DATA FROM MULTIPLE MODELS.
C        NOVEMBER  2010   VEENHUIS    ADDED A CHECK TO MAKE SURE PQPF IS 
C                                     BEING PROCESSED. ADDED ITABLE TO HOLD
C                                     LIST OF ALLOWED IDS.
C        PURPOSE
C            AVGPRB IS CALLED FROM OPTY AND RETURNS THE PROBABILITY FORECAST
C            FROM EACH ENSEMBLE MEMBER IN THE ARRAY XDATA(1:NSTA,1:M). WHERE
C            NSTA IS THE NUMBER OF STATIONS AND M IS THE NUMBER OF ENSEMBLE
C            MEMBERS.
C   
C        DATA SET USE 
...
C        ITABLE IS THE LIST OF IDS THAT CAN BE HANDLED
      DATA ITABLE /2032201,2033301,2034301,2032101/
...
C
        IF(IDFOUND .EQ. 0) THEN
          WRITE(KFILDO,*) '****ERROR: ENTERED AVGPRB FOR VARIALBE',
 ...
 121      FORMAT(/,' ****VRBL ',
     ...
     1     ' AVGPRB WILL ONLY ACCEPT IDS LISTED IN ITABLE'
          WRITE(KFILDO,*) '****CHECK ITABLE IN AVGPRB FOR VALID IDS.'
...
C
  ...
  125       FORMAT(/,' ****VARIABLE NOT RETRIEVED BY RETVEC IN AVGPRB',
...
C      FOR MISSING VALUES AND TRUNCATE THE VALUES TO BE BETWEEN
C      0 AND 1.  THE DATA FOR EACH MEMBER WILL BE RETURNED IN XDATA.
C    
      DO 250 K=1,NSTA
        DO 240 J=1,M-1
...
C
            IF(XDATA(K,J).LT.0) XDATA(K,J)=0.
	    IF(XDATA(K,J).GT.1) XDATA(K,J)=1.
C	    
C            MEMCNT(K)=MEMCNT(K)+1
C            SUMPRB(K)=XDATA(K,J)+SUMPRB(K)
          ELSE
C            SUMPRB(K)=XDATA(K,J)
            XDATA(K,J)=9999
          ENDIF
  240    CONTINUE
  250  CONTINUE
