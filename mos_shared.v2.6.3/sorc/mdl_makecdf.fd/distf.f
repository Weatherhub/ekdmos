...
C                                     FINAL PDF.-A WORK IN PROGRESS.
C        FEBRUARY 2011    VEENHUIS    MODFIED CALL LIST FOR RDSPSK. ADDED
C                                     NEW ARRAYS. CODE NOW WORKS. NEEDS MORE 
C                                     TESTING.
...
C
C     ADDED FOR SPREAD SKILL
C
      CHARACTER*8 CCALLSP(3000)
...
C        ASCII FILL ON KFILAI. IF THE ID CANNOT BE LOCATED THE CODE
...
C
 ...
 100       FORMAT(/,'**** IN SUBROUTINE DISTF: DID NOT FIND SPREAD-',
     ...
     2             '**** THE SPREAD ADJUSTMENT FROM THE ID INPUT',
     3             ' WILL BE USED.')  
           ISTOP=ISTOP+1
         ENDIF
C
C         DO XXX=1,NCALLSP
C           PRINT *,CCALLSP(XXX),XB0(XXX),XB1(XXX),XMIN(XXX),XMAX(XXX)
C         ENDDO
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C
...
C
            IF(LDPARS(8).EQ.4)THEN
               CALL OPTX(KFILDO,KFIL10,IP12,KFILRA,RACESS,NUMRA,
     1               LD,LDPARS,TRESHL(N),JD(1,N),ITAU(N),
     2               NDATE,MDATE,CCALL,ISDATA,SDDATA(1,M),ND1,NCAT,NSTA,
     3               ICALLD,CCALLD,IPACK,IWORK,DATA,ND5,
     4               LSTORE,ND9,LITEMS,CORE,ND10,
     5               LASTL,LASTD,NBLOCK,NSTORE,NFETCH,
     6               IS0,IS1,IS2,IS4,ND7,
...
C
      DO 210 L=1,M
      IF(XDATA(K,L).NE.9999..AND.SDDATA(K,L).NE.9999.)THEN
         MM=MM+1
         RDATA(MM)=XDATA(K,L)
         SDATA(MM)=SDDATA(K,L)
C**D         WRITE(KFILDO,2075)M,K,MM,L
C**D 2075    FORMAT(' AT 2075--M,K,MM,L',4I6)
C      ELSE
C         WRITE(KFILDO,*)"MISSING MEMBER ",L," FOR STATION ",K
      ENDIF
C      
 210  CONTINUE
C
...
C           ONE BELOW WILL EXPLAIN WHICH VARIABLE AND STATION.
      ...
      ENDIF  
C
C        NOW CREATE KERNEL - THIS PART OF CODE WAS ADAPTED FROM KERNEL
C        AND KERNELW
C
      FMAXVAL=-9999999
      FMINVAL=+9999999
...
C
C         MAKE SURE XMEMSP FALLS WITHIN THE VALID RANGE
C
        IF(XMEMSP.LT.XMIN(KK))THEN
          XMEMSP=XMIN(KK)
        ENDIF 
        IF(XMEMSP.GT.XMAX(KK))THEN
          XMEMSP=XMAX(KK)
        ENDIF 
C
        FAC=(XB0(KK)+XB1(KK)*XMEMSP)/XSRAW   
C
      ELSE
C
C       COULD NOT FIND THE STATION/ID COMBO IN THE SPREAD
C       SKILL FILE.  USE THE SPREAD ADJUSTMENT FROM THE 
C       INPUT VARIABLE LIST.
C
...
C
C
C               IF PDF(J-1) IS 0.0, YOU END UP DIVIDING BY 0.
C
         IF(CMED.GE.TOT2.AND.PDF(J-1).GT.0.0)THEN
C         IF(CMED.GE.TOT2)THEN
...
C           (XAXIS(J)-XMED) CAN BE PLUS OR US AND DEPART FROM XMED.
C
 167     CONTINUE
C
      ENDIF
...
C
 ...
 180  CONTINUE
C
D     WRITE(KFILDO,185),TOT,(J,CDF(J),J=1,NPCDF)
D185  FORMAT(/' AT 185 IN KERNEL--TOT,(J,CDF(J),J=1,NPCDF)'
D    1       ,F10.4/,(5(I7,F10.5)))
C
