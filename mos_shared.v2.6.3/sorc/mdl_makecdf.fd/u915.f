...
C
C ABSTRACT: PROGRAM U915 IS USED TO READ DATA FROM A MOS-2000
...
C   10-11-29  VEENHUIS   ADDED ND2 TO CALL TO INT915.
...
C   12-10-10  WAGNER     ADDED CALL TO RDTDLMC TO READ STATION
C                        CALL LETTERS. 
C   13-03-05  VEENHUIS   THIS VERSION DOES NOT PACK THE MEAN AND
C                        STANDARD DEVIATION. THIS WAS DONE TO
C                        ACCOMODATE THE ECMOS ENSEMBLE. ALSO
C                        I REMOVED THE CODE THE READ THE SPREAD-SKILL
C                        FILE INT915. 
C
C USAGE:    CALL U915(KFILDI,KFILDO,ICALL,CCALL,ISDATA,SDATA,NGP,
...
C                       STALON( ) IN INT915.
C         SDDATA(K,L) - USED FOR THE STATDARD ERRORS (K=1,NSTA) (L=1,ND2).
...
C            KFILC    - UNIT NUMBER FOR PREDICTOR CONSTANT FILE.
C                       (INPUT)
...
C           SDDATA(K,1) = THE SD VALUES FOR EACH ENSEMBLE MEMBER FOR
C                         STATION K AND ENSEMBLE MEMBER J (J=1,M).
...
C        IN THE DATA TO BE PACKED.
C
C      CALL SETMIS(KFILDO,XDATA,NSTA,XMISSP,XMISSS)
C
...
C      PLAINT=PLAIN(NS)
C      PLAINT(24:27)='MEAN'
C      CALL PACKV(KFILDO,KFILIO,LD,LDPARS,
C     1           JP(1,NS),ISCALD(NS),0,
C     2           IPLAINT,PLAINT,NDATE,NYR,NMO,NDA,NHR,
C     3           CCALL,ISDATA,XDATA,ND1,NSTA,IPACK,ND5,MINPK,
C     4           IS0,IS1,IS2,IS4,ND7,XMISSP,XMISSS,
C     5           IP(15),LWORDS,NTOTBY,NTOTRC,
C     6           L3264B,L3264W,ISTOP(1),IER)
C      IF(KFILX.NE.0)THEN
C         CALL WRTDLR(KFILDO,KFILX,CFILX,IS1(9),ICALL,CCALL,ND1,NSTA,
C     1               ICALLD,CCALLD,ND5,IPACK,LWORDS,
C     2               NREPLA,NCHECK,L3264B,L3264W,IER) 
C         IF(IER.NE.0)ISTOP(1)=ISTOP(1)+1
C        AN ERROR IN WRTDLR WILL PRINT A DIAGNOSTIC.
C      ENDIF
C
...
C        IN THE DATA TO BE PACKED.
C
C      CALL SETMIS(KFILDO,SDDATA(1,1),NSTA,XMISSP,XMISSS)
C
C        PACK AND WRITE THE DATA.
C
C      CALL PRSID1(KFILDO,LD,LDPARS)
...
C      PLAINT=PLAIN(NS)
C      PLAINT(24:25)='SD'
C      ISCALDT=ISCALD(NS)+1
C        INCREASE THE PACKED PRECISION FOR THE AVERAGE SD'S.
C      CALL PACKV(KFILDO,KFILIO,LD,LDPARS,
C     1           JP(1,NS),ISCALDT,0,
C     2           IPLAINT,PLAINT,NDATE,NYR,NMO,NDA,NHR,
C     3           CCALL,ISDATA,SDDATA,ND1,NSTA,IPACK,ND5,MINPK,
C     4           IS0,IS1,IS2,IS4,ND7,XMISSP,XMISSS,
C     5           IP(15),LWORDS,NTOTBY,NTOTRC,
C     6           L3264B,L3264W,ISTOP(1),IER)
C      IF(KFILX.NE.0)THEN
