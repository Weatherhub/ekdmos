...
C                                 
C        March     2010   FAN     ADDED OpenMP PARALLEL DIRECTIVES FOR
C                                 MULTI-TASK CAPABILITY; MOVED
C                                 WRITE(KFILDO,2228) FROM INSIDE DO 240
C                                 AND RENAMED TO WRITE(KFILDO,1028)
C        NOVEMBER  2011   SCALLION  PUT IN CHECKS FOR DIVISION BY 
C                                   COUNT( , ) = 0. THIS IS A TEMPORARY
C                                   FIX, IN SUPPORT OF 2.5 KM CONUS 
C                                   GMOS INITIAL RFC. ANY FUTURE UPDATES 
C                                   TO THE SYSTEM SHOULD NOT NEED THIS 
C                                   UPDATE, SINCE IT IS ALREADY INCLUDED 
C                                   IN MORE RECENT VERSIONS OF THIS CODE.
C        FEBRUARY  2013   ENGLE     COMMENTED IFIRST=1 TO RESOLVE SLOW RUN-
C                         FAN       TIME USING OPENMP ON WCOSS.
C                         SCALLION
C
C        PURPOSE
...
C
      IF(LNDWAT.LT.0.OR.LNDWAT.GT.3) THEN
         WRITE(KFILDO,1028)LNDWAT
 1028    FORMAT(/' ****LNDWAT =',I4,' NOT EQUAL TO 0, 1, 2, OR 3.',
     1           '   FATAL ERROR AT 1028 IN CORBC5.')
         IER=777
         GO TO 500
      ENDIF
C
...
C
!$OMP PARALLEL DO PRIVATE(JY,JYE,DISTSQ,IX,IXE,WTLW,ELEDIF,IER,
!$OMP&                    BBPLUS,WT,WHT,W,IWINDX,IWINDY,F)
      DO 238 JY=JB,JE
C
      IF(MESH.EQ.MESHE)THEN
         JYE=JY
