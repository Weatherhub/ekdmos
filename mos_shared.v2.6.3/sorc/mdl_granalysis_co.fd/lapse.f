     ...
     6                 IBKPN,LPNO,LAPFG,MGUESS,N4P,ISTOP,IER)
C
...
C                       OPTION USEDS FOR THE 1ST PASS MODIFIED BY
...
C                LLMT = DIAGNOSTIC IS WRITTEN WHEN NUMBER OF PAIRS
C                       USED IS LT LLMT*NORUNS.  SET TO 4 IN DATA
C                       STATEMENT.  XLAPSE( ) SET TO ZERO.  (INTERNAL)
...
C
     ...
     1          XLAPSE(NSTA),XP(NSTA),YP(NSTA),XPL(NSTA),YPL(NSTA)
      ...
      DIMENSION MPAIRS(NSTA)
C        MPAIRS( ) IS AN AUTOMATIC ARRAY.
...
C
C     WRITE(KFILDO,126)K,CCALL(K)
C126  FORMAT(/' IN LAPSE--K,CCALL(K)',I6,2X,A8)
C
C     IF(CCALL(K).EQ.'ZKMEI  '.OR.
C    1   CCALL(K).EQ.'ZKSLC  ')THEN 
C        WRITE(KFILDO,127)
C127     FORMAT(/'   K CALL(K)         NAME(K)',
C    1           '        XDATA(K,L,J)    XPL(K)         YPL(K),',
C    2           '       ELEV(K)         NOPAR(K)',/,   
C    3           '   JJ CCALL(IALOC(JJ)) NAME(IALOC(JJ))',
...
C        IF(CCALL(K).EQ.'ZKMEI  '.OR.
C    1      CCALL(K).EQ.'ZKSLC  ')THEN 
C           WRITE(KFILDO,135)K,CCALL(K),NAME(K),
C    1                 XDATA(K,L,J),XPL(K),YPL(K),
C    2                 ELEV(K),NOPAR(K),
...
C           THE FIRST VARIABLE.
 ...
 207     FORMAT(/,' LISTING OF COMPUTED LAPSE RATE WHEN NON-ZERO.',
     ...
     2           ' STATION NO.  J CALL LETTERS AND NAME      NO. PAIRS',
