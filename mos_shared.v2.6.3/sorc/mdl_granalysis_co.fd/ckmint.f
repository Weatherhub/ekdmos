      SUBROUTINE CKMINT(KFILDO,KFIL10,P,FD6,NX,NY,NAREA,
     ...
     5                  IPACK,IWORK,ND5,
     6                  CORE,ND10,NBLOCK,NFETCH,L3264B,ISTOP,IER)
C
C        APRIL     2008   GLAHN   MDL   MOS-2000
C        AUGUST    2008   GLAHN   ADDED COUNTS AND AVERAGES
C        AUGUST    2008   GLAHN   ADDED DIAGNOSTIC FOR LT -15 CHANGE
...
C                                 SET DIFMAX = 0 WHEN NO CORRECTIONS MADE
C 
C        PURPOSE 
C            TO CHECK A SURFACE NIGHTTIME MINIMUM TEMPERATURE ANALYSIS
C            IN P( , ) WITH UP TO 5 SURFACE TEMPERATURE ANALYSES.
C            THE MIN TEMP AT EVERY GRIDPOINT IS MADE AS LOW AS ANY
...
C
C            CORRECTING MIN TEMPERATURE WITH TEMPERATURE MAY CREATE
C            PROBLEMS.  MIN TEMPERATURE HAS MORE FORECASTS, BUT
...
C            MADE IN DEG F.  THESE WILL BE NEGATIVE.
C
C            CALLED WITH  CCCFFF = 222220 MIN TEMP.
...
C            P(IX,JY) = THE MIN TEMP ANALYSIS TO CHECK (IX=1,NX)
...
C                       3 = HAWAII,
C                       4 = PUERTO RICO.
C               ID(J) = THE 4-WORD ID OF THE VARIABLE MIN TEMP GRID
C                       (J=1,4).  (INPUT)
...
C
      DATA ITABLE/24, 27, 30, 33, 36, 99, 99,
     1            24, 27, 30, 33, 36, 99, 99,
     2            99, 27, 30, 33, 36, 99, 99,
     3            99 ,27, 30, 33, 36, 39, 99,
     4            99, 27, 30, 33, 36, 39, 99,
     5            99, 99, 30, 33, 36, 39, 99,
     6            99, 99, 30, 33, 36, 39, 42,
     7            99, 99, 30, 33, 36, 39, 42,
C
     8            24, 27, 30, 33, 99, 99, 99,
     1            24, 27, 30, 33, 36, 99, 99,
     2            24, 27, 30, 33, 36, 99, 99,
     3            99 ,27, 30, 33, 36, 99, 99,
     4            99, 27, 30, 33, 36, 39, 99,
     5            99, 27, 30, 33, 36, 39, 99,
     6            99, 99, 30, 33, 36, 39, 99,
     7            99, 99, 30, 33, 36, 39, 42/
      DATA IROW/2, 6, 7, 1/
      DATA ICOL/1, 3, 3, 1,
     1          6, 6, 7, 5,
...
C
 ...
 103     FORMAT(/,' ****MESHE =',I4,' NOT EQUAL TO MESH =',I4,
     1            ' IN CKMINT.  DIFFERENCES NOT COMPUTED.')
         ISTOP(1)=ISTOP(1)+1
         GO TO 800
      ENDIF
C
 ...
 104     FORMAT(/,' ****CLIPPING GRID NOT AVAILABLE IN CKNINT.',
...
C
C        CHECK THE CCCFFF = 222220 FOR MIN TEMP.
C
      IF(ID(1)/1000.NE.222220)THEN
         WRITE(KFILDO,105)(ID(J),J=1,4)
 105     FORMAT(/' ****CCCFFF NOT CORRECT IN CKMINT.  ID = ',
     1            3I10.9,I10.3,
     2           '.  CHECKING OF MIN TEMPERATURE GRID ABORTED.')
         GO TO 800
      ENDIF
C
...
C
 ...
 106     FORMAT(/' ****NAREA =',I2,' AND NPROJ =',I2,
     1           ' NOT COMPATIBLE.  ID = ',
     2            3I10.9,I5.3,
     3           '.  CHECKING OF MIN TEMPERATURE GRID ABORTED.')
         GO TO 800
      ENDIF
C
...
C
      ...
      XMESH=XMESH*1000.
C        XMESH IS NEEDED BELOW IN M, NOT KM.
CD
CD     WRITE(KFILDO,107)MESH,XMESH
CD107  FORMAT(/' AT 107 IN CKMINT--MESH,XMESH',I4,F12.5)
C
...
C        3-H TEMP TO CHECK.
      LD(1)=ID(1)-200000
C        THIS MAKES THE MIN TEMP FFF OF 220 = 020 FOR 3-H TEMP
      LD(2)=ID(2)
      LD(3)=(ID(3)/1000)*1000+(M-1)*3+(IDPARS(12)/24)*24-IC
C        ITABLE HAS 3-H PROJECTIONS FOR 42-H MIN.  THEY MUST
C        BE INCREASED BY 24 HOURS EVERY 24-H CYCLE INDICATED
C        BY IDPARS(12) OF THE MIN TEMP.
      LD(4)=ID(4)
      ITIME=0
C
CD     WRITE(KFILDO,110)(LD(J),J=1,4),M
CD110  FORMAT(/,' AT 110 IN CKMINT--(LD(J),J=1,4),M',4I10,I4)
C
...
C 
 ...
 140  CONTINUE
C      
C        CHECK GRIDS.  WHEN 3-H TEMP IS LOWER THAN MIN TEMP, SET MIN
C        TEMP TO TEMPERATURE.  MULTIPLE LOOPS AND SOME HARDWIRING KEEP
C        DOWN COMPLEXITY.
C
...
C
CD        WRITE(KFILDO,144)NAREA,IC,IT,IROW(NAREA),IROW(NAREA)
CD144     FORMAT(' IN CKMINT--NAREA,IC,IT,IROW(NAREA),IROW(NAREA)',
CD    1            5I6)
CD        WRITE(KFILDO,145)ICK,NINT(X1),NINT(X2),NINT(X3)
CD145     FORMAT(' IN CKMINT--ICK,NINT(X1),NINT(X2),NINT(X3)',4I4)
C
...
C
               IF(TEMP(IX,JY,ICK).LT.P(IX,JY))THEN
C
                  IF(CPNDFD(IX,JY).NE.0.)THEN
C                       DIFFERENCES ARE COMPUTED ONLY OVER THE NDFD
C                       GRID.
                     ICOUNT=ICOUNT+1
                     DIF=TEMP(IX,JY,ICK)-P(IX,JY)
                     FD6(IX,JY)=FD6(IX,JY)+DIF*10.
CD                    IF(DIF.LT.-8.)WRITE(KFILDO,146)DIF,IX,JY,ICK
CD146                 FORMAT(/,'****LARGE MIN TEMP CHANGE--',
CD    1                        'DIF,IX,JY,ICK',F6.1,3I6)
                     DIFAVG=DIFAVG+DIF
                     DIFMAX=MIN(DIFMAX,DIF)
C                       THE DIFFERENCES ARE ALWAYS NEGATIVE.
                     P(IX,JY)=TEMP(IX,JY,ICK)
                  ENDIF
C
               ELSE
C
                  IF(CPNDFD(IX,JY).NE.0..AND.FD6(IX,JY).GT.9998.5)THEN
C                       DIFFERENCES ARE RECORDED AS ZERO ONLY OVER
C                       THE NDFD GRID.
                     FD6(IX,JY)=0.
                  ENDIF
C
...
C
 ...
 150     CONTINUE
C
CD        WRITE(KFILDO,151)ICOUNT
CD151     FORMAT(' IN CKMINT--ICOUNT',I7)    
C
...
C         
      ...
      ENDIF
C
      WRITE(KFILDO,255)ICOUNT,IDPARS(12),DIFAVG,DIFMAX
 255  FORMAT(/' MIN TEMP GRID WAS DECREASED BY TEMPERATURE',I8,' TIMES',
     1        ' FOR PROJECTION',I4,':',
     2        '  AVERAGE CORRECTION =',F5.1,';',
     3        '  MAXIMUM CORRECTION =',F5.1)
C
 800  CONTINUE
C
      RETURN
