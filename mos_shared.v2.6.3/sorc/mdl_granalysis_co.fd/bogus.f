...
C           VALUE INTERPOLATED FROM CURRENT ANALYSIS IS NOW IN BB.
...
CD135     FORMAT(/,' AT 135 IN BOGUS--K,CCALL(K),XP(K),YP(K),BB,IER',
...
CD234  FORMAT(/' AT 334--TEMPST,CCALL(K),(TEMP(J),WT(J),J=1,10)',/,
...
CD              ENDIF
C
CD              WRITE(KFILDO,239)TEMP(L),TEMPST
CD239           FORMAT(' MISSING DATA FOR STATION ',A8,
CD    1                ' AVERAGING FOR STATION ',A8)
            ENDIF
C
            ISTART=MIN(N+1,NSTA)
