      SUBROUTINE OSMTH1(KFILDO,P,NX,NY,MESH,
     1                  SEALND,NXE,NYE,MESHE,
     2                  IOCEXT,IOCINC,ISTOP,IER)
C
C        SEPTEMBER 2008   GLAHN   MDL   MOS-2000
C                                 MODIFIED FROM OSMTH
C        PURPOSE
C            TO SMOOTH THE OCEAN POINTS IN A GRID OVER A CIRCLE
C            OF RADIUS 2*IOCEXT.  IT IS ASSUMED P(NX,JY) AND 
...
C                       (OUTPUT)
C                DIST = THE CIRCLE RADIUS FOR SMOOTHING.  (INTERNAL)
C        1         2         3         4         5         6         7 X
C
      DIMENSION P(NX,NY)
...
D103     FORMAT(/' IN OSMTH1--NX,NY,NXE,NYE,MESH,MESHE,IOCEXT,IOCINC',
D    1           8I8)
C
...
C
      DIST=IOCEXT**2+.1
C        DIST IS THE RADIUS OF THE CIRCLE.  THE CONSTANT .1 IS TO ASSURE
C        A FULL CIRCLE.
C      
C        SMOOTH THE POINTS.
C
...
C                 THE POINT IS WATER AND NOT MISSING.
               IF((IX1-IX)**2+(JY1-JY)**2.GT.DIST)GO TO 149
