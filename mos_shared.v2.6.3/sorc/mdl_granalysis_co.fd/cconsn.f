      SUBROUTINE CCONSN(KFILDO,KFILOG,KFILRA,RACESS,NUMRA,
...
C
C        FEBRUARY  2010   WAGNER  MDL   MOS-2000
C                                 ADAPTED FROM CAKSNO
C
C        PURPOSE
C            TO ACCESS THE MOS-2000 EXTERNAL RANDOM ACCESS FILE SYSTEM
C            AND RETRIEVE THE CONUS SNOW MASK GRID WITH WHICH
C            TO CLIP THE CONUS SNOW GRIDS.  NORMALLY, THIS IS THE NDFD
C            AREA AND IS HANDLED ELSEWHERE, BUT CONUS SNOW REQUIRES
...
C
C            THE CLIPPING IS CONTROLLED BY 'CCONSN' BEING SPECIFIED
...
C              FD4(J) = WORK ARRAY TO HOLD CONUS SNOW MASK
...
C              IDMESH = THE PROPER VALUE FOR NOMINAL MESHLENGTH IN
C                       THE MOS2000 ID.  THIS IS SET BASED ON THE
C                       NOMINAL MESH LENGTH MESH, AND CAN ADAPT TO
C                       5KM OR 2.5KM GRIDS.
...
C                       THIS IS THE CONUS SNOW MASK.  (INTERNAL)
C        1         2         3         4         5         6         7 X
C  
...
C
      ...
      DIMENSION IDMESH(5)
C
      DATA IDMESH/6,99,5,99,4/
CD     CALL TIMPR(KFILDO,KFILDO,'START CCONSN        ')
CD     WRITE(KFILDO,105)MESH,NPROJ,ORIENT,XLAT
CD105  FORMAT(/' AT 105 IN CCONSN--MESH,NPROJ,ORIENT,XLAT',2I10,2F10.2)
C
...
C
      ...
      ENDIF
C
C         TO BUILD THE CORRECT ID FOR THE SNOW CLIPPING
C         MASK, READ THE VALUE IN IDMESH FOR THE NOMINAL
C         MESH LENGTH MESH.  IF THIS VALUE IS NOT 5, 3,
C         OR 1, THE NOMINAL MESH LENGTH IS NOT SUPPORTED
C         YET, AND NO CLIPPING WILL BE DONE.
C
      IF(MESH.GT.5.OR.MESH.LT.1.OR.IDMESH(MESH).EQ.99)THEN
         WRITE(KFILDO,131)MESH
         ISTOP=ISTOP+1
         GO TO 700
      ENDIF
C
 131  FORMAT(/'****UNABLE TO RECOGNIZE NOMINAL MESH LENGTH ',
     1        I2,' IN ROUTINE CCONSN.  THE SNOW ANALYSIS',
     2        'CANNOT BE CUT.  NOT FATAL, BUT COUNTED AS',
     3        'AN ERROR.  PROCEEDING.')
C
C     THE FOLLOWING WRITES CAN BE USED TO ASSESS IF THE ID
C     OF THE SNOW CLIPPING MASK IS BEING BUILT CORRECTLY
C     FOR THE NOMINAL MESH LENGTH.
C
C     WRITE(KFILDO,*)"NOMINAL MESH LENGTH IS ",MESH
C     WRITE(KFILDO,*)"SO IDMESH IS ",IDMESH(MESH)
      LD(1)=400306000+(IDMESH(MESH)*10000)
C     WRITE(KFILDO,*)"CLIPPING MASK ID IS ",LD(1)
C
C        THIS IS THE SNOW MASK FOR CONUS.
      LD(2)=0
      LD(3)=0
      LD(4)=0
      PLAINT='NDGD CONUS SNOW MASK           '
...
C           PROVIDED.
         WRITE(KFILDO,151)
 151     FORMAT(/' ****THE NDGD CONUS SNOW MASK WAS NOT FOUND.',
...
C
         WRITE(KFILDO,349)
 349     FORMAT(/,' CLIPPING ARCHIVE GRID IN CCONSN.')
C
         DO 350 JJ=1,NX*NY
CCCCD        WRITE(KFILDO,3495)JJ,P(JJ),FD4(JJ)
CCCCD3495    FORMAT(' IN CCONSN--JJ,P(JJ),FD4(JJ)',I8,2F10.2)
C
         IF(FD4(JJ).LT..5)THEN
            P(JJ)=9999.
         ENDIF
...
C
      ENDIF
C
CD     CALL TIMPR(KFILDO,KFILDO,'END   CCONSN        ')
C
 700  RETURN
      END
